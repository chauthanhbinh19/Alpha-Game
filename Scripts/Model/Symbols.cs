...
public class Symbols
{
    ...
    public List<Symbols> GetQualityPower(List<Symbols> list)
    {
    ...
    {
        Symbols orginCard = new Symbols();
        orginCard = orginCard.GetSymbolsById(c.id);
    ...
    public List<Symbols> GetSymbols(string type, int pageSize, int offset)
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                symbolsList = GetQualityPower(symbolsList);
            }
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                INSERT INTO user_symbols (
                    user_id, symbol_id, level, experiment, star, quality, block, quantity, power, health, physical_attack, 
                    ...
                    command.Parameters.AddWithValue("@quality", PowerManager.CheckQuality(symbols.rare));
    ...
    public Symbols SumPowerSymbolsPercent()
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                from symbols a, user_symbols ua
                where a.id=ua.symbol_id and ua.user_id=@user_id;";
                MySqlCommand command = new MySqlCommand(query, connection);
                command.Parameters.AddWithValue("@user_id", User.CurrentUserId);
                using (MySqlDataReader reader = command.ExecuteReader())
                {
