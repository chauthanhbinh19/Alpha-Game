...
public class Puppet
{
    ...
    {
        Puppet orginCard = new Puppet();
        orginCard = orginCard.GetPuppetById(c.id);
        ...
        Puppet.power = PowerManager.CalculatePower(
            Puppet.health,
    ...
    public static List<string> GetUniquePuppetTypes()
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            ...
            string query = "Select distinct type from puppet";
    ...
    public List<Puppet> GetUserPuppet(string type, int pageSize, int offset)
    {
        List<Puppet> Puppets = new List<Puppet>();
        int user_id = User.CurrentUserId;
        string connectionString = DatabaseConfig.ConnectionString;
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            ...
            {
                connection.Open();
                string query = @"Select um.*, m.id, m.name, m.image, m.rare from puppet m, user_puppet um where m.id=um.puppet_id and um.user_id=@userId and m.type=@type 
                ORDER BY m.name REGEXP '[0-9]+$',CAST(REGEXP_SUBSTR(m.name, '[0-9]+$') AS UNSIGNED), m.name limit @limit offset @offset";
                MySqlCommand command = new MySqlCommand(query, connection);
                command.Parameters.AddWithValue("@userId", user_id);
                command.Parameters.AddWithValue("@type", type);
    ...
    public int GetUserPuppetCount(string type)
    {
        int count = 0;
        int user_id = User.CurrentUserId;
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                if (count == 0)
                {
                    ...
                    user_id, puppet_id, level, experiment, star, quality, block, quantity, power, health, physical_attack, 
                    ...
                    command.Parameters.AddWithValue("@quality", PowerManager.CheckQuality(Puppet.rare));
    ...
    public List<Puppet> GetPuppetWithPrice(string type, int pageSize, int offset)
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                from puppet m, puppet_trade mt, currency cu
                where m.id=mt.puppet_id and mt.currency_id = cu.id and m.type =@type
                ORDER BY m.name REGEXP '[0-9]+$',CAST(REGEXP_SUBSTR(m.name, '[0-9]+$') AS UNSIGNED), m.name limit @limit offset @offset";
                MySqlCommand command = new MySqlCommand(query, connection);
                command.Parameters.AddWithValue("@type", type);
                command.Parameters.AddWithValue("@limit", pageSize);
    ...
    public Puppet GetPuppetById(int Id)
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            ...
            {
                connection.Open();
                string query = "Select * from puppet where id=@id";
                MySqlCommand command = new MySqlCommand(query, connection);
    ...
    public Puppet GetUserPuppetById(int Id)
    {
    ...
    public void InsertPuppetGallery(int Id)
    {
        Puppet PuppetFromDB = GetPuppetById(Id);
        int percent = 0;
        if (PuppetFromDB.rare.Equals("LG"))
        {
            percent = 20;
        }
        else if (PuppetFromDB.rare.Equals("UR"))
        {
            percent = 10;
        }
        else if (PuppetFromDB.rare.Equals("SSR"))
        {
            percent = 5;
        }
        else if (PuppetFromDB.rare.Equals("SR"))
        {
            percent = 2;
        }
        else if (PuppetFromDB.rare.Equals("MR"))
        {
            percent = 30;
        }
        string connectionString = DatabaseConfig.ConnectionString;
    ...
    public void UpdateStatusPuppetGallery(int Id)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
    ...
    public Puppet SumPowerPuppetPercent()
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                from puppet a, user_puppet ua
                where a.id=ua.puppet_id and ua.user_id=@user_id;";
                MySqlCommand command = new MySqlCommand(query, connection);
                command.Parameters.AddWithValue("@user_id", User.CurrentUserId);
                using (MySqlDataReader reader = command.ExecuteReader())
                {
