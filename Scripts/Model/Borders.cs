...
public class Borders
{
    ...
    public List<Borders> GetQualityPower(List<Borders> list)
    {
    ...
    public List<Borders> GetBorders(int pageSize, int offset)
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                borders = GetQualityPower(borders);
            }
    ...
    public List<Borders> GetUserBorders(int user_id, int pageSize, int offset)
    {
        List<Borders> borders = new List<Borders>();
        // int user_id=User.CurrentUserId;
        string connectionString = DatabaseConfig.ConnectionString;
    ...
    public int GetUserBordersCount(int user_id){
        int count =0;
        // int user_id=User.CurrentUserId;
        string connectionString = DatabaseConfig.ConnectionString;
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                INSERT INTO user_borders (
                    user_id, border_id, level, experiment, star, quality, block, is_used, quantity, power, health, physical_attack, 
                    ...
                    command.Parameters.AddWithValue("@quality", PowerManager.CheckQuality(borders.rare));
    ...
    public Borders GetBordersByUsed()
    {
        Borders borders = new Borders();
        string connectionString = DatabaseConfig.ConnectionString;
        using (MySqlConnection connection = new MySqlConnection(connectionString))
    ...
    public Borders SumPowerBordersPercent()
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                from borders a, user_borders ua
                where a.id=ua.border_id and ua.user_id=@user_id;";
                MySqlCommand command = new MySqlCommand(query, connection);
                command.Parameters.AddWithValue("@user_id", User.CurrentUserId);
                using (MySqlDataReader reader = command.ExecuteReader())
                {
