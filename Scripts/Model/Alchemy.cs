...
public class Alchemy
{
    ...
    public List<Alchemy> GetUserAlchemy(string type, int pageSize, int offset)
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            ...
            {
                connection.Open();
                string query = @"Select m.* from alchemy m, user_alchemy um where m.id=um.alchemy_id and um.user_id=@userId and m.type=@type 
                ORDER BY m.name REGEXP '[0-9]+$',CAST(REGEXP_SUBSTR(m.name, '[0-9]+$') AS UNSIGNED), m.name limit @limit offset @offset";
                MySqlCommand command = new MySqlCommand(query, connection);
                command.Parameters.AddWithValue("@userId", user_id);
                command.Parameters.AddWithValue("@type", type);
    ...
    public bool InsertUserAlchemyCircle(Alchemy Alchemy)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                if (count == 0)
                {
                    ...
                    user_id, alchemy_id, level, experiment, star, block, quantity, power, health, physical_attack, 
