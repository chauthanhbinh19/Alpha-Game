...
public class CardMilitary
{
    ...
    public List<CardMilitary> GetAllAnimeStatsPower(int user_id, List<CardMilitary> CardMilitaryList)
    {
    ...
    public List<CardMilitary> GetUserCardMilitary(int user_id, string type,int pageSize, int offset)
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                CardMilitaryList = GetAllAnimeStatsPower(user_id, CardMilitaryList);
                CardMilitaryList = GetQualityPower(CardMilitaryList);
            }
            catch (MySqlException ex)
            {
    ...
    public List<CardMilitary> GetUserCardMilitaryTeam(int user_id, int teamId, string position)
    {
        List<CardMilitary> CardMilitaryList = new List<CardMilitary>();
        string connectionString = DatabaseConfig.ConnectionString;
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                LEFT JOIN card_military m ON um.card_military_id = m.id 
                LEFT JOIN fact_card_military fcm ON fcm.user_id = um.user_id AND fcm.user_card_military_id = um.card_military_id
                WHERE um.user_id = @userId AND fcm.team_id = @team_id and fcm.position like @position
                ORDER BY m.name REGEXP '[0-9]+$', CAST(REGEXP_SUBSTR(m.name, '[0-9]+$') AS UNSIGNED), m.name;
                ";
                MySqlCommand command = new MySqlCommand(query, connection);
                command.Parameters.AddWithValue("@userId", user_id);
    ...
    public int GetUserCardMilitaryTeamsPositionCount(int user_id, int team_id, string position){
        int count =0;
        // int user_id=User.CurrentUserId;
        string connectionString = DatabaseConfig.ConnectionString;
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                INSERT INTO user_card_military (
                    ...
                    command.Parameters.AddWithValue("@quality", PowerManager.CheckQuality(CardMilitary.rare));
