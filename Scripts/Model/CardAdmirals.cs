...
public class CardAdmirals
{
    ...
    public List<CardAdmirals> GetUserCardAdmirals(string type,int pageSize, int offset)
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            ...
            {
                connection.Open();
                string query = @"Select uc.*, c.*, fca.* from card_admirals c, user_card_admirals uc, fact_card_admirals fca where c.id=uc.card_admiral_id and uc.user_id=@userId 
                and c.type= @type and fca.user_id= uc.user_id and fca.user_card_admiral_id =uc.card_admiral_id
                ORDER BY c.name REGEXP '[0-9]+$',CAST(REGEXP_SUBSTR(c.name, '[0-9]+$') AS UNSIGNED), c.name limit @limit offset @offset";
                MySqlCommand command = new MySqlCommand(query, connection);
                command.Parameters.AddWithValue("@userId", user_id);
                command.Parameters.AddWithValue("@type", type);
