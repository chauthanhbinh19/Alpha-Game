...
public class HIDCManager : MonoBehaviour
{
    ...
    public void CreateHIDC()
    {
        ...
        titleText.text = LocalizationManager.Get(AppDisplayConstants.Title.HIDC);
        TextMeshProUGUI titleText2 = transform.Find("HIDCContent/TitleText").GetComponent<TextMeshProUGUI>();
        titleText2.text = LocalizationManager.Get(AppDisplayConstants.Title.HIDC);
    ...
    private void CreateHIDCButtonUI(int index, string itemName, Texture2D _itemImage, Transform panel)
    {
        ...
        if (borderImage != null)
        {
            borderImage.texture = TextureHelper.LoadTexture2DCached(ImageConstants.HIDC.HIDC_BORDER_URL);
        }

        RawImage iconImage = transform.Find("IconImage").GetComponent<RawImage>();
        if (iconImage != null)
        {
            iconImage.texture = TextureHelper.LoadTexture2DCached(ImageConstants.HIDC.HIDC_ICON_URL);
        }
    }
