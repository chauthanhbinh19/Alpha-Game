...
public class HIHNManager : MonoBehaviour
{
    ...
    private void CreateHIHNButtonUI(int index, string itemName, Texture2D _itemImage, Transform panel)
    {
        ...
        if (borderImage != null)
        {
            borderImage.texture = TextureHelper.LoadTexture2DCached(ImageConstants.HIHN.HIHN_BORDER_URL);
        }

        RawImage iconImage = transform.Find("IconImage").GetComponent<RawImage>();
        if (iconImage != null)
        {
            iconImage.texture = TextureHelper.LoadTexture2DCached(ImageConstants.HIHN.HIHN_ICON_URL);
        }
    }
