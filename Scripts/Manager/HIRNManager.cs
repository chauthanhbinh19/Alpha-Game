...
public class HIRNManager : MonoBehaviour
{
    ...
    private void CreateHIRNButtonUI(int index, string itemName, Texture2D _itemImage, Transform panel)
    {
        ...
        if (borderImage != null)
        {
            borderImage.texture = TextureHelper.LoadTexture2DCached(ImageConstants.HIRN.HIRN_BORDER_URL);
        }

        RawImage iconImage = transform.Find("IconImage").GetComponent<RawImage>();
        if (iconImage != null)
        {
            iconImage.texture = TextureHelper.LoadTexture2DCached(ImageConstants.HIRN.HIRN_ICON_URL);
        }
    }
