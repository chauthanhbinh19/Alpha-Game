...
public class SummonManager : MonoBehaviour
{
    ...
    public void Initialize()
    {
        ...
        SummonTabButtonPrefab = UIManager.Instance.Get(AppConstants.Prefab.Component.SUMMON_TAB_BUTTON_PREFAB);
        MainPanel = UIManager.Instance.GetTransform(AppConstants.Transform.MAIN_PANEL);
        SummonPanelPrefab = UIManager.Instance.Get(AppConstants.Prefab.General.SUMMON_PANEL_PREFAB);
    ...
    public async Task LoadCurrentPageAsync()
    {
        ...
        if (MainType.Equals(AppConstants.MainType.SUMMON_CARD_HERO))
        {
            ...
            List<Items> items = new List<Items> { await UserItemsService.Create().GetUserItemByNameAsync(User.CurrentUserId, ItemConstants.Ticket.CARD_HERO_TICKET) };
            ...
            SummonOneButton.onClick.AddListener(() =>
            {
                ...
                FindObjectOfType<GachaSystem>().Summon(MainType, Type, SummonObject, 1, items, async (success) =>
                {
                    ...
                    {
                        CurrenciesManager.Instance.GetTicketsCurrency(
                        new List<Items> { await UserItemsService.Create().GetUserItemByNameAsync(User.CurrentUserId, ItemConstants.Ticket.CARD_HERO_TICKET) },
                        CurrencyPanel
                        );
                    }
                    else
        ...
        else if (MainType.Equals(AppConstants.MainType.SUMMON_CARD_CAPTAIN))
        {
            ...
            List<Items> items = new List<Items> { await UserItemsService.Create().GetUserItemByNameAsync(User.CurrentUserId, ItemConstants.Ticket.CARD_CAPTAIN_TICKET) };
            CurrenciesManager.Instance.GetTicketsCurrency(
                items,
                CurrencyPanel
            );
            ...
            SummonOneButton.onClick.AddListener(() =>
            {
                ...
                FindObjectOfType<GachaSystem>().Summon(MainType, Type, SummonObject, 1, items, async (success) =>
                {
                    ...
                    {
                        CurrenciesManager.Instance.GetTicketsCurrency(
                        new List<Items> { await UserItemsService.Create().GetUserItemByNameAsync(User.CurrentUserId, ItemConstants.Ticket.CARD_CAPTAIN_TICKET) },
                        CurrencyPanel
                        );
                    }
                    else
        ...
        else if (MainType.Equals(AppConstants.MainType.SUMMON_CARD_MILITARY))
        {
            ...
            List<Items> items = new List<Items> { await UserItemsService.Create().GetUserItemByNameAsync(User.CurrentUserId, ItemConstants.Ticket.CARD_MILITARY_TICKET) };
            CurrenciesManager.Instance.GetTicketsCurrency(
                items,
                CurrencyPanel
            );
            ...
            SummonOneButton.onClick.AddListener(() =>
            {
                ...
                FindObjectOfType<GachaSystem>().Summon(MainType, Type, SummonObject, 1, items, async (success) =>
                {
                    ...
                    {
                        CurrenciesManager.Instance.GetTicketsCurrency(
                        new List<Items> { await UserItemsService.Create().GetUserItemByNameAsync(User.CurrentUserId, ItemConstants.Ticket.CARD_MILITARY_TICKET) },
                        CurrencyPanel
                        );
                    }
                    else
        ...
        else if (MainType.Equals(AppConstants.MainType.SUMMON_CARD_SPELL))
        {
            ...
            List<Items> items = new List<Items> { await UserItemsService.Create().GetUserItemByNameAsync(User.CurrentUserId, ItemConstants.Ticket.CARD_SPELL_TICKET) };
            CurrenciesManager.Instance.GetTicketsCurrency(
                items,
                CurrencyPanel
            );
            ...
            SummonOneButton.onClick.AddListener(() =>
            {
                ...
                FindObjectOfType<GachaSystem>().Summon(MainType, Type, SummonObject, 1, items, async (success) =>
                {
                    ...
                    {
                        CurrenciesManager.Instance.GetTicketsCurrency(
                        new List<Items> { await UserItemsService.Create().GetUserItemByNameAsync(User.CurrentUserId, ItemConstants.Ticket.CARD_SPELL_TICKET) },
                        CurrencyPanel
                        );
                    }
                    else
        ...
        else if (MainType.Equals(AppConstants.MainType.SUMMON_CARD_MONSTER))
        {
            ...
            List<Items> items = new List<Items> { await UserItemsService.Create().GetUserItemByNameAsync(User.CurrentUserId, ItemConstants.Ticket.CARD_MONSTER_TICKET) };
            CurrenciesManager.Instance.GetTicketsCurrency(
                items,
                CurrencyPanel
            );
            ...
            SummonOneButton.onClick.AddListener(() =>
            {
                ...
                FindObjectOfType<GachaSystem>().Summon(MainType, Type, SummonObject, 1, items, async (success) =>
                {
                    ...
                    {
                        CurrenciesManager.Instance.GetTicketsCurrency(
                        new List<Items> { await UserItemsService.Create().GetUserItemByNameAsync(User.CurrentUserId, ItemConstants.Ticket.CARD_MONSTER_TICKET) },
                        CurrencyPanel
                        );
                    }
                    else
        ...
        else if (MainType.Equals(AppConstants.MainType.SUMMON_CARD_COLONEL))
        {
            ...
            List<Items> items = new List<Items> { await UserItemsService.Create().GetUserItemByNameAsync(User.CurrentUserId, ItemConstants.Ticket.CARD_COLONEL_TICKET) };
            CurrenciesManager.Instance.GetTicketsCurrency(
                items,
                CurrencyPanel
            );
            ...
            SummonOneButton.onClick.AddListener(() =>
            {
                ...
                FindObjectOfType<GachaSystem>().Summon(MainType, Type, SummonObject, 1, items, async (success) =>
                {
                    ...
                    {
                        CurrenciesManager.Instance.GetTicketsCurrency(
                        new List<Items> { await UserItemsService.Create().GetUserItemByNameAsync(User.CurrentUserId, ItemConstants.Ticket.CARD_COLONEL_TICKET) },
                        CurrencyPanel
                        );
                    }
                    else
        ...
        else if (MainType.Equals(AppConstants.MainType.SUMMON_CARD_GENERAL))
        {
            ...
            List<Items> items = new List<Items> { await UserItemsService.Create().GetUserItemByNameAsync(User.CurrentUserId, ItemConstants.Ticket.CARD_GENERAL_TICKET) };
            CurrenciesManager.Instance.GetTicketsCurrency(
                items,
                CurrencyPanel
            );
            ...
            SummonOneButton.onClick.AddListener(() =>
            {
                ...
                FindObjectOfType<GachaSystem>().Summon(MainType, Type, SummonObject, 1, items, async (success) =>
                {
                    ...
                    {
                        CurrenciesManager.Instance.GetTicketsCurrency(
                        new List<Items> { await UserItemsService.Create().GetUserItemByNameAsync(User.CurrentUserId, ItemConstants.Ticket.CARD_GENERAL_TICKET) },
                        CurrencyPanel
                        );
                    }
                    else
        ...
        else if (MainType.Equals(AppConstants.MainType.SUMMON_CARD_ADMIRAL))
        {
            ...
            List<Items> items = new List<Items> { await UserItemsService.Create().GetUserItemByNameAsync(User.CurrentUserId, ItemConstants.Ticket.CARD_ADMIRAL_TICKET) };
            CurrenciesManager.Instance.GetTicketsCurrency(
                items,
                CurrencyPanel
            );
            ...
            SummonOneButton.onClick.AddListener(() =>
            {
                ...
                FindObjectOfType<GachaSystem>().Summon(MainType, Type, SummonObject, 1, items, async (success) =>
                {
                    ...
                    {
                        CurrenciesManager.Instance.GetTicketsCurrency(
                        new List<Items> { await UserItemsService.Create().GetUserItemByNameAsync(User.CurrentUserId, ItemConstants.Ticket.CARD_ADMIRAL_TICKET) },
                        CurrencyPanel
                        );
                    }
                    else
