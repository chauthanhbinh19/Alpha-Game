...
public class HIINManager : MonoBehaviour
{
    ...
    public void CreateHIIN()
    {
        ...
        titleText.text = LocalizationManager.Get(AppDisplayConstants.MainType.HIIN);
        TextMeshProUGUI titleText2 = transform.Find("HIINContent/TitleText").GetComponent<TextMeshProUGUI>();
        titleText2.text = LocalizationManager.Get(AppDisplayConstants.MainType.HIIN);
    ...
    private void CreateHIINButtonUI(int index, string itemName, Texture2D _itemImage, Transform panel)
    {
        ...
        if (borderImage != null)
        {
            borderImage.texture = TextureHelper.LoadTexture2DCached(ImageConstants.HIIN.HIIN_BORDER_URL);
        }

        RawImage iconImage = transform.Find("IconImage").GetComponent<RawImage>();
        if (iconImage != null)
        {
            iconImage.texture = TextureHelper.LoadTexture2DCached(ImageConstants.HIIN.HIIN_ICON_URL);
        }
    }
