...
public class HIENManager : MonoBehaviour
{
    ...
    public void CreateHIEN()
    {
        ...
        titleText2.text = LocalizationManager.Get(AppDisplayConstants.Title.HIEN);
    ...
    private void CreateHIENButtonUI(int index, string itemName, Texture2D _itemImage, Transform panel)
    {
        ...
        if (borderImage != null)
        {
            borderImage.texture = TextureHelper.LoadTexture2DCached(ImageConstants.HIEN.HIEN_BORDER_URL);
        }

        RawImage iconImage = transform.Find("IconImage").GetComponent<RawImage>();
        if (iconImage != null)
        {
            iconImage.texture = TextureHelper.LoadTexture2DCached(ImageConstants.HIEN.HIEN_ICON_URL);
        }
    }
