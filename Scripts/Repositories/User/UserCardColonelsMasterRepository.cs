    ...
    public async Task<Master> GetCardColonelMasterAsync(string type, string card_id)
    {
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                WHERE user_id = @user_id AND Master_type = @type AND user_card_colonel_id = @card_id;
            ";
    ...
    public async Task InsertOrUpdateCardColonelMasterAsync(Master Master, string type, string card_id)
    {
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                SELECT COUNT(*) 
                FROM user_card_colonels_Master 
                WHERE user_id = @user_id AND user_card_colonel_id = @card_id AND Master_type = @Master_type;
            ";
                ...
                await using (MySqlCommand checkCmd = new MySqlCommand(checkQuery, connection))
                {
                    ...
                    else
                    {
                        ...
                        (
                            user_id, user_card_colonel_id, Master_type, Master_level, 
