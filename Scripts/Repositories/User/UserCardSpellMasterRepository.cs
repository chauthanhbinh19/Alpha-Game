...
public class UserCardSpellMasterRepository : IUserCardSpellMasterRepository
{
    public Master GetCardSpellMaster(string type, string card_id)
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                connection.Open();
                string query = @"select *
                from user_card_spell_master
                where user_id = @user_id AND master_type = @type AND user_card_spell_id = @card_id";
    ...
    public void InsertOrUpdateCardSpellMaster(Master master, string type, string card_id)
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                connection.Open();
                string checkQuery = @"
                SELECT COUNT(*) FROM user_card_spell_master 
                WHERE user_id = @user_id AND user_card_spell_id = @card_id AND master_type = @master_type";
                ...
                using (MySqlCommand checkCmd = new MySqlCommand(checkQuery, connection))
                {
                    ...
                    {
                        string insertQuery = @"
                        INSERT INTO user_card_spell_master 
