    ...
    public Master GetCardCaptainsMaster(string type, string card_id)
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                where user_id = @user_id AND master_type = @type AND user_card_captain_id = @card_id";
    ...
    public void InsertOrUpdateCardCaptainsMaster(Master master, string type, string card_id)
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                connection.Open();
                string checkQuery = @"
                SELECT COUNT(*) FROM user_card_captains_master 
                WHERE user_id = @user_id AND user_card_captain_id = @card_id AND master_type = @master_type";
                ...
                using (MySqlCommand checkCmd = new MySqlCommand(checkQuery, connection))
                {
                    ...
                    else
                    {
                        ...
                        (
                            user_id, user_card_captain_id, master_type, master_level, 
