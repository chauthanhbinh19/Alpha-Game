...
public class UserBadgesRepository : IUserBadgesRepository
{
    public List<Badges> GetUserBadges(string user_id, int pageSize, int offset, string rare)
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            ...
            {
                connection.Open();
                string query = @"Select ut.*, t.id, t.name, t.image, t.rare, t.description from Badges t, user_Badges ut 
                where t.id=ut.Badge_id and ut.user_id=@userId AND (@rare = 'All' or t.rare = @rare)
    ...
    public int GetUserBadgesCount(string user_id, string rare)
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            ...
            {
                connection.Open();
                string query = @"Select count(*) from Badges t, user_Badges ut 
                where t.id=ut.Badge_id and ut.user_id=@userId AND (@rare = 'All' or t.rare = @rare)";
                MySqlCommand command = new MySqlCommand(query, connection);
                command.Parameters.AddWithValue("@userId", user_id);
                command.Parameters.AddWithValue("@rare", rare);
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                INSERT INTO user_Badges (
                    ...
                    command.Parameters.AddWithValue("@rare", Badges.Rare);
                    command.Parameters.AddWithValue("@level", 0);
                    command.Parameters.AddWithValue("@experiment", 0);
                    command.Parameters.AddWithValue("@star", 0);
                    command.Parameters.AddWithValue("@quality", QualityEvaluator.CheckQuality(Badges.Rare));
                    command.Parameters.AddWithValue("@block", false);
                    command.Parameters.AddWithValue("@quantity", Badges.Quantity);
    ...
    public Badges GetUserBadgesById(string user_id, string Id)
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            ...
            {
                connection.Open();
                string query = @"Select * from user_Badges where user_Badges.Badge_id=@id 
