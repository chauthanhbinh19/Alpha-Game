    ...
    public async Task<Rank> GetPetRankAsync(string id, string card_id)
    {
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                SELECT *
                FROM user_pets_rank
                WHERE user_id = @user_id AND rank_id = @id AND user_pet_id = @card_id;
            ";
    ...
    public async Task InsertOrUpdatePetRankAsync(Rank rank, string card_id)
    {
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                SELECT COUNT(*) 
                FROM user_pets_rank 
                WHERE user_id = @user_id AND user_pet_id = @card_id AND rank_id = @rank_id;
            ";
                ...
                await using (MySqlCommand checkCommand = new MySqlCommand(checkSQL, connection))
                {
                    ...
                    else
                    {
                        ...
                        (
                            user_id, user_pet_id, rank_id, rank_level, 
