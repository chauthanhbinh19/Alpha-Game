...
public class UserCardMilitaryMasterRepository : IUserCardMilitaryMasterRepository
{
    public Master GetCardMilitaryMaster(string type, string card_id)
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                connection.Open();
                string query = @"select *
                from user_card_military_master
                where user_id = @user_id AND master_type = @type AND user_card_military_id = @card_id";
    ...
    public void InsertOrUpdateCardMilitaryMaster(Master master, string type, string card_id)
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                connection.Open();
                string checkQuery = @"
                SELECT COUNT(*) FROM user_card_military_master 
                WHERE user_id = @user_id AND user_card_military_id = @card_id AND master_type = @master_type";
                ...
                using (MySqlCommand checkCmd = new MySqlCommand(checkQuery, connection))
                {
                    ...
                    if (count > 0)
                    {
                        ...
                        string updateQuery = @"
                        UPDATE user_card_military_master
                    ...
                    {
                        string insertQuery = @"
                        INSERT INTO user_card_military_master 
