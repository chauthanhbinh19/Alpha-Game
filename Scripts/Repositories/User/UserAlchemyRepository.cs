...
public class UserAlchemyRepository : IUserAlchemyRepository
{
    public List<Alchemy> GetUserAlchemy(string user_id, string type, int pageSize, int offset, string rare)
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            ...
            {
                connection.Open();
                string query = @"Select um.*, m.id, m.name, m.image, m.rare, m.description from alchemy m, user_alchemy um 
                where m.id=um.alchemy_id and um.user_id=@userId and m.type=@type AND (@rare = 'All' or m.rare = @rare)
    ...
    public int GetUserAlchemyCount(string user_id, string type, string rare)
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            ...
            {
                connection.Open();
                string query = @"Select count(*) from alchemy m, user_alchemy um 
                where m.id=um.alchemy_id and um.user_id=@userId and m.type= @type AND (@rare = 'All' or m.rare = @rare)";
                MySqlCommand command = new MySqlCommand(query, connection);
                command.Parameters.AddWithValue("@userId", user_id);
                command.Parameters.AddWithValue("@type", type);
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                if (count == 0)
                {
                    ...
                    user_id, alchemy_id, level, experiment, star, quality, block, quantity,
