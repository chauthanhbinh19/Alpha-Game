...
public class UserTalismanRepository : IUserTalismanRepository
{
    public List<Talisman> GetUserTalisman(string user_id, string type, int pageSize, int offset, string rare)
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            ...
            {
                connection.Open();
                string query = @"Select um.*, m.id, m.name, m.image, m.rare, m.description from talisman m, user_talisman um 
                where m.id=um.talisman_id and um.user_id=@userId and m.type=@type AND (@rare = 'All' or m.rare = @rare)
    ...
    public int GetUserTalismanCount(string user_id, string type, string rare)
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            ...
            {
                connection.Open();
                string query = @"Select count(*) from talisman m, user_talisman um 
                where m.id=um.talisman_id and um.user_id=@userId and m.type= @type AND (@rare = 'All' or m.rare = @rare)";
                MySqlCommand command = new MySqlCommand(query, connection);
                command.Parameters.AddWithValue("@userId", user_id);
                command.Parameters.AddWithValue("@type", type);
