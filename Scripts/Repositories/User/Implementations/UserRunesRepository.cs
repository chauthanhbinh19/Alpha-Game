...
public class UserRunesRepository : IUserRunesRepository
{
    ...
    public async Task<int> GetUserRunesCountAsync(string userId, string search, string rare)
    {
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                SELECT COUNT(*) 
                FROM Runes t
                INNER JOIN user_runes ut ON t.id = ut.rune_id AND t.is_active = TRUE AND t.is_deleted = FALSE
    ...
    public async Task<InsertOrUpdateResult<Runes>> InsertOrUpdateUserRuneAsync(string userId, Runes rune)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        await using MySqlConnection connection = new MySqlConnection(connectionString);
        ...
        try
        {
            ...
            // Query thực hiện Insert hoặc Update nếu đã tồn tại Composite Primary Key (user_id, rune_id)
            string upsertSQL = @"
            INSERT INTO user_runes (
                ...
                @user_id, @rune_id, @rare, 0, 0, 0, @quality, false, @quantity,
                @power, @health, @physical_attack, @physical_defense, @magical_attack, @magical_defense,
                @chemical_attack, @chemical_defense, @atomic_attack, @atomic_defense, @mental_attack, @mental_defense,
                @speed, @critical_damage_rate, @critical_rate, @critical_resistance_rate, @ignore_critical_rate,
                @penetration_rate, @penetration_resistance_rate,
            ...
            return InsertOrUpdateResult<Runes>.Failure();
        }
        catch (MySqlException ex)
        {
    ...
    public async Task<InsertOrUpdateResult<BatchOperationResultDTO<Runes>>> InsertOrUpdateUserRunesBatchAsync(
    string userId, List<Runes> runes)
    {
        if (runes == null || runes.Count == 0)
        {
            return new InsertOrUpdateResult<BatchOperationResultDTO<Runes>>
            {
                Data = new BatchOperationResultDTO<Runes>(),
        ...
        try
        {
            ...
            // 1. Query lấy TOÀN BỘ rune_id hiện có của User (Cực nhanh nhờ Index user_id)
            var existingIds = new HashSet<string>();
            string checkSql = "SELECT rune_id FROM user_runes WHERE user_id = @user_id;";
            ...
            // 2. Phân loại Runes giữ NGUYÊN VẸN OBJECT thuộc tính trong RAM C#
            var batchResult = new BatchOperationResultDTO<Runes>();
            foreach (var card in runes)
            {
            ...
            {
                var batch = runes.Skip(i).Take(batchSize).ToList();
            ...
            INSERT INTO user_runes (
                ...
                for (int j = 0; j < batch.Count; j++)
                {
                    ...
                    stringBuilder.Append($@"
                (@user_id, @rune_id_{j}, @rare_{j}, 0, 0, 0, @quality_{j}, 0, @quantity_{j},
                ...
                @skill_damage_rate_{j}, @skill_resistance_rate_{j}
                    ...
                    {
                    new MySqlParameter($"@rune_id_{j}", c.Id),
                ...
                stringBuilder.Append(@"
            ON DUPLICATE KEY UPDATE
                quantity = COALESCE(user_runes.quantity, 0) + VALUES(quantity);
            ");
        ...
        catch (Exception ex)
        {
            ...
            return InsertOrUpdateResult<BatchOperationResultDTO<Runes>>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateUserRuneLevelAsync(string userId, Runes rune)
    {
        ...
        try
        {
            ...
            string updateSQL = @"
            UPDATE user_runes uc INNER JOIN runes c ON c.id = uc.rune_id
            SET 
                uc.level = @level, 
                uc.experience = @experience
            WHERE uc.user_id = @user_id 
              AND uc.rune_id = @rune_id
        ...
        {
            Debug.LogError("Error UpdateUserRuneLevel: " + ex.Message);
    ...
    public async Task<InsertOrUpdateResult<bool>> UpdateUserRuneStarAsync(string userId, Runes rune)
    {
        ...
        {
            Debug.LogError("Error UpdateUserRuneStar: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
            ...
            ";
                await using (MySqlCommand selectCommand = new MySqlCommand(selectSQL, connection))
                {
                    ...
                    await using (MySqlDataReader reader = await selectCommand.ExecuteReaderAsync())
                    {
                        ...
                        {
                            sumRunes.Health = reader.GetDoubleSafe("health");
                            sumRunes.PhysicalAttack = reader.GetDoubleSafe("physical_attack");
                            sumRunes.PhysicalDefense = reader.GetDoubleSafe("physical_defense");
                            sumRunes.MagicalAttack = reader.GetDoubleSafe("magical_attack");
                            sumRunes.MagicalDefense = reader.GetDoubleSafe("magical_defense");
                            sumRunes.ChemicalAttack = reader.GetDoubleSafe("chemical_attack");
                            sumRunes.ChemicalDefense = reader.GetDoubleSafe("chemical_defense");
                            sumRunes.AtomicAttack = reader.GetDoubleSafe("atomic_attack");
                            sumRunes.AtomicDefense = reader.GetDoubleSafe("atomic_defense");
                            sumRunes.MentalAttack = reader.GetDoubleSafe("mental_attack");
                            sumRunes.MentalDefense = reader.GetDoubleSafe("mental_defense");
                            sumRunes.Speed = reader.GetDoubleSafe("speed");
                            sumRunes.CriticalDamageRate = reader.GetDoubleSafe("critical_damage_rate");
                            sumRunes.CriticalRate = reader.GetDoubleSafe("critical_rate");
                            sumRunes.CriticalResistanceRate = reader.GetDoubleSafe("critical_resistance_rate");
                            sumRunes.IgnoreCriticalRate = reader.GetDoubleSafe("ignore_critical_rate");
                            sumRunes.PenetrationRate = reader.GetDoubleSafe("penetration_rate");
                            sumRunes.PenetrationResistanceRate = reader.GetDoubleSafe("penetration_resistance_rate");
                            sumRunes.EvasionRate = reader.GetDoubleSafe("evasion_rate");
                            sumRunes.DamageAbsorptionRate = reader.GetDoubleSafe("damage_absorption_rate");
                            sumRunes.IgnoreDamageAbsorptionRate = reader.GetDoubleSafe("ignore_damage_absorption_rate");
                            sumRunes.AbsorbedDamageRate = reader.GetDoubleSafe("absorbed_damage_rate");
                            sumRunes.VitalityRegenerationRate = reader.GetDoubleSafe("vitality_regeneration_rate");
                            sumRunes.VitalityRegenerationResistanceRate = reader.GetDoubleSafe("vitality_regeneration_resistance_rate");
                            sumRunes.AccuracyRate = reader.GetDoubleSafe("accuracy_rate");
                            sumRunes.LifestealRate = reader.GetDoubleSafe("lifesteal_rate");
                            sumRunes.ShieldStrength = reader.GetDoubleSafe("shield_strength");
                            sumRunes.Tenacity = reader.GetDoubleSafe("tenacity");
                            sumRunes.ResistanceRate = reader.GetDoubleSafe("resistance_rate");
                            sumRunes.ComboRate = reader.GetDoubleSafe("combo_rate");
                            sumRunes.IgnoreComboRate = reader.GetDoubleSafe("ignore_combo_rate");
                            sumRunes.ComboDamageRate = reader.GetDoubleSafe("combo_damage_rate");
                            sumRunes.ComboResistanceRate = reader.GetDoubleSafe("combo_resistance_rate");
                            sumRunes.StunRate = reader.GetDoubleSafe("stun_rate");
                            sumRunes.IgnoreStunRate = reader.GetDoubleSafe("ignore_stun_rate");
                            sumRunes.ReflectionRate = reader.GetDoubleSafe("reflection_rate");
                            sumRunes.IgnoreReflectionRate = reader.GetDoubleSafe("ignore_reflection_rate");
                            sumRunes.ReflectionDamageRate = reader.GetDoubleSafe("reflection_damage_rate");
                            sumRunes.ReflectionResistanceRate = reader.GetDoubleSafe("reflection_resistance_rate");
                            sumRunes.Mana = reader.GetDoubleSafe("mana");
                            sumRunes.ManaRegenerationRate = reader.GetDoubleSafe("mana_regeneration_rate");
                            sumRunes.DamageToDifferentFactionRate = reader.GetDoubleSafe("damage_to_different_faction_rate");
                            sumRunes.ResistanceToDifferentFactionRate = reader.GetDoubleSafe("resistance_to_different_faction_rate");
                            sumRunes.DamageToSameFactionRate = reader.GetDoubleSafe("damage_to_same_faction_rate");
                            sumRunes.ResistanceToSameFactionRate = reader.GetDoubleSafe("resistance_to_same_faction_rate");
                            sumRunes.NormalDamageRate = reader.GetDoubleSafe("normal_damage_rate");
                            sumRunes.NormalResistanceRate = reader.GetDoubleSafe("normal_resistance_rate");
                            sumRunes.SkillDamageRate = reader.GetDoubleSafe("skill_damage_rate");
                            sumRunes.SkillResistanceRate = reader.GetDoubleSafe("skill_resistance_rate");
                        }
                    }
                }
            }
