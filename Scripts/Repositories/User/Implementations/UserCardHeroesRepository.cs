...
public class UserCardHeroesRepository : IUserCardHeroesRepository
{
    ...
    public async Task<bool> UpdateTeamUserCardHeroAsync(string userId, string teamId, string position, string cardId)
    {
        ...
        try
        {
            ...
            string updateSQL = @"
            UPDATE user_card_heroes uc INNER JOIN card_heroes c ON c.id = uc.card_hero_id 
            SET team_id = @team_id, position = @position 
            WHERE uc.user_id = @user_id AND uc.card_hero_id = @card_hero_id AND c.is_active = TRUE AND c.is_deleted = FALSE;
        ";
    ...
    public async Task<int> GetUserCardHeroesCountAsync(string userId, string search, string type, string rare)
    {
        ...
        try
        {
            ...
            SELECT COUNT(*) 
            FROM card_heroes c
            JOIN user_card_heroes uc ON c.id = uc.card_hero_id AND c.is_active = TRUE AND c.is_deleted = FALSE
            WHERE uc.user_id = @userId  AND c.is_active = TRUE AND c.is_deleted = FALSE
        ";
            if (!string.IsNullOrEmpty(type) && type != "All")
            {
    ...
    string userId, List<CardHeroes> cardHeroes)
    {
        ...
        try
        {
            ...
            int totalCards = cardHeroes.Count;
            ...
            INSERT INTO user_card_heroes (
                ...
                for (int j = 0; j < currentBatchCount; j++)
                {
                    ...
                    var c = cardHeroes[i + j];

                    stringBuilder.Append($"(@user_id, @card_hero_id_{j}, @rare_{j}, 0, 0, 0, @quality_{j}, 0, @quantity_{j}, ")
                    ...
                    p.AddWithValue($"@card_hero_id_{j}", c.Id);
    ...
    public async Task<CardHeroes> GetUserCardHeroByIdAsync(string userId, string Id)
    {
        ...
        try
        {
            ...
            INNER JOIN card_heroes c ON uc.card_hero_id = c.id
                LEFT JOIN AggregatedModules am ON uc.card_hero_id = am.user_card_hero_id
                LEFT JOIN AggregatedUpgrades au ON uc.card_hero_id = au.user_card_hero_id
            WHERE uc.card_hero_id = @id AND uc.user_id = @user_id AND c.is_active = TRUE AND c.is_deleted = FALSE";
    ...
    {
        List<string> cardHeroIds = new List<string>();
        string connectionString = DatabaseConfig.ConnectionString;

        string selectSQL = @"
        SELECT uc.card_hero_id
        FROM user_card_heroes uc
            ...
            try
            {
                ...
                await using (MySqlCommand selectCommand = new MySqlCommand(selectSQL, connection))
                {
                    ...
                    await using (MySqlDataReader reader = (MySqlDataReader)await selectCommand.ExecuteReaderAsync())
                    {
                        while (await reader.ReadAsync())
                        {
                            if (!reader.IsDBNull(reader.GetOrdinal("card_hero_id")))
                            {
                                cardHeroIds.Add(reader.GetString("card_hero_id"));
                            }
                        }
                    }
                }
