...
public class UserFurnituresRepository : IUserFurnituresRepository
{
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                WITH AggregatedModules AS (
                    SELECT user_furniture_id, SUM(current_multiplier) AS total_module_mult
                    FROM user_furnitures_module
                    GROUP BY user_furniture_id
                ),
                AggregatedUpgrades AS (
                    SELECT user_furniture_id, SUM(current_multiplier) AS total_upgrade_mult
                ...
                await using (MySqlCommand selectCommand = new MySqlCommand(selectSQL, connection))
                {
                    ...
                    await using (MySqlDataReader reader = await selectCommand.ExecuteReaderAsync())
                    {
                        while (await reader.ReadAsync())
                        {
                            ...
                            furniture.UserModules = userModule;
                            furniture.UserUpgrades = userUpgrade;
    ...
    public async Task<InsertOrUpdateResult<Furnitures>> InsertOrUpdateUserFurnitureAsync(string userId, Furnitures furniture)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        await using MySqlConnection connection = new MySqlConnection(connectionString);
        ...
        try
        {
            ...
            // Query thực hiện Insert hoặc Update nếu đã tồn tại Composite Primary Key (user_id, furniture_id)
            string upsertSQL = @"
            INSERT INTO user_furnitures (
                ...
                @user_id, @furniture_id, @rare, 0, 0, 0, @quality, false, @quantity,
                @power, @health, @physical_attack, @physical_defense, @magical_attack, @magical_defense,
                @chemical_attack, @chemical_defense, @atomic_attack, @atomic_defense, @mental_attack, @mental_defense,
                @speed, @critical_damage_rate, @critical_rate, @critical_resistance_rate, @ignore_critical_rate,
                @penetration_rate, @penetration_resistance_rate,
            ...
            return InsertOrUpdateResult<Furnitures>.Failure();
        }
        catch (MySqlException ex)
        {
    ...
    string userId, List<Furnitures> furnitures)
    {
        ...
        {
            return new InsertOrUpdateResult<BatchOperationResultDTO<Furnitures>>
            {
                Data = new BatchOperationResultDTO<Furnitures>(),
        ...
        try
        {
            ...
            // 1. Query lấy TOÀN BỘ furniture_id hiện có của User (Cực nhanh nhờ Index user_id)
            var existingIds = new HashSet<string>();
            string checkSql = "SELECT furniture_id FROM user_furnitures WHERE user_id = @user_id;";
            ...
            // 2. Phân loại Furnitures giữ NGUYÊN VẸN OBJECT thuộc tính trong RAM C#
            var batchResult = new BatchOperationResultDTO<Furnitures>();
            foreach (var card in furnitures)
            {
            ...
            {
                var batch = furnitures.Skip(i).Take(batchSize).ToList();
            ...
            INSERT INTO user_furnitures (
                ...
                for (int j = 0; j < batch.Count; j++)
                {
                    ...
                    stringBuilder.Append($@"
                (@user_id, @furniture_id_{j}, @rare_{j}, 0, 0, 0, @quality_{j}, 0, @quantity_{j},
                ...
                @skill_damage_rate_{j}, @skill_resistance_rate_{j}
                    ...
                    {
                    new MySqlParameter($"@furniture_id_{j}", c.Id),
                ...
                stringBuilder.Append(@"
            ON DUPLICATE KEY UPDATE
                quantity = COALESCE(user_furnitures.quantity, 0) + VALUES(quantity);
            ");
        ...
        catch (Exception ex)
        {
            ...
            return InsertOrUpdateResult<BatchOperationResultDTO<Furnitures>>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateUserFurnitureLevelAsync(string userId, Furnitures furniture)
    {
        ...
        {
            Debug.LogError("Error UpdateUserFurnitureLevel: " + ex.Message);
    ...
    public async Task<InsertOrUpdateResult<bool>> UpdateUserFurnitureStarAsync(string userId, Furnitures furniture)
    {
        ...
        {
            Debug.LogError("Error UpdateUserFurnitureStar: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
