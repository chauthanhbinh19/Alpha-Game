...
public class UserCardCaptainsMasterRepository : IUserCardCaptainsMasterRepository
{
    public async Task<Master> GetCardCaptainMasterAsync(string id, string cardId)
    {
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                string selectSQL = @"
                SELECT *
                FROM user_card_captains_master
                WHERE user_id = @user_id AND master_id = @id AND user_card_captain_id = @card_id;
            ";
    ...
    public async Task InsertOrUpdateCardCaptainMasterAsync(string userId, UserMasters userMaster, string cardId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                SELECT COUNT(*) 
                FROM user_card_captains_master 
                WHERE user_id = @user_id AND user_card_captain_id = @card_id AND master_id = @master_id;
            ";
                ...
                await using (MySqlCommand checkCommand = new MySqlCommand(checkSQL, connection))
                {
                    ...
                    if (count > 0)
                    {
                        // UPDATE
                        string updateSQL = @"
                        UPDATE user_card_captains_master
                    ...
                    else
                    {
                        // INSERT
                        string insertSQL = @"
                        INSERT INTO user_card_captains_master 
                        (
                            user_id, user_card_captain_id, master_id, master_level, 
    ...
    public async Task<Master> GetSumCardCaptainsMasterAsync(string userId, string cardId)
    {
        Master master = new Master();
        string connectionString = DatabaseConfig.ConnectionString;
