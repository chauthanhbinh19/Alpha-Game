        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                WITH AggregatedModules AS (
                    SELECT user_technology_id, SUM(current_multiplier) AS total_module_mult
                    FROM user_technologies_module
                    GROUP BY user_technology_id
                ),
                AggregatedUpgrades AS (
                    SELECT user_technology_id, SUM(current_multiplier) AS total_upgrade_mult
                ...
                await using (MySqlCommand selectCommand = new MySqlCommand(selectSQL, connection))
                {
                    ...
                    await using (MySqlDataReader reader = await selectCommand.ExecuteReaderAsync())
                    {
                        while (await reader.ReadAsync())
                        {
                            ...
                            technology.UserModules = userModule;
                            technology.UserUpgrades = userUpgrade;
