...
public class UserAlchemiesRepository : IUserAlchemiesRepository
{
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                WITH AggregatedModules AS (
                    SELECT user_alchemy_id, SUM(current_multiplier) AS total_module_mult
                    FROM user_alchemies_module
                    GROUP BY user_alchemy_id
                ),
                AggregatedUpgrades AS (
                    SELECT user_alchemy_id, SUM(current_multiplier) AS total_upgrade_mult
                ...
                await using (MySqlCommand selectCommand = new MySqlCommand(selectSQL, connection))
                {
                    ...
                    await using (MySqlDataReader reader = await selectCommand.ExecuteReaderAsync())
                    {
                        while (await reader.ReadAsync())
                        {
                            ...
                            alchemy.UserModules = userModule;
                            alchemy.UserUpgrades = userUpgrade;
    ...
    public async Task<int> GetUserAlchemiesCountAsync(string userId, string search, string type, string rare)
    {
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                SELECT COUNT(*) 
                FROM alchemies m
                JOIN user_alchemies um ON m.id = um.alchemy_id AND m.is_active = TRUE AND m.is_deleted = FALSE
    ...
    public async Task<InsertOrUpdateResult<Alchemies>> InsertOrUpdateUserAlchemyAsync(string userId, Alchemies alchemy)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        await using MySqlConnection connection = new MySqlConnection(connectionString);
        ...
        try
        {
            ...
            // Query thực hiện Insert hoặc Update nếu đã tồn tại Composite Primary Key (user_id, alchemy_id)
            string upsertSQL = @"
            INSERT INTO user_alchemies (
                user_id, alchemy_id, rare, level, experience, star, quality, block, quantity,
                ...
                @user_id, @alchemy_id, @rare, 0, 0, 0, @quality, false, @quantity,
                @power, @health, @physical_attack, @physical_defense, @magical_attack, @magical_defense,
                @chemical_attack, @chemical_defense, @atomic_attack, @atomic_defense, @mental_attack, @mental_defense,
                @speed, @critical_damage_rate, @critical_rate, @critical_resistance_rate, @ignore_critical_rate,
                @penetration_rate, @penetration_resistance_rate,
            ...
            return InsertOrUpdateResult<Alchemies>.Failure();
        }
        catch (MySqlException ex)
        {
    ...
    string userId, List<Alchemies> alchemies)
    {
        ...
        {
            return new InsertOrUpdateResult<BatchOperationResultDTO<Alchemies>>
            {
                Data = new BatchOperationResultDTO<Alchemies>(),
        ...
        try
        {
            ...
            // 1. Query lấy TOÀN BỘ alchemy_id hiện có của User (Cực nhanh nhờ Index user_id)
            var existingIds = new HashSet<string>();
            string checkSql = "SELECT alchemy_id FROM user_alchemies WHERE user_id = @user_id;";
            ...
            // 2. Phân loại Alchemies giữ NGUYÊN VẸN OBJECT thuộc tính trong RAM C#
            var batchResult = new BatchOperationResultDTO<Alchemies>();
            foreach (var card in alchemies)
            {
            ...
            {
                var batch = alchemies.Skip(i).Take(batchSize).ToList();
            ...
            INSERT INTO user_alchemies (
                ...
                for (int j = 0; j < batch.Count; j++)
                {
                    ...
                    stringBuilder.Append($@"
                (@user_id, @alchemy_id_{j}, @rare_{j}, 0, 0, 0, @quality_{j}, 0, @quantity_{j},
                ...
                stringBuilder.Append(@"
            ON DUPLICATE KEY UPDATE
                quantity = COALESCE(user_alchemies.quantity, 0) + VALUES(quantity);
            ");
        ...
        catch (Exception ex)
        {
            ...
            return InsertOrUpdateResult<BatchOperationResultDTO<Alchemies>>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateUserAlchemyLevelAsync(string userId, Alchemies alchemy)
    {
        ...
        try
        {
            ...
            string updateSQL = @"
            UPDATE user_alchemies uc INNER JOIN alchemies c ON c.id = uc.alchemy_id
            SET 
                uc.level = @level, 
                uc.experience = @experience
            WHERE uc.user_id = @user_id 
              AND uc.alchemy_id = @alchemy_id
 ...
 AND c.is_active = TRUE AND c.is_deleted = FALSE;
        ...
        {
            Debug.LogError("Error UpdateUserAlchemyLevel: " + ex.Message);
 ...
 AND c.is_active = TRUE AND c.is_deleted = FALSE;
        ...
        {
            Debug.LogError("Error UpdateUserAlchemyStar: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
            ...
            try
            {
                ...
                await using (MySqlCommand selectCommand = new MySqlCommand(selectSQL, connection))
                {
                    ...
                    await using (var reader = await selectCommand.ExecuteReaderAsync())
                    {
                        if (await reader.ReadAsync())
                        {
                            sumAlchemies.Health = reader.GetDoubleSafe("health");
                            sumAlchemies.PhysicalAttack = reader.GetDoubleSafe("physical_attack");
                            sumAlchemies.PhysicalDefense = reader.GetDoubleSafe("physical_defense");
                            sumAlchemies.MagicalAttack = reader.GetDoubleSafe("magical_attack");
                            sumAlchemies.MagicalDefense = reader.GetDoubleSafe("magical_defense");
                            sumAlchemies.ChemicalAttack = reader.GetDoubleSafe("chemical_attack");
                            sumAlchemies.ChemicalDefense = reader.GetDoubleSafe("chemical_defense");
                            sumAlchemies.AtomicAttack = reader.GetDoubleSafe("atomic_attack");
                            sumAlchemies.AtomicDefense = reader.GetDoubleSafe("atomic_defense");
                            sumAlchemies.MentalAttack = reader.GetDoubleSafe("mental_attack");
                            sumAlchemies.MentalDefense = reader.GetDoubleSafe("mental_defense");
                            sumAlchemies.Speed = reader.GetDoubleSafe("speed");
                            sumAlchemies.CriticalDamageRate = reader.GetDoubleSafe("critical_damage_rate");
                            sumAlchemies.CriticalRate = reader.GetDoubleSafe("critical_rate");
                            sumAlchemies.CriticalResistanceRate = reader.GetDoubleSafe("critical_resistance_rate");
                            sumAlchemies.IgnoreCriticalRate = reader.GetDoubleSafe("ignore_critical_rate");
                            sumAlchemies.PenetrationRate = reader.GetDoubleSafe("penetration_rate");
                            sumAlchemies.PenetrationResistanceRate = reader.GetDoubleSafe("penetration_resistance_rate");
                            sumAlchemies.EvasionRate = reader.GetDoubleSafe("evasion_rate");
                            sumAlchemies.DamageAbsorptionRate = reader.GetDoubleSafe("damage_absorption_rate");
                            sumAlchemies.IgnoreDamageAbsorptionRate = reader.GetDoubleSafe("ignore_damage_absorption_rate");
                            sumAlchemies.AbsorbedDamageRate = reader.GetDoubleSafe("absorbed_damage_rate");
                            sumAlchemies.VitalityRegenerationRate = reader.GetDoubleSafe("vitality_regeneration_rate");
                            sumAlchemies.VitalityRegenerationResistanceRate = reader.GetDoubleSafe("vitality_regeneration_resistance_rate");
                            sumAlchemies.AccuracyRate = reader.GetDoubleSafe("accuracy_rate");
                            sumAlchemies.LifestealRate = reader.GetDoubleSafe("lifesteal_rate");
                            sumAlchemies.ShieldStrength = reader.GetDoubleSafe("shield_strength");
                            sumAlchemies.Tenacity = reader.GetDoubleSafe("tenacity");
                            sumAlchemies.ResistanceRate = reader.GetDoubleSafe("resistance_rate");
                            sumAlchemies.ComboRate = reader.GetDoubleSafe("combo_rate");
                            sumAlchemies.IgnoreComboRate = reader.GetDoubleSafe("ignore_combo_rate");
                            sumAlchemies.ComboDamageRate = reader.GetDoubleSafe("combo_damage_rate");
                            sumAlchemies.ComboResistanceRate = reader.GetDoubleSafe("combo_resistance_rate");
                            sumAlchemies.StunRate = reader.GetDoubleSafe("stun_rate");
                            sumAlchemies.IgnoreStunRate = reader.GetDoubleSafe("ignore_stun_rate");
                            sumAlchemies.ReflectionRate = reader.GetDoubleSafe("reflection_rate");
                            sumAlchemies.IgnoreReflectionRate = reader.GetDoubleSafe("ignore_reflection_rate");
                            sumAlchemies.ReflectionDamageRate = reader.GetDoubleSafe("reflection_damage_rate");
                            sumAlchemies.ReflectionResistanceRate = reader.GetDoubleSafe("reflection_resistance_rate");
                            sumAlchemies.Mana = reader.GetDoubleSafe("mana");
                            sumAlchemies.ManaRegenerationRate = reader.GetDoubleSafe("mana_regeneration_rate");
                            sumAlchemies.DamageToDifferentFactionRate = reader.GetDoubleSafe("damage_to_different_faction_rate");
                            sumAlchemies.ResistanceToDifferentFactionRate = reader.GetDoubleSafe("resistance_to_different_faction_rate");
                            sumAlchemies.DamageToSameFactionRate = reader.GetDoubleSafe("damage_to_same_faction_rate");
                            sumAlchemies.ResistanceToSameFactionRate = reader.GetDoubleSafe("resistance_to_same_faction_rate");
                            sumAlchemies.NormalDamageRate = reader.GetDoubleSafe("normal_damage_rate");
                            sumAlchemies.NormalResistanceRate = reader.GetDoubleSafe("normal_resistance_rate");
                            sumAlchemies.SkillDamageRate = reader.GetDoubleSafe("skill_damage_rate");
                            sumAlchemies.SkillResistanceRate = reader.GetDoubleSafe("skill_resistance_rate");
                        }
                    }
                }
            }
