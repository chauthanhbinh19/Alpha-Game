...
public class UserCardMilitariesRepository : IUserCardMilitariesRepository
{
    public async Task<List<CardMilitaries>> GetUserCardMilitariesAsync(string userId, string search, string type, int pageSize, int offset, string rare)
    {
        ...
        try
        {
            ...
            WITH AggregatedModules AS (
                    SELECT user_card_military_id, SUM(current_multiplier) AS total_module_mult
                    FROM user_card_militaries_module
                    GROUP BY user_card_military_id
                ),
                AggregatedUpgrades AS (
                    SELECT user_card_military_id, SUM(current_multiplier) AS total_upgrade_mult
                    FROM user_card_militaries_upgrade
                    GROUP BY user_card_military_id
                )
            SELECT 
                    uc.*, 
                ...
                INNER JOIN card_militaries c ON uc.card_military_id = c.id
                LEFT JOIN AggregatedModules am ON uc.card_military_id = am.user_card_military_id
                LEFT JOIN AggregatedUpgrades au ON uc.card_military_id = au.user_card_military_id
        ...
        ";
            ...
            while (await reader.ReadAsync())
            {
                ...
                cardMilitary.UserModules = userModule;
                cardMilitary.UserUpgrades = userUpgrade;
    ...
    public async Task<List<CardMilitaries>> GetUserCardMilitariesInTeamAsync(string userId)
    {
        List<CardMilitaries> cardMilitaries = new List<CardMilitaries>();
        string connectionString = DatabaseConfig.ConnectionString;
    ...
    public async Task<List<CardMilitaries>> GetUserCardMilitariesInTeamSimpleAsync(string userId, string teamId)
    {
        List<CardMilitaries> cardMilitaries = new List<CardMilitaries>();
        string connectionString = DatabaseConfig.ConnectionString;
    ...
    public async Task<bool> UpdateTeamUserCardMilitaryAsync(string userId, string teamId, string position, string cardId)
    {
        ...
        try
        {
            ...
            string updateSQL = @"
            UPDATE user_card_militaries uc INNER JOIN card_militaries c ON c.id = uc.card_military_id 
            SET team_id = @team_id, position = @position 
            WHERE uc.user_id = @user_id AND uc.card_military_id = @card_military_id AND c.is_active = TRUE AND c.is_deleted = FALSE;
        ";
    ...
    public async Task<int> GetUserCardMilitariesCountAsync(string userId, string search, string type, string rare)
    {
        ...
        try
        {
            ...
            JOIN user_card_militaries uc ON c.id = uc.card_military_id AND c.is_active = TRUE AND c.is_deleted = FALSE
            WHERE uc.user_id = @userId  AND c.is_active = TRUE AND c.is_deleted = FALSE
        ";
            if (!string.IsNullOrEmpty(type) && type != "All")
            {
    ...
    public async Task<InsertOrUpdateResult<CardMilitaries>> InsertOrUpdateUserCardMilitaryAsync(string userId, CardMilitaries cardMilitary)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        await using MySqlConnection connection = new MySqlConnection(connectionString);
        ...
        try
        {
            ...
            // Query thực hiện Insert hoặc Update nếu đã tồn tại Composite Primary Key (user_id, card_military_id)
            string upsertSQL = @"
            INSERT INTO user_card_militaries (
                user_id, card_military_id, rare, level, experience, star, quality, block, quantity,
                ...
                @user_id, @card_military_id, @rare, 0, 0, 0, @quality, false, @quantity,
                @power, @health, @physical_attack, @physical_defense, @magical_attack, @magical_defense,
                @chemical_attack, @chemical_defense, @atomic_attack, @atomic_defense, @mental_attack, @mental_defense,
                @speed, @critical_damage_rate, @critical_rate, @critical_resistance_rate, @ignore_critical_rate,
                @penetration_rate, @penetration_resistance_rate,
            ...
            return InsertOrUpdateResult<CardMilitaries>.Failure();
        }
        catch (MySqlException ex)
        {
    ...
    public async Task<InsertOrUpdateResult<BatchOperationResultDTO<CardMilitaries>>> InsertOrUpdateUserCardMilitariesBatchAsync(
    string userId, List<CardMilitaries> cardMilitaries)
    {
        if (cardMilitaries == null || cardMilitaries.Count == 0)
        {
            return new InsertOrUpdateResult<BatchOperationResultDTO<CardMilitaries>>
            {
                Data = new BatchOperationResultDTO<CardMilitaries>(),
        ...
        catch (Exception ex)
        {
            ...
            return InsertOrUpdateResult<BatchOperationResultDTO<CardMilitaries>>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateUserCardMilitaryLevelAsync(string userId, CardMilitaries cardMilitary)
    {
        ...
        ";
            ...
            updateCommand.Parameters.AddWithValue("@level", cardMilitary.Level);
            updateCommand.Parameters.AddWithValue("@experience", cardMilitary.Experience);
        ...
        {
            Debug.LogError("Error UpdateUserCardMilitaryLevel: " + ex.Message);
    ...
    public async Task<InsertOrUpdateResult<bool>> UpdateUserCardMilitaryStarAsync(string userId, CardMilitaries cardMilitary)
    {
        ...
        {
            Debug.LogError("Error UpdateUserCardMilitaryStar: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
    public async Task<CardMilitaries> GetUserCardMilitaryByIdAsync(string userId, string Id)
    {
        ...
        try
        {
            ...
            INNER JOIN card_militaries c ON uc.card_military_id = c.id
                LEFT JOIN AggregatedModules am ON uc.card_military_id = am.user_card_military_id
                LEFT JOIN AggregatedUpgrades au ON uc.card_military_id = au.user_card_military_id
            WHERE uc.card_military_id = @id AND uc.user_id = @user_id AND c.is_active = TRUE AND c.is_deleted = FALSE";
    ...
    public async Task<BaseStats> GetTeamTotalStatsAsync(string userId)
    {
        ...
        try
        {
            ...
            WITH CalculatedCards AS (
                ...
                FROM user_card_militaries uc
                INNER JOIN teams t ON uc.team_id = t.team_id AND t.is_main = 1
                LEFT JOIN user_card_militaries_module ubm ON uc.card_military_id = ubm.user_card_military_id
                LEFT JOIN user_card_militaries_upgrade ubu ON uc.card_military_id = ubu.user_card_military_id
                WHERE uc.user_id = @user_id AND uc.team_id IS NOT NULL
            )
            SELECT 
