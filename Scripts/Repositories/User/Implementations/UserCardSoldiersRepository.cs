...
public class UserCardSoldiersRepository : IUserCardSoldiersRepository
{
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                WITH AggregatedModules AS (
                    SELECT user_card_soldier_id, SUM(current_multiplier) AS total_module_mult
                    FROM user_card_soldiers_module
                    GROUP BY user_card_soldier_id
                ),
                AggregatedUpgrades AS (
                    SELECT user_card_soldier_id, SUM(current_multiplier) AS total_upgrade_mult
                    FROM user_card_soldiers_upgrade
                    GROUP BY user_card_soldier_id
                )
                SELECT 
                uc.*, 
            ...
            INNER JOIN card_soldiers c ON uc.card_soldier_id = c.id
                LEFT JOIN AggregatedModules am ON uc.card_soldier_id = am.user_card_soldier_id
                LEFT JOIN AggregatedUpgrades au ON uc.card_soldier_id = au.user_card_soldier_id
            ...
            ";
                ...
                while (await reader.ReadAsync())
                {
                    ...
                    cardSoldier.UserModules = userModule;
                    cardSoldier.UserUpgrades = userUpgrade;
    ...
    public async Task<List<CardSoldiers>> GetUserCardSoldiersInTeamAsync(string userId)
    {
        List<CardSoldiers> cardSoldiers = new List<CardSoldiers>();
        string connectionString = DatabaseConfig.ConnectionString;
    ...
    public async Task<List<CardSoldiers>> GetUserCardSoldiersInTeamSimpleAsync(string userId, string teamId)
    {
        List<CardSoldiers> cardSoldiers = new List<CardSoldiers>();
        string connectionString = DatabaseConfig.ConnectionString;
    ...
    public async Task<Dictionary<string, int>> GetUniqueCardSoldiersTypesTeamAsync(string userId, string teamId)
    {
    ...
    public async Task<bool> UpdateTeamCardSoldierAsync(string userId, string teamId, string position, string cardId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                string updateSQL = @"
            UPDATE user_card_soldiers uc INNER JOIN card_soldiers c ON c.id = uc.card_soldier_id 
            SET team_id = @team_id, position = @position 
            WHERE uc.user_id = @user_id AND uc.card_soldier_id = @card_soldier_id AND c.is_active = TRUE AND c.is_deleted = FALSE;
        ";
    ...
    public async Task<bool> IsCardInTeamAsync(string userId, string cardId)
    {
        ...
        ";
            ...
            command.Parameters.AddWithValue("@card_soldier_id", cardId);
    ...
    public async Task<int> GetUserCardSoldiersCountAsync(string userId, string search, string type, string rare)
    {
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                JOIN user_card_soldiers uc ON c.id = uc.card_soldier_id AND c.is_active = TRUE AND c.is_deleted = FALSE
                WHERE uc.user_id = @userId  AND c.is_active = TRUE AND c.is_deleted = FALSE
            ";
                if (!string.IsNullOrEmpty(type) && type != "All")
                {
    ...
    public async Task<InsertOrUpdateResult<CardSoldiers>> InsertOrUpdateUserCardSoldierAsync(string userId, CardSoldiers cardSoldier)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        await using MySqlConnection connection = new MySqlConnection(connectionString);
        ...
        try
        {
            ...
            // Query thực hiện Insert hoặc Update nếu đã tồn tại Composite Primary Key (user_id, card_soldier_id)
            string upsertSQL = @"
            INSERT INTO user_card_soldiers (
                ...
                @user_id, @card_soldier_id, @rare, 0, 0, 0, @quality, false, @quantity,
                @power, @health, @physical_attack, @physical_defense, @magical_attack, @magical_defense,
                @chemical_attack, @chemical_defense, @atomic_attack, @atomic_defense, @mental_attack, @mental_defense,
                @speed, @critical_damage_rate, @critical_rate, @critical_resistance_rate, @ignore_critical_rate,
                @penetration_rate, @penetration_resistance_rate,
            ...
            // Add Parameters
            command.Parameters.AddWithValue("@user_id", userId);
            command.Parameters.AddWithValue("@card_soldier_id", cardSoldier.Id);
            ...
            return InsertOrUpdateResult<CardSoldiers>.Failure();
        }
        catch (MySqlException ex)
        {
    ...
    public async Task<InsertOrUpdateResult<BatchOperationResultDTO<CardSoldiers>>> InsertOrUpdateUserCardSoldiersBatchAsync(
    string userId, List<CardSoldiers> cardSoldiers)
    {
        if (cardSoldiers == null || cardSoldiers.Count == 0)
        {
            return new InsertOrUpdateResult<BatchOperationResultDTO<CardSoldiers>>
            {
                Data = new BatchOperationResultDTO<CardSoldiers>(),
        ...
        try
        {
            ...
            string checkSql = "SELECT card_soldier_id FROM user_card_soldiers WHERE user_id = @user_id;";
            ...
            var batchResult = new BatchOperationResultDTO<CardSoldiers>();
            foreach (var card in cardSoldiers)
            {
            ...
            int totalCards = cardSoldiers.Count;
            ...
            INSERT INTO user_card_soldiers (
                ...
                for (int j = 0; j < currentBatchCount; j++)
                {
                    ...
                    var c = cardSoldiers[i + j];

                    stringBuilder.Append($"(@user_id, @card_soldier_id_{j}, @rare_{j}, 0, 0, 0, @quality_{j}, 0, @quantity_{j}, ")
                    ...
                    p.AddWithValue($"@card_soldier_id_{j}", c.Id);
        ...
        catch (Exception ex)
        {
            ...
            return InsertOrUpdateResult<BatchOperationResultDTO<CardSoldiers>>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateUserCardSoldierLevelAsync(string userId, CardSoldiers cardSoldier)
    {
        if (cardSoldier == null)
        {
        ...
        {
            Debug.LogError("Error UpdateUserCardSoldierLevel: " + ex.Message);
    ...
    public async Task<InsertOrUpdateResult<bool>> UpdateUserCardSoldierStarAsync(string userId, CardSoldiers cardSoldier)
    {
        ...
        {
            Debug.LogError("Error UpdateUserCardSoldierStar: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                INNER JOIN card_soldiers c ON uc.card_soldier_id = c.id
                    LEFT JOIN AggregatedModules am ON uc.card_soldier_id = am.user_card_soldier_id
                    LEFT JOIN AggregatedUpgrades au ON uc.card_soldier_id = au.user_card_soldier_id
                WHERE uc.card_soldier_id = @id AND uc.user_id = @user_id AND c.is_active = TRUE AND c.is_deleted = FALSE";
    ...
    {
        List<string> cardSoldierIds = new List<string>();
        string connectionString = DatabaseConfig.ConnectionString;

        string selectSQL = @"
        SELECT uc.card_soldier_id
        FROM user_card_soldiers uc
            ...
            try
            {
                ...
                await using (MySqlCommand selectCommand = new MySqlCommand(selectSQL, connection))
                {
                    ...
                    await using (MySqlDataReader reader = (MySqlDataReader)await selectCommand.ExecuteReaderAsync())
                    {
                        while (await reader.ReadAsync())
                        {
                            if (!reader.IsDBNull(reader.GetOrdinal("card_soldier_id")))
                            {
                                cardSoldierIds.Add(reader.GetString("card_soldier_id"));
                            }
                        }
                    }
                }
    ...
    public async Task<BaseStats> GetTeamTotalStatsAsync(string userId)
    {
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                FROM user_card_soldiers uc
                INNER JOIN teams t ON uc.team_id = t.team_id AND t.is_main = 1
                LEFT JOIN user_card_soldiers_module ubm ON uc.card_soldier_id = ubm.user_card_soldier_id
                LEFT JOIN user_card_soldiers_upgrade ubu ON uc.card_soldier_id = ubu.user_card_soldier_id
                WHERE uc.user_id = @user_id AND uc.team_id IS NOT NULL
            )
            SELECT 
