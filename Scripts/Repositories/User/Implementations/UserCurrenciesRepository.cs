...
public class UserCurrenciesRepository : IUserCurrenciesRepository
{
                ...
                await using (MySqlCommand selectCommand = new MySqlCommand(selectSQL, connection))
                {
                    ...
                    await using (MySqlDataReader reader = await selectCommand.ExecuteReaderAsync())
                    {
                        while (await reader.ReadAsync())
                        {
                            ...
                            int quantity = reader.GetIntSafe("quantity");
    ...
    public async Task UpdateUserCurrencyAsync(string userId, string currencyId, double price)
    {
        string connectionString = DatabaseConfig.ConnectionString;
                    ...
                    await using (MySqlDataReader reader = await selectCommand.ExecuteReaderAsync())
                    {
                        if (await reader.ReadAsync())
                        {
                            currency = new Currencies
                            {
                                ...
                                Name = reader.GetStringSafe("name"),
                                Image = reader.GetStringSafe("image"),
                                Quantity = reader.GetIntSafe("price"), // price lưu vào Quantity
                            };
                        }
                    }
                }
                    ...
                    await using (MySqlDataReader reader = await selectCommand.ExecuteReaderAsync())
                    {
                        if (await reader.ReadAsync())
                        {
                            currency = new Currencies
                            {
                                ...
                                Quantity = reader.GetIntSafe("trade_price")
                            };
                        }
                    }
                }
    ...
    public async Task<Currencies> GetUserOutfitPriceAsync(string Id)
    {
        Currencies currency = new Currencies();
        string connectionString = DatabaseConfig.ConnectionString;
    ...
    public async Task<List<Currencies>> GetAvatarsCurrencyAsync(string type)
    {
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                string selectSQL = @"SELECT DISTINCT c.id, c.image, c.name, uc.quantity
                             FROM avatars a
                             JOIN avatar_trade at ON a.id = at.avatar_id
                             JOIN currencies c ON at.currency_id = c.id
                             JOIN user_currencies uc ON c.id = uc.currency_id;";
    ...
    public async Task<List<Currencies>> GetOutfitsCurrencyAsync(string type)
    {
        List<Currencies> currencies = new List<Currencies>();
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                SELECT DISTINCT c.id, c.image, c.name, uc.quantity
                FROM outfits a
                JOIN outfit_trade at ON a.id = at.outfit_id
                JOIN currencies c ON at.currency_id = c.id
                JOIN user_currencies uc ON c.id = uc.currency_id";
