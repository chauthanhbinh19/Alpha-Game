...
public class UserBooksRepository : IUserBooksRepository
{
    ...
    public async Task<InsertOrUpdateResult<Books>> InsertOrUpdateUserBookAsync(string userId, Books book)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        await using MySqlConnection connection = new MySqlConnection(connectionString);
        ...
        try
        {
            ...
            // Query thực hiện Insert hoặc Update nếu đã tồn tại Composite Primary Key (user_id, book_id)
            string upsertSQL = @"
            INSERT INTO user_books (
                ...
                @user_id, @book_id, @rare, 0, 0, 0, @quality, false, @quantity,
                @power, @health, @physical_attack, @physical_defense, @magical_attack, @magical_defense,
                @chemical_attack, @chemical_defense, @atomic_attack, @atomic_defense, @mental_attack, @mental_defense,
                @speed, @critical_damage_rate, @critical_rate, @critical_resistance_rate, @ignore_critical_rate,
                @penetration_rate, @penetration_resistance_rate,
            ...
            return InsertOrUpdateResult<Books>.Failure();
        }
        catch (MySqlException ex)
        {
    ...
    string userId, List<Books> books)
    {
        ...
        {
            return new InsertOrUpdateResult<BatchOperationResultDTO<Books>>
            {
                Data = new BatchOperationResultDTO<Books>(),
        ...
        try
        {
            ...
            // 1. Query lấy TOÀN BỘ book_id hiện có của User (Cực nhanh nhờ Index user_id)
            var existingIds = new HashSet<string>();
            string checkSql = "SELECT book_id FROM user_books WHERE user_id = @user_id;";
            ...
            // 2. Phân loại Books giữ NGUYÊN VẸN OBJECT thuộc tính trong RAM C#
            var batchResult = new BatchOperationResultDTO<Books>();
        ...
        catch (Exception ex)
        {
            ...
            return InsertOrUpdateResult<BatchOperationResultDTO<Books>>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateUserBookLevelAsync(string userId, Books book)
    {
        ...
        {
            Debug.LogError("Error UpdateUserBookLevel: " + ex.Message);
    ...
    public async Task<InsertOrUpdateResult<bool>> UpdateUserBookStarAsync(string userId, Books book)
    {
        ...
        {
            Debug.LogError("Error UpdateUserBookStar: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
