        ...
        try
        {
            ...
            for (int i = 0; i < books.Count; i += batchSize)
            {
                ...
                ON DUPLICATE KEY UPDATE
                    quantity = user_books.quantity + VALUES(quantity);
                ");
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                await using (MySqlCommand updateCommand = new MySqlCommand(updateSQL, connection))
                {
                    ...
                    updateCommand.Parameters.AddWithValue("@comboRate", book.ComboRate);
    ...
    public async Task<List<Books>> GetAllUserBooksInTeamAsync(string user_id)
    {
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                string selectSQL = @"SELECT uc.*, c.name, c.image, c.type, c.description
                FROM user_books uc
                LEFT JOIN books c ON uc.book_id = c.id 
