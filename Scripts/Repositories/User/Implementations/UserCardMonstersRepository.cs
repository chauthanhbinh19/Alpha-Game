...
public class UserCardMonstersRepository : IUserCardMonstersRepository
{
    ...
    public async Task<List<CardMonsters>> GetUserCardMonstersInTeamAsync(string userId)
    {
        List<CardMonsters> cardMonsters = new List<CardMonsters>();
        string connectionString = DatabaseConfig.ConnectionString;
    ...
    public async Task<bool> UpdateTeamUserCardMonsterAsync(string userId, string teamId, string position, string cardId)
    {
        ...
        try
        {
            ...
            string updateSQL = @"
            UPDATE user_card_monsters uc INNER JOIN card_monsters c ON c.id = uc.card_monster_id 
            SET team_id = @team_id, position = @position 
            WHERE uc.user_id = @user_id AND uc.card_monster_id = @card_monster_id AND c.is_active = TRUE AND c.is_deleted = FALSE;
        ";
    ...
    public async Task<int> GetUserCardMonstersCountAsync(string userId, string search, string type, string rare)
    {
        ...
        try
        {
            ...
            JOIN user_card_monsters uc ON c.id = uc.card_monster_id AND c.is_active = TRUE AND c.is_deleted = FALSE
            WHERE uc.user_id = @userId  AND c.is_active = TRUE AND c.is_deleted = FALSE
        ";
            if (!string.IsNullOrEmpty(type) && type != "All")
            {
    ...
    public async Task<CardMonsters> GetUserCardMonsterByIdAsync(string userId, string Id)
    {
        ...
        try
        {
            ...
            INNER JOIN card_monsters c ON uc.card_monster_id = c.id
                LEFT JOIN AggregatedModules am ON uc.card_monster_id = am.user_card_monster_id
                LEFT JOIN AggregatedUpgrades au ON uc.card_monster_id = au.user_card_monster_id
            WHERE uc.card_monster_id = @id AND uc.user_id = @user_id AND c.is_active = TRUE AND c.is_deleted = FALSE";
    ...
    {
        List<string> cardMonsterIds = new List<string>();
        string connectionString = DatabaseConfig.ConnectionString;

        string selectSQL = @"
        SELECT uc.card_monster_id
        FROM user_card_monsters uc
            ...
            try
            {
                ...
                await using (MySqlCommand selectCommand = new MySqlCommand(selectSQL, connection))
                {
                    ...
                    await using (MySqlDataReader reader = (MySqlDataReader)await selectCommand.ExecuteReaderAsync())
                    {
                        while (await reader.ReadAsync())
                        {
                            if (!reader.IsDBNull(reader.GetOrdinal("card_monster_id")))
                            {
                                cardMonsterIds.Add(reader.GetString("card_monster_id"));
                            }
                        }
                    }
                }
