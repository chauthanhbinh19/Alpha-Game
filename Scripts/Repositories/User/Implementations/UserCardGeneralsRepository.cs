...
{
    public async Task<List<CardGenerals>> GetUserCardGeneralsAsync(string userId, string search, string type, int pageSize, int offset, string rare)
    {
        ...
        try
        {
            ...
            WITH AggregatedModules AS (
                    SELECT user_card_general_id, SUM(current_multiplier) AS total_module_mult
                    FROM user_card_generals_module
                    GROUP BY user_card_general_id
                ),
                AggregatedUpgrades AS (
                    SELECT user_card_general_id, SUM(current_multiplier) AS total_upgrade_mult
                    FROM user_card_generals_upgrade
                    GROUP BY user_card_general_id
                )
            SELECT 
                    uc.*, 
                ...
                INNER JOIN card_generals c ON uc.card_general_id = c.id
                LEFT JOIN AggregatedModules am ON uc.card_general_id = am.user_card_general_id
                LEFT JOIN AggregatedUpgrades au ON uc.card_general_id = au.user_card_general_id
        ...
        ";
            ...
            while (await reader.ReadAsync())
            {
                ...
                cardGeneral.UserModules = userModule;
                cardGeneral.UserUpgrades = userUpgrade;
    ...
    public async Task<List<CardGenerals>> GetUserCardGeneralsTeamAsync(string userId, string teamId, string position)
    {
        List<CardGenerals> cardGenerals = new List<CardGenerals>();
        string connectionString = DatabaseConfig.ConnectionString;
    ...
    public async Task<List<CardGenerals>> GetUserCardGeneralsTeamWithoutPositionAsync(string userId, string teamId)
    {
        List<CardGenerals> cardGenerals = new List<CardGenerals>();
        string connectionString = DatabaseConfig.ConnectionString;
    ...
    public async Task<List<CardGenerals>> GetUserCardGeneralsInTeamAsync(string userId)
    {
        List<CardGenerals> cardGenerals = new List<CardGenerals>();
        string connectionString = DatabaseConfig.ConnectionString;
    ...
    public async Task<List<CardGenerals>> GetUserCardGeneralsInTeamSimpleAsync(string userId, string teamId)
    {
        List<CardGenerals> cardGenerals = new List<CardGenerals>();
        string connectionString = DatabaseConfig.ConnectionString;
    ...
    public async Task<bool> UpdateTeamUserCardGeneralAsync(string userId, string teamId, string position, string cardId)
    {
        ...
        try
        {
            ...
            string updateSQL = @"
            UPDATE user_card_generals uc INNER JOIN card_generals c ON c.id = uc.card_general_id 
            SET team_id = @team_id, position = @position 
            WHERE uc.user_id = @user_id AND uc.card_general_id = @card_general_id AND c.is_active = TRUE AND c.is_deleted = FALSE;
        ";
            ...
            updateCommand.Parameters.AddWithValue("@card_general_id", cardId);
    ...
    public async Task<int> GetUserCardGeneralsCountAsync(string userId, string search, string type, string rare)
    {
        ...
        try
        {
            ...
            JOIN user_card_generals uc ON c.id = uc.card_general_id AND c.is_active = TRUE AND c.is_deleted = FALSE
            WHERE uc.user_id = @userId  AND c.is_active = TRUE AND c.is_deleted = FALSE
        ";
            if (!string.IsNullOrEmpty(type) && type != "All")
            {
    ...
    public async Task<InsertOrUpdateResult<CardGenerals>> InsertOrUpdateUserCardGeneralAsync(string userId, CardGenerals cardGeneral)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        await using MySqlConnection connection = new MySqlConnection(connectionString);
        ...
        try
        {
            ...
            // Query thực hiện Insert hoặc Update nếu đã tồn tại Composite Primary Key (user_id, card_general_id)
            string upsertSQL = @"
            INSERT INTO user_card_generals (
                ...
                @user_id, @card_general_id, @rare, 0, 0, 0, @quality, false, @quantity,
                @power, @health, @physical_attack, @physical_defense, @magical_attack, @magical_defense,
                @chemical_attack, @chemical_defense, @atomic_attack, @atomic_defense, @mental_attack, @mental_defense,
                @speed, @critical_damage_rate, @critical_rate, @critical_resistance_rate, @ignore_critical_rate,
                @penetration_rate, @penetration_resistance_rate,
            ...
            return InsertOrUpdateResult<CardGenerals>.Failure();
        }
        catch (MySqlException ex)
        {
    ...
    public async Task<InsertOrUpdateResult<BatchOperationResultDTO<CardGenerals>>> InsertOrUpdateUserCardGeneralsBatchAsync(
    string userId, List<CardGenerals> cardGenerals)
    {
        if (cardGenerals == null || cardGenerals.Count == 0)
        {
            return new InsertOrUpdateResult<BatchOperationResultDTO<CardGenerals>>
            {
                Data = new BatchOperationResultDTO<CardGenerals>(),
        ...
        try
        {
            ...
            string checkSql = "SELECT card_general_id FROM user_card_generals WHERE user_id = @user_id;";
            ...
            var batchResult = new BatchOperationResultDTO<CardGenerals>();
            ...
            int totalCards = cardGenerals.Count;
            ...
            INSERT INTO user_card_generals (
                ...
                for (int j = 0; j < currentBatchCount; j++)
                {
                    ...
                    var c = cardGenerals[i + j];

                    stringBuilder.Append($"(@user_id, @card_general_id_{j}, @rare_{j}, 0, 0, 0, @quality_{j}, 0, @quantity_{j}, ")
                    ...
                    p.AddWithValue($"@card_general_id_{j}", c.Id);
                ...
                stringBuilder.Append(@"
            ON DUPLICATE KEY UPDATE
                quantity = COALESCE(user_card_generals.quantity, 0) + VALUES(quantity);");
        ...
        catch (Exception ex)
        {
            ...
            return InsertOrUpdateResult<BatchOperationResultDTO<CardGenerals>>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateUserCardGeneralLevelAsync(string userId, CardGenerals cardGeneral)
    {
        if (cardGeneral == null)
        {
        ...
        ";
            ...
            updateCommand.Parameters.AddWithValue("@level", cardGeneral.Level);
            updateCommand.Parameters.AddWithValue("@experience", cardGeneral.Experience);
        ...
        {
            Debug.LogError("Error UpdateUserCardGeneralLevel: " + ex.Message);
    ...
    public async Task<InsertOrUpdateResult<bool>> UpdateUserCardGeneralStarAsync(string userId, CardGenerals cardGeneral)
    {
        ...
        ";
            ...
            updateCommand.Parameters.AddWithValue("@user_id", userId);
            updateCommand.Parameters.AddWithValue("@card_general_id", cardGeneral.Id);
            updateCommand.Parameters.AddWithValue("@star", cardGeneral.Star);
        ...
        {
            Debug.LogError("Error UpdateUserCardGeneralStar: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
    public async Task<CardGenerals> GetUserCardGeneralByIdAsync(string userId, string Id)
    {
        CardGenerals cardGeneral = new CardGenerals();
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            INNER JOIN card_generals c ON uc.card_general_id = c.id
                LEFT JOIN AggregatedModules am ON uc.card_general_id = am.user_card_general_id
                LEFT JOIN AggregatedUpgrades au ON uc.card_general_id = au.user_card_general_id
            WHERE uc.card_general_id = @id AND uc.user_id = @user_id AND c.is_active = TRUE AND c.is_deleted = FALSE";
    ...
    {
        List<string> cardGeneralIds = new List<string>();
        string connectionString = DatabaseConfig.ConnectionString;

        string selectSQL = @"
        SELECT uc.card_general_id
        FROM user_card_generals uc
            ...
            try
            {
                ...
                await using (MySqlCommand selectCommand = new MySqlCommand(selectSQL, connection))
                {
                    ...
                    await using (MySqlDataReader reader = (MySqlDataReader)await selectCommand.ExecuteReaderAsync())
                    {
                        while (await reader.ReadAsync())
                        {
                            if (!reader.IsDBNull(reader.GetOrdinal("card_general_id")))
                            {
                                cardGeneralIds.Add(reader.GetString("card_general_id"));
                            }
                        }
                    }
                }
    ...
    public async Task<BaseStats> GetTeamTotalStatsAsync(string userId)
    {
        ...
        try
        {
            ...
            WITH CalculatedCards AS (
                ...
                FROM user_card_generals uc
                INNER JOIN teams t ON uc.team_id = t.team_id AND t.is_main = 1
                LEFT JOIN user_card_generals_module ubm ON uc.card_general_id = ubm.user_card_general_id
                LEFT JOIN user_card_generals_upgrade ubu ON uc.card_general_id = ubu.user_card_general_id
                WHERE uc.user_id = @user_id AND uc.team_id IS NOT NULL
            )
            SELECT 
