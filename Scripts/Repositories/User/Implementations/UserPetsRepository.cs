...
public class UserPetsRepository : IUserPetsRepository
{
    ...
    public async Task<InsertOrUpdateResult<Pets>> InsertOrUpdateUserPetAsync(string userId, Pets pet)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        await using MySqlConnection connection = new MySqlConnection(connectionString);
        ...
        try
        {
            ...
            // Query thực hiện Insert hoặc Update nếu đã tồn tại Composite Primary Key (user_id, pet_id)
            string upsertSQL = @"
            INSERT INTO user_pets (
                ...
                @user_id, @pet_id, @rare, 0, 0, 0, @quality, false, @quantity,
                @power, @health, @physical_attack, @physical_defense, @magical_attack, @magical_defense,
                @chemical_attack, @chemical_defense, @atomic_attack, @atomic_defense, @mental_attack, @mental_defense,
                @speed, @critical_damage_rate, @critical_rate, @critical_resistance_rate, @ignore_critical_rate,
                @penetration_rate, @penetration_resistance_rate,
            ...
            return InsertOrUpdateResult<Pets>.Failure();
        }
        catch (MySqlException ex)
        {
    ...
    public async Task<InsertOrUpdateResult<BatchOperationResultDTO<Pets>>> InsertOrUpdateUserPetsBatchAsync(
    string userId, List<Pets> pets)
    {
        if (pets == null || pets.Count == 0)
        {
            return new InsertOrUpdateResult<BatchOperationResultDTO<Pets>>
            {
                Data = new BatchOperationResultDTO<Pets>(),
        ...
        try
        {
            ...
            // 1. Query lấy TOÀN BỘ pet_id hiện có của User (Cực nhanh nhờ Index user_id)
            var existingIds = new HashSet<string>();
            string checkSql = "SELECT pet_id FROM user_pets WHERE user_id = @user_id;";
            ...
            // 2. Phân loại Pets giữ NGUYÊN VẸN OBJECT thuộc tính trong RAM C#
            var batchResult = new BatchOperationResultDTO<Pets>();
            foreach (var card in pets)
            {
            ...
            {
                var batch = pets.Skip(i).Take(batchSize).ToList();
            ...
            INSERT INTO user_pets (
                ...
                for (int j = 0; j < batch.Count; j++)
                {
                    ...
                    stringBuilder.Append($@"
                (@user_id, @pet_id_{j}, @rare_{j}, 0, 0, 0, @quality_{j}, 0, @quantity_{j},
                ...
                @skill_damage_rate_{j}, @skill_resistance_rate_{j}
                    ...
                    {
                    new MySqlParameter($"@pet_id_{j}", c.Id),
                ...
                stringBuilder.Append(@"
            ON DUPLICATE KEY UPDATE
                quantity = COALESCE(user_pets.quantity, 0) + VALUES(quantity);
            ");
        ...
        catch (Exception ex)
        {
            ...
            return InsertOrUpdateResult<BatchOperationResultDTO<Pets>>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateUserPetLevelAsync(string userId, Pets pet)
    {
        ...
        ";
            ...
            updateCommand.Parameters.AddWithValue("@level", pet.Level);
            updateCommand.Parameters.AddWithValue("@experience", pet.Experience);
        ...
        {
            Debug.LogError("Error UpdateUserPetLevel: " + ex.Message);
    ...
    public async Task<InsertOrUpdateResult<bool>> UpdateUserPetStarAsync(string userId, Pets pet)
    {
        ...
        {
            Debug.LogError("Error UpdateUserPetStar: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
