...
public class UserCardCaptainsRepository : IUserCardCaptainsRepository
{
    ...
    public async Task<List<CardCaptains>> GetUserCardCaptainsInTeamAsync(string userId)
    {
        List<CardCaptains> cardCaptains = new List<CardCaptains>();
        string connectionString = DatabaseConfig.ConnectionString;
    ...
    string userId, List<CardCaptains> cardHeroes)
    {
        ...
        try
        {
            ...
            INSERT INTO user_card_captains (
                ...
                for (int j = 0; j < currentBatchCount; j++)
                {
                    ...
                    stringBuilder.Append($"(@user_id, @card_captain_id_{j}, @rare_{j}, 0, 0, 0, @quality_{j}, 0, @quantity_{j}, ")
                    ...
                    p.AddWithValue($"@card_captain_id_{j}", c.Id);
 ...
 AND c.is_active = TRUE AND c.is_deleted = FALSE;
    ...
    {
        List<string> cardCaptainIds = new List<string>();
        string connectionString = DatabaseConfig.ConnectionString;

        string selectSQL = @"
        SELECT uc.card_captain_id
        FROM user_card_captains uc
            ...
            try
            {
                ...
                await using (MySqlCommand selectCommand = new MySqlCommand(selectSQL, connection))
                {
                    ...
                    await using (MySqlDataReader reader = (MySqlDataReader)await selectCommand.ExecuteReaderAsync())
                    {
                        while (await reader.ReadAsync())
                        {
                            if (!reader.IsDBNull(reader.GetOrdinal("card_captain_id")))
                            {
                                cardCaptainIds.Add(reader.GetString("card_captain_id"));
                            }
                        }
                    }
                }
