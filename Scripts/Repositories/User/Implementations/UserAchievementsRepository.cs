...
public class UserAchievementsRepository : IUserAchievementsRepository
{
    ...
    public async Task<InsertOrUpdateResult<Achievements>> InsertOrUpdateUserAchievementAsync(string userId, Achievements achievement)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        await using MySqlConnection connection = new MySqlConnection(connectionString);
        ...
        try
        {
            ...
            // Query thực hiện Insert hoặc Update nếu đã tồn tại Composite Primary Key (user_id, achievement_id)
            string upsertSQL = @"
            INSERT INTO user_achievements (
                ...
                @user_id, @achievement_id, @rare, 0, 0, 0, @quality, false, @quantity,
                @power, @health, @physical_attack, @physical_defense, @magical_attack, @magical_defense,
                @chemical_attack, @chemical_defense, @atomic_attack, @atomic_defense, @mental_attack, @mental_defense,
                @speed, @critical_damage_rate, @critical_rate, @critical_resistance_rate, @ignore_critical_rate,
                @penetration_rate, @penetration_resistance_rate,
    ...
    string userId, List<Achievements> achievements)
    {
        ...
        {
            return new InsertOrUpdateResult<BatchOperationResultDTO<Achievements>>
            {
                Data = new BatchOperationResultDTO<Achievements>(),
        ...
        try
        {
            ...
            // 1. Query lấy TOÀN BỘ achievement_id hiện có của User (Cực nhanh nhờ Index user_id)
            var existingIds = new HashSet<string>();
            string checkSql = "SELECT achievement_id FROM user_achievements WHERE user_id = @user_id;";
            ...
            // 2. Phân loại Achievements giữ NGUYÊN VẸN OBJECT thuộc tính trong RAM C#
            var batchResult = new BatchOperationResultDTO<Achievements>();
            foreach (var card in achievements)
            {
        ...
        catch (Exception ex)
        {
            ...
            return InsertOrUpdateResult<BatchOperationResultDTO<Achievements>>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateUserAchievementLevelAsync(string userId, Achievements achievement)
    {
        ...
        {
            Debug.LogError("Error UpdateUserAchievementLevel: " + ex.Message);
    ...
    public async Task<InsertOrUpdateResult<bool>> UpdateUserAchievementStarAsync(string userId, Achievements achievement)
    {
        ...
        {
            Debug.LogError("Error UpdateUserAchievementStar: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
