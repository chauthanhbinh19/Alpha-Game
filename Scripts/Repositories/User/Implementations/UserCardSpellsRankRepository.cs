...
public class UserCardSpellsRankRepository : IUserCardSpellsRankRepository
{
    public async Task<Rank> GetCardSpellRankAsync(string id, string cardId)
    {
        Rank rank = new Rank();
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                string selectSQL = @"
                SELECT *
                FROM user_card_spells_rank
                WHERE user_id = @user_id AND rank_id = @id AND user_card_spell_id = @card_id;
            ";
    ...
    public async Task InsertOrUpdateCardSpellRankAsync(string userId, UserRanks userRank, string cardId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                SELECT COUNT(*) 
                FROM user_card_spells_rank 
                WHERE user_id = @user_id AND user_card_spell_id = @card_id AND rank_id = @rank_id;
            ";
                ...
                await using (MySqlCommand checkCommand = new MySqlCommand(checkSQL, connection))
                {
                    ...
                    else
                    {
                        // INSERT
                        string insertSQL = @"
                        INSERT INTO user_card_spells_rank 
                        (
                            user_id, user_card_spell_id, rank_id, rank_level, 
    ...
    public async Task<Rank> GetSumCardSpellsRankAsync(string userId, string cardId)
    {
        Rank rank = new Rank();
        string connectionString = DatabaseConfig.ConnectionString;
