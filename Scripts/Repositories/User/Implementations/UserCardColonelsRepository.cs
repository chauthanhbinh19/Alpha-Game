...
{
    public async Task<List<CardColonels>> GetUserCardColonelsAsync(string userId, string search, string type, int pageSize, int offset, string rare)
    {
        ...
        try
        {
            ...
            WITH AggregatedModules AS (
                    SELECT user_card_colonel_id, SUM(current_multiplier) AS total_module_mult
                    FROM user_card_colonels_module
                    GROUP BY user_card_colonel_id
                ),
                AggregatedUpgrades AS (
                    SELECT user_card_colonel_id, SUM(current_multiplier) AS total_upgrade_mult
                    FROM user_card_colonels_upgrade
                    GROUP BY user_card_colonel_id
                )
            SELECT 
                    uc.*, 
                ...
                INNER JOIN card_colonels c ON uc.card_colonel_id = c.id
                LEFT JOIN AggregatedModules am ON uc.card_colonel_id = am.user_card_colonel_id
                LEFT JOIN AggregatedUpgrades au ON uc.card_colonel_id = au.user_card_colonel_id
        ...
        ";
            ...
            while (await reader.ReadAsync())
            {
                ...
                cardColonel.UserModules = userModule;
                cardColonel.UserUpgrades = userUpgrade;
    ...
    public async Task<List<CardColonels>> GetUserCardColonelsTeamAsync(string userId, string teamId, string position)
    {
        List<CardColonels> cardColonels = new List<CardColonels>();
        string connectionString = DatabaseConfig.ConnectionString;
    ...
    public async Task<List<CardColonels>> GetUserCardColonelsTeamWithoutPositionAsync(string userId, string teamId)
    {
        List<CardColonels> cardColonels = new List<CardColonels>();
        string connectionString = DatabaseConfig.ConnectionString;
    ...
    public async Task<List<CardColonels>> GetUserCardColonelsInTeamAsync(string userId)
    {
        List<CardColonels> cardColonels = new List<CardColonels>();
        string connectionString = DatabaseConfig.ConnectionString;
    ...
    public async Task<bool> UpdateTeamUserCardColonelAsync(string userId, string teamId, string position, string cardId)
    {
        ...
        try
        {
            ...
            WHERE uc.user_id = @user_id AND uc.card_colonel_id = @card_colonel_id AND c.is_active = TRUE AND c.is_deleted = FALSE;
        ";
    ...
    public async Task<int> GetUserCardColonelsCountAsync(string userId, string search, string type, string rare)
    {
        ...
        try
        {
            ...
            JOIN user_card_colonels uc ON c.id = uc.card_colonel_id AND c.is_active = TRUE AND c.is_deleted = FALSE
            WHERE uc.user_id = @userId  AND c.is_active = TRUE AND c.is_deleted = FALSE
        ";
            if (!string.IsNullOrEmpty(type) && type != "All")
            {
    ...
    public async Task<InsertOrUpdateResult<CardColonels>> InsertOrUpdateUserCardColonelAsync(string userId, CardColonels cardColonel)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        await using MySqlConnection connection = new MySqlConnection(connectionString);
        ...
        try
        {
            ...
            // Query thực hiện Insert hoặc Update nếu đã tồn tại Composite Primary Key (user_id, card_colonel_id)
            string upsertSQL = @"
            INSERT INTO user_card_colonels (
                ...
                @user_id, @card_colonel_id, @rare, 0, 0, 0, @quality, false, @quantity,
                @power, @health, @physical_attack, @physical_defense, @magical_attack, @magical_defense,
                @chemical_attack, @chemical_defense, @atomic_attack, @atomic_defense, @mental_attack, @mental_defense,
                @speed, @critical_damage_rate, @critical_rate, @critical_resistance_rate, @ignore_critical_rate,
                @penetration_rate, @penetration_resistance_rate,
            ...
            // Add Parameters
            command.Parameters.AddWithValue("@user_id", userId);
            command.Parameters.AddWithValue("@card_colonel_id", cardColonel.Id);
    ...
    public async Task<InsertOrUpdateResult<BatchOperationResultDTO<CardColonels>>> InsertOrUpdateUserCardColonelsBatchAsync(
    string userId, List<CardColonels> cardColonels)
    {
        if (cardColonels == null || cardColonels.Count == 0)
        {
            return new InsertOrUpdateResult<BatchOperationResultDTO<CardColonels>>
            {
                Data = new BatchOperationResultDTO<CardColonels>(),
        ...
        try
        {
            ...
            string checkSql = "SELECT card_colonel_id FROM user_card_colonels WHERE user_id = @user_id;";
            ...
            var batchResult = new BatchOperationResultDTO<CardColonels>();
            ...
            int totalCards = cardColonels.Count;
            ...
            INSERT INTO user_card_colonels (
                ...
                for (int j = 0; j < currentBatchCount; j++)
                {
                    ...
                    var c = cardColonels[i + j];

                    stringBuilder.Append($"(@user_id, @card_colonel_id_{j}, @rare_{j}, 0, 0, 0, @quality_{j}, 0, @quantity_{j}, ")
                    ...
                    p.AddWithValue($"@card_colonel_id_{j}", c.Id);
                ...
                stringBuilder.Append(@"
            ON DUPLICATE KEY UPDATE
                quantity = COALESCE(user_card_colonels.quantity, 0) + VALUES(quantity);");
        ...
        catch (Exception ex)
        {
            ...
            return InsertOrUpdateResult<BatchOperationResultDTO<CardColonels>>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateUserCardColonelLevelAsync(string userId, CardColonels cardColonel)
    {
        ...
        {
            Debug.LogError("Error UpdateUserCardColonelLevel: " + ex.Message);
    ...
    public async Task<InsertOrUpdateResult<bool>> UpdateUserCardColonelStarAsync(string userId, CardColonels cardColonel)
    {
        ...
        ";
            ...
            updateCommand.Parameters.AddWithValue("@quantity", cardColonel.Quantity);
        ...
        {
            Debug.LogError("Error UpdateUserCardColonelStar: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
    public async Task<CardColonels> GetUserCardColonelByIdAsync(string userId, string Id)
    {
        CardColonels cardColonel = new CardColonels();
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            INNER JOIN card_colonels c ON uc.card_colonel_id = c.id
                LEFT JOIN AggregatedModules am ON uc.card_colonel_id = am.user_card_colonel_id
                LEFT JOIN AggregatedUpgrades au ON uc.card_colonel_id = au.user_card_colonel_id
            WHERE uc.card_colonel_id = @id AND uc.user_id = @user_id AND c.is_active = TRUE AND c.is_deleted = FALSE";
    ...
    {
        List<string> cardColonelIds = new List<string>();
        string connectionString = DatabaseConfig.ConnectionString;

        string selectSQL = @"
        SELECT uc.card_colonel_id
        FROM user_card_colonels uc
            ...
            try
            {
                ...
                await using (MySqlCommand selectCommand = new MySqlCommand(selectSQL, connection))
                {
                    ...
                    await using (MySqlDataReader reader = (MySqlDataReader)await selectCommand.ExecuteReaderAsync())
                    {
                        while (await reader.ReadAsync())
                        {
                            if (!reader.IsDBNull(reader.GetOrdinal("card_colonel_id")))
                            {
                                cardColonelIds.Add(reader.GetString("card_colonel_id"));
                            }
                        }
                    }
                }
    ...
    public async Task<BaseStats> GetTeamTotalStatsAsync(string userId)
    {
        ...
        try
        {
            ...
            WITH CalculatedCards AS (
                ...
                FROM user_card_colonels uc
                INNER JOIN teams t ON uc.team_id = t.team_id AND t.is_main = 1
                LEFT JOIN user_card_colonels_module ubm ON uc.card_colonel_id = ubm.user_card_colonel_id
                LEFT JOIN user_card_colonels_upgrade ubu ON uc.card_colonel_id = ubu.user_card_colonel_id
                WHERE uc.user_id = @user_id AND uc.team_id IS NOT NULL
            )
            SELECT 
