...
public class UserBuildingsRepository : IUserBuildingsRepository
{
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                WITH AggregatedModules AS (
                    SELECT user_building_id, SUM(current_multiplier) AS total_module_mult
                    FROM user_buildings_module
                    GROUP BY user_building_id
                ),
                AggregatedUpgrades AS (
                    SELECT user_building_id, SUM(current_multiplier) AS total_upgrade_mult
                ...
                await using (MySqlCommand selectCommand = new MySqlCommand(selectSQL, connection))
                {
                    ...
                    await using (MySqlDataReader reader = await selectCommand.ExecuteReaderAsync())
                    {
                        while (await reader.ReadAsync())
                        {
                            ...
                            building.UserModules = userModule;
                            building.UserUpgrades = userUpgrade;
    ...
    public async Task<int> GetUserBuildingsCountAsync(string userId, string search, string type, string rare)
    {
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                SELECT COUNT(*) 
                FROM Buildings m
                JOIN user_buildings um ON m.id = um.building_id AND m.is_active = TRUE AND m.is_deleted = FALSE
    ...
    public async Task<InsertOrUpdateResult<Buildings>> InsertOrUpdateUserBuildingAsync(string userId, Buildings building)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        await using MySqlConnection connection = new MySqlConnection(connectionString);
        ...
        try
        {
            ...
            // Query thực hiện Insert hoặc Update nếu đã tồn tại Composite Primary Key (user_id, building_id)
            string upsertSQL = @"
            INSERT INTO user_buildings (
                ...
                @user_id, @building_id, @rare, 0, 0, 0, @quality, false, @quantity,
                @power, @health, @physical_attack, @physical_defense, @magical_attack, @magical_defense,
                @chemical_attack, @chemical_defense, @atomic_attack, @atomic_defense, @mental_attack, @mental_defense,
                @speed, @critical_damage_rate, @critical_rate, @critical_resistance_rate, @ignore_critical_rate,
                @penetration_rate, @penetration_resistance_rate,
            ...
            return InsertOrUpdateResult<Buildings>.Failure();
        }
        catch (MySqlException ex)
        {
    ...
    string userId, List<Buildings> buildings)
    {
        ...
        {
            return new InsertOrUpdateResult<BatchOperationResultDTO<Buildings>>
            {
                Data = new BatchOperationResultDTO<Buildings>(),
        ...
        try
        {
            ...
            // 1. Query lấy TOÀN BỘ building_id hiện có của User (Cực nhanh nhờ Index user_id)
            var existingIds = new HashSet<string>();
            string checkSql = "SELECT building_id FROM user_buildings WHERE user_id = @user_id;";
            ...
            // 2. Phân loại Buildings giữ NGUYÊN VẸN OBJECT thuộc tính trong RAM C#
            var batchResult = new BatchOperationResultDTO<Buildings>();
            foreach (var card in buildings)
            {
            ...
            {
                var batch = buildings.Skip(i).Take(batchSize).ToList();
            ...
            INSERT INTO user_buildings (
                ...
                for (int j = 0; j < batch.Count; j++)
                {
                    ...
                    stringBuilder.Append($@"
                (@user_id, @building_id_{j}, @rare_{j}, 0, 0, 0, @quality_{j}, 0, @quantity_{j},
                ...
                @skill_damage_rate_{j}, @skill_resistance_rate_{j}
                    ...
                    {
                    new MySqlParameter($"@building_id_{j}", c.Id),
                ...
                stringBuilder.Append(@"
            ON DUPLICATE KEY UPDATE
                quantity = COALESCE(user_buildings.quantity, 0) + VALUES(quantity);
            ");
        ...
        catch (Exception ex)
        {
            ...
            return InsertOrUpdateResult<BatchOperationResultDTO<Buildings>>.Failure(ex.Message);
        }
    }
 ...
 AND c.is_active = TRUE AND c.is_deleted = FALSE;
        ...
        {
            Debug.LogError("Error UpdateUserBuildingLevel: " + ex.Message);
 ...
 AND c.is_active = TRUE AND c.is_deleted = FALSE;
        ...
        {
            Debug.LogError("Error UpdateUserBuildingStar: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
            ...
            try
            {
                ...
                await using (MySqlCommand selectCommand = new MySqlCommand(selectSQL, connection))
                {
                    ...
                    await using (var reader = await selectCommand.ExecuteReaderAsync())
                    {
                        if (await reader.ReadAsync())
                        {
                            sumBuildings.Health = reader.GetDoubleSafe("health");
                            sumBuildings.PhysicalAttack = reader.GetDoubleSafe("physical_attack");
                            sumBuildings.PhysicalDefense = reader.GetDoubleSafe("physical_defense");
                            sumBuildings.MagicalAttack = reader.GetDoubleSafe("magical_attack");
                            sumBuildings.MagicalDefense = reader.GetDoubleSafe("magical_defense");
                            sumBuildings.ChemicalAttack = reader.GetDoubleSafe("chemical_attack");
                            sumBuildings.ChemicalDefense = reader.GetDoubleSafe("chemical_defense");
                            sumBuildings.AtomicAttack = reader.GetDoubleSafe("atomic_attack");
                            sumBuildings.AtomicDefense = reader.GetDoubleSafe("atomic_defense");
                            sumBuildings.MentalAttack = reader.GetDoubleSafe("mental_attack");
                            sumBuildings.MentalDefense = reader.GetDoubleSafe("mental_defense");
                            sumBuildings.Speed = reader.GetDoubleSafe("speed");
                            sumBuildings.CriticalDamageRate = reader.GetDoubleSafe("critical_damage_rate");
                            sumBuildings.CriticalRate = reader.GetDoubleSafe("critical_rate");
                            sumBuildings.CriticalResistanceRate = reader.GetDoubleSafe("critical_resistance_rate");
                            sumBuildings.IgnoreCriticalRate = reader.GetDoubleSafe("ignore_critical_rate");
                            sumBuildings.PenetrationRate = reader.GetDoubleSafe("penetration_rate");
                            sumBuildings.PenetrationResistanceRate = reader.GetDoubleSafe("penetration_resistance_rate");
                            sumBuildings.EvasionRate = reader.GetDoubleSafe("evasion_rate");
                            sumBuildings.DamageAbsorptionRate = reader.GetDoubleSafe("damage_absorption_rate");
                            sumBuildings.IgnoreDamageAbsorptionRate = reader.GetDoubleSafe("ignore_damage_absorption_rate");
                            sumBuildings.AbsorbedDamageRate = reader.GetDoubleSafe("absorbed_damage_rate");
                            sumBuildings.VitalityRegenerationRate = reader.GetDoubleSafe("vitality_regeneration_rate");
                            sumBuildings.VitalityRegenerationResistanceRate = reader.GetDoubleSafe("vitality_regeneration_resistance_rate");
                            sumBuildings.AccuracyRate = reader.GetDoubleSafe("accuracy_rate");
                            sumBuildings.LifestealRate = reader.GetDoubleSafe("lifesteal_rate");
                            sumBuildings.ShieldStrength = reader.GetDoubleSafe("shield_strength");
                            sumBuildings.Tenacity = reader.GetDoubleSafe("tenacity");
                            sumBuildings.ResistanceRate = reader.GetDoubleSafe("resistance_rate");
                            sumBuildings.ComboRate = reader.GetDoubleSafe("combo_rate");
                            sumBuildings.IgnoreComboRate = reader.GetDoubleSafe("ignore_combo_rate");
                            sumBuildings.ComboDamageRate = reader.GetDoubleSafe("combo_damage_rate");
                            sumBuildings.ComboResistanceRate = reader.GetDoubleSafe("combo_resistance_rate");
                            sumBuildings.StunRate = reader.GetDoubleSafe("stun_rate");
                            sumBuildings.IgnoreStunRate = reader.GetDoubleSafe("ignore_stun_rate");
                            sumBuildings.ReflectionRate = reader.GetDoubleSafe("reflection_rate");
                            sumBuildings.IgnoreReflectionRate = reader.GetDoubleSafe("ignore_reflection_rate");
                            sumBuildings.ReflectionDamageRate = reader.GetDoubleSafe("reflection_damage_rate");
                            sumBuildings.ReflectionResistanceRate = reader.GetDoubleSafe("reflection_resistance_rate");
                            sumBuildings.Mana = reader.GetDoubleSafe("mana");
                            sumBuildings.ManaRegenerationRate = reader.GetDoubleSafe("mana_regeneration_rate");
                            sumBuildings.DamageToDifferentFactionRate = reader.GetDoubleSafe("damage_to_different_faction_rate");
                            sumBuildings.ResistanceToDifferentFactionRate = reader.GetDoubleSafe("resistance_to_different_faction_rate");
                            sumBuildings.DamageToSameFactionRate = reader.GetDoubleSafe("damage_to_same_faction_rate");
                            sumBuildings.ResistanceToSameFactionRate = reader.GetDoubleSafe("resistance_to_same_faction_rate");
                            sumBuildings.NormalDamageRate = reader.GetDoubleSafe("normal_damage_rate");
                            sumBuildings.NormalResistanceRate = reader.GetDoubleSafe("normal_resistance_rate");
                            sumBuildings.SkillDamageRate = reader.GetDoubleSafe("skill_damage_rate");
                            sumBuildings.SkillResistanceRate = reader.GetDoubleSafe("skill_resistance_rate");
                        }
                    }
                }
            }
