...
public class UserHIRNsRepository : IUserHIRNsRepository
{
    public async Task<UserHIRNs> GetUserHIRNsAsync(string userId, string id)
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                string selectSQL = @"
                SELECT uc.*
                FROM user_hirns uc INNER JOIN hirns c ON c.id = uc.hirn_id
                WHERE uc.user_id = @user_id AND uc.hirn_id = @hirn_id AND c.is_active = TRUE AND c.is_deleted = FALSE;
            ";
    ...
    public async Task InsertOrUpdateUserHIRNsAsync(string userId, UserHIRNs userHIRN, string id)
    {
        ...
        try
        {
            ...
            string checkSQL = @"
            SELECT COUNT(*) FROM user_hirns uc INNER JOIN hirns c ON c.id = uc.hirn_id 
            WHERE uc.user_id = @user_id AND hirn_id = @hirn_id AND c.is_active = TRUE AND c.is_deleted = FALSE;";
            ...
            await using (var checkCommand = new MySqlCommand(checkSQL, connection))
            {
                ...
                if (count > 0)
                {
                    // -------- UPDATE ----------
                    string updateSQL = @"
                    UPDATE user_hirns uc INNER JOIN hirns c ON c.id = uc.hirn_id
                    SET
                    ...
                    AND uc.hirn_id = @hirn_id AND c.is_active = TRUE AND c.is_deleted = FALSE;
                ";
