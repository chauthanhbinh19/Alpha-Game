...
public class UserHITNsRepository : IUserHITNsRepository
{
    public async Task<UserHITNs> GetUserHITNsAsync(string userId, string id)
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                string selectSQL = @"
                SELECT uc.*
                FROM user_hitns uc INNER JOIN hitns c ON c.id = uc.hitn_id
                WHERE uc.user_id = @user_id AND uc.hitn_id = @hitn_id AND c.is_active = TRUE AND c.is_deleted = FALSE;
            ";
    ...
    public async Task InsertOrUpdateUserHITNsAsync(string userId, UserHITNs userHITN, string id)
    {
        ...
        try
        {
            ...
            string checkSQL = @"
            SELECT COUNT(*) FROM user_hitns uc INNER JOIN hitns c ON c.id = uc.hitn_id 
            WHERE uc.user_id = @user_id AND hitn_id = @hitn_id AND c.is_active = TRUE AND c.is_deleted = FALSE;";
            ...
            await using (var checkCommand = new MySqlCommand(checkSQL, connection))
            {
                ...
                if (count > 0)
                {
                    // -------- UPDATE ----------
                    string updateSQL = @"
                    UPDATE user_hitns uc INNER JOIN hitns c ON c.id = uc.hitn_id
                    SET
                    ...
                    AND uc.hitn_id = @hitn_id AND c.is_active = TRUE AND c.is_deleted = FALSE;
                ";
