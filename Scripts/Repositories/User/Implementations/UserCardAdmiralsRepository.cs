...
{
    public async Task<List<CardAdmirals>> GetUserCardAdmiralsAsync(string userId, string search, string type, int pageSize, int offset, string rare)
    {
    ...
    public async Task<List<CardAdmirals>> GetUserCardAdmiralsTeamAsync(string userId, string teamId, string position)
    {
        List<CardAdmirals> cardAdmirals = new List<CardAdmirals>();
        string connectionString = DatabaseConfig.ConnectionString;
    ...
    public async Task<List<CardAdmirals>> GetUserCardAdmiralsTeamWithoutPositionAsync(string userId, string teamId)
    {
        List<CardAdmirals> cardAdmirals = new List<CardAdmirals>();
        string connectionString = DatabaseConfig.ConnectionString;
    ...
    public async Task<List<CardAdmirals>> GetUserCardAdmiralsInTeamAsync(string userId)
    {
        List<CardAdmirals> cardAdmirals = new List<CardAdmirals>();
        string connectionString = DatabaseConfig.ConnectionString;
    ...
    public async Task<bool> UpdateTeamUserCardAdmiralAsync(string userId, string teamId, string position, string cardId)
    {
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                string updateSQL = @"
            UPDATE user_card_admirals uc INNER JOIN card_admirals c ON c.id = uc.card_admiral_id 
            SET team_id = @team_id, position = @position 
            WHERE uc.user_id = @user_id AND uc.card_admiral_id = @card_admiral_id AND c.is_active = TRUE AND c.is_deleted = FALSE;
        ";
                ...
                updateCommand.Parameters.AddWithValue("@card_admiral_id", cardId);
    ...
    public async Task<int> GetUserCardAdmiralsCountAsync(string userId, string search, string type, string rare)
    {
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                JOIN user_card_admirals uc ON c.id = uc.card_admiral_id AND c.is_active = TRUE AND c.is_deleted = FALSE
                WHERE uc.user_id = @userId  AND c.is_active = TRUE AND c.is_deleted = FALSE";
                if (!string.IsNullOrEmpty(type) && type != "All")
                {
                    selectSQL += " AND c.type = @type";
    ...
    public async Task<InsertOrUpdateResult<CardAdmirals>> InsertOrUpdateUserCardAdmiralAsync(string userId, CardAdmirals cardAdmiral)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        await using MySqlConnection connection = new MySqlConnection(connectionString);
        ...
        try
        {
            ...
            // Query thực hiện Insert hoặc Update nếu đã tồn tại Composite Primary Key (user_id, card_admiral_id)
            string upsertSQL = @"
            INSERT INTO user_card_admirals (
                ...
                @user_id, @card_admiral_id, @rare, 0, 0, 0, @quality, false, @quantity,
                @power, @health, @physical_attack, @physical_defense, @magical_attack, @magical_defense,
                @chemical_attack, @chemical_defense, @atomic_attack, @atomic_defense, @mental_attack, @mental_defense,
                @speed, @critical_damage_rate, @critical_rate, @critical_resistance_rate, @ignore_critical_rate,
                @penetration_rate, @penetration_resistance_rate,
            ...
            return InsertOrUpdateResult<CardAdmirals>.Failure();
        }
        catch (MySqlException ex)
        {
    ...
    public async Task<InsertOrUpdateResult<BatchOperationResultDTO<CardAdmirals>>> InsertOrUpdateUserCardAdmiralsBatchAsync(
    string userId, List<CardAdmirals> cardAdmirals)
    {
        if (cardAdmirals == null || cardAdmirals.Count == 0)
        {
            return new InsertOrUpdateResult<BatchOperationResultDTO<CardAdmirals>>
            {
                Data = new BatchOperationResultDTO<CardAdmirals>(),
        ...
        try
        {
            ...
            string checkSql = "SELECT card_admiral_id FROM user_card_admirals WHERE user_id = @user_id;";
            ...
            var batchResult = new BatchOperationResultDTO<CardAdmirals>();
            ...
            int totalCards = cardAdmirals.Count;
            ...
            INSERT INTO user_card_admirals (
                ...
                for (int j = 0; j < currentBatchCount; j++)
                {
                    ...
                    var c = cardAdmirals[i + j];

                    stringBuilder.Append($"(@user_id, @card_admiral_id_{j}, @rare_{j}, 0, 0, 0, @quality_{j}, 0, @quantity_{j}, ")
                    ...
                    p.AddWithValue($"@card_admiral_id_{j}", c.Id);
                ...
                stringBuilder.Append(@"
            ON DUPLICATE KEY UPDATE
                quantity = COALESCE(user_card_admirals.quantity, 0) + VALUES(quantity);");
        ...
        catch (Exception ex)
        {
            ...
            return InsertOrUpdateResult<BatchOperationResultDTO<CardAdmirals>>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateUserCardAdmiralLevelAsync(string userId, CardAdmirals cardAdmiral)
    {
        if (cardAdmiral == null)
        {
        ...
        {
            Debug.LogError("Error UpdateUserCardAdmiralLevel: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateUserCardAdmiralStarAsync(string userId, CardAdmirals cardAdmiral)
    {
        if (cardAdmiral == null)
        {
            return new InsertOrUpdateResult<bool>
 ...
 AND c.is_active = TRUE AND c.is_deleted = FALSE;
        ...
        {
            Debug.LogError("Error UpdateUserCardAdmiralStar: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
    public async Task<CardAdmirals> GetUserCardAdmiralByIdAsync(string userId, string Id)
    {
        CardAdmirals cardAdmiral = new CardAdmirals();
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                LEFT JOIN AggregatedModules am ON uc.card_admiral_id = am.user_card_admiral_id
                LEFT JOIN AggregatedUpgrades au ON uc.card_admiral_id = au.user_card_admiral_id
                WHERE uc.card_admiral_id = @id AND uc.user_id = @user_id AND c.is_active = TRUE AND c.is_deleted = FALSE";
    ...
    {
        List<string> cardAdmiralIds = new List<string>();
        string connectionString = DatabaseConfig.ConnectionString;

        string selectSQL = @"
        SELECT uc.card_admiral_id
        FROM user_card_admirals uc
            ...
            try
            {
                ...
                await using (MySqlCommand selectCommand = new MySqlCommand(selectSQL, connection))
                {
                    ...
                    await using (MySqlDataReader reader = (MySqlDataReader)await selectCommand.ExecuteReaderAsync())
                    {
                        while (await reader.ReadAsync())
                        {
                            if (!reader.IsDBNull(reader.GetOrdinal("card_admiral_id")))
                            {
                                cardAdmiralIds.Add(reader.GetString("card_admiral_id"));
                            }
                        }
                    }
                }
