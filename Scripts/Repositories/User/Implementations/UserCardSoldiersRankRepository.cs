...
public class UserCardSoldiersRankRepository : IUserCardSoldiersRankRepository
{
    public async Task<Rank> GetCardSoldierRankAsync(string id, string cardId)
    {
        Rank rank = new Rank();
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                string selectSQL = @"
                SELECT *
                FROM user_card_soldiers_Rank
                WHERE user_id = @user_id AND rank_id = @id AND user_card_soldier_id = @card_id;
            ";
    ...
    public async Task InsertOrUpdateCardSoldierRankAsync(string userId, UserRanks userRank, string cardId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                SELECT COUNT(*) 
                FROM user_card_soldiers_Rank 
                WHERE user_id = @user_id AND user_card_soldier_id = @card_id AND rank_id = @rank_id;
            ";
                ...
                await using (MySqlCommand checkCommand = new MySqlCommand(checkSQL, connection))
                {
                    ...
                    if (count > 0)
                    {
                        // UPDATE
                        string updateSQL = @"
                        UPDATE user_card_soldiers_Rank
                    ...
                    else
                    {
                        // INSERT
                        string insertSQL = @"
                        INSERT INTO user_card_soldiers_Rank 
                        (
                            user_id, user_card_soldier_id, rank_id, rank_level, 
    ...
    public async Task<Rank> GetSumCardSoldiersRankAsync(string userId, string cardId)
    {
        Rank rank = new Rank();
        string connectionString = DatabaseConfig.ConnectionString;
