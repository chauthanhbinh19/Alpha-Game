...
public class UserCardMilitariesMasterRepository : IUserCardMilitariesMasterRepository
{
    public async Task<Master> GetCardMilitaryMasterAsync(string id, string cardId)
    {
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                string selectSQL = @"
                SELECT *
                FROM user_card_militaries_master
                WHERE user_id = @user_id AND master_id = @id AND user_card_military_id = @card_id;
            ";
    ...
    public async Task InsertOrUpdateCardMilitaryMasterAsync(string userId, UserMasters userMaster, string cardId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                SELECT COUNT(*) 
                FROM user_card_militaries_master 
                WHERE user_id = @user_id AND user_card_military_id = @card_id AND master_id = @master_id;
            ";
                ...
                await using (MySqlCommand checkCommand = new MySqlCommand(checkSQL, connection))
                {
                    ...
                    if (count > 0)
                    {
                        // UPDATE
                        string updateSQL = @"
                        UPDATE user_card_militaries_master
                    ...
                    else
                    {
                        // INSERT
                        string insertSQL = @"
                        INSERT INTO user_card_militaries_master 
                        (
                            user_id, user_card_military_id, master_id, master_level, 
