...
public class UserAvatarsRepository : IUserAvatarsRepository
{
    ...
    public async Task<InsertOrUpdateResult<Avatars>> InsertOrUpdateUserAvatarAsync(string userId, Avatars avatar)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        await using MySqlConnection connection = new MySqlConnection(connectionString);
        ...
        try
        {
            ...
            // Query thực hiện Insert hoặc Update nếu đã tồn tại Composite Primary Key (user_id, avatar_id)
            string upsertSQL = @"
            INSERT INTO user_avatars (
                ...
                @user_id, @avatar_id, @rare, 0, 0, 0, @quality, false, @quantity,
                @power, @health, @physical_attack, @physical_defense, @magical_attack, @magical_defense,
                @chemical_attack, @chemical_defense, @atomic_attack, @atomic_defense, @mental_attack, @mental_defense,
                @speed, @critical_damage_rate, @critical_rate, @critical_resistance_rate, @ignore_critical_rate,
                @penetration_rate, @penetration_resistance_rate,
            ...
            command.Parameters.AddWithValue("@avatar_id", avatar.Id);
            ...
            return InsertOrUpdateResult<Avatars>.Failure();
        }
        catch (MySqlException ex)
        {
    ...
    string userId, List<Avatars> avatars)
    {
        ...
        {
            return new InsertOrUpdateResult<BatchOperationResultDTO<Avatars>>
            {
                Data = new BatchOperationResultDTO<Avatars>(),
        ...
        try
        {
            ...
            // 1. Query lấy TOÀN BỘ avatar_id hiện có của User (Cực nhanh nhờ Index user_id)
            var existingIds = new HashSet<string>();
            string checkSql = "SELECT avatar_id FROM user_avatars WHERE user_id = @user_id;";
            ...
            // 2. Phân loại Avatars giữ NGUYÊN VẸN OBJECT thuộc tính trong RAM C#
            var batchResult = new BatchOperationResultDTO<Avatars>();
        ...
        catch (Exception ex)
        {
            ...
            return InsertOrUpdateResult<BatchOperationResultDTO<Avatars>>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateUserAvatarLevelAsync(string userId, Avatars avatar)
    {
        ...
        {
            Debug.LogError("Error UpdateUserAvatarLevel: " + ex.Message);
    ...
    public async Task<InsertOrUpdateResult<bool>> UpdateUserAvatarStarAsync(string userId, Avatars avatar)
    {
        ...
        {
            Debug.LogError("Error UpdateUserAvatarStar: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
