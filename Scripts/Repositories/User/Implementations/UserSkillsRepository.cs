...
public class UserSkillsRepository : IUserSkillsRepository
{
    ...
    public async Task<InsertOrUpdateResult<Skills>> InsertOrUpdateUserSkillAsync(string userId, Skills skill)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        await using MySqlConnection connection = new MySqlConnection(connectionString);
        ...
        try
        {
            ...
            // Query thực hiện Insert hoặc Update nếu đã tồn tại Composite Primary Key (user_id, skill_id)
            string upsertSQL = @"
            INSERT INTO user_skills (
                ...
                @user_id, @skill_id, @rare, 0, 0, 0, @quality, false, @quantity,
                @power, @health, @physical_attack, @physical_defense, @magical_attack, @magical_defense,
                @chemical_attack, @chemical_defense, @atomic_attack, @atomic_defense, @mental_attack, @mental_defense,
                @speed, @critical_damage_rate, @critical_rate, @critical_resistance_rate, @ignore_critical_rate,
                @penetration_rate, @penetration_resistance_rate,
    ...
    string userId, List<Skills> skills)
    {
        ...
        {
            return new InsertOrUpdateResult<BatchOperationResultDTO<Skills>>
            {
                Data = new BatchOperationResultDTO<Skills>(),
        ...
        try
        {
            ...
            // 1. Query lấy TOÀN BỘ skill_id hiện có của User (Cực nhanh nhờ Index user_id)
            var existingIds = new HashSet<string>();
            string checkSql = "SELECT skill_id FROM user_skills WHERE user_id = @user_id;";
            ...
            // 2. Phân loại Skills giữ NGUYÊN VẸN OBJECT thuộc tính trong RAM C#
            var batchResult = new BatchOperationResultDTO<Skills>();
        ...
        catch (Exception ex)
        {
            ...
            return InsertOrUpdateResult<BatchOperationResultDTO<Skills>>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateUserSkillLevelAsync(string userId, Skills skill)
    {
        ...
        {
            Debug.LogError("Error UpdateUserSkillLevel: " + ex.Message);
    ...
    public async Task<InsertOrUpdateResult<bool>> UpdateUserSkillStarAsync(string userId, Skills skill)
    {
        ...
        {
            Debug.LogError("Error UpdateUserSkillStar: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
