        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                WITH AggregatedModules AS (
                    SELECT user_core_id, SUM(current_multiplier) AS total_module_mult
                    FROM user_cores_module
                    GROUP BY user_core_id
                ),
                AggregatedUpgrades AS (
                    SELECT user_core_id, SUM(current_multiplier) AS total_upgrade_mult
                    FROM user_cores_upgrade
                    GROUP BY user_core_id
                )
                SELECT 
                    uc.*, 
                    c.id AS base_core_id, 
                    c.name, 
                    c.image, 
                    c.rare, 
                    c.description,
            ...
            ";
                ...
                while (await reader.ReadAsync())
                {
                    ...
                    UserModules userModule = new UserModules
                            ...
                            core.UserModules = userModule;
                            core.UserUpgrades = userUpgrade;
    ...
    public async Task<Cores> GetUserCoreByIdAsync(string userId, string Id)
    {
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                LEFT JOIN AggregatedModules am ON uc.core_id = am.user_core_id
                LEFT JOIN AggregatedUpgrades au ON uc.core_id = au.user_core_id
                WHERE uc.core_id = @id AND uc.user_id = @user_id";
