...
public class UserVehiclesRepository : IUserVehiclesRepository
{
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                WITH AggregatedModules AS (
                    SELECT user_vehicle_id, SUM(current_multiplier) AS total_module_mult
                    FROM user_vehicles_module
                    GROUP BY user_vehicle_id
                ),
                AggregatedUpgrades AS (
                    SELECT user_vehicle_id, SUM(current_multiplier) AS total_upgrade_mult
                    FROM user_vehicles_upgrade
                    GROUP BY user_vehicle_id
                )
                SELECT 
                    uc.*, 
                    c.id AS base_vehicle_id, 
                    c.name, 
                    c.image, 
                    c.rare, 
                    c.description,
                ...
                await using (MySqlCommand selectCommand = new MySqlCommand(selectSQL, connection))
                {
                    ...
                    await using (MySqlDataReader reader = await selectCommand.ExecuteReaderAsync())
                    {
                        while (await reader.ReadAsync())
                        {
                            ...
                            vehicle.UserModules = userModule;
                            vehicle.UserUpgrades = userUpgrade;
    ...
    public async Task<InsertOrUpdateResult<Vehicles>> InsertOrUpdateUserVehicleAsync(string userId, Vehicles vehicle)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        await using MySqlConnection connection = new MySqlConnection(connectionString);
        ...
        try
        {
            ...
            // Query thực hiện Insert hoặc Update nếu đã tồn tại Composite Primary Key (user_id, vehicle_id)
            string upsertSQL = @"
            INSERT INTO user_vehicles (
                ...
                @user_id, @vehicle_id, @rare, 0, 0, 0, @quality, false, @quantity,
                @power, @health, @physical_attack, @physical_defense, @magical_attack, @magical_defense,
                @chemical_attack, @chemical_defense, @atomic_attack, @atomic_defense, @mental_attack, @mental_defense,
                @speed, @critical_damage_rate, @critical_rate, @critical_resistance_rate, @ignore_critical_rate,
                @penetration_rate, @penetration_resistance_rate,
            ...
            return InsertOrUpdateResult<Vehicles>.Failure();
        }
        catch (MySqlException ex)
        {
    ...
    public async Task<InsertOrUpdateResult<BatchOperationResultDTO<Vehicles>>> InsertOrUpdateUserVehiclesBatchAsync(
    string userId, List<Vehicles> vehicles)
    {
        if (vehicles == null || vehicles.Count == 0)
        {
            return new InsertOrUpdateResult<BatchOperationResultDTO<Vehicles>>
            {
                Data = new BatchOperationResultDTO<Vehicles>(),
        ...
        try
        {
            ...
            // 1. Query lấy TOÀN BỘ vehicle_id hiện có của User (Cực nhanh nhờ Index user_id)
            var existingIds = new HashSet<string>();
            string checkSql = "SELECT vehicle_id FROM user_vehicles WHERE user_id = @user_id;";
            ...
            // 2. Phân loại Vehicles giữ NGUYÊN VẸN OBJECT thuộc tính trong RAM C#
            var batchResult = new BatchOperationResultDTO<Vehicles>();
            foreach (var card in vehicles)
            {
            ...
            INSERT INTO user_vehicles (
                ...
                stringBuilder.Append(@"
            ON DUPLICATE KEY UPDATE
                quantity = COALESCE(user_vehicles.quantity, 0) + VALUES(quantity);
            ");
        ...
        catch (Exception ex)
        {
            ...
            return InsertOrUpdateResult<BatchOperationResultDTO<Vehicles>>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateUserVehicleLevelAsync(string userId, Vehicles vehicle)
    {
        ...
        {
            Debug.LogError("Error UpdateUserVehicleLevel: " + ex.Message);
    ...
    public async Task<InsertOrUpdateResult<bool>> UpdateUserVehicleStarAsync(string userId, Vehicles vehicle)
    {
        ...
        {
            Debug.LogError("Error UpdateUserVehicleStar: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
    public async Task<Vehicles> GetUserVehicleByIdAsync(string userId, string Id)
    {
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                LEFT JOIN AggregatedModules am ON uc.vehicle_id = am.user_vehicle_id
                LEFT JOIN AggregatedUpgrades au ON uc.vehicle_id = au.user_vehicle_id
                WHERE uc.vehicle_id = @id AND uc.user_id = @user_id";
            ...
            try
            {
                ...
                await using (MySqlCommand selectCommand = new MySqlCommand(selectSQL, connection))
                {
                    ...
                    await using (var reader = await selectCommand.ExecuteReaderAsync())
                    {
                        if (await reader.ReadAsync())
                        {
                            sumVehicles.Health = reader.GetDoubleSafe("health");
                            sumVehicles.PhysicalAttack = reader.GetDoubleSafe("physical_attack");
                            sumVehicles.PhysicalDefense = reader.GetDoubleSafe("physical_defense");
                            sumVehicles.MagicalAttack = reader.GetDoubleSafe("magical_attack");
                            sumVehicles.MagicalDefense = reader.GetDoubleSafe("magical_defense");
                            sumVehicles.ChemicalAttack = reader.GetDoubleSafe("chemical_attack");
                            sumVehicles.ChemicalDefense = reader.GetDoubleSafe("chemical_defense");
                            sumVehicles.AtomicAttack = reader.GetDoubleSafe("atomic_attack");
                            sumVehicles.AtomicDefense = reader.GetDoubleSafe("atomic_defense");
                            sumVehicles.MentalAttack = reader.GetDoubleSafe("mental_attack");
                            sumVehicles.MentalDefense = reader.GetDoubleSafe("mental_defense");
                            sumVehicles.Speed = reader.GetDoubleSafe("speed");
                            sumVehicles.CriticalDamageRate = reader.GetDoubleSafe("critical_damage_rate");
                            sumVehicles.CriticalRate = reader.GetDoubleSafe("critical_rate");
                            sumVehicles.CriticalResistanceRate = reader.GetDoubleSafe("critical_resistance_rate");
                            sumVehicles.IgnoreCriticalRate = reader.GetDoubleSafe("ignore_critical_rate");
                            sumVehicles.PenetrationRate = reader.GetDoubleSafe("penetration_rate");
                            sumVehicles.PenetrationResistanceRate = reader.GetDoubleSafe("penetration_resistance_rate");
                            sumVehicles.EvasionRate = reader.GetDoubleSafe("evasion_rate");
                            sumVehicles.DamageAbsorptionRate = reader.GetDoubleSafe("damage_absorption_rate");
                            sumVehicles.IgnoreDamageAbsorptionRate = reader.GetDoubleSafe("ignore_damage_absorption_rate");
                            sumVehicles.AbsorbedDamageRate = reader.GetDoubleSafe("absorbed_damage_rate");
                            sumVehicles.VitalityRegenerationRate = reader.GetDoubleSafe("vitality_regeneration_rate");
                            sumVehicles.VitalityRegenerationResistanceRate = reader.GetDoubleSafe("vitality_regeneration_resistance_rate");
                            sumVehicles.AccuracyRate = reader.GetDoubleSafe("accuracy_rate");
                            sumVehicles.LifestealRate = reader.GetDoubleSafe("lifesteal_rate");
                            sumVehicles.ShieldStrength = reader.GetDoubleSafe("shield_strength");
                            sumVehicles.Tenacity = reader.GetDoubleSafe("tenacity");
                            sumVehicles.ResistanceRate = reader.GetDoubleSafe("resistance_rate");
                            sumVehicles.ComboRate = reader.GetDoubleSafe("combo_rate");
                            sumVehicles.IgnoreComboRate = reader.GetDoubleSafe("ignore_combo_rate");
                            sumVehicles.ComboDamageRate = reader.GetDoubleSafe("combo_damage_rate");
                            sumVehicles.ComboResistanceRate = reader.GetDoubleSafe("combo_resistance_rate");
                            sumVehicles.StunRate = reader.GetDoubleSafe("stun_rate");
                            sumVehicles.IgnoreStunRate = reader.GetDoubleSafe("ignore_stun_rate");
                            sumVehicles.ReflectionRate = reader.GetDoubleSafe("reflection_rate");
                            sumVehicles.IgnoreReflectionRate = reader.GetDoubleSafe("ignore_reflection_rate");
                            sumVehicles.ReflectionDamageRate = reader.GetDoubleSafe("reflection_damage_rate");
                            sumVehicles.ReflectionResistanceRate = reader.GetDoubleSafe("reflection_resistance_rate");
                            sumVehicles.Mana = reader.GetDoubleSafe("mana");
                            sumVehicles.ManaRegenerationRate = reader.GetDoubleSafe("mana_regeneration_rate");
                            sumVehicles.DamageToDifferentFactionRate = reader.GetDoubleSafe("damage_to_different_faction_rate");
                            sumVehicles.ResistanceToDifferentFactionRate = reader.GetDoubleSafe("resistance_to_different_faction_rate");
                            sumVehicles.DamageToSameFactionRate = reader.GetDoubleSafe("damage_to_same_faction_rate");
                            sumVehicles.ResistanceToSameFactionRate = reader.GetDoubleSafe("resistance_to_same_faction_rate");
                            sumVehicles.NormalDamageRate = reader.GetDoubleSafe("normal_damage_rate");
                            sumVehicles.NormalResistanceRate = reader.GetDoubleSafe("normal_resistance_rate");
                            sumVehicles.SkillDamageRate = reader.GetDoubleSafe("skill_damage_rate");
                            sumVehicles.SkillResistanceRate = reader.GetDoubleSafe("skill_resistance_rate");
                        }
                    }
                }
            }
