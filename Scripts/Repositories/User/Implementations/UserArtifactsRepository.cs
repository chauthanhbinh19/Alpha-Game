...
public class UserArtifactsRepository : IUserArtifactsRepository
{
    ...
    public async Task<InsertOrUpdateResult<Artifacts>> InsertOrUpdateUserArtifactAsync(string userId, Artifacts artifact)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        await using MySqlConnection connection = new MySqlConnection(connectionString);
        ...
        try
        {
            ...
            // Query thực hiện Insert hoặc Update nếu đã tồn tại Composite Primary Key (user_id, artifact_id)
            string upsertSQL = @"
            INSERT INTO user_artifacts (
                ...
                @user_id, @artifact_id, @rare, 0, 0, 0, @quality, false, @quantity,
                @power, @health, @physical_attack, @physical_defense, @magical_attack, @magical_defense,
                @chemical_attack, @chemical_defense, @atomic_attack, @atomic_defense, @mental_attack, @mental_defense,
                @speed, @critical_damage_rate, @critical_rate, @critical_resistance_rate, @ignore_critical_rate,
                @penetration_rate, @penetration_resistance_rate,
    ...
    string userId, List<Artifacts> artifacts)
    {
        ...
        {
            return new InsertOrUpdateResult<BatchOperationResultDTO<Artifacts>>
            {
                Data = new BatchOperationResultDTO<Artifacts>(),
        ...
        try
        {
            ...
            // 1. Query lấy TOÀN BỘ artifact_id hiện có của User (Cực nhanh nhờ Index user_id)
            var existingIds = new HashSet<string>();
            string checkSql = "SELECT artifact_id FROM user_artifacts WHERE user_id = @user_id;";
            ...
            // 2. Phân loại Artifacts giữ NGUYÊN VẸN OBJECT thuộc tính trong RAM C#
            var batchResult = new BatchOperationResultDTO<Artifacts>();
        ...
        catch (Exception ex)
        {
            ...
            return InsertOrUpdateResult<BatchOperationResultDTO<Artifacts>>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateUserArtifactLevelAsync(string userId, Artifacts artifact)
    {
        ...
        {
            Debug.LogError("Error UpdateUserArtifactLevel: " + ex.Message);
    ...
    public async Task<InsertOrUpdateResult<bool>> UpdateUserArtifactStarAsync(string userId, Artifacts artifact)
    {
        ...
        {
            Debug.LogError("Error UpdateUserArtifactStar: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
