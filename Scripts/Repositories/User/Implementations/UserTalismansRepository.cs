...
public class UserTalismansRepository : IUserTalismansRepository
{
    ...
    public async Task<InsertOrUpdateResult<Talismans>> InsertOrUpdateUserTalismanAsync(string userId, Talismans talisman)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        await using MySqlConnection connection = new MySqlConnection(connectionString);
        ...
        try
        {
            ...
            // Query thực hiện Insert hoặc Update nếu đã tồn tại Composite Primary Key (user_id, talisman_id)
            string upsertSQL = @"
            INSERT INTO user_talismans (
                ...
                @user_id, @talisman_id, @rare, 0, 0, 0, @quality, false, @quantity,
                @power, @health, @physical_attack, @physical_defense, @magical_attack, @magical_defense,
                @chemical_attack, @chemical_defense, @atomic_attack, @atomic_defense, @mental_attack, @mental_defense,
                @speed, @critical_damage_rate, @critical_rate, @critical_resistance_rate, @ignore_critical_rate,
                @penetration_rate, @penetration_resistance_rate,
    ...
    string userId, List<Talismans> talismans)
    {
        ...
        {
            return new InsertOrUpdateResult<BatchOperationResultDTO<Talismans>>
            {
                Data = new BatchOperationResultDTO<Talismans>(),
        ...
        try
        {
            ...
            // 1. Query lấy TOÀN BỘ talisman_id hiện có của User (Cực nhanh nhờ Index user_id)
            var existingIds = new HashSet<string>();
            string checkSql = "SELECT talisman_id FROM user_talismans WHERE user_id = @user_id;";
            ...
            // 2. Phân loại Talismans giữ NGUYÊN VẸN OBJECT thuộc tính trong RAM C#
            var batchResult = new BatchOperationResultDTO<Talismans>();
            foreach (var card in talismans)
            {
        ...
        catch (Exception ex)
        {
            ...
            return InsertOrUpdateResult<BatchOperationResultDTO<Talismans>>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateUserTalismanLevelAsync(string userId, Talismans talisman)
    {
        ...
        {
            Debug.LogError("Error UpdateUserTalismanLevel: " + ex.Message);
    ...
    public async Task<InsertOrUpdateResult<bool>> UpdateUserTalismanStarAsync(string userId, Talismans talisman)
    {
        ...
        {
            Debug.LogError("Error UpdateUserTalismanStar: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
