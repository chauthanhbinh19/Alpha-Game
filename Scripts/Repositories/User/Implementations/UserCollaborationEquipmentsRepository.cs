...
public class UserCollaborationEquipmentsRepository : IUserCollaborationEquipmentsRepository
{
    ...
    public async Task<InsertOrUpdateResult<CollaborationEquipments>> InsertOrUpdateUserCollaborationEquipmentAsync(string userId, CollaborationEquipments collaborationEquipment)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        await using MySqlConnection connection = new MySqlConnection(connectionString);
        ...
        try
        {
            ...
            // Query thực hiện Insert hoặc Update nếu đã tồn tại Composite Primary Key (user_id, collaboration_equipment_id)
            string upsertSQL = @"
            INSERT INTO user_collaboration_equipments (
                ...
                @user_id, @collaboration_equipment_id, @rare, 0, 0, 0, @quality, false, @quantity,
                @power, @health, @physical_attack, @physical_defense, @magical_attack, @magical_defense,
                @chemical_attack, @chemical_defense, @atomic_attack, @atomic_defense, @mental_attack, @mental_defense,
                @speed, @critical_damage_rate, @critical_rate, @critical_resistance_rate, @ignore_critical_rate,
                @penetration_rate, @penetration_resistance_rate,
            ...
            return InsertOrUpdateResult<CollaborationEquipments>.Failure();
        }
        catch (MySqlException ex)
        {
    ...
    string userId, List<CollaborationEquipments> collaborationEquipments)
    {
        ...
        {
            return new InsertOrUpdateResult<BatchOperationResultDTO<CollaborationEquipments>>
            {
                Data = new BatchOperationResultDTO<CollaborationEquipments>(),
        ...
        try
        {
            ...
            // 1. Query lấy TOÀN BỘ collaboration_equipment_id hiện có của User (Cực nhanh nhờ Index user_id)
            var existingIds = new HashSet<string>();
            string checkSql = "SELECT collaboration_equipment_id FROM user_collaboration_equipments WHERE user_id = @user_id;";
            ...
            // 2. Phân loại CollaborationEquipments giữ NGUYÊN VẸN OBJECT thuộc tính trong RAM C#
            var batchResult = new BatchOperationResultDTO<CollaborationEquipments>();
            foreach (var card in collaborationEquipments)
            {
        ...
        catch (Exception ex)
        {
            ...
            return InsertOrUpdateResult<BatchOperationResultDTO<CollaborationEquipments>>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateUserCollaborationEquipmentLevelAsync(string userId, CollaborationEquipments collaborationEquipment)
    {
        ...
        {
            Debug.LogError("Error UpdateUserCollaborationEquipmentLevel: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateUserCollaborationEquipmentStarAsync(string userId, CollaborationEquipments collaborationEquipment)
    {
        if (collaborationEquipment == null)
        {
            return new InsertOrUpdateResult<bool>
        ...
        {
            Debug.LogError("Error UpdateUserCollaborationEquipmentStar: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
