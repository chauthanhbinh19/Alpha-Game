...
public class UserCardLivesRepository : IUserCardLivesRepository
{
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                WITH AggregatedModules AS (
                    SELECT user_card_life_id, SUM(current_multiplier) AS total_module_mult
                    FROM user_card_lives_module
                    GROUP BY user_card_life_id
                ),
                AggregatedUpgrades AS (
                    SELECT user_card_life_id, SUM(current_multiplier) AS total_upgrade_mult
                    FROM user_card_lives_upgrade
                    GROUP BY user_card_life_id
                )
                SELECT 
                    uc.*, 
                    c.id AS base_card_life_id, 
                    c.name, 
                    c.image, 
                    c.rare, 
                    c.description,
                ...
                await using (MySqlCommand selectCommand = new MySqlCommand(selectSQL, connection))
                {
                    ...
                    await using (MySqlDataReader reader = await selectCommand.ExecuteReaderAsync())
                    {
                        while (await reader.ReadAsync())
                        {
                            ...
                            cardLife.UserModules = userModule;
                            cardLife.UserUpgrades = userUpgrade;
    ...
    public async Task<InsertOrUpdateResult<CardLives>> InsertOrUpdateUserCardLifeAsync(string userId, CardLives cardLife)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        await using MySqlConnection connection = new MySqlConnection(connectionString);
        ...
        try
        {
            ...
            // Query thực hiện Insert hoặc Update nếu đã tồn tại Composite Primary Key (user_id, card_life_id)
            string upsertSQL = @"
            INSERT INTO user_card_lives (
                user_id, card_life_id, rare, level, experience, star, quality, block, quantity,
                ...
                @user_id, @card_life_id, @rare, 0, 0, 0, @quality, false, @quantity,
                @power, @health, @physical_attack, @physical_defense, @magical_attack, @magical_defense,
                @chemical_attack, @chemical_defense, @atomic_attack, @atomic_defense, @mental_attack, @mental_defense,
                @speed, @critical_damage_rate, @critical_rate, @critical_resistance_rate, @ignore_critical_rate,
                @penetration_rate, @penetration_resistance_rate,
    ...
    public async Task<InsertOrUpdateResult<BatchOperationResultDTO<CardLives>>> InsertOrUpdateUserCardLivesBatchAsync(
    string userId, List<CardLives> cardLives)
    {
        if (cardLives == null || cardLives.Count == 0)
        {
            return new InsertOrUpdateResult<BatchOperationResultDTO<CardLives>>
            {
                Data = new BatchOperationResultDTO<CardLives>(),
        ...
        try
        {
            ...
            // 1. Query lấy TOÀN BỘ card_life_id hiện có của User (Cực nhanh nhờ Index user_id)
            var existingIds = new HashSet<string>();
            string checkSql = "SELECT card_life_id FROM user_card_lives WHERE user_id = @user_id;";
            ...
            // 2. Phân loại CardLives giữ NGUYÊN VẸN OBJECT thuộc tính trong RAM C#
            var batchResult = new BatchOperationResultDTO<CardLives>();
            foreach (var card in cardLives)
            {
            ...
            {
                var batch = cardLives.Skip(i).Take(batchSize).ToList();
            ...
            INSERT INTO user_card_lives (
                ...
                for (int j = 0; j < batch.Count; j++)
                {
                    ...
                    stringBuilder.Append($@"
                (@user_id, @card_life_id_{j}, @rare_{j}, 0, 0, 0, @quality_{j}, 0, @quantity_{j},
                ...
                @skill_damage_rate_{j}, @skill_resistance_rate_{j}
                    ...
                    {
                    new MySqlParameter($"@card_life_id_{j}", c.Id),
                ...
                stringBuilder.Append(@"
            ON DUPLICATE KEY UPDATE
                quantity = COALESCE(user_card_lives.quantity, 0) + VALUES(quantity);
            ");
        ...
        catch (Exception ex)
        {
            ...
            return InsertOrUpdateResult<BatchOperationResultDTO<CardLives>>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateUserCardLifeLevelAsync(string userId, CardLives cardLife)
    {
        if (cardLife == null)
        {
        ...
        {
            Debug.LogError("Error UpdateUserCardLifeLevel: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateUserCardLifeStarAsync(string userId, CardLives cardLife)
    {
        if (cardLife == null)
        {
            return new InsertOrUpdateResult<bool>
        ...
        {
            Debug.LogError("Error UpdateUserCardLifeStar: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
    public async Task<CardLives> GetUserCardLifeByIdAsync(string userId, string Id)
    {
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                LEFT JOIN AggregatedModules am ON uc.card_life_id = am.user_card_life_id
                LEFT JOIN AggregatedUpgrades au ON uc.card_life_id = au.user_card_life_id
                WHERE uc.card_life_id = @id AND uc.user_id = @user_id";
            ...
            try
            {
                ...
                await using (MySqlCommand selectCommand = new MySqlCommand(selectSQL, connection))
                {
                    ...
                    await using (var reader = await selectCommand.ExecuteReaderAsync())
                    {
                        if (await reader.ReadAsync())
                        {
                            sumCardLives.Health = reader.GetDoubleSafe("health");
                            sumCardLives.PhysicalAttack = reader.GetDoubleSafe("physical_attack");
                            sumCardLives.PhysicalDefense = reader.GetDoubleSafe("physical_defense");
                            sumCardLives.MagicalAttack = reader.GetDoubleSafe("magical_attack");
                            sumCardLives.MagicalDefense = reader.GetDoubleSafe("magical_defense");
                            sumCardLives.ChemicalAttack = reader.GetDoubleSafe("chemical_attack");
                            sumCardLives.ChemicalDefense = reader.GetDoubleSafe("chemical_defense");
                            sumCardLives.AtomicAttack = reader.GetDoubleSafe("atomic_attack");
                            sumCardLives.AtomicDefense = reader.GetDoubleSafe("atomic_defense");
                            sumCardLives.MentalAttack = reader.GetDoubleSafe("mental_attack");
                            sumCardLives.MentalDefense = reader.GetDoubleSafe("mental_defense");
                            sumCardLives.Speed = reader.GetDoubleSafe("speed");
                            sumCardLives.CriticalDamageRate = reader.GetDoubleSafe("critical_damage_rate");
                            sumCardLives.CriticalRate = reader.GetDoubleSafe("critical_rate");
                            sumCardLives.CriticalResistanceRate = reader.GetDoubleSafe("critical_resistance_rate");
                            sumCardLives.IgnoreCriticalRate = reader.GetDoubleSafe("ignore_critical_rate");
                            sumCardLives.PenetrationRate = reader.GetDoubleSafe("penetration_rate");
                            sumCardLives.PenetrationResistanceRate = reader.GetDoubleSafe("penetration_resistance_rate");
                            sumCardLives.EvasionRate = reader.GetDoubleSafe("evasion_rate");
                            sumCardLives.DamageAbsorptionRate = reader.GetDoubleSafe("damage_absorption_rate");
                            sumCardLives.IgnoreDamageAbsorptionRate = reader.GetDoubleSafe("ignore_damage_absorption_rate");
                            sumCardLives.AbsorbedDamageRate = reader.GetDoubleSafe("absorbed_damage_rate");
                            sumCardLives.VitalityRegenerationRate = reader.GetDoubleSafe("vitality_regeneration_rate");
                            sumCardLives.VitalityRegenerationResistanceRate = reader.GetDoubleSafe("vitality_regeneration_resistance_rate");
                            sumCardLives.AccuracyRate = reader.GetDoubleSafe("accuracy_rate");
                            sumCardLives.LifestealRate = reader.GetDoubleSafe("lifesteal_rate");
                            sumCardLives.ShieldStrength = reader.GetDoubleSafe("shield_strength");
                            sumCardLives.Tenacity = reader.GetDoubleSafe("tenacity");
                            sumCardLives.ResistanceRate = reader.GetDoubleSafe("resistance_rate");
                            sumCardLives.ComboRate = reader.GetDoubleSafe("combo_rate");
                            sumCardLives.IgnoreComboRate = reader.GetDoubleSafe("ignore_combo_rate");
                            sumCardLives.ComboDamageRate = reader.GetDoubleSafe("combo_damage_rate");
                            sumCardLives.ComboResistanceRate = reader.GetDoubleSafe("combo_resistance_rate");
                            sumCardLives.StunRate = reader.GetDoubleSafe("stun_rate");
                            sumCardLives.IgnoreStunRate = reader.GetDoubleSafe("ignore_stun_rate");
                            sumCardLives.ReflectionRate = reader.GetDoubleSafe("reflection_rate");
                            sumCardLives.IgnoreReflectionRate = reader.GetDoubleSafe("ignore_reflection_rate");
                            sumCardLives.ReflectionDamageRate = reader.GetDoubleSafe("reflection_damage_rate");
                            sumCardLives.ReflectionResistanceRate = reader.GetDoubleSafe("reflection_resistance_rate");
                            sumCardLives.Mana = reader.GetDoubleSafe("mana");
                            sumCardLives.ManaRegenerationRate = reader.GetDoubleSafe("mana_regeneration_rate");
                            sumCardLives.DamageToDifferentFactionRate = reader.GetDoubleSafe("damage_to_different_faction_rate");
                            sumCardLives.ResistanceToDifferentFactionRate = reader.GetDoubleSafe("resistance_to_different_faction_rate");
                            sumCardLives.DamageToSameFactionRate = reader.GetDoubleSafe("damage_to_same_faction_rate");
                            sumCardLives.ResistanceToSameFactionRate = reader.GetDoubleSafe("resistance_to_same_faction_rate");
                            sumCardLives.NormalDamageRate = reader.GetDoubleSafe("normal_damage_rate");
                            sumCardLives.NormalResistanceRate = reader.GetDoubleSafe("normal_resistance_rate");
                            sumCardLives.SkillDamageRate = reader.GetDoubleSafe("skill_damage_rate");
                            sumCardLives.SkillResistanceRate = reader.GetDoubleSafe("skill_resistance_rate");
                        }
                    }
                }
            }
