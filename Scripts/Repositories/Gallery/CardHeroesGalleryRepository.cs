...
public class CardHeroesGalleryRepository : ICardHeroesGalleryRepository
{
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                SELECT m.*, mg.current_star, mg.temp_star,
                    ...
                    END AS status 
                FROM card_heroes m 
                LEFT JOIN card_heroes_gallery mg 
                    ON m.id = mg.card_hero_id AND mg.user_id = @userId 
                WHERE m.type = @type 
    ...
    {
        int percent = QualityEvaluator.CheckQuality(CardHeroFromDB.Type);
        string connectionString = DatabaseConfig.ConnectionString;
                ...
                await using (MySqlCommand command = new MySqlCommand(query, connection))
                {
                    ...
                    await using (MySqlDataReader reader = (MySqlDataReader)await command.ExecuteReaderAsync())
                    {
                        if (await reader.ReadAsync())
                        {
                            ...
                            sumCardHeroes.Mana = reader.IsDBNull(reader.GetOrdinal("total_mana")) ? 0 : reader.GetDouble("total_mana");
