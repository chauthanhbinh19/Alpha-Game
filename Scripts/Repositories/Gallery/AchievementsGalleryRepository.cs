...
public class AchievementsGalleryRepository : IAchievementsGalleryRepository
{
    ...
    public void UpdateStatusAchievementsGallery(string Id)
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            ...
            {
                connection.Open();
                string query = "update achievements_gallery set status=@status where user_id=@user_id and achievement_id=@achievement_id";
                MySqlCommand command = new MySqlCommand(query, connection);
                command.Parameters.AddWithValue("@user_id", User.CurrentUserId);
                command.Parameters.AddWithValue("@achievement_id", Id);
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                if (reader != null)
                {
                    ...
                    if (tempStar < star)
                    {
                        string updateQuery = @"update achievements_gallery 
