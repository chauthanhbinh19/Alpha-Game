...
public class CardCaptainsGalleryRepository : ICardCaptainsGalleryRepository
{
    public async Task<List<CardCaptains>> GetCardCaptainsCollectionAsync(string type, int pageSize, int offset, string rare)
    {
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                LEFT JOIN card_captains_gallery mg 
                    ON m.id = mg.card_captain_id AND mg.user_id = @userId 
                WHERE m.type = @type 
    ...
    {
        int percent = QualityEvaluator.CheckQuality(CardCaptainFromDB.Type);
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                if (recordCount == 0)
                {
                    ...
                    command.Parameters.AddWithValue("@mental_attack", CardCaptainFromDB.MentalAttack);
                    command.Parameters.AddWithValue("@mental_defense", CardCaptainFromDB.MentalDefense);
    ...
    public async Task UpdateStatusCardCaptainGalleryAsync(string Id)
    {
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                string query = "UPDATE card_captains_gallery SET status=@status WHERE user_id=@user_id AND card_captain_id=@card_captain_id";
                MySqlCommand command = new MySqlCommand(query, connection);
                command.Parameters.AddWithValue("@user_id", User.CurrentUserId);
                command.Parameters.AddWithValue("@card_captain_id", Id);
                ...
                await using (MySqlCommand command = new MySqlCommand(query, connection))
                {
                    ...
                    await using (MySqlDataReader reader = (MySqlDataReader)await command.ExecuteReaderAsync())
                    {
                        if (await reader.ReadAsync())
                        {
                            ...
                            sumCardCaptains.Mana = reader.IsDBNull(reader.GetOrdinal("total_mana")) ? 0 : reader.GetDouble("total_mana");
