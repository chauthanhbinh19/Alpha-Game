...
public class MechaBeastsGalleryRepository : IMechaBeastsGalleryRepository
{
    ...
    public async Task<int> GetMechaBeastsCountAsync(string search, string rare)
    {
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                string selectSQL = @"SELECT COUNT(*) FROM mecha_beasts c 
                WHERE 1=1 AND c.is_active = TRUE AND c.is_deleted = FALSE";
                if (!string.IsNullOrEmpty(rare) && rare != "All")
                {
                    selectSQL += " AND rare = @rare";
    ...
    public async Task<InsertOrUpdateResult<MechaBeasts>> InsertMechaBeastGalleryAsync(string userId, string id, MechaBeasts mechaBeast)
    {
        int percent = QualityEvaluatorHelper.CheckQuality(mechaBeast.Rarity);
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        ";
            ...
            if (recordCount > 0)
            {
                return new InsertOrUpdateResult<MechaBeasts>
                {
                    Data = mechaBeast,
            ...
            INSERT INTO mecha_beasts_gallery (
                user_id, mecha_beast_id, status, current_star, temp_star, power, health, 
    ...
    public async Task<InsertOrUpdateResult<List<MechaBeasts>>> InsertBatchMechaBeastsGalleryAsync(string userId, List<MechaBeasts> mechaBeasts)
    {
        if (mechaBeasts == null || mechaBeasts.Count == 0)
        {
            return InsertOrUpdateResult<List<MechaBeasts>>.Inserted(new List<MechaBeasts>());
        }

        string connectionString = DatabaseConfig.ConnectionString;
        var insertedList = new List<MechaBeasts>();
        int batchSize = 300;
        ...
        try
        {
            ...
            {
                var chunk = mechaBeasts.Skip(i).Take(batchSize).ToList();
                ...
                try
                {
                    ...
                    // INSERT IGNORE: Tự động bỏ qua các dòng đã trùng Primary Key (user_id, mecha_beast_id)
                    sb.Append(@"
                    INSERT IGNORE INTO mecha_beasts_gallery (
                        user_id, mecha_beast_id, status, current_star, temp_star, power, health, 
                        physical_attack, physical_defense, magical_attack, magical_defense, 
                        chemical_attack, chemical_defense, atomic_attack, atomic_defense, 
                        mental_attack, mental_defense, speed, critical_damage_rate, critical_rate,
                    ...
                    for (int j = 0; j < chunk.Count; j++)
                    {
                        ...
                        valueSqls.Add($@"
                        (@user_id{suffix}, @mecha_beast_id{suffix}, 'pending', 0, 0, @power{suffix}, @health{suffix},
                        ...
                        command.Parameters.AddWithValue($"@mecha_beast_id{suffix}", item.Id);
            ...
            return InsertOrUpdateResult<List<MechaBeasts>>.Inserted(insertedList);
        }
        catch (MySqlException ex)
        {
            Debug.LogError("Error Batch Insert: " + ex.Message);
            return InsertOrUpdateResult<List<MechaBeasts>>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateStatusMechaBeastGalleryAsync(string userId, string id, string status = "available")
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            string updateSQL = @"UPDATE mechaBeasts_gallery 
                             SET status = @status 
                             WHERE user_id = @user_id 
                               AND mechaBeast_id = @mechaBeast_id
                               AND (status IS NULL OR status != @status);";

            await using MySqlCommand updateCommand = new MySqlCommand(updateSQL, connection);
            updateCommand.Parameters.AddWithValue("@user_id", userId);
            updateCommand.Parameters.AddWithValue("@mechaBeast_id", id);
        ...
        {
            Debug.LogError("Error UpdateStatusMechaBeastGallery: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateBatchStatusMechaBeastsGalleryAsync(string userId, string status = "available")
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        {
            Debug.LogError("Error UpdateBatchStatusMechaBeastsGallery: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<double>> UpdateTempStarMechaBeastGalleryAsync(string userId, string mechaBeastId, double star)
    {
        ...
        try
        {
            ...
            await using MySqlCommand updateCommand = new MySqlCommand(updateSQL, connection);
            updateCommand.Parameters.AddWithValue("@user_id", userId);
            updateCommand.Parameters.AddWithValue("@mecha_beast_id", mechaBeastId);
    ...
    public async Task<InsertOrUpdateResult<double>> UpdateCurrentStarMechaBeastGalleryAsync(string userId, string mechaBeastId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            if (rowsAffected > 0)
            {
                ...
                selectCommand.Parameters.AddWithValue("@mecha_beast_id", mechaBeastId);
    ...
    public async Task<InsertOrUpdateResult<List<(string MechaBeastId, double CurrentStar)>>> UpdateBatchCurrentStarMechaBeastsGalleryAsync(string userId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            if (rowsAffected > 0)
            {
                // Bước 2: Lấy danh sách mecha_beast_id và current_star vừa được cập nhật (current_star == temp_star)
                string selectSQL = @"
                SELECT mecha_beast_id, current_star 
                FROM mecha_beasts_gallery 
                WHERE user_id = @user_id 
                ...
                var updatedList = new List<(string MechaBeastId, double CurrentStar)>();

                while (await reader.ReadAsync())
                {
                    string id = reader.GetString("mecha_beast_id");
                    double star = reader.GetDouble("current_star");
                    updatedList.Add((id, star));
                }

                return InsertOrUpdateResult<List<(string MechaBeastId, double CurrentStar)>>.Updated(updatedList);
            }

            return new InsertOrUpdateResult<List<(string MechaBeastId, double CurrentStar)>>
            {
                Data = new List<(string MechaBeastId, double CurrentStar)>(),
                OperationType = DatabaseOperationType.None,
                Message = MessageConstants.NOTHING_WAS_UPDATED
            };
        }
        ...
        {
            Debug.LogError("Error: " + ex.Message);
            return InsertOrUpdateResult<List<(string MechaBeastId, double CurrentStar)>>.Failure(ex.Message);
        }
    }
    public async Task<MechaBeasts> GetMechaBeastCollectionByIdAsync(string userId, string objectId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            string selectSQL = @"
            SELECT cg.* 
            FROM mecha_beasts_gallery cg, mecha_beasts c 
            WHERE cg.user_id = @userId 
                AND cg.mecha_beast_id = @objectId 
                AND cg.status = 'available'
                AND cg.mecha_beast_id = c.id
    ...
    public async Task<InsertOrUpdateResult<bool>> UpdateMechaBeastGalleryPowerAsync(string userId, string id, MechaBeasts mechaBeast)
    {
        string connectionString = DatabaseConfig.ConnectionString;
