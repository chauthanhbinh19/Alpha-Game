...
public class CardHeroesGalleryRepository : ICardHeroesGalleryRepository
{
    ...
    public async Task<int> GetCardHeroesCountAsync(string search, string type, string rare)
    {
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                string selectSQL = @"SELECT COUNT(*) FROM card_heroes m 
                WHERE 1=1 AND m.is_active = TRUE AND m.is_deleted = FALSE";
                if (!string.IsNullOrEmpty(type) && type != "All")
                {
    ...
    public async Task<InsertOrUpdateResult<CardHeroes>> InsertCardHeroGalleryAsync(string userId, string id, CardHeroes cardHero)
    {
        int percent = QualityEvaluatorHelper.CheckQuality(cardHero.Rarity);
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        ";
            ...
            if (recordCount > 0)
            {
                return new InsertOrUpdateResult<CardHeroes>
                {
                    Data = cardHero,
    ...
    public async Task<InsertOrUpdateResult<List<CardHeroes>>> InsertBatchCardHeroesGalleryAsync(string userId, List<CardHeroes> cardHeroes)
    {
        if (cardHeroes == null || cardHeroes.Count == 0)
        {
            return InsertOrUpdateResult<List<CardHeroes>>.Inserted(new List<CardHeroes>());
        }

        string connectionString = DatabaseConfig.ConnectionString;
        var insertedList = new List<CardHeroes>();
        int batchSize = 300;
        ...
        try
        {
            ...
            {
                var chunk = cardHeroes.Skip(i).Take(batchSize).ToList();
                ...
                try
                {
                    ...
                    // INSERT IGNORE: Tự động bỏ qua các dòng đã trùng Primary Key (user_id, card_hero_id)
                    sb.Append(@"
                    INSERT IGNORE INTO card_heroes_gallery (
                        user_id, card_hero_id, status, current_star, temp_star, power, health, 
                        physical_attack, physical_defense, magical_attack, magical_defense, 
                        chemical_attack, chemical_defense, atomic_attack, atomic_defense, 
                        mental_attack, mental_defense, speed, critical_damage_rate, critical_rate,
                    ...
                    for (int j = 0; j < chunk.Count; j++)
                    {
                        ...
                        valueSqls.Add($@"
                        (@user_id{suffix}, @card_hero_id{suffix}, 'pending', 0, 0, @power{suffix}, @health{suffix},
                        ...
                        command.Parameters.AddWithValue($"@card_hero_id{suffix}", item.Id);
            ...
            return InsertOrUpdateResult<List<CardHeroes>>.Inserted(insertedList);
        }
        catch (MySqlException ex)
        {
            Debug.LogError("Error Batch Insert: " + ex.Message);
            return InsertOrUpdateResult<List<CardHeroes>>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateStatusCardHeroGalleryAsync(string userId, string id, string status = "available")
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            string updateSQL = @"UPDATE card_heroes_gallery g INNER JOIN card_heroes p ON p.id = g.card_hero_id 
                             SET status = @status 
                             WHERE g.user_id = @user_id 
                               AND g.card_hero_id = @card_hero_id
        ...
        {
            Debug.LogError("Error UpdateStatusCardHeroGallery: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateBatchStatusCardHeroesGalleryAsync(string userId, string status = "available")
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        {
            Debug.LogError("Error UpdateBatchStatusCardHeroesGallery: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<double>> UpdateTempStarCardHeroGalleryAsync(string userId, string cardHeroId, double star)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            string updateSQL = @"
            UPDATE card_heroes_gallery g INNER JOIN card_heroes p ON p.id = g.card_hero_id 
            ...
            await using MySqlCommand updateCommand = new MySqlCommand(updateSQL, connection);
            updateCommand.Parameters.AddWithValue("@user_id", userId);
            updateCommand.Parameters.AddWithValue("@card_hero_id", cardHeroId);
    ...
    public async Task<InsertOrUpdateResult<double>> UpdateCurrentStarCardHeroGalleryAsync(string userId, string cardHeroId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
    ...
    public async Task<InsertOrUpdateResult<List<(string CardHeroId, double CurrentStar)>>> UpdateBatchCurrentStarCardHeroesGalleryAsync(string userId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            if (rowsAffected > 0)
            {
                // Bước 2: Lấy danh sách card_hero_id và current_star vừa được cập nhật (current_star == temp_star)
                string selectSQL = @"
                SELECT card_hero_id, current_star 
                FROM card_heroes_gallery 
                ...
                var updatedList = new List<(string CardHeroId, double CurrentStar)>();

                while (await reader.ReadAsync())
                {
                    string id = reader.GetString("card_hero_id");
                    double star = reader.GetDouble("current_star");
                    updatedList.Add((id, star));
                }

                return InsertOrUpdateResult<List<(string CardHeroId, double CurrentStar)>>.Updated(updatedList);
            }

            return new InsertOrUpdateResult<List<(string CardHeroId, double CurrentStar)>>
            {
                Data = new List<(string CardHeroId, double CurrentStar)>(),
                OperationType = DatabaseOperationType.None,
                Message = MessageConstants.NOTHING_WAS_UPDATED
            };
        }
        ...
        {
            Debug.LogError("Error: " + ex.Message);
            return InsertOrUpdateResult<List<(string CardHeroId, double CurrentStar)>>.Failure(ex.Message);
        }
    }
    public async Task<CardHeroes> GetCardHeroCollectionByIdAsync(string userId, string objectId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            string selectSQL = @"
            SSELECT cg.* 
            FROM card_heroes_gallery cg, card_heroes c 
            WHERE cg.user_id = @userId 
                AND cg.card_hero_id = @objectId 
                AND cg.status = 'available'
                AND cg.card_hero_id = c.id
    ...
    public async Task<InsertOrUpdateResult<bool>> UpdateCardHeroGalleryPowerAsync(string userId, string id, CardHeroes cardHero)
    {
        string connectionString = DatabaseConfig.ConnectionString;
