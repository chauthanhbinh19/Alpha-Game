...
public class RelicsGalleryRepository : IRelicsGalleryRepository
{
    ...
    public async Task<InsertOrUpdateResult<Relics>> InsertRelicGalleryAsync(string userId, string id, Relics relic)
    {
        int percent = QualityEvaluatorHelper.CheckQuality(relic.Rarity);
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        ";
            ...
            if (recordCount > 0)
            {
                return new InsertOrUpdateResult<Relics>
                {
                    Data = relic,
    ...
    public async Task<InsertOrUpdateResult<List<Relics>>> InsertBatchRelicsGalleryAsync(string userId, List<Relics> relics)
    {
        if (relics == null || relics.Count == 0)
        {
            return InsertOrUpdateResult<List<Relics>>.Inserted(new List<Relics>());
        }

        string connectionString = DatabaseConfig.ConnectionString;
        var insertedList = new List<Relics>();
        int batchSize = 300;
        ...
        try
        {
            ...
            {
                var chunk = relics.Skip(i).Take(batchSize).ToList();
                ...
                try
                {
                    ...
                    // INSERT IGNORE: Tự động bỏ qua các dòng đã trùng Primary Key (user_id, relic_id)
                    sb.Append(@"
                    INSERT IGNORE INTO relics_gallery (
                        user_id, relic_id, status, current_star, temp_star, power, health, 
                        physical_attack, physical_defense, magical_attack, magical_defense, 
                        chemical_attack, chemical_defense, atomic_attack, atomic_defense, 
                        mental_attack, mental_defense, speed, critical_damage_rate, critical_rate,
                    ...
                    for (int j = 0; j < chunk.Count; j++)
                    {
                        ...
                        valueSqls.Add($@"
                        (@user_id{suffix}, @relic_id{suffix}, 'pending', 0, 0, @power{suffix}, @health{suffix},
                        ...
                        command.Parameters.AddWithValue($"@relic_id{suffix}", item.Id);
            ...
            return InsertOrUpdateResult<List<Relics>>.Inserted(insertedList);
        }
        catch (MySqlException ex)
        {
            Debug.LogError("Error Batch Insert: " + ex.Message);
            return InsertOrUpdateResult<List<Relics>>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateStatusRelicGalleryAsync(string userId, string id, string status = "available")
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        {
            Debug.LogError("Error UpdateStatusRelicGallery: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateBatchStatusRelicsGalleryAsync(string userId, string status = "available")
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        {
            Debug.LogError("Error UpdateBatchStatusRelicsGallery: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<double>> UpdateTempStarRelicGalleryAsync(string userId, string relicId, double star)
    {
        ...
        try
        {
            ...
            await using MySqlCommand updateCommand = new MySqlCommand(updateSQL, connection);
            updateCommand.Parameters.AddWithValue("@user_id", userId);
            updateCommand.Parameters.AddWithValue("@relic_id", relicId);
    ...
    public async Task<InsertOrUpdateResult<double>> UpdateCurrentStarRelicGalleryAsync(string userId, string relicId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            if (rowsAffected > 0)
            {
                ...
                selectCommand.Parameters.AddWithValue("@relic_id", relicId);
    ...
    public async Task<InsertOrUpdateResult<List<(string RelicId, double CurrentStar)>>> UpdateBatchCurrentStarRelicsGalleryAsync(string userId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            if (rowsAffected > 0)
            {
                // Bước 2: Lấy danh sách relic_id và current_star vừa được cập nhật (current_star == temp_star)
                string selectSQL = @"
                SELECT relic_id, current_star 
                FROM relics_gallery 
                WHERE user_id = @user_id 
                ...
                var updatedList = new List<(string RelicId, double CurrentStar)>();

                while (await reader.ReadAsync())
                {
                    string id = reader.GetString("relic_id");
                    double star = reader.GetDouble("current_star");
                    updatedList.Add((id, star));
                }

                return InsertOrUpdateResult<List<(string RelicId, double CurrentStar)>>.Updated(updatedList);
            }

            return new InsertOrUpdateResult<List<(string RelicId, double CurrentStar)>>
            {
                Data = new List<(string RelicId, double CurrentStar)>(),
                OperationType = DatabaseOperationType.None,
                Message = MessageConstants.NOTHING_WAS_UPDATED
            };
        }
        ...
        {
            Debug.LogError("Error: " + ex.Message);
            return InsertOrUpdateResult<List<(string RelicId, double CurrentStar)>>.Failure(ex.Message);
        }
    }
    public async Task<Relics> GetRelicCollectionByIdAsync(string userId, string objectId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            string selectSQL = @"
            SELECT cg.* 
            FROM relics_gallery cg, relics c 
            WHERE cg.user_id = @userId 
                AND cg.relic_id = @objectId 
                AND cg.status = 'available'
                AND cg.relic_id = c.id
    ...
    public async Task<InsertOrUpdateResult<bool>> UpdateRelicGalleryPowerAsync(string userId, string Id, Relics relic)
    {
        string connectionString = DatabaseConfig.ConnectionString;
