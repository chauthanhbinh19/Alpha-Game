...
public class TechnologiesGalleryRepository : ITechnologiesGalleryRepository
{
    ...
    public async Task<int> GetTechnologiesCountAsync(string search, string rare)
    {
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                string selectSQL = @"SELECT COUNT(*) FROM Technologies c 
                WHERE 1=1 AND c.is_active = TRUE AND c.is_deleted = FALSE";
                if (!string.IsNullOrEmpty(rare) && rare != "All")
                {
                    selectSQL += " AND rare = @rare";
    ...
    public async Task<Technologies> GetTechnologyCollectionByIdAsync(string userId, string objectId)
    {
        ...
        try
        {
            ...
            string selectSQL = @"
            SELECT cg.* 
            FROM technologies_gallery cg, technologies c 
            WHERE cg.user_id = @userId 
                AND cg.technology_id = @objectId 
                AND cg.status = 'available'
                AND cg.technology_id = c.id
    ...
    public async Task<InsertOrUpdateResult<bool>> UpdateTechnologyGalleryPowerAsync(string userId, string id, Technologies technology)
    {
        string connectionString = DatabaseConfig.ConnectionString;
