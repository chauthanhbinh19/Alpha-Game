...
public class ArchitecturesGalleryRepository : IArchitecturesGalleryRepository
{
    ...
    public async Task<InsertOrUpdateResult<Architectures>> InsertArchitectureGalleryAsync(string userId, string id, Architectures architecture)
    {
        int percent = QualityEvaluatorHelper.CheckQuality(architecture.Rarity);
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        ";
            ...
            if (recordCount > 0)
            {
                return new InsertOrUpdateResult<Architectures>
                {
                    Data = architecture,
            ...
            INSERT INTO architectures_gallery (
                user_id, architecture_id, status, current_star, temp_star, power, health, 
    ...
    public async Task<InsertOrUpdateResult<List<Architectures>>> InsertBatchArchitecturesGalleryAsync(string userId, List<Architectures> architectures)
    {
        if (architectures == null || architectures.Count == 0)
        {
            return InsertOrUpdateResult<List<Architectures>>.Inserted(new List<Architectures>());
        }

        string connectionString = DatabaseConfig.ConnectionString;
        var insertedList = new List<Architectures>();
        int batchSize = 300;
        ...
        try
        {
            ...
            {
                var chunk = architectures.Skip(i).Take(batchSize).ToList();
                ...
                try
                {
                    ...
                    // INSERT IGNORE: Tự động bỏ qua các dòng đã trùng Primary Key (user_id, architecture_id)
                    sb.Append(@"
                    INSERT IGNORE INTO architectures_gallery (
                        user_id, architecture_id, status, current_star, temp_star, power, health, 
                        physical_attack, physical_defense, magical_attack, magical_defense, 
                        chemical_attack, chemical_defense, atomic_attack, atomic_defense, 
                        mental_attack, mental_defense, speed, critical_damage_rate, critical_rate,
                    ...
                    for (int j = 0; j < chunk.Count; j++)
                    {
                        ...
                        valueSqls.Add($@"
                        (@user_id{suffix}, @architecture_id{suffix}, 'pending', 0, 0, @power{suffix}, @health{suffix},
                        ...
                        command.Parameters.AddWithValue($"@architecture_id{suffix}", item.Id);
            ...
            return InsertOrUpdateResult<List<Architectures>>.Inserted(insertedList);
        }
        catch (MySqlException ex)
        {
            Debug.LogError("Error Batch Insert: " + ex.Message);
            return InsertOrUpdateResult<List<Architectures>>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateStatusArchitectureGalleryAsync(string userId, string id, string status = "available")
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        {
            Debug.LogError("Error UpdateStatusArchitectureGallery: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateBatchStatusArchitecturesGalleryAsync(string userId, string status = "available")
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        {
            Debug.LogError("Error UpdateBatchStatusArchitecturesGallery: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
    ...
    public async Task<InsertOrUpdateResult<double>> UpdateCurrentStarArchitectureGalleryAsync(string userId, string architectureId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            if (rowsAffected > 0)
            {
                ...
                selectCommand.Parameters.AddWithValue("@architecture_id", architectureId);
    ...
    public async Task<InsertOrUpdateResult<List<(string ArchitectureId, double CurrentStar)>>> UpdateBatchCurrentStarArchitecturesGalleryAsync(string userId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            if (rowsAffected > 0)
            {
                // Bước 2: Lấy danh sách architecture_id và current_star vừa được cập nhật (current_star == temp_star)
                string selectSQL = @"
                SELECT architecture_id, current_star 
                FROM architectures_gallery 
                WHERE user_id = @user_id 
                ...
                var updatedList = new List<(string ArchitectureId, double CurrentStar)>();

                while (await reader.ReadAsync())
                {
                    string id = reader.GetString("architecture_id");
                    double star = reader.GetDouble("current_star");
                    updatedList.Add((id, star));
                }

                return InsertOrUpdateResult<List<(string ArchitectureId, double CurrentStar)>>.Updated(updatedList);
            }

            return new InsertOrUpdateResult<List<(string ArchitectureId, double CurrentStar)>>
            {
                Data = new List<(string ArchitectureId, double CurrentStar)>(),
                OperationType = DatabaseOperationType.None,
                Message = MessageConstants.NOTHING_WAS_UPDATED
            };
        }
        ...
        {
            Debug.LogError("Error: " + ex.Message);
            return InsertOrUpdateResult<List<(string ArchitectureId, double CurrentStar)>>.Failure(ex.Message);
        }
    }
    public async Task<Architectures> GetArchitectureCollectionByIdAsync(string userId, string objectId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            string selectSQL = @"
            SELECT cg.* 
            FROM architectures_gallery cg, architectures c 
            WHERE cg.user_id = @userId 
                AND cg.architecture_id = @objectId 
                AND cg.status = 'available'
                AND cg.architecture_id = c.id
