...
public class AchievementsGalleryRepository : IAchievementsGalleryRepository
{
    ...
    public async Task<InsertOrUpdateResult<Achievements>> InsertAchievementGalleryAsync(string userId, string id, Achievements achievement)
    {
        int percent = QualityEvaluatorHelper.CheckQuality(achievement.Rarity);
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        ";
            ...
            if (recordCount > 0)
            {
                return new InsertOrUpdateResult<Achievements>
                {
                    Data = achievement,
            ...
            INSERT INTO achievements_gallery (
                user_id, achievement_id, status, current_star, temp_star, power, health, 
    ...
    public async Task<InsertOrUpdateResult<List<Achievements>>> InsertBatchAchievementsGalleryAsync(string userId, List<Achievements> achievements)
    {
        if (achievements == null || achievements.Count == 0)
        {
            return InsertOrUpdateResult<List<Achievements>>.Inserted(new List<Achievements>());
        }

        string connectionString = DatabaseConfig.ConnectionString;
        var insertedList = new List<Achievements>();
        int batchSize = 300;
        ...
        try
        {
            ...
            {
                var chunk = achievements.Skip(i).Take(batchSize).ToList();
                ...
                try
                {
                    ...
                    // INSERT IGNORE: Tự động bỏ qua các dòng đã trùng Primary Key (user_id, achievement_id)
                    sb.Append(@"
                    INSERT IGNORE INTO achievements_gallery (
                        user_id, achievement_id, status, current_star, temp_star, power, health, 
                        physical_attack, physical_defense, magical_attack, magical_defense, 
                        chemical_attack, chemical_defense, atomic_attack, atomic_defense, 
                        mental_attack, mental_defense, speed, critical_damage_rate, critical_rate,
                    ...
                    for (int j = 0; j < chunk.Count; j++)
                    {
                        ...
                        valueSqls.Add($@"
                        (@user_id{suffix}, @achievement_id{suffix}, 'pending', 0, 0, @power{suffix}, @health{suffix},
                        ...
                        command.Parameters.AddWithValue($"@achievement_id{suffix}", item.Id);
            ...
            return InsertOrUpdateResult<List<Achievements>>.Inserted(insertedList);
        }
        catch (MySqlException ex)
        {
            Debug.LogError("Error Batch Insert: " + ex.Message);
            return InsertOrUpdateResult<List<Achievements>>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateStatusAchievementGalleryAsync(string userId, string id, string status = "available")
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        {
            Debug.LogError("Error UpdateStatusAchievementGallery: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateBatchStatusAchievementsGalleryAsync(string userId, string status = "available")
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        {
            Debug.LogError("Error UpdateBatchStatusAchievementsGallery: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
    ...
    public async Task<InsertOrUpdateResult<double>> UpdateCurrentStarAchievementGalleryAsync(string userId, string achievementId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            if (rowsAffected > 0)
            {
                ...
                selectCommand.Parameters.AddWithValue("@achievement_id", achievementId);
    ...
    public async Task<InsertOrUpdateResult<List<(string AchievementId, double CurrentStar)>>> UpdateBatchCurrentStarAchievementsGalleryAsync(string userId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            if (rowsAffected > 0)
            {
                // Bước 2: Lấy danh sách achievement_id và current_star vừa được cập nhật (current_star == temp_star)
                string selectSQL = @"
                SELECT achievement_id, current_star 
                FROM achievements_gallery 
                WHERE user_id = @user_id 
                ...
                var updatedList = new List<(string AchievementId, double CurrentStar)>();

                while (await reader.ReadAsync())
                {
                    string id = reader.GetString("achievement_id");
                    double star = reader.GetDouble("current_star");
                    updatedList.Add((id, star));
                }

                return InsertOrUpdateResult<List<(string AchievementId, double CurrentStar)>>.Updated(updatedList);
            }

            return new InsertOrUpdateResult<List<(string AchievementId, double CurrentStar)>>
            {
                Data = new List<(string AchievementId, double CurrentStar)>(),
                OperationType = DatabaseOperationType.None,
                Message = MessageConstants.NOTHING_WAS_UPDATED
            };
        }
        ...
        {
            Debug.LogError("Error: " + ex.Message);
            return InsertOrUpdateResult<List<(string AchievementId, double CurrentStar)>>.Failure(ex.Message);
        }
    }
    public async Task<Achievements> GetAchievementCollectionByIdAsync(string userId, string objectId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            string selectSQL = @"
            SELECT cg.* 
            FROM achievements_gallery cg, achievements c 
            WHERE cg.user_id = @userId 
                AND cg.achievement_id = @objectId 
                AND cg.status = 'available'
                AND cg.achievement_id = c.id
    ...
    public async Task<InsertOrUpdateResult<bool>> UpdateAchievementGalleryPowerAsync(string userId, string id, Achievements achievement)
    {
        string connectionString = DatabaseConfig.ConnectionString;
