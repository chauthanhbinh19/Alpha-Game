...
public class TalismansGalleryRepository : ITalismansGalleryRepository
{
    ...
    public async Task<int> GetTalismansCountAsync(string search, string type, string rare)
    {
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                string selectSQL = @"SELECT COUNT(*) FROM Talismans m 
                WHERE 1=1 AND m.is_active = TRUE AND m.is_deleted = FALSE";
                if (!string.IsNullOrEmpty(type) && type != "All")
                {
    ...
    public async Task<InsertOrUpdateResult<Talismans>> InsertTalismanGalleryAsync(string userId, string id, Talismans talisman)
    {
        int percent = QualityEvaluatorHelper.CheckQuality(talisman.Rarity);
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        ";
            ...
            if (recordCount > 0)
            {
                return new InsertOrUpdateResult<Talismans>
                {
                    Data = talisman,
            ...
            insertCommand.Parameters.AddWithValue("@talisman_id", id);
    ...
    public async Task<InsertOrUpdateResult<List<Talismans>>> InsertBatchTalismansGalleryAsync(string userId, List<Talismans> talismans)
    {
        if (talismans == null || talismans.Count == 0)
        {
            return InsertOrUpdateResult<List<Talismans>>.Inserted(new List<Talismans>());
        }

        string connectionString = DatabaseConfig.ConnectionString;
        var insertedList = new List<Talismans>();
        int batchSize = 300;
        ...
        try
        {
            ...
            {
                var chunk = talismans.Skip(i).Take(batchSize).ToList();
                ...
                try
                {
                    ...
                    // INSERT IGNORE: Tự động bỏ qua các dòng đã trùng Primary Key (user_id, talisman_id)
                    sb.Append(@"
                    INSERT IGNORE INTO talismans_gallery (
                        user_id, talisman_id, status, current_star, temp_star, power, health, 
                        physical_attack, physical_defense, magical_attack, magical_defense, 
                        chemical_attack, chemical_defense, atomic_attack, atomic_defense, 
                        mental_attack, mental_defense, speed, critical_damage_rate, critical_rate,
                    ...
                    for (int j = 0; j < chunk.Count; j++)
                    {
                        ...
                        valueSqls.Add($@"
                        (@user_id{suffix}, @talisman_id{suffix}, 'pending', 0, 0, @power{suffix}, @health{suffix},
                        ...
                        command.Parameters.AddWithValue($"@talisman_id{suffix}", item.Id);
            ...
            return InsertOrUpdateResult<List<Talismans>>.Inserted(insertedList);
        }
        catch (MySqlException ex)
        {
            Debug.LogError("Error Batch Insert: " + ex.Message);
            return InsertOrUpdateResult<List<Talismans>>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateStatusTalismanGalleryAsync(string userId, string id, string status = "available")
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            string updateSQL = @"UPDATE talismans_gallery 
                             SET status = @status 
        ...
        {
            Debug.LogError("Error UpdateStatusTalismanGallery: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateBatchStatusTalismansGalleryAsync(string userId, string status = "available")
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        {
            Debug.LogError("Error UpdateBatchStatusTalismansGallery: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<double>> UpdateTempStarTalismanGalleryAsync(string userId, string talismanId, double star)
    {
        ...
        try
        {
            ...
            await using MySqlCommand updateCommand = new MySqlCommand(updateSQL, connection);
            updateCommand.Parameters.AddWithValue("@user_id", userId);
            updateCommand.Parameters.AddWithValue("@talisman_id", talismanId);
    ...
    public async Task<InsertOrUpdateResult<double>> UpdateCurrentStarTalismanGalleryAsync(string userId, string talismanId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            if (rowsAffected > 0)
            {
                ...
                selectCommand.Parameters.AddWithValue("@talisman_id", talismanId);
    ...
    public async Task<InsertOrUpdateResult<List<(string TalismanId, double CurrentStar)>>> UpdateBatchCurrentStarTalismansGalleryAsync(string userId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            if (rowsAffected > 0)
            {
                // Bước 2: Lấy danh sách talisman_id và current_star vừa được cập nhật (current_star == temp_star)
                string selectSQL = @"
                SELECT talisman_id, current_star 
                FROM talismans_gallery 
                WHERE user_id = @user_id 
                ...
                var updatedList = new List<(string TalismanId, double CurrentStar)>();

                while (await reader.ReadAsync())
                {
                    string id = reader.GetString("talisman_id");
                    double star = reader.GetDouble("current_star");
                    updatedList.Add((id, star));
                }

                return InsertOrUpdateResult<List<(string TalismanId, double CurrentStar)>>.Updated(updatedList);
            }

            return new InsertOrUpdateResult<List<(string TalismanId, double CurrentStar)>>
            {
                Data = new List<(string TalismanId, double CurrentStar)>(),
                OperationType = DatabaseOperationType.None,
                Message = MessageConstants.NOTHING_WAS_UPDATED
            };
        }
        ...
        {
            Debug.LogError("Error: " + ex.Message);
            return InsertOrUpdateResult<List<(string TalismanId, double CurrentStar)>>.Failure(ex.Message);
        }
    }
    public async Task<Talismans> GetTalismanCollectionByIdAsync(string userId, string objectId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            string selectSQL = @"
            SELECT cg.* 
            FROM talismans_gallery cg, talismans c 
            WHERE cg.user_id = @userId 
                AND cg.talisman_id = @objectId 
                AND cg.status = 'available'
                AND cg.talisman_id = c.id
    ...
    public async Task<InsertOrUpdateResult<bool>> UpdateTalismanGalleryPowerAsync(string userId, string Id, Talismans talisman)
    {
        string connectionString = DatabaseConfig.ConnectionString;
                ...
                await using (MySqlCommand selectCommand = new MySqlCommand(selectSQL, connection))
                {
                    ...
                    await using (MySqlDataReader reader = (MySqlDataReader)await selectCommand.ExecuteReaderAsync())
                    {
                        if (await reader.ReadAsync())
                        {
                            ...
                            sumTalismans.PercentAllHealth = reader.IsDBNull(reader.GetOrdinal("total_percent_all_health")) ? 0 : reader.GetDoubleSafe("total_percent_all_health");
                            sumTalismans.PercentAllPhysicalAttack = reader.IsDBNull(reader.GetOrdinal("total_percent_all_physical_attack")) ? 0 : reader.GetDoubleSafe("total_percent_all_physical_attack");
                            sumTalismans.PercentAllPhysicalDefense = reader.IsDBNull(reader.GetOrdinal("total_percent_all_physical_defense")) ? 0 : reader.GetDoubleSafe("total_percent_all_physical_defense");
                            sumTalismans.PercentAllMagicalAttack = reader.IsDBNull(reader.GetOrdinal("total_percent_all_magical_attack")) ? 0 : reader.GetDoubleSafe("total_percent_all_magical_attack");
                            sumTalismans.PercentAllMagicalDefense = reader.IsDBNull(reader.GetOrdinal("total_percent_all_magical_defense")) ? 0 : reader.GetDoubleSafe("total_percent_all_magical_defense");
                            sumTalismans.PercentAllChemicalAttack = reader.IsDBNull(reader.GetOrdinal("total_percent_all_chemical_attack")) ? 0 : reader.GetDoubleSafe("total_percent_all_chemical_attack");
                            sumTalismans.PercentAllChemicalDefense = reader.IsDBNull(reader.GetOrdinal("total_percent_all_chemical_defense")) ? 0 : reader.GetDoubleSafe("total_percent_all_chemical_defense");
                            sumTalismans.PercentAllAtomicAttack = reader.IsDBNull(reader.GetOrdinal("total_percent_all_atomic_attack")) ? 0 : reader.GetDoubleSafe("total_percent_all_atomic_attack");
                            sumTalismans.PercentAllAtomicDefense = reader.IsDBNull(reader.GetOrdinal("total_percent_all_atomic_defense")) ? 0 : reader.GetDoubleSafe("total_percent_all_atomic_defense");
                            sumTalismans.PercentAllMentalAttack = reader.IsDBNull(reader.GetOrdinal("total_percent_all_mental_attack")) ? 0 : reader.GetDoubleSafe("total_percent_all_mental_attack");
                            sumTalismans.PercentAllMentalDefense = reader.IsDBNull(reader.GetOrdinal("total_percent_all_mental_defense")) ? 0 : reader.GetDoubleSafe("total_percent_all_mental_defense");
                        }
                    }
                }
            }
