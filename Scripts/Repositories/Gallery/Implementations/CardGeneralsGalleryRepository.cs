...
public class CardGeneralsGalleryRepository : ICardGeneralsGalleryRepository
{
                        ...
                        while (await reader.ReadAsync())
                        {
                            ...
                            if (!string.IsNullOrEmpty(classesJson))
                            {
                                try
                                {
                                    ...
                                    cardGeneral.Class = JsonHelper.DeserializeClasses(classesJson);
                                }
                                catch
                                {
                                    // Phòng trường hợp Hero không có class, MySQL sinh ra chuỗi "[null]"
                                    cardGeneral.Class = new Classes();
                                }
                            }
    ...
    public async Task<int> GetCardGeneralsCountAsync(string search, string type, string rare)
    {
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                string selectSQL = @"SELECT COUNT(*) FROM card_generals m 
                WHERE 1=1 AND m.is_active = TRUE AND m.is_deleted = FALSE";
                if (!string.IsNullOrEmpty(type) && type != "All")
                {
    ...
    public async Task<InsertOrUpdateResult<CardGenerals>> InsertCardGeneralGalleryAsync(string userId, string id, CardGenerals cardGeneral)
    {
        int percent = QualityEvaluatorHelper.CheckQuality(cardGeneral.Rarity);
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        ";
            ...
            if (recordCount > 0)
            {
                return new InsertOrUpdateResult<CardGenerals>
                {
                    Data = cardGeneral,
    ...
    public async Task<InsertOrUpdateResult<List<CardGenerals>>> InsertBatchCardGeneralsGalleryAsync(string userId, List<CardGenerals> cardGenerals)
    {
        if (cardGenerals == null || cardGenerals.Count == 0)
        {
            return InsertOrUpdateResult<List<CardGenerals>>.Inserted(new List<CardGenerals>());
        }

        string connectionString = DatabaseConfig.ConnectionString;
        var insertedList = new List<CardGenerals>();
        int batchSize = 300;
        ...
        try
        {
            ...
            for (int i = 0; i < cardGenerals.Count; i += batchSize)
            {
                var chunk = cardGenerals.Skip(i).Take(batchSize).ToList();
                ...
                try
                {
                    ...
                    // INSERT IGNORE: Tự động bỏ qua các dòng đã trùng Primary Key (user_id, card_general_id)
                    sb.Append(@"
                    INSERT IGNORE INTO card_generals_gallery (
                        user_id, card_general_id, status, current_star, temp_star, power, health, 
                        physical_attack, physical_defense, magical_attack, magical_defense, 
                        chemical_attack, chemical_defense, atomic_attack, atomic_defense, 
                        mental_attack, mental_defense, speed, critical_damage_rate, critical_rate,
                    ...
                    for (int j = 0; j < chunk.Count; j++)
                    {
                        ...
                        valueSqls.Add($@"
                        (@user_id{suffix}, @card_general_id{suffix}, 'pending', 0, 0, @power{suffix}, @health{suffix},
                        ...
                        command.Parameters.AddWithValue($"@card_general_id{suffix}", item.Id);
            ...
            return InsertOrUpdateResult<List<CardGenerals>>.Inserted(insertedList);
        }
        catch (MySqlException ex)
        {
            Debug.LogError("Error Batch Insert: " + ex.Message);
            return InsertOrUpdateResult<List<CardGenerals>>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateStatusCardGeneralGalleryAsync(string userId, string id, string status = "available")
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            string updateSQL = @"UPDATE card_generals_gallery g INNER JOIN card_generals p ON p.id = g.card_general_id 
                             SET status = @status 
                             WHERE g.user_id = @user_id 
                               AND g.card_general_id = @card_general_id
        ...
        {
            Debug.LogError("Error UpdateStatusCardGeneralGallery: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateBatchStatusCardGeneralsGalleryAsync(string userId, string status = "available")
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        {
            Debug.LogError("Error UpdateBatchStatusCardGeneralsGallery: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<double>> UpdateTempStarCardGeneralGalleryAsync(string userId, string cardGeneralId, double star)
    {
        ...
        try
        {
            ...
            string updateSQL = @"
            UPDATE card_generals_gallery g INNER JOIN card_generals p ON p.id = g.card_general_id 
    ...
    public async Task<InsertOrUpdateResult<double>> UpdateCurrentStarCardGeneralGalleryAsync(string userId, string cardGeneralId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
    ...
    public async Task<InsertOrUpdateResult<List<(string CardGeneralId, double CurrentStar)>>> UpdateBatchCurrentStarCardGeneralsGalleryAsync(string userId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            if (rowsAffected > 0)
            {
                // Bước 2: Lấy danh sách card_general_id và current_star vừa được cập nhật (current_star == temp_star)
                string selectSQL = @"
                SELECT card_general_id, current_star 
                FROM card_generals_gallery 
                ...
                var updatedList = new List<(string CardGeneralId, double CurrentStar)>();

                while (await reader.ReadAsync())
                {
                    string id = reader.GetString("card_general_id");
                    double star = reader.GetDouble("current_star");
                    updatedList.Add((id, star));
                }

                return InsertOrUpdateResult<List<(string CardGeneralId, double CurrentStar)>>.Updated(updatedList);
            }

            return new InsertOrUpdateResult<List<(string CardGeneralId, double CurrentStar)>>
            {
                Data = new List<(string CardGeneralId, double CurrentStar)>(),
                OperationType = DatabaseOperationType.None,
                Message = MessageConstants.NOTHING_WAS_UPDATED
            };
        }
        ...
        {
            Debug.LogError("Error: " + ex.Message);
            return InsertOrUpdateResult<List<(string CardGeneralId, double CurrentStar)>>.Failure(ex.Message);
        }
    }
    public async Task<CardGenerals> GetCardGeneralCollectionByIdAsync(string userId, string objectId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            string selectSQL = @"
            SELECT cg.* 
            FROM card_generals_gallery cg, card_generals c 
            WHERE cg.user_id = @userId 
                AND cg.card_general_id = @objectId 
                AND cg.status = 'available'
                AND cg.card_general_id = c.id
    ...
    public async Task<InsertOrUpdateResult<bool>> UpdateCardGeneralGalleryPowerAsync(string userId, string id, CardGenerals cardGeneral)
    {
        string connectionString = DatabaseConfig.ConnectionString;
