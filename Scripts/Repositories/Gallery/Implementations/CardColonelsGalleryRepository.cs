...
public class CardColonelsGalleryRepository : ICardColonelsGalleryRepository
{
    ...
    public async Task<int> GetCardColonelsCountAsync(string search, string type, string rare)
    {
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                string selectSQL = @"SELECT COUNT(*) FROM card_colonels m 
                WHERE 1=1 AND m.is_active = TRUE AND m.is_deleted = FALSE";
                if (!string.IsNullOrEmpty(type) && type != "All")
                {
    ...
    public async Task<InsertOrUpdateResult<bool>> UpdateStatusCardColonelGalleryAsync(string userId, string id, string status = "available")
    {
        ...
        try
        {
            ...
            string updateSQL = @"UPDATE card_colonels_gallery g INNER JOIN card_colonels p ON p.id = g.card_colonel_id 
                             SET status = @status 
                             WHERE g.user_id = @user_id 
                               AND g.card_colonel_id = @card_colonel_id
    ...
    public async Task<InsertOrUpdateResult<double>> UpdateTempStarCardColonelGalleryAsync(string userId, string cardColonelId, double star)
    {
        ...
        try
        {
            ...
            string updateSQL = @"
            UPDATE card_colonels_gallery g INNER JOIN card_colonels p ON p.id = g.card_colonel_id 
    ...
    public async Task<CardColonels> GetCardColonelCollectionByIdAsync(string userId, string objectId)
    {
        ...
        try
        {
            ...
            string selectSQL = @"
            SELECT cg.* 
            FROM card_colonels_gallery cg, card_colonels c 
            WHERE cg.user_id = @userId 
                AND cg.card_colonel_id = @objectId 
                AND cg.status = 'available'
                AND cg.card_colonel_id = c.id
    ...
    public async Task<InsertOrUpdateResult<bool>> UpdateCardColonelGalleryPowerAsync(string userId, string id, CardColonels cardColonel)
    {
        string connectionString = DatabaseConfig.ConnectionString;
