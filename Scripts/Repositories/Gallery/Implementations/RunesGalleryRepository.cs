...
public class RunesGalleryRepository : IRunesGalleryRepository
{
    ...
    public async Task<int> GetRunesCountAsync(string search, string rare)
    {
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                string selectSQL = @"SELECT COUNT(*) FROM Runes c 
                WHERE 1=1 AND c.is_active = TRUE AND c.is_deleted = FALSE";
                if (!string.IsNullOrEmpty(rare) && rare != "All")
                {
                    selectSQL += " AND rare = @rare";
    ...
    public async Task<InsertOrUpdateResult<Runes>> InsertRuneGalleryAsync(string userId, string id, Runes rune)
    {
        int percent = QualityEvaluatorHelper.CheckQuality(rune.Rarity);
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        ";
            ...
            if (recordCount > 0)
            {
                return new InsertOrUpdateResult<Runes>
                {
                    Data = rune,
            ...
            INSERT INTO runes_gallery (
                user_id, rune_id, status, current_star, temp_star, power, health, 
    ...
    public async Task<InsertOrUpdateResult<List<Runes>>> InsertBatchRunesGalleryAsync(string userId, List<Runes> runes)
    {
        if (runes == null || runes.Count == 0)
        {
            return InsertOrUpdateResult<List<Runes>>.Inserted(new List<Runes>());
        }
        ...
        try
        {
            ...
            for (int i = 0; i < runes.Count; i += batchSize)
            {
                var chunk = runes.Skip(i).Take(batchSize).ToList();
                ...
                try
                {
                    ...
                    // INSERT IGNORE: Tự động bỏ qua các dòng đã trùng Primary Key (user_id, rune_id)
                    sb.Append(@"
                    INSERT IGNORE INTO runes_gallery (
                        user_id, rune_id, status, current_star, temp_star, power, health, 
                        physical_attack, physical_defense, magical_attack, magical_defense, 
                        chemical_attack, chemical_defense, atomic_attack, atomic_defense, 
                        mental_attack, mental_defense, speed, critical_damage_rate, critical_rate,
                    ...
                    for (int j = 0; j < chunk.Count; j++)
                    {
                        ...
                        valueSqls.Add($@"
                        (@user_id{suffix}, @rune_id{suffix}, 'pending', 0, 0, @power{suffix}, @health{suffix},
                        ...
                        command.Parameters.AddWithValue($"@rune_id{suffix}", item.Id);
            ...
            return InsertOrUpdateResult<List<Runes>>.Inserted(insertedList);
        }
        catch (MySqlException ex)
        {
            Debug.LogError("Error Batch Insert: " + ex.Message);
            return InsertOrUpdateResult<List<Runes>>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateStatusRuneGalleryAsync(string userId, string id, string status = "available")
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        {
            Debug.LogError("Error UpdateStatusRuneGallery: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateBatchStatusRunesGalleryAsync(string userId, string status = "available")
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        {
            Debug.LogError("Error UpdateBatchStatusRunesGallery: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<double>> UpdateTempStarRuneGalleryAsync(string userId, string runeId, double star)
    {
        ...
        try
        {
            ...
            await using MySqlCommand updateCommand = new MySqlCommand(updateSQL, connection);
            updateCommand.Parameters.AddWithValue("@user_id", userId);
            updateCommand.Parameters.AddWithValue("@rune_id", runeId);
    ...
    public async Task<InsertOrUpdateResult<double>> UpdateCurrentStarRuneGalleryAsync(string userId, string runeId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            if (rowsAffected > 0)
            {
                ...
                selectCommand.Parameters.AddWithValue("@rune_id", runeId);
    ...
    public async Task<InsertOrUpdateResult<List<(string RuneId, double CurrentStar)>>> UpdateBatchCurrentStarRunesGalleryAsync(string userId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            if (rowsAffected > 0)
            {
                // Bước 2: Lấy danh sách rune_id và current_star vừa được cập nhật (current_star == temp_star)
                string selectSQL = @"
                SELECT rune_id, current_star 
                FROM runes_gallery 
                WHERE user_id = @user_id 
                ...
                var updatedList = new List<(string RuneId, double CurrentStar)>();

                while (await reader.ReadAsync())
                {
                    string id = reader.GetString("rune_id");
                    double star = reader.GetDouble("current_star");
                    updatedList.Add((id, star));
                }

                return InsertOrUpdateResult<List<(string RuneId, double CurrentStar)>>.Updated(updatedList);
            }

            return new InsertOrUpdateResult<List<(string RuneId, double CurrentStar)>>
            {
                Data = new List<(string RuneId, double CurrentStar)>(),
                OperationType = DatabaseOperationType.None,
                Message = MessageConstants.NOTHING_WAS_UPDATED
            };
        }
        ...
        {
            Debug.LogError("Error: " + ex.Message);
            return InsertOrUpdateResult<List<(string RuneId, double CurrentStar)>>.Failure(ex.Message);
        }
    }
    public async Task<Runes> GetRuneCollectionByIdAsync(string userId, string objectId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            string selectSQL = @"
            SELECT cg.* 
            FROM runes_gallery cg, runes c 
            WHERE cg.user_id = @userId 
                AND cg.rune_id = @objectId 
    ...
    public async Task<InsertOrUpdateResult<bool>> UpdateRuneGalleryPowerAsync(string userId, string id, Runes rune)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            ...
            ";
                ...
                updateCommand.Parameters.AddWithValue("@mental_attack", rune.MagicalAttack);
                updateCommand.Parameters.AddWithValue("@mental_defense", rune.MagicalDefense);
