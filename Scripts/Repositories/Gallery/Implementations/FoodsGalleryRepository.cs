...
public class FoodsGalleryRepository : IFoodsGalleryRepository
{
    ...
    public async Task<int> GetFoodsCountAsync(string search, string rare)
    {
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                string selectSQL = @"SELECT COUNT(*) FROM Foods c 
                WHERE 1=1 AND c.is_active = TRUE AND c.is_deleted = FALSE";
                if (!string.IsNullOrEmpty(rare) && rare != "All")
                {
                    selectSQL += " AND rare = @rare";
    ...
    public async Task<Foods> GetFoodCollectionByIdAsync(string userId, string objectId)
    {
        ...
        try
        {
            ...
            string selectSQL = @"
            SELECT cg.* 
            FROM foods_gallery cg, foods c 
            WHERE cg.user_id = @userId 
                AND cg.food_id = @objectId 
    ...
    public async Task<InsertOrUpdateResult<bool>> UpdateFoodGalleryPowerAsync(string userId, string id, Foods food)
    {
        string connectionString = DatabaseConfig.ConnectionString;
