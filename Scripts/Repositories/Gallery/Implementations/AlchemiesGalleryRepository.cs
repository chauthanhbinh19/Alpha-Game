...
public class AlchemiesGalleryRepository : IAlchemiesGalleryRepository
{
    ...
    public async Task<InsertOrUpdateResult<Alchemies>> InsertAlchemyGalleryAsync(string userId, string id, Alchemies alchemy)
    {
        int percent = QualityEvaluatorHelper.CheckQuality(alchemy.Rarity);
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        ";
            ...
            if (recordCount > 0)
            {
                return new InsertOrUpdateResult<Alchemies>
                {
                    Data = alchemy,
    ...
    public async Task<InsertOrUpdateResult<List<Alchemies>>> InsertBatchAlchemiesGalleryAsync(string userId, List<Alchemies> alchemies)
    {
        if (alchemies == null || alchemies.Count == 0)
        {
            return InsertOrUpdateResult<List<Alchemies>>.Inserted(new List<Alchemies>());
        }
        ...
        try
        {
            ...
            for (int i = 0; i < alchemies.Count; i += batchSize)
            {
                var chunk = alchemies.Skip(i).Take(batchSize).ToList();
                ...
                try
                {
                    ...
                    // INSERT IGNORE: Tự động bỏ qua các dòng đã trùng Primary Key (user_id, alchemy_id)
                    sb.Append(@"
                    INSERT IGNORE INTO alchemies_gallery (
                        user_id, alchemy_id, status, current_star, temp_star, power, health, 
                        physical_attack, physical_defense, magical_attack, magical_defense, 
                        chemical_attack, chemical_defense, atomic_attack, atomic_defense, 
                        mental_attack, mental_defense, speed, critical_damage_rate, critical_rate,
                    ...
                    for (int j = 0; j < chunk.Count; j++)
                    {
                        ...
                        valueSqls.Add($@"
                        (@user_id{suffix}, @alchemy_id{suffix}, 'pending', 0, 0, @power{suffix}, @health{suffix},
                        ...
                        command.Parameters.AddWithValue($"@alchemy_id{suffix}", item.Id);
            ...
            return InsertOrUpdateResult<List<Alchemies>>.Inserted(insertedList);
        }
        catch (MySqlException ex)
        {
            Debug.LogError("Error Batch Insert: " + ex.Message);
            return InsertOrUpdateResult<List<Alchemies>>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateStatusAlchemyGalleryAsync(string userId, string id, string status = "available")
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        {
            Debug.LogError("Error UpdateStatusAlchemyGallery: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateBatchStatusAlchemiesGalleryAsync(string userId, string status = "available")
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        {
            Debug.LogError("Error UpdateBatchStatusAlchemiesGallery: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<double>> UpdateTempStarAlchemyGalleryAsync(string userId, string alchemyId, double star)
    {
        ...
        try
        {
            ...
            await using MySqlCommand updateCommand = new MySqlCommand(updateSQL, connection);
            updateCommand.Parameters.AddWithValue("@user_id", userId);
            updateCommand.Parameters.AddWithValue("@alchemy_id", alchemyId);
    ...
    public async Task<InsertOrUpdateResult<double>> UpdateCurrentStarAlchemyGalleryAsync(string userId, string alchemyId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            if (rowsAffected > 0)
            {
                ...
                selectCommand.Parameters.AddWithValue("@alchemy_id", alchemyId);
    ...
    public async Task<InsertOrUpdateResult<List<(string AlchemyId, double CurrentStar)>>> UpdateBatchCurrentStarAlchemiesGalleryAsync(string userId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            if (rowsAffected > 0)
            {
                // Bước 2: Lấy danh sách alchemy_id và current_star vừa được cập nhật (current_star == temp_star)
                string selectSQL = @"
                SELECT alchemy_id, current_star 
                FROM alchemies_gallery 
                WHERE user_id = @user_id 
                ...
                var updatedList = new List<(string AlchemyId, double CurrentStar)>();

                while (await reader.ReadAsync())
                {
                    string id = reader.GetString("alchemy_id");
                    double star = reader.GetDouble("current_star");
                    updatedList.Add((id, star));
                }

                return InsertOrUpdateResult<List<(string AlchemyId, double CurrentStar)>>.Updated(updatedList);
            }

            return new InsertOrUpdateResult<List<(string AlchemyId, double CurrentStar)>>
            {
                Data = new List<(string AlchemyId, double CurrentStar)>(),
                OperationType = DatabaseOperationType.None,
                Message = MessageConstants.NOTHING_WAS_UPDATED
            };
        }
        ...
        {
            Debug.LogError("Error: " + ex.Message);
            return InsertOrUpdateResult<List<(string AlchemyId, double CurrentStar)>>.Failure(ex.Message);
        }
    }
    public async Task<Alchemies> GetAlchemyCollectionByIdAsync(string userId, string objectId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            string selectSQL = @"
            SELECT cg.* 
            FROM alchemies_gallery cg, alchemies c 
            WHERE cg.user_id = @userId 
                AND cg.alchemy_id = @objectId 
                AND cg.status = 'available'
                AND cg.alchemy_id = c.id
    ...
    public async Task<InsertOrUpdateResult<bool>> UpdateAlchemyGalleryPowerAsync(string userId, string id, Alchemies alchemy)
    {
        string connectionString = DatabaseConfig.ConnectionString;
