...
public class BadgesGalleryRepository : IBadgesGalleryRepository
{
    ...
    public async Task<InsertOrUpdateResult<Badges>> InsertBadgeGalleryAsync(string userId, string id, Badges badge)
    {
        int percent = QualityEvaluatorHelper.CheckQuality(badge.Rarity);
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        ";
            ...
            if (recordCount > 0)
            {
                return new InsertOrUpdateResult<Badges>
                {
                    Data = badge,
            ...
            INSERT INTO badges_gallery (
                user_id, badge_id, status, current_star, temp_star, power, health, 
    ...
    public async Task<InsertOrUpdateResult<List<Badges>>> InsertBatchBadgesGalleryAsync(string userId, List<Badges> badges)
    {
        if (badges == null || badges.Count == 0)
        {
            return InsertOrUpdateResult<List<Badges>>.Inserted(new List<Badges>());
        }

        string connectionString = DatabaseConfig.ConnectionString;
        var insertedList = new List<Badges>();
        int batchSize = 300;
        ...
        try
        {
            ...
            {
                var chunk = badges.Skip(i).Take(batchSize).ToList();
                ...
                try
                {
                    ...
                    // INSERT IGNORE: Tự động bỏ qua các dòng đã trùng Primary Key (user_id, badge_id)
                    sb.Append(@"
                    INSERT IGNORE INTO badges_gallery (
                        user_id, badge_id, status, current_star, temp_star, power, health, 
                        physical_attack, physical_defense, magical_attack, magical_defense, 
                        chemical_attack, chemical_defense, atomic_attack, atomic_defense, 
                        mental_attack, mental_defense, speed, critical_damage_rate, critical_rate,
                    ...
                    for (int j = 0; j < chunk.Count; j++)
                    {
                        ...
                        valueSqls.Add($@"
                        (@user_id{suffix}, @badge_id{suffix}, 'pending', 0, 0, @power{suffix}, @health{suffix},
                        ...
                        command.Parameters.AddWithValue($"@badge_id{suffix}", item.Id);
            ...
            return InsertOrUpdateResult<List<Badges>>.Inserted(insertedList);
        }
        catch (MySqlException ex)
        {
            Debug.LogError("Error Batch Insert: " + ex.Message);
            return InsertOrUpdateResult<List<Badges>>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateStatusBadgeGalleryAsync(string userId, string id, string status = "available")
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        {
            Debug.LogError("Error UpdateStatusBadgeGallery: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateBatchStatusBadgesGalleryAsync(string userId, string status = "available")
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        {
            Debug.LogError("Error UpdateBatchStatusBadgesGallery: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<double>> UpdateTempStarBadgeGalleryAsync(string userId, string badgeId, double star)
    {
        ...
        try
        {
            ...
            await using MySqlCommand updateCommand = new MySqlCommand(updateSQL, connection);
            updateCommand.Parameters.AddWithValue("@user_id", userId);
            updateCommand.Parameters.AddWithValue("@badge_id", badgeId);
    ...
    public async Task<InsertOrUpdateResult<double>> UpdateCurrentStarBadgeGalleryAsync(string userId, string badgeId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            if (rowsAffected > 0)
            {
                ...
                selectCommand.Parameters.AddWithValue("@badge_id", badgeId);
    ...
    public async Task<InsertOrUpdateResult<List<(string BadgeId, double CurrentStar)>>> UpdateBatchCurrentStarBadgesGalleryAsync(string userId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            if (rowsAffected > 0)
            {
                // Bước 2: Lấy danh sách badge_id và current_star vừa được cập nhật (current_star == temp_star)
                string selectSQL = @"
                SELECT badge_id, current_star 
                FROM badges_gallery 
                WHERE user_id = @user_id 
                ...
                var updatedList = new List<(string BadgeId, double CurrentStar)>();

                while (await reader.ReadAsync())
                {
                    string id = reader.GetString("badge_id");
                    double star = reader.GetDouble("current_star");
                    updatedList.Add((id, star));
                }

                return InsertOrUpdateResult<List<(string BadgeId, double CurrentStar)>>.Updated(updatedList);
            }

            return new InsertOrUpdateResult<List<(string BadgeId, double CurrentStar)>>
            {
                Data = new List<(string BadgeId, double CurrentStar)>(),
                OperationType = DatabaseOperationType.None,
                Message = MessageConstants.NOTHING_WAS_UPDATED
            };
        }
        ...
        {
            Debug.LogError("Error: " + ex.Message);
            return InsertOrUpdateResult<List<(string BadgeId, double CurrentStar)>>.Failure(ex.Message);
        }
    }
    public async Task<Badges> GetBadgeCollectionByIdAsync(string userId, string objectId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            string selectSQL = @"
            SELECT cg.* 
            FROM badges_gallery cg, badges c 
            WHERE cg.user_id = @userId 
                AND cg.badge_id = @objectId 
                AND cg.status = 'available'
                AND cg.badge_id = c.id
    ...
    public async Task<InsertOrUpdateResult<bool>> UpdateBadgeGalleryPowerAsync(string userId, string id, Badges badge)
    {
        string connectionString = DatabaseConfig.ConnectionString;
