...
public class SpiritCardsGalleryRepository : ISpiritCardsGalleryRepository
{
    ...
    public async Task<InsertOrUpdateResult<SpiritCards>> InsertSpiritCardGalleryAsync(string userId, string id, SpiritCards spiritCard)
    {
        int percent = QualityEvaluatorHelper.CheckQuality(spiritCard.Rarity);
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        ";
            ...
            if (recordCount > 0)
            {
                return new InsertOrUpdateResult<SpiritCards>
                {
                    Data = spiritCard,
    ...
    public async Task<InsertOrUpdateResult<List<SpiritCards>>> InsertBatchSpiritCardsGalleryAsync(string userId, List<SpiritCards> spiritCards)
    {
        if (spiritCards == null || spiritCards.Count == 0)
        {
            return InsertOrUpdateResult<List<SpiritCards>>.Inserted(new List<SpiritCards>());
        }

        string connectionString = DatabaseConfig.ConnectionString;
        var insertedList = new List<SpiritCards>();
        int batchSize = 300;
        ...
        try
        {
            ...
            {
                var chunk = spiritCards.Skip(i).Take(batchSize).ToList();
                ...
                try
                {
                    ...
                    // INSERT IGNORE: Tự động bỏ qua các dòng đã trùng Primary Key (user_id, spirit_card_id)
                    sb.Append(@"
                    INSERT IGNORE INTO spirit_cards_gallery (
                        user_id, spirit_card_id, status, current_star, temp_star, power, health, 
                        physical_attack, physical_defense, magical_attack, magical_defense, 
                        chemical_attack, chemical_defense, atomic_attack, atomic_defense, 
                        mental_attack, mental_defense, speed, critical_damage_rate, critical_rate,
                    ...
                    for (int j = 0; j < chunk.Count; j++)
                    {
                        ...
                        valueSqls.Add($@"
                        (@user_id{suffix}, @spirit_card_id{suffix}, 'pending', 0, 0, @power{suffix}, @health{suffix},
                        ...
                        command.Parameters.AddWithValue($"@spirit_card_id{suffix}", item.Id);
            ...
            return InsertOrUpdateResult<List<SpiritCards>>.Inserted(insertedList);
        }
        catch (MySqlException ex)
        {
            Debug.LogError("Error Batch Insert: " + ex.Message);
            return InsertOrUpdateResult<List<SpiritCards>>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateStatusSpiritCardGalleryAsync(string userId, string id, string status = "available")
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        {
            Debug.LogError("Error UpdateStatusSpiritCardGallery: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateBatchStatusSpiritCardsGalleryAsync(string userId, string status = "available")
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        {
            Debug.LogError("Error UpdateBatchStatusSpiritCardsGallery: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<double>> UpdateTempStarSpiritCardGalleryAsync(string userId, string spiritCardId, double star)
    {
        ...
        try
        {
            ...
            await using MySqlCommand updateCommand = new MySqlCommand(updateSQL, connection);
            updateCommand.Parameters.AddWithValue("@user_id", userId);
            updateCommand.Parameters.AddWithValue("@spirit_card_id", spiritCardId);
    ...
    public async Task<InsertOrUpdateResult<double>> UpdateCurrentStarSpiritCardGalleryAsync(string userId, string spiritCardId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            if (rowsAffected > 0)
            {
                ...
                selectCommand.Parameters.AddWithValue("@spirit_card_id", spiritCardId);
    ...
    public async Task<InsertOrUpdateResult<List<(string SpiritCardId, double CurrentStar)>>> UpdateBatchCurrentStarSpiritCardsGalleryAsync(string userId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            if (rowsAffected > 0)
            {
                // Bước 2: Lấy danh sách spirit_card_id và current_star vừa được cập nhật (current_star == temp_star)
                string selectSQL = @"
                SELECT spirit_card_id, current_star 
                FROM spirit_cards_gallery 
                WHERE user_id = @user_id 
                ...
                var updatedList = new List<(string SpiritCardId, double CurrentStar)>();

                while (await reader.ReadAsync())
                {
                    string id = reader.GetString("spirit_card_id");
                    double star = reader.GetDouble("current_star");
                    updatedList.Add((id, star));
                }

                return InsertOrUpdateResult<List<(string SpiritCardId, double CurrentStar)>>.Updated(updatedList);
            }

            return new InsertOrUpdateResult<List<(string SpiritCardId, double CurrentStar)>>
            {
                Data = new List<(string SpiritCardId, double CurrentStar)>(),
                OperationType = DatabaseOperationType.None,
                Message = MessageConstants.NOTHING_WAS_UPDATED
            };
        }
        ...
        {
            Debug.LogError("Error: " + ex.Message);
            return InsertOrUpdateResult<List<(string SpiritCardId, double CurrentStar)>>.Failure(ex.Message);
        }
    }
    public async Task<SpiritCards> GetSpiritCardCollectionByIdAsync(string userId, string objectId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            string selectSQL = @"
            SELECT cg.* 
            FROM spirit_cards_gallery cg, spirit_cards c 
            WHERE cg.user_id = @userId 
                AND cg.spirit_card_id = @objectId 
                AND cg.status = 'available'
                AND cg.spirit_card_id = c.id
    ...
    public async Task<InsertOrUpdateResult<bool>> UpdateSpiritCardGalleryPowerAsync(string userId, string Id, SpiritCards spiritCard)
    {
        string connectionString = DatabaseConfig.ConnectionString;
