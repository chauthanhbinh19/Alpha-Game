...
public class CardMilitariesGalleryRepository : ICardMilitariesGalleryRepository
{
    ...
    public async Task<int> GetCardMilitariesCountAsync(string search, string type, string rare)
    {
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                string selectSQL = @"SELECT COUNT(*) FROM card_militaries m 
                WHERE 1=1 AND m.is_active = TRUE AND m.is_deleted = FALSE";
                if (!string.IsNullOrEmpty(type) && type != "All")
                {
    ...
    public async Task<InsertOrUpdateResult<CardMilitaries>> InsertCardMilitaryGalleryAsync(string userId, string id, CardMilitaries cardMilitary)
    {
        int percent = QualityEvaluatorHelper.CheckQuality(cardMilitary.Rarity);
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        ";
            ...
            if (recordCount > 0)
            {
                return new InsertOrUpdateResult<CardMilitaries>
                {
                    Data = cardMilitary,
    ...
    public async Task<InsertOrUpdateResult<List<CardMilitaries>>> InsertBatchCardMilitariesGalleryAsync(string userId, List<CardMilitaries> cardMilitaries)
    {
        if (cardMilitaries == null || cardMilitaries.Count == 0)
        {
            return InsertOrUpdateResult<List<CardMilitaries>>.Inserted(new List<CardMilitaries>());
        }

        string connectionString = DatabaseConfig.ConnectionString;
        var insertedList = new List<CardMilitaries>();
        int batchSize = 300;
        ...
        try
        {
            ...
            {
                var chunk = cardMilitaries.Skip(i).Take(batchSize).ToList();
                ...
                try
                {
                    ...
                    // INSERT IGNORE: Tự động bỏ qua các dòng đã trùng Primary Key (user_id, card_military_id)
                    sb.Append(@"
                    INSERT IGNORE INTO card_militaries_gallery (
                        user_id, card_military_id, status, current_star, temp_star, power, health, 
                        physical_attack, physical_defense, magical_attack, magical_defense, 
                        chemical_attack, chemical_defense, atomic_attack, atomic_defense, 
                        mental_attack, mental_defense, speed, critical_damage_rate, critical_rate,
                    ...
                    for (int j = 0; j < chunk.Count; j++)
                    {
                        ...
                        valueSqls.Add($@"
                        (@user_id{suffix}, @card_military_id{suffix}, 'pending', 0, 0, @power{suffix}, @health{suffix},
                        ...
                        command.Parameters.AddWithValue($"@card_military_id{suffix}", item.Id);
            ...
            return InsertOrUpdateResult<List<CardMilitaries>>.Inserted(insertedList);
        }
        catch (MySqlException ex)
        {
            Debug.LogError("Error Batch Insert: " + ex.Message);
            return InsertOrUpdateResult<List<CardMilitaries>>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateStatusCardMilitaryGalleryAsync(string userId, string id, string status = "available")
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            string updateSQL = @"UPDATE card_militaries_gallery g INNER JOIN card_militaries p ON p.id = g.card_military_id 
                             SET status = @status 
                             WHERE g.user_id = @user_id 
                               AND g.card_military_id = @card_military_id
        ...
        {
            Debug.LogError("Error UpdateStatusCardMilitaryGallery: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateBatchStatusCardMilitariesGalleryAsync(string userId, string status = "available")
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        {
            Debug.LogError("Error UpdateBatchStatusCardMilitariesGallery: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<double>> UpdateTempStarCardMilitaryGalleryAsync(string userId, string cardMilitaryId, double star)
    {
        ...
        try
        {
            ...
            string updateSQL = @"
            UPDATE card_militaries_gallery g INNER JOIN card_militaries p ON p.id = g.card_military_id 
            ...
            await using MySqlCommand updateCommand = new MySqlCommand(updateSQL, connection);
            updateCommand.Parameters.AddWithValue("@user_id", userId);
            updateCommand.Parameters.AddWithValue("@card_military_id", cardMilitaryId);
    ...
    public async Task<InsertOrUpdateResult<double>> UpdateCurrentStarCardMilitaryGalleryAsync(string userId, string cardMilitaryId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
    ...
    public async Task<InsertOrUpdateResult<List<(string CardMilitaryId, double CurrentStar)>>> UpdateBatchCurrentStarCardMilitariesGalleryAsync(string userId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            if (rowsAffected > 0)
            {
                // Bước 2: Lấy danh sách card_military_id và current_star vừa được cập nhật (current_star == temp_star)
                string selectSQL = @"
                SELECT card_military_id, current_star 
                FROM card_militaries_gallery 
                ...
                var updatedList = new List<(string CardMilitaryId, double CurrentStar)>();

                while (await reader.ReadAsync())
                {
                    string id = reader.GetString("card_military_id");
                    double star = reader.GetDouble("current_star");
                    updatedList.Add((id, star));
                }

                return InsertOrUpdateResult<List<(string CardMilitaryId, double CurrentStar)>>.Updated(updatedList);
            }

            return new InsertOrUpdateResult<List<(string CardMilitaryId, double CurrentStar)>>
            {
                Data = new List<(string CardMilitaryId, double CurrentStar)>(),
                OperationType = DatabaseOperationType.None,
                Message = MessageConstants.NOTHING_WAS_UPDATED
            };
        }
        ...
        {
            Debug.LogError("Error: " + ex.Message);
            return InsertOrUpdateResult<List<(string CardMilitaryId, double CurrentStar)>>.Failure(ex.Message);
        }
    }
    public async Task<CardMilitaries> GetCardMilitaryCollectionByIdAsync(string userId, string objectId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            string selectSQL = @"
            SELECT cg.* 
            FROM card_militaries_gallery cg, card_militaries c 
            WHERE cg.user_id = @userId 
                AND cg.card_military_id = @objectId 
                AND cg.status = 'available'
                AND cg.card_military_id = c.id
    ...
    public async Task<InsertOrUpdateResult<bool>> UpdateCardMilitaryGalleryPowerAsync(string userId, string id, CardMilitaries cardMilitary)
    {
        string connectionString = DatabaseConfig.ConnectionString;
