...
public class CardCaptainsGalleryRepository : ICardCaptainsGalleryRepository
{
    ...
    public async Task<InsertOrUpdateResult<CardCaptains>> InsertCardCaptainGalleryAsync(string userId, string id, CardCaptains cardCaptain)
    {
        int percent = QualityEvaluatorHelper.CheckQuality(cardCaptain.Rarity);
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        ";
            ...
            if (recordCount > 0)
            {
                return new InsertOrUpdateResult<CardCaptains>
                {
                    Data = cardCaptain,
    ...
    public async Task<InsertOrUpdateResult<List<CardCaptains>>> InsertBatchCardCaptainsGalleryAsync(string userId, List<CardCaptains> cardCaptains)
    {
        if (cardCaptains == null || cardCaptains.Count == 0)
        {
            return InsertOrUpdateResult<List<CardCaptains>>.Inserted(new List<CardCaptains>());
        }

        string connectionString = DatabaseConfig.ConnectionString;
        var insertedList = new List<CardCaptains>();
        int batchSize = 300;
        ...
        try
        {
            ...
            {
                var chunk = cardCaptains.Skip(i).Take(batchSize).ToList();
                ...
                try
                {
                    ...
                    // INSERT IGNORE: Tự động bỏ qua các dòng đã trùng Primary Key (user_id, card_captain_id)
                    sb.Append(@"
                    INSERT IGNORE INTO card_captains_gallery (
                        user_id, card_captain_id, status, current_star, temp_star, power, health, 
                        physical_attack, physical_defense, magical_attack, magical_defense, 
                        chemical_attack, chemical_defense, atomic_attack, atomic_defense, 
                        mental_attack, mental_defense, speed, critical_damage_rate, critical_rate,
                    ...
                    for (int j = 0; j < chunk.Count; j++)
                    {
                        ...
                        valueSqls.Add($@"
                        (@user_id{suffix}, @card_captain_id{suffix}, 'pending', 0, 0, @power{suffix}, @health{suffix},
                        ...
                        command.Parameters.AddWithValue($"@card_captain_id{suffix}", item.Id);
            ...
            return InsertOrUpdateResult<List<CardCaptains>>.Inserted(insertedList);
        }
        catch (MySqlException ex)
        {
            Debug.LogError("Error Batch Insert: " + ex.Message);
            return InsertOrUpdateResult<List<CardCaptains>>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateStatusCardCaptainGalleryAsync(string userId, string id, string status = "available")
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        {
            Debug.LogError("Error UpdateStatusCardCaptainGallery: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateBatchStatusCardCaptainsGalleryAsync(string userId, string status = "available")
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        {
            Debug.LogError("Error UpdateBatchStatusCardCaptainsGallery: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<double>> UpdateTempStarCardCaptainGalleryAsync(string userId, string cardCaptainId, double star)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            await using MySqlCommand updateCommand = new MySqlCommand(updateSQL, connection);
            updateCommand.Parameters.AddWithValue("@user_id", userId);
            updateCommand.Parameters.AddWithValue("@card_captain_id", cardCaptainId);
    ...
    public async Task<InsertOrUpdateResult<double>> UpdateCurrentStarCardCaptainGalleryAsync(string userId, string cardCaptainId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
    ...
    public async Task<InsertOrUpdateResult<List<(string CardCaptainId, double CurrentStar)>>> UpdateBatchCurrentStarCardCaptainsGalleryAsync(string userId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            if (rowsAffected > 0)
            {
                // Bước 2: Lấy danh sách card_captain_id và current_star vừa được cập nhật (current_star == temp_star)
                string selectSQL = @"
                SELECT card_captain_id, current_star 
                FROM card_captains_gallery 
                ...
                var updatedList = new List<(string CardCaptainId, double CurrentStar)>();

                while (await reader.ReadAsync())
                {
                    string id = reader.GetString("card_captain_id");
                    double star = reader.GetDouble("current_star");
                    updatedList.Add((id, star));
                }

                return InsertOrUpdateResult<List<(string CardCaptainId, double CurrentStar)>>.Updated(updatedList);
            }

            return new InsertOrUpdateResult<List<(string CardCaptainId, double CurrentStar)>>
            {
                Data = new List<(string CardCaptainId, double CurrentStar)>(),
                OperationType = DatabaseOperationType.None,
                Message = MessageConstants.NOTHING_WAS_UPDATED
            };
        }
        ...
        {
            Debug.LogError("Error: " + ex.Message);
            return InsertOrUpdateResult<List<(string CardCaptainId, double CurrentStar)>>.Failure(ex.Message);
        }
    }
    public async Task<CardCaptains> GetCardCaptainCollectionByIdAsync(string userId, string objectId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            string selectSQL = @"
            SELECT cg.* 
            FROM card_captains_gallery cg, card_captains c 
            WHERE cg.user_id = @userId 
                AND cg.card_captain_id = @objectId 
                AND cg.status = 'available'
                AND cg.card_captain_id = c.id
    ...
    public async Task<InsertOrUpdateResult<bool>> UpdateCardCaptainGalleryPowerAsync(string userId, string id, CardCaptains cardCaptain)
    {
        string connectionString = DatabaseConfig.ConnectionString;
