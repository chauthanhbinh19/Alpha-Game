...
public class CoresGalleryRepository : ICoresGalleryRepository
{
    ...
    public async Task<Cores> GetCoreCollectionByIdAsync(string userId, string objectId)
    {
        ...
        try
        {
            ...
            string selectSQL = @"
            SELECT cg.* 
            FROM cores_gallery cg, cores c 
            WHERE cg.user_id = @userId 
                AND cg.core_id = @objectId 
    ...
    public async Task<InsertOrUpdateResult<bool>> UpdateCoreGalleryPowerAsync(string userId, string id, Cores core)
    {
        string connectionString = DatabaseConfig.ConnectionString;
