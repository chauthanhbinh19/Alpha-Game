...
public class SpiritBeastsGalleryRepository : ISpiritBeastsGalleryRepository
{
    ...
    public async Task<int> GetSpiritBeastsCountAsync(string search, string rare)
    {
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                string selectSQL = @"SELECT COUNT(*) FROM spirit_beasts c 
                WHERE 1=1 AND c.is_active = TRUE AND c.is_deleted = FALSE";
                if (!string.IsNullOrEmpty(rare) && rare != "All")
                {
                    selectSQL += " AND rare = @rare";
    ...
    public async Task<SpiritBeasts> GetSpiritBeastCollectionByIdAsync(string userId, string objectId)
    {
        ...
        try
        {
            ...
            string selectSQL = @"
            SELECT cg.* 
            FROM spirit_beasts_gallery cg, spirit_beasts c 
            WHERE cg.user_id = @userId 
                AND cg.spirit_beast_id = @objectId 
                AND cg.status = 'available'
                AND cg.spirit_beast_id = c.id
    ...
    public async Task<InsertOrUpdateResult<bool>> UpdateSpiritBeastGalleryPowerAsync(string userId, string id, SpiritBeasts spiritBeast)
    {
        string connectionString = DatabaseConfig.ConnectionString;
