...
public class CollaborationsGalleryRepository : ICollaborationsGalleryRepository
{
    public async Task<List<Collaborations>> GetCollaborationsCollectionAsync(string userId, string search, int pageSize, int offset, string rare)
    {
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                SELECT c.*, 
                       CASE 
                           WHEN cg.collaboration_id IS NULL THEN 'block' 
                           WHEN cg.status = 'pending' THEN 'pending' 
                           WHEN cg.status = 'available' THEN 'available' 
                       END AS status 
                FROM Collaborations c 
                LEFT JOIN collaborations_gallery cg 
                       ON c.id = cg.collaboration_id AND cg.user_id = @userId 
    ...
    public async Task<int> GetCollaborationsCountAsync(string search, string rare)
    {
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                string selectSQL = @"SELECT COUNT(*) FROM Collaborations c WHERE 1=1 AND c.is_active = TRUE AND c.is_deleted = FALSE";
    ...
    public async Task<InsertOrUpdateResult<Collaborations>> InsertCollaborationGalleryAsync(string userId, string id, Collaborations collaboration)
    {
        int percent = QualityEvaluatorHelper.CheckQuality(collaboration.Rarity);
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        ";
            ...
            if (recordCount > 0)
            {
                return new InsertOrUpdateResult<Collaborations>
                {
                    Data = collaboration,
            ...
            INSERT INTO collaborations_gallery (
                user_id, collaboration_id, status, current_star, temp_star, power, health, 
            ...
            insertCommand.Parameters.AddWithValue("@collaboration_id", id);
    ...
    public async Task<InsertOrUpdateResult<List<Collaborations>>> InsertBatchCollaborationsGalleryAsync(string userId, List<Collaborations> collaborations)
    {
        if (collaborations == null || collaborations.Count == 0)
        {
            return InsertOrUpdateResult<List<Collaborations>>.Inserted(new List<Collaborations>());
        }

        string connectionString = DatabaseConfig.ConnectionString;
        var insertedList = new List<Collaborations>();
        int batchSize = 300;
        ...
        try
        {
            ...
            {
                var chunk = collaborations.Skip(i).Take(batchSize).ToList();
                ...
                try
                {
                    ...
                    // INSERT IGNORE: Tự động bỏ qua các dòng đã trùng Primary Key (user_id, collaboration_id)
                    sb.Append(@"
                    INSERT IGNORE INTO collaborations_gallery (
                        user_id, collaboration_id, status, current_star, temp_star, power, health, 
                        physical_attack, physical_defense, magical_attack, magical_defense, 
                        chemical_attack, chemical_defense, atomic_attack, atomic_defense, 
                        mental_attack, mental_defense, speed, critical_damage_rate, critical_rate,
                    ...
                    for (int j = 0; j < chunk.Count; j++)
                    {
                        ...
                        valueSqls.Add($@"
                        (@user_id{suffix}, @collaboration_id{suffix}, 'pending', 0, 0, @power{suffix}, @health{suffix},
                        ...
                        command.Parameters.AddWithValue($"@collaboration_id{suffix}", item.Id);
            ...
            return InsertOrUpdateResult<List<Collaborations>>.Inserted(insertedList);
        }
        catch (MySqlException ex)
        {
            Debug.LogError("Error Batch Insert: " + ex.Message);
            return InsertOrUpdateResult<List<Collaborations>>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateStatusCollaborationGalleryAsync(string userId, string id, string status = "available")
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            string updateSQL = @"UPDATE collaborations_gallery 
                             SET status = @status 
        ...
        {
            Debug.LogError("Error UpdateStatusCollaborationGallery: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateBatchStatusCollaborationsGalleryAsync(string userId, string status = "available")
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        {
            Debug.LogError("Error UpdateBatchStatusCollaborationsGallery: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<double>> UpdateTempStarCollaborationGalleryAsync(string userId, string collaborationId, double star)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            await using MySqlCommand updateCommand = new MySqlCommand(updateSQL, connection);
            updateCommand.Parameters.AddWithValue("@user_id", userId);
            updateCommand.Parameters.AddWithValue("@collaboration_id", collaborationId);
    ...
    public async Task<InsertOrUpdateResult<double>> UpdateCurrentStarCollaborationGalleryAsync(string userId, string collaborationId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            if (rowsAffected > 0)
            {
                ...
                selectCommand.Parameters.AddWithValue("@collaboration_id", collaborationId);
    ...
    public async Task<InsertOrUpdateResult<List<(string CollaborationId, double CurrentStar)>>> UpdateBatchCurrentStarCollaborationsGalleryAsync(string userId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            if (rowsAffected > 0)
            {
                // Bước 2: Lấy danh sách collaboration_id và current_star vừa được cập nhật (current_star == temp_star)
                string selectSQL = @"
                SELECT collaboration_id, current_star 
                FROM collaborations_gallery 
                WHERE user_id = @user_id 
                ...
                var updatedList = new List<(string CollaborationId, double CurrentStar)>();

                while (await reader.ReadAsync())
                {
                    string id = reader.GetString("collaboration_id");
                    double star = reader.GetDouble("current_star");
                    updatedList.Add((id, star));
                }

                return InsertOrUpdateResult<List<(string CollaborationId, double CurrentStar)>>.Updated(updatedList);
            }

            return new InsertOrUpdateResult<List<(string CollaborationId, double CurrentStar)>>
            {
                Data = new List<(string CollaborationId, double CurrentStar)>(),
                OperationType = DatabaseOperationType.None,
                Message = MessageConstants.NOTHING_WAS_UPDATED
            };
        }
        ...
        {
            Debug.LogError("Error: " + ex.Message);
            return InsertOrUpdateResult<List<(string CollaborationId, double CurrentStar)>>.Failure(ex.Message);
        }
    }
    public async Task<Collaborations> GetCollaborationCollectionByIdAsync(string userId, string objectId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            string selectSQL = @"
            SELECT cg.* 
            FROM collaborations_gallery cg, collaborations c 
            WHERE cg.user_id = @userId 
                AND cg.collaboration_id = @objectId 
                AND cg.status = 'available'
                AND cg.collaboration_id = c.id
    ...
    public async Task<InsertOrUpdateResult<bool>> UpdateCollaborationGalleryPowerAsync(string userId, string id, Collaborations collaboration)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                await using (MySqlDataReader reader = await selectCommand.ExecuteReaderAsync())
                {
                    if (await reader.ReadAsync())
                    {
                        sumCollaborations.Power = reader["total_power"] as double? ?? 0;
                        sumCollaborations.Health = reader["total_health"] as double? ?? 0;
                        sumCollaborations.Mana = reader["total_mana"] as double? ?? 0f;

                        sumCollaborations.PhysicalAttack = reader["total_physical_attack"] as double? ?? 0;
                        sumCollaborations.PhysicalDefense = reader["total_physical_defense"] as double? ?? 0;
                        sumCollaborations.MagicalAttack = reader["total_magical_attack"] as double? ?? 0;
                        sumCollaborations.MagicalDefense = reader["total_magical_defense"] as double? ?? 0;
                        sumCollaborations.ChemicalAttack = reader["total_chemical_attack"] as double? ?? 0;
                        sumCollaborations.ChemicalDefense = reader["total_chemical_defense"] as double? ?? 0;
                        sumCollaborations.AtomicAttack = reader["total_atomic_attack"] as double? ?? 0;
                        sumCollaborations.AtomicDefense = reader["total_atomic_defense"] as double? ?? 0;
                        sumCollaborations.MentalAttack = reader["total_mental_attack"] as double? ?? 0;
                        sumCollaborations.MentalDefense = reader["total_mental_defense"] as double? ?? 0;

                        sumCollaborations.Speed = reader["total_speed"] as double? ?? 0;
                        sumCollaborations.CriticalDamageRate = reader["total_critical_damage_rate"] as double? ?? 0;
                        sumCollaborations.CriticalRate = reader["total_critical_rate"] as double? ?? 0;
                        sumCollaborations.CriticalResistanceRate = reader["total_critical_resistance_rate"] as double? ?? 0;

                        sumCollaborations.IgnoreCriticalRate = reader["total_ignore_critical_rate"] as double? ?? 0;
                        sumCollaborations.PenetrationRate = reader["total_penetration_rate"] as double? ?? 0;
                        sumCollaborations.PenetrationResistanceRate = reader["total_penetration_resistance_rate"] as double? ?? 0;

                        sumCollaborations.EvasionRate = reader["total_evasion_rate"] as double? ?? 0;
                        sumCollaborations.DamageAbsorptionRate = reader["total_damage_absorption_rate"] as double? ?? 0;
                        sumCollaborations.IgnoreDamageAbsorptionRate = reader["total_ignore_damage_absorption_rate"] as double? ?? 0;
                        sumCollaborations.AbsorbedDamageRate = reader["total_absorbed_damage_rate"] as double? ?? 0;

                        sumCollaborations.VitalityRegenerationRate = reader["total_vitality_regeneration_rate"] as double? ?? 0;
                        sumCollaborations.VitalityRegenerationResistanceRate = reader["total_vitality_regeneration_resistance_rate"] as double? ?? 0;

                        sumCollaborations.AccuracyRate = reader["total_accuracy_rate"] as double? ?? 0;
                        sumCollaborations.LifestealRate = reader["total_lifesteal_rate"] as double? ?? 0;
                        sumCollaborations.ShieldStrength = reader["total_shield_strength"] as double? ?? 0;

                        sumCollaborations.Tenacity = reader["total_tenacity"] as double? ?? 0;
                        sumCollaborations.ResistanceRate = reader["total_resistance_rate"] as double? ?? 0;

                        sumCollaborations.ComboRate = reader["total_combo_rate"] as double? ?? 0;
                        sumCollaborations.IgnoreComboRate = reader["total_ignore_combo_rate"] as double? ?? 0;
                        sumCollaborations.ComboDamageRate = reader["total_combo_damage_rate"] as double? ?? 0;
                        sumCollaborations.ComboResistanceRate = reader["total_combo_resistance_rate"] as double? ?? 0;

                        sumCollaborations.StunRate = reader["total_stun_rate"] as double? ?? 0;
                        sumCollaborations.IgnoreStunRate = reader["total_ignore_stun_rate"] as double? ?? 0;

                        sumCollaborations.ReflectionRate = reader["total_reflection_rate"] as double? ?? 0;
                        sumCollaborations.IgnoreReflectionRate = reader["total_ignore_reflection_rate"] as double? ?? 0;
                        sumCollaborations.ReflectionDamageRate = reader["total_reflection_damage_rate"] as double? ?? 0;
                        sumCollaborations.ReflectionResistanceRate = reader["total_reflection_resistance_rate"] as double? ?? 0;

                        sumCollaborations.ManaRegenerationRate = reader["total_mana_regeneration_rate"] as double? ?? 0;

                        sumCollaborations.DamageToDifferentFactionRate = reader["total_damage_to_different_faction_rate"] as double? ?? 0;
                        sumCollaborations.ResistanceToDifferentFactionRate = reader["total_resistance_to_different_faction_rate"] as double? ?? 0;

                        sumCollaborations.DamageToSameFactionRate = reader["total_damage_to_same_faction_rate"] as double? ?? 0;
                        sumCollaborations.ResistanceToSameFactionRate = reader["total_resistance_to_same_faction_rate"] as double? ?? 0;

                        sumCollaborations.NormalDamageRate = reader["total_normal_damage_rate"] as double? ?? 0;
                        sumCollaborations.NormalResistanceRate = reader["total_normal_resistance_rate"] as double? ?? 0;

                        sumCollaborations.SkillDamageRate = reader["total_skill_damage_rate"] as double? ?? 0;
                        sumCollaborations.SkillResistanceRate = reader["total_skill_resistance_rate"] as double? ?? 0;

                        sumCollaborations.PercentAllHealth = reader["total_percent_all_health"] as double? ?? 0;
                        sumCollaborations.PercentAllPhysicalAttack = reader["total_percent_all_physical_attack"] as double? ?? 0;
                        sumCollaborations.PercentAllPhysicalDefense = reader["total_percent_all_physical_defense"] as double? ?? 0;
                        sumCollaborations.PercentAllMagicalAttack = reader["total_percent_all_magical_attack"] as double? ?? 0;
                        sumCollaborations.PercentAllMagicalDefense = reader["total_percent_all_magical_defense"] as double? ?? 0;
                        sumCollaborations.PercentAllChemicalAttack = reader["total_percent_all_chemical_attack"] as double? ?? 0;
                        sumCollaborations.PercentAllChemicalDefense = reader["total_percent_all_chemical_defense"] as double? ?? 0;
                        sumCollaborations.PercentAllAtomicAttack = reader["total_percent_all_atomic_attack"] as double? ?? 0;
                        sumCollaborations.PercentAllAtomicDefense = reader["total_percent_all_atomic_defense"] as double? ?? 0;
                        sumCollaborations.PercentAllMentalAttack = reader["total_percent_all_mental_attack"] as double? ?? 0;
                        sumCollaborations.PercentAllMentalDefense = reader["total_percent_all_mental_defense"] as double? ?? 0;
                    }
                }
            }
            catch (MySqlException ex)
