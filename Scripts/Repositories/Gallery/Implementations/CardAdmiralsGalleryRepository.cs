...
public class CardAdmiralsGalleryRepository : ICardAdmiralsGalleryRepository
{
    ...
    public async Task<InsertOrUpdateResult<bool>> UpdateStatusCardAdmiralGalleryAsync(string userId, string id, string status = "available")
    {
        ...
        try
        {
            ...
            string updateSQL = @"UPDATE card_admirals_gallery g INNER JOIN card_admirals p ON p.id = g.card_admiral_id 
                             SET status = @status 
                             WHERE g.user_id = @user_id 
                               AND g.card_admiral_id = @card_admiral_id
    ...
    public async Task<InsertOrUpdateResult<double>> UpdateTempStarCardAdmiralGalleryAsync(string userId, string cardAdmiralId, double star)
    {
        ...
        try
        {
            ...
            string updateSQL = @"
            UPDATE card_admirals_gallery g INNER JOIN card_admirals p ON p.id = g.card_admiral_id 
    ...
    public async Task<CardAdmirals> GetCardAdmiralCollectionByIdAsync(string userId, string objectId)
    {
        ...
        try
        {
            ...
            string selectSQL = @"
            SELECT cg.* 
            FROM card_admirals_gallery cg, card_admirals c 
            WHERE cg.user_id = @userId 
                AND cg.card_admiral_id = @objectId 
                AND cg.status = 'available'
                AND cg.card_admiral_id = c.id
    ...
    public async Task<InsertOrUpdateResult<bool>> UpdateCardAdmiralGalleryPowerAsync(string userId, string id, CardAdmirals cardAdmiral)
    {
        string connectionString = DatabaseConfig.ConnectionString;
