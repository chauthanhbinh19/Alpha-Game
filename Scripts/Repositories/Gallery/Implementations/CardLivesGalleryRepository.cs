...
public class CardLivesGalleryRepository : ICardLivesGalleryRepository
{
    ...
    public async Task<int> GetCardLivesCountAsync(string search, string type, string rare)
    {
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                string selectSQL = @"SELECT COUNT(*) FROM card_lives m 
                WHERE 1=1 AND m.is_active = TRUE AND m.is_deleted = FALSE";
                if (!string.IsNullOrEmpty(type) && type != "All")
                {
    ...
    public async Task<CardLives> GetCardLifeCollectionByIdAsync(string userId, string objectId)
    {
        ...
        try
        {
            ...
            string selectSQL = @"
            SELECT cg.* 
            FROM card_lives_gallery cg, card_lives c 
            WHERE cg.user_id = @userId 
                AND cg.card_life_id = @objectId 
                AND cg.status = 'available'
                AND cg.card_life_id = c.id
    ...
    public async Task<InsertOrUpdateResult<bool>> UpdateCardLifeGalleryPowerAsync(string userId, string id, CardLives cardLife)
    {
        string connectionString = DatabaseConfig.ConnectionString;
