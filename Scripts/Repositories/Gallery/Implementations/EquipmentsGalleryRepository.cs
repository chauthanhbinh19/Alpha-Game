...
public class EquipmentsGalleryRepository : IEquipmentsGalleryRepository
{
    ...
    public async Task<InsertOrUpdateResult<Equipments>> InsertEquipmentGalleryAsync(string userId, string id, Equipments equipment)
    {
        int percent = QualityEvaluatorHelper.CheckQuality(equipment.Rarity);
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        ";
            ...
            if (recordCount > 0)
            {
                return new InsertOrUpdateResult<Equipments>
                {
                    Data = equipment,
    ...
    public async Task<InsertOrUpdateResult<List<Equipments>>> InsertBatchEquipmentsGalleryAsync(string userId, List<Equipments> equipments)
    {
        if (equipments == null || equipments.Count == 0)
        {
            return InsertOrUpdateResult<List<Equipments>>.Inserted(new List<Equipments>());
        }

        string connectionString = DatabaseConfig.ConnectionString;
        var insertedList = new List<Equipments>();
        int batchSize = 300;
        ...
        try
        {
            ...
            {
                var chunk = equipments.Skip(i).Take(batchSize).ToList();
                ...
                try
                {
                    ...
                    // INSERT IGNORE: Tự động bỏ qua các dòng đã trùng Primary Key (user_id, equipment_id)
                    sb.Append(@"
                    INSERT IGNORE INTO equipments_gallery (
                        user_id, equipment_id, status, current_star, temp_star, power, health, 
                        physical_attack, physical_defense, magical_attack, magical_defense, 
                        chemical_attack, chemical_defense, atomic_attack, atomic_defense, 
                        mental_attack, mental_defense, speed, critical_damage_rate, critical_rate,
                    ...
                    for (int j = 0; j < chunk.Count; j++)
                    {
                        ...
                        valueSqls.Add($@"
                        (@user_id{suffix}, @equipment_id{suffix}, 'pending', 0, 0, @power{suffix}, @health{suffix},
                        ...
                        command.Parameters.AddWithValue($"@equipment_id{suffix}", item.Id);
            ...
            return InsertOrUpdateResult<List<Equipments>>.Inserted(insertedList);
        }
        catch (MySqlException ex)
        {
            Debug.LogError("Error Batch Insert: " + ex.Message);
            return InsertOrUpdateResult<List<Equipments>>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateStatusEquipmentGalleryAsync(string userId, string id, string status = "available")
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        {
            Debug.LogError("Error UpdateStatusEquipmentGallery: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateBatchStatusEquipmentsGalleryAsync(string userId, string status = "available")
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        {
            Debug.LogError("Error UpdateBatchStatusEquipmentsGallery: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<double>> UpdateTempStarEquipmentGalleryAsync(string userId, string equipmentId, double star)
    {
        ...
        try
        {
            ...
            await using MySqlCommand updateCommand = new MySqlCommand(updateSQL, connection);
            updateCommand.Parameters.AddWithValue("@user_id", userId);
            updateCommand.Parameters.AddWithValue("@equipment_id", equipmentId);
    ...
    public async Task<InsertOrUpdateResult<double>> UpdateCurrentStarEquipmentGalleryAsync(string userId, string equipmentId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
    ...
    public async Task<InsertOrUpdateResult<List<(string EquipmentId, double CurrentStar)>>> UpdateBatchCurrentStarEquipmentsGalleryAsync(string userId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            if (rowsAffected > 0)
            {
                // Bước 2: Lấy danh sách equipment_id và current_star vừa được cập nhật (current_star == temp_star)
                string selectSQL = @"
                SELECT equipment_id, current_star 
                FROM equipments_gallery 
                WHERE user_id = @user_id 
                ...
                var updatedList = new List<(string EquipmentId, double CurrentStar)>();

                while (await reader.ReadAsync())
                {
                    string id = reader.GetString("equipment_id");
                    double star = reader.GetDouble("current_star");
                    updatedList.Add((id, star));
                }

                return InsertOrUpdateResult<List<(string EquipmentId, double CurrentStar)>>.Updated(updatedList);
            }

            return new InsertOrUpdateResult<List<(string EquipmentId, double CurrentStar)>>
            {
                Data = new List<(string EquipmentId, double CurrentStar)>(),
                OperationType = DatabaseOperationType.None,
                Message = MessageConstants.NOTHING_WAS_UPDATED
            };
        }
        ...
        {
            Debug.LogError("Error: " + ex.Message);
            return InsertOrUpdateResult<List<(string EquipmentId, double CurrentStar)>>.Failure(ex.Message);
        }
    }
    public async Task<Equipments> GetEquipmentCollectionByIdAsync(string userId, string objectId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            string selectSQL = @"
            SELECT cg.* 
            FROM equipments_gallery cg, equipments c 
            WHERE cg.user_id = @userId 
                AND cg.equipment_id = @objectId 
                AND cg.status = 'available'
                AND cg.equipment_id = c.id
    ...
    public async Task<InsertOrUpdateResult<bool>> UpdateEquipmentGalleryPowerAsync(string userId, string Id, Equipments equipment)
    {
        string connectionString = DatabaseConfig.ConnectionString;
