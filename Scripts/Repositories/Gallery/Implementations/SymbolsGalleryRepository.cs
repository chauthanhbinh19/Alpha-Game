...
public class SymbolsGalleryRepository : ISymbolsGalleryRepository
{
    ...
    public async Task<Symbols> GetSymbolCollectionByIdAsync(string userId, string objectId)
    {
        ...
        try
        {
            ...
            string selectSQL = @"
            SELECT cg.* 
            FROM symbols_gallery cg, symbols c 
            WHERE cg.user_id = @userId 
                AND cg.symbol_id = @objectId 
                AND cg.status = 'available'
                AND cg.symbol_id = c.id
    ...
    public async Task<InsertOrUpdateResult<bool>> UpdateSymbolGalleryPowerAsync(string userId, string Id, Symbols symbol)
    {
        string connectionString = DatabaseConfig.ConnectionString;
