...
public class VehiclesGalleryRepository : IVehiclesGalleryRepository
{
    ...
    public async Task<int> GetVehiclesCountAsync(string search, string type, string rare)
    {
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                string selectSQL = @"SELECT COUNT(*) FROM Vehicles m 
                WHERE 1=1 AND m.is_active = TRUE AND m.is_deleted = FALSE";
                if (!string.IsNullOrEmpty(type) && type != "All")
                {
    ...
    public async Task<Vehicles> GetVehicleCollectionByIdAsync(string userId, string objectId)
    {
        ...
        try
        {
            ...
            string selectSQL = @"
            SELECT cg.* 
            FROM vehicles_gallery cg, vehicles c 
            WHERE cg.user_id = @userId 
                AND cg.vehicle_id = @objectId 
                AND cg.status = 'available'
                AND cg.vehicle_id = c.id
    ...
    public async Task<InsertOrUpdateResult<bool>> UpdateVehicleGalleryPowerAsync(string userId, string Id, Vehicles vehicle)
    {
        string connectionString = DatabaseConfig.ConnectionString;
