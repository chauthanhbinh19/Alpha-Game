...
public class PetsGalleryRepository : IPetsGalleryRepository
{
    ...
    public async Task<int> GetPetsCountAsync(string search, string type, string rare)
    {
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                string selectSQL = @"SELECT COUNT(*) FROM Pets m 
                WHERE 1=1 AND m.is_active = TRUE AND m.is_deleted = FALSE";
                if (!string.IsNullOrEmpty(type) && type != "All")
                {
    ...
    public async Task<InsertOrUpdateResult<Pets>> InsertPetGalleryAsync(string userId, string id, Pets pet)
    {
        int percent = QualityEvaluatorHelper.CheckQuality(pet.Rarity);
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        ";
            ...
            if (recordCount > 0)
            {
                return new InsertOrUpdateResult<Pets>
                {
                    Data = pet,
    ...
    public async Task<InsertOrUpdateResult<List<Pets>>> InsertBatchPetsGalleryAsync(string userId, List<Pets> pets)
    {
        if (pets == null || pets.Count == 0)
        {
            return InsertOrUpdateResult<List<Pets>>.Inserted(new List<Pets>());
        }

        string connectionString = DatabaseConfig.ConnectionString;
        var insertedList = new List<Pets>();
        int batchSize = 300;
        ...
        try
        {
            ...
            for (int i = 0; i < pets.Count; i += batchSize)
            {
                var chunk = pets.Skip(i).Take(batchSize).ToList();
                ...
                try
                {
                    ...
                    // INSERT IGNORE: Tự động bỏ qua các dòng đã trùng Primary Key (user_id, pet_id)
                    sb.Append(@"
                    INSERT IGNORE INTO pets_gallery (
                        user_id, pet_id, status, current_star, temp_star, power, health, 
                        physical_attack, physical_defense, magical_attack, magical_defense, 
                        chemical_attack, chemical_defense, atomic_attack, atomic_defense, 
                        mental_attack, mental_defense, speed, critical_damage_rate, critical_rate,
                    ...
                    for (int j = 0; j < chunk.Count; j++)
                    {
                        ...
                        valueSqls.Add($@"
                        (@user_id{suffix}, @pet_id{suffix}, 'pending', 0, 0, @power{suffix}, @health{suffix},
                        ...
                        command.Parameters.AddWithValue($"@pet_id{suffix}", item.Id);
            ...
            return InsertOrUpdateResult<List<Pets>>.Inserted(insertedList);
        }
        catch (MySqlException ex)
        {
            Debug.LogError("Error Batch Insert: " + ex.Message);
            return InsertOrUpdateResult<List<Pets>>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateStatusPetGalleryAsync(string userId, string id, string status = "available")
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        {
            Debug.LogError("Error UpdateStatusPetGallery: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateBatchStatusPetsGalleryAsync(string userId, string status = "available")
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            string updateSQL = @"UPDATE card_heroes_gallery 
                             SET status = @status 
                             WHERE user_id = @user_id 
                               AND (status IS NULL OR status != @status);";
        ...
        {
            Debug.LogError("Error UpdateBatchStatusPetsGallery: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
    ...
    public async Task<InsertOrUpdateResult<double>> UpdateCurrentStarPetGalleryAsync(string userId, string petId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
    ...
    public async Task<InsertOrUpdateResult<List<(string PetId, double CurrentStar)>>> UpdateBatchCurrentStarPetsGalleryAsync(string userId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            if (rowsAffected > 0)
            {
                // Bước 2: Lấy danh sách pet_id và current_star vừa được cập nhật (current_star == temp_star)
                string selectSQL = @"
                SELECT pet_id, current_star 
                FROM pets_gallery 
                WHERE user_id = @user_id 
                ...
                var updatedList = new List<(string PetId, double CurrentStar)>();

                while (await reader.ReadAsync())
                {
                    string id = reader.GetString("pet_id");
                    double star = reader.GetDouble("current_star");
                    updatedList.Add((id, star));
                }

                return InsertOrUpdateResult<List<(string PetId, double CurrentStar)>>.Updated(updatedList);
            }

            return new InsertOrUpdateResult<List<(string PetId, double CurrentStar)>>
            {
                Data = new List<(string PetId, double CurrentStar)>(),
                OperationType = DatabaseOperationType.None,
                Message = MessageConstants.NOTHING_WAS_UPDATED
            };
        }
        ...
        {
            Debug.LogError("Error: " + ex.Message);
            return InsertOrUpdateResult<List<(string PetId, double CurrentStar)>>.Failure(ex.Message);
        }
    }
    public async Task<Pets> GetPetCollectionByIdAsync(string userId, string objectId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            string selectSQL = @"
            SELECT cg.* 
            FROM pets_gallery cg, pets c 
            WHERE cg.user_id = @userId 
                AND cg.pet_id = @objectId 
    ...
    public async Task<InsertOrUpdateResult<bool>> UpdatePetGalleryPowerAsync(string userId, string Id, Pets pet)
    {
        string connectionString = DatabaseConfig.ConnectionString;
