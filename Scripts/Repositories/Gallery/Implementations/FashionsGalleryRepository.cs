...
public class FashionsGalleryRepository : IFashionsGalleryRepository
{
    ...
    public async Task<int> GetFashionsCountAsync(string search, string type, string rare)
    {
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                string selectSQL = @"SELECT COUNT(*) FROM Fashions m 
                WHERE 1=1 AND m.is_active = TRUE AND m.is_deleted = FALSE";
                if (!string.IsNullOrEmpty(type) && type != "All")
                {
    ...
    public async Task<InsertOrUpdateResult<double>> UpdateTempStarFashionGalleryAsync(string userId, string fashionId, double star)
    {
        string connectionString = DatabaseConfig.ConnectionString;
    ...
    public async Task<Fashions> GetFashionCollectionByIdAsync(string userId, string objectId)
    {
        ...
        try
        {
            ...
            string selectSQL = @"
            SELECT cg.* 
            FROM fashions_gallery cg, fashions c 
            WHERE cg.user_id = @userId 
                AND cg.fashion_id = @objectId 
                AND cg.status = 'available'
                AND cg.fashion_id = c.id
    ...
    public async Task<InsertOrUpdateResult<bool>> UpdateFashionGalleryPowerAsync(string userId, string Id, Fashions fashion)
    {
        string connectionString = DatabaseConfig.ConnectionString;
