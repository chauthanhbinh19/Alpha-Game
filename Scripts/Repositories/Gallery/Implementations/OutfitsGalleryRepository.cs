...
public class OutfitsGalleryRepository : IOutfitsGalleryRepository
{
    ...
    public async Task<int> GetOutfitsCountAsync(string search, string type, string rare)
    {
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                string selectSQL = @"SELECT COUNT(*) FROM Outfits c 
                WHERE 1=1 AND c.is_active = TRUE AND c.is_deleted = FALSE";
                if (!string.IsNullOrEmpty(type) && type != "All")
                {
    ...
    public async Task<Outfits> GetOutfitCollectionByIdAsync(string userId, string objectId)
    {
        ...
        try
        {
            ...
            string selectSQL = @"
            SELECT cg.* 
            FROM outfits_gallery cg, outfits c 
            WHERE cg.user_id = @userId 
                AND cg.outfit_id = @objectId 
                AND cg.status = 'available'
                AND cg.outfit_id = c.id
    ...
    public async Task<InsertOrUpdateResult<bool>> UpdateOutfitGalleryPowerAsync(string userId, string id, Outfits outfit)
    {
        string connectionString = DatabaseConfig.ConnectionString;
