...
public class BuildingsGalleryRepository : IBuildingsGalleryRepository
{
    ...
    public async Task<InsertOrUpdateResult<Buildings>> InsertBuildingGalleryAsync(string userId, string id, Buildings building)
    {
        int percent = QualityEvaluatorHelper.CheckQuality(building.Rarity);
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        ";
            ...
            if (recordCount > 0)
            {
                return new InsertOrUpdateResult<Buildings>
                {
                    Data = building,
    ...
    public async Task<InsertOrUpdateResult<List<Buildings>>> InsertBatchBuildingsGalleryAsync(string userId, List<Buildings> buildings)
    {
        if (buildings == null || buildings.Count == 0)
        {
            return InsertOrUpdateResult<List<Buildings>>.Inserted(new List<Buildings>());
        }
        ...
        try
        {
            ...
            for (int i = 0; i < buildings.Count; i += batchSize)
            {
                var chunk = buildings.Skip(i).Take(batchSize).ToList();
                ...
                try
                {
                    ...
                    // INSERT IGNORE: Tự động bỏ qua các dòng đã trùng Primary Key (user_id, building_id)
                    sb.Append(@"
                    INSERT IGNORE INTO buildings_gallery (
                        user_id, building_id, status, current_star, temp_star, power, health, 
                        physical_attack, physical_defense, magical_attack, magical_defense, 
                        chemical_attack, chemical_defense, atomic_attack, atomic_defense, 
                        mental_attack, mental_defense, speed, critical_damage_rate, critical_rate,
                    ...
                    for (int j = 0; j < chunk.Count; j++)
                    {
                        ...
                        valueSqls.Add($@"
                        (@user_id{suffix}, @building_id{suffix}, 'pending', 0, 0, @power{suffix}, @health{suffix},
                        ...
                        command.Parameters.AddWithValue($"@building_id{suffix}", item.Id);
            ...
            return InsertOrUpdateResult<List<Buildings>>.Inserted(insertedList);
        }
        catch (MySqlException ex)
        {
            Debug.LogError("Error Batch Insert: " + ex.Message);
            return InsertOrUpdateResult<List<Buildings>>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateStatusBuildingGalleryAsync(string userId, string id, string status = "available")
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        {
            Debug.LogError("Error UpdateStatusBuildingGallery: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateBatchStatusBuildingsGalleryAsync(string userId, string status = "available")
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        {
            Debug.LogError("Error UpdateBatchStatusBuildingsGallery: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<double>> UpdateTempStarBuildingGalleryAsync(string userId, string buildingId, double star)
    {
        ...
        try
        {
            ...
            await using MySqlCommand updateCommand = new MySqlCommand(updateSQL, connection);
            updateCommand.Parameters.AddWithValue("@user_id", userId);
            updateCommand.Parameters.AddWithValue("@building_id", buildingId);
    ...
    public async Task<InsertOrUpdateResult<double>> UpdateCurrentStarBuildingGalleryAsync(string userId, string buildingId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            if (rowsAffected > 0)
            {
                ...
                selectCommand.Parameters.AddWithValue("@building_id", buildingId);
    ...
    public async Task<InsertOrUpdateResult<List<(string BuildingId, double CurrentStar)>>> UpdateBatchCurrentStarBuildingsGalleryAsync(string userId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            if (rowsAffected > 0)
            {
                // Bước 2: Lấy danh sách building_id và current_star vừa được cập nhật (current_star == temp_star)
                string selectSQL = @"
                SELECT building_id, current_star 
                FROM buildings_gallery 
                WHERE user_id = @user_id 
                ...
                var updatedList = new List<(string BuildingId, double CurrentStar)>();

                while (await reader.ReadAsync())
                {
                    string id = reader.GetString("building_id");
                    double star = reader.GetDouble("current_star");
                    updatedList.Add((id, star));
                }

                return InsertOrUpdateResult<List<(string BuildingId, double CurrentStar)>>.Updated(updatedList);
            }

            return new InsertOrUpdateResult<List<(string BuildingId, double CurrentStar)>>
            {
                Data = new List<(string BuildingId, double CurrentStar)>(),
                OperationType = DatabaseOperationType.None,
                Message = MessageConstants.NOTHING_WAS_UPDATED
            };
        }
        ...
        {
            Debug.LogError("Error: " + ex.Message);
            return InsertOrUpdateResult<List<(string BuildingId, double CurrentStar)>>.Failure(ex.Message);
        }
    }
    public async Task<Buildings> GetBuildingCollectionByIdAsync(string userId, string objectId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            string selectSQL = @"
            SELECT cg.* 
            FROM buildings_gallery cg, buildings c 
            WHERE cg.user_id = @userId 
                AND cg.building_id = @objectId 
                AND cg.status = 'available'
                AND cg.building_id = c.id
    ...
    public async Task<InsertOrUpdateResult<bool>> UpdateBuildingGalleryPowerAsync(string userId, string id, Buildings building)
    {
        string connectionString = DatabaseConfig.ConnectionString;
