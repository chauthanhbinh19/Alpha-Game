...
public class SkillsGalleryRepository : ISkillsGalleryRepository
{
    ...
    public async Task<int> GetSkillsCountAsync(string search, string type, string rare)
    {
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                string selectSQL = @"SELECT COUNT(*) FROM Skills m
                WHERE 1=1 AND m.is_active = TRUE AND m.is_deleted = FALSE";
                if (!string.IsNullOrEmpty(type) && type != "All")
                {
    ...
    public async Task<InsertOrUpdateResult<Skills>> InsertSkillGalleryAsync(string userId, string id, Skills skill)
    {
        int percent = QualityEvaluatorHelper.CheckQuality(skill.Rarity);
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        ";
            ...
            if (recordCount > 0)
            {
                return new InsertOrUpdateResult<Skills>
                {
                    Data = skill,
    ...
    public async Task<InsertOrUpdateResult<List<Skills>>> InsertBatchSkillsGalleryAsync(string userId, List<Skills> skills)
    {
        if (skills == null || skills.Count == 0)
        {
            return InsertOrUpdateResult<List<Skills>>.Inserted(new List<Skills>());
        }

        string connectionString = DatabaseConfig.ConnectionString;
        var insertedList = new List<Skills>();
        int batchSize = 300;
        ...
        try
        {
            ...
            {
                var chunk = skills.Skip(i).Take(batchSize).ToList();
                ...
                try
                {
                    ...
                    // INSERT IGNORE: Tự động bỏ qua các dòng đã trùng Primary Key (user_id, skill_id)
                    sb.Append(@"
                    INSERT IGNORE INTO skills_gallery (
                        user_id, skill_id, status, current_star, temp_star, power, health, 
                        physical_attack, physical_defense, magical_attack, magical_defense, 
                        chemical_attack, chemical_defense, atomic_attack, atomic_defense, 
                        mental_attack, mental_defense, speed, critical_damage_rate, critical_rate,
                    ...
                    for (int j = 0; j < chunk.Count; j++)
                    {
                        ...
                        valueSqls.Add($@"
                        (@user_id{suffix}, @skill_id{suffix}, 'pending', 0, 0, @power{suffix}, @health{suffix},
                        ...
                        command.Parameters.AddWithValue($"@skill_id{suffix}", item.Id);
            ...
            return InsertOrUpdateResult<List<Skills>>.Inserted(insertedList);
        }
        catch (MySqlException ex)
        {
            Debug.LogError("Error Batch Insert: " + ex.Message);
            return InsertOrUpdateResult<List<Skills>>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateStatusSkillGalleryAsync(string userId, string id, string status = "available")
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        {
            Debug.LogError("Error UpdateStatusSkillGallery: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateBatchStatusSkillsGalleryAsync(string userId, string status = "available")
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        {
            Debug.LogError("Error UpdateBatchStatusSkillsGallery: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
    ...
    public async Task<InsertOrUpdateResult<double>> UpdateCurrentStarSkillGalleryAsync(string userId, string skillId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            if (rowsAffected > 0)
            {
                ...
                selectCommand.Parameters.AddWithValue("@skill_id", skillId);
    ...
    public async Task<InsertOrUpdateResult<List<(string SkillId, double CurrentStar)>>> UpdateBatchCurrentStarSkillsGalleryAsync(string userId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            if (rowsAffected > 0)
            {
                // Bước 2: Lấy danh sách skill_id và current_star vừa được cập nhật (current_star == temp_star)
                string selectSQL = @"
                SELECT skill_id, current_star 
                FROM skills_gallery 
                WHERE user_id = @user_id 
                ...
                var updatedList = new List<(string SkillId, double CurrentStar)>();

                while (await reader.ReadAsync())
                {
                    string id = reader.GetString("skill_id");
                    double star = reader.GetDouble("current_star");
                    updatedList.Add((id, star));
                }

                return InsertOrUpdateResult<List<(string SkillId, double CurrentStar)>>.Updated(updatedList);
            }

            return new InsertOrUpdateResult<List<(string SkillId, double CurrentStar)>>
            {
                Data = new List<(string SkillId, double CurrentStar)>(),
                OperationType = DatabaseOperationType.None,
                Message = MessageConstants.NOTHING_WAS_UPDATED
            };
        }
        ...
        {
            Debug.LogError("Error: " + ex.Message);
            return InsertOrUpdateResult<List<(string SkillId, double CurrentStar)>>.Failure(ex.Message);
        }
    }
    public async Task<Skills> GetSkillCollectionByIdAsync(string userId, string objectId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            string selectSQL = @"
            SELECT cg.* 
            FROM skills_gallery cg, skills c 
            WHERE cg.user_id = @userId 
                AND cg.skill_id = @objectId 
                AND cg.status = 'available'
                AND cg.skill_id = c.id
    ...
    public async Task<InsertOrUpdateResult<bool>> UpdateSkillGalleryPowerAsync(string userId, string Id, Skills skill)
    {
        string connectionString = DatabaseConfig.ConnectionString;
