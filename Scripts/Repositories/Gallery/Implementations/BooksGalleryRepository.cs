...
public class BooksGalleryRepository : IBooksGalleryRepository
{
    ...
    public async Task<int> GetBooksCountAsync(string search, string type, string rare)
    {
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                string selectSQL = @"SELECT COUNT(*) FROM Books m 
                WHERE 1=1 AND m.is_active = TRUE AND m.is_deleted = FALSE";
                if (!string.IsNullOrEmpty(type) && type != "All")
                {
    ...
    public async Task<InsertOrUpdateResult<Books>> InsertBookGalleryAsync(string userId, string id, Books book)
    {
        int percent = QualityEvaluatorHelper.CheckQuality(book.Rarity);
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        ";
            ...
            if (recordCount > 0)
            {
                return new InsertOrUpdateResult<Books>
                {
                    Data = book,
    ...
    public async Task<InsertOrUpdateResult<List<Books>>> InsertBatchBooksGalleryAsync(string userId, List<Books> books)
    {
        if (books == null || books.Count == 0)
        {
            return InsertOrUpdateResult<List<Books>>.Inserted(new List<Books>());
        }
        ...
        try
        {
            ...
            {
                var chunk = books.Skip(i).Take(batchSize).ToList();
                ...
                try
                {
                    ...
                    // INSERT IGNORE: Tự động bỏ qua các dòng đã trùng Primary Key (user_id, book_id)
                    sb.Append(@"
                    INSERT IGNORE INTO books_gallery (
                        user_id, book_id, status, current_star, temp_star, power, health, 
                        physical_attack, physical_defense, magical_attack, magical_defense, 
                        chemical_attack, chemical_defense, atomic_attack, atomic_defense, 
                        mental_attack, mental_defense, speed, critical_damage_rate, critical_rate,
                    ...
                    for (int j = 0; j < chunk.Count; j++)
                    {
                        ...
                        valueSqls.Add($@"
                        (@user_id{suffix}, @book_id{suffix}, 'pending', 0, 0, @power{suffix}, @health{suffix},
                        ...
                        command.Parameters.AddWithValue($"@book_id{suffix}", item.Id);
            ...
            return InsertOrUpdateResult<List<Books>>.Inserted(insertedList);
        }
        catch (MySqlException ex)
        {
            Debug.LogError("Error Batch Insert: " + ex.Message);
            return InsertOrUpdateResult<List<Books>>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateStatusBookGalleryAsync(string userId, string id, string status = "available")
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        {
            Debug.LogError("Error UpdateStatusBookGallery: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateBatchStatusBooksGalleryAsync(string userId, string status = "available")
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        {
            Debug.LogError("Error UpdateBatchStatusBooksGallery: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
    ...
    public async Task<InsertOrUpdateResult<double>> UpdateCurrentStarBookGalleryAsync(string userId, string bookId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
    ...
    public async Task<InsertOrUpdateResult<List<(string BookId, double CurrentStar)>>> UpdateBatchCurrentStarBooksGalleryAsync(string userId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            if (rowsAffected > 0)
            {
                // Bước 2: Lấy danh sách book_id và current_star vừa được cập nhật (current_star == temp_star)
                string selectSQL = @"
                SELECT book_id, current_star 
                FROM books_gallery 
                WHERE user_id = @user_id 
                ...
                var updatedList = new List<(string BookId, double CurrentStar)>();

                while (await reader.ReadAsync())
                {
                    string id = reader.GetString("book_id");
                    double star = reader.GetDouble("current_star");
                    updatedList.Add((id, star));
                }

                return InsertOrUpdateResult<List<(string BookId, double CurrentStar)>>.Updated(updatedList);
            }

            return new InsertOrUpdateResult<List<(string BookId, double CurrentStar)>>
            {
                Data = new List<(string BookId, double CurrentStar)>(),
                OperationType = DatabaseOperationType.None,
                Message = MessageConstants.NOTHING_WAS_UPDATED
            };
        }
        ...
        {
            Debug.LogError("Error: " + ex.Message);
            return InsertOrUpdateResult<List<(string BookId, double CurrentStar)>>.Failure(ex.Message);
        }
    }
    public async Task<Books> GetBookCollectionByIdAsync(string userId, string objectId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            string selectSQL = @"
            SELECT cg.* 
            FROM books_gallery cg, books c 
            WHERE cg.user_id = @userId 
                AND cg.book_id = @objectId 
    ...
    public async Task<InsertOrUpdateResult<bool>> UpdateBookGalleryPowerAsync(string userId, string id, Books book)
    {
        string connectionString = DatabaseConfig.ConnectionString;
