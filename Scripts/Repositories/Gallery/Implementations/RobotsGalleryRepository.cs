...
public class RobotsGalleryRepository : IRobotsGalleryRepository
{
    ...
    public async Task<int> GetRobotsCountAsync(string search, string rare)
    {
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                string selectSQL = @"SELECT COUNT(*) FROM Robots c WHERE 1=1 AND c.is_active = TRUE AND c.is_deleted = FALSE";
                if (!string.IsNullOrEmpty(rare) && rare != "All")
                {
                    selectSQL += " AND rare = @rare";
                }
    ...
    public async Task<InsertOrUpdateResult<Robots>> InsertRobotGalleryAsync(string userId, string id, Robots robot)
    {
        int percent = QualityEvaluatorHelper.CheckQuality(robot.Rarity);
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        ";
            ...
            if (recordCount > 0)
            {
                return new InsertOrUpdateResult<Robots>
                {
                    Data = robot,
            ...
            INSERT INTO robots_gallery (
                user_id, robot_id, status, current_star, temp_star, power, health, 
    ...
    public async Task<InsertOrUpdateResult<List<Robots>>> InsertBatchRobotsGalleryAsync(string userId, List<Robots> robots)
    {
        if (robots == null || robots.Count == 0)
        {
            return InsertOrUpdateResult<List<Robots>>.Inserted(new List<Robots>());
        }

        string connectionString = DatabaseConfig.ConnectionString;
        var insertedList = new List<Robots>();
        int batchSize = 300;
        ...
        try
        {
            ...
            {
                var chunk = robots.Skip(i).Take(batchSize).ToList();
                ...
                try
                {
                    ...
                    // INSERT IGNORE: Tự động bỏ qua các dòng đã trùng Primary Key (user_id, robot_id)
                    sb.Append(@"
                    INSERT IGNORE INTO robots_gallery (
                        user_id, robot_id, status, current_star, temp_star, power, health, 
                        physical_attack, physical_defense, magical_attack, magical_defense, 
                        chemical_attack, chemical_defense, atomic_attack, atomic_defense, 
                        mental_attack, mental_defense, speed, critical_damage_rate, critical_rate,
                    ...
                    for (int j = 0; j < chunk.Count; j++)
                    {
                        ...
                        valueSqls.Add($@"
                        (@user_id{suffix}, @robot_id{suffix}, 'pending', 0, 0, @power{suffix}, @health{suffix},
                        ...
                        command.Parameters.AddWithValue($"@robot_id{suffix}", item.Id);
            ...
            return InsertOrUpdateResult<List<Robots>>.Inserted(insertedList);
        }
        catch (MySqlException ex)
        {
            Debug.LogError("Error Batch Insert: " + ex.Message);
            return InsertOrUpdateResult<List<Robots>>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateStatusRobotGalleryAsync(string userId, string id, string status = "available")
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        {
            Debug.LogError("Error UpdateStatusRobotGallery: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateBatchStatusRobotsGalleryAsync(string userId, string status = "available")
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        {
            Debug.LogError("Error UpdateBatchStatusRobotsGallery: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<double>> UpdateTempStarRobotGalleryAsync(string userId, string robotId, double star)
    {
        ...
        try
        {
            ...
            await using MySqlCommand updateCommand = new MySqlCommand(updateSQL, connection);
            updateCommand.Parameters.AddWithValue("@user_id", userId);
            updateCommand.Parameters.AddWithValue("@robot_id", robotId);
    ...
    public async Task<InsertOrUpdateResult<double>> UpdateCurrentStarRobotGalleryAsync(string userId, string robotId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            if (rowsAffected > 0)
            {
                ...
                selectCommand.Parameters.AddWithValue("@robot_id", robotId);
    ...
    public async Task<InsertOrUpdateResult<List<(string RobotId, double CurrentStar)>>> UpdateBatchCurrentStarRobotsGalleryAsync(string userId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            if (rowsAffected > 0)
            {
                // Bước 2: Lấy danh sách robot_id và current_star vừa được cập nhật (current_star == temp_star)
                string selectSQL = @"
                SELECT robot_id, current_star 
                FROM robots_gallery 
                WHERE user_id = @user_id 
                ...
                var updatedList = new List<(string RobotId, double CurrentStar)>();

                while (await reader.ReadAsync())
                {
                    string id = reader.GetString("robot_id");
                    double star = reader.GetDouble("current_star");
                    updatedList.Add((id, star));
                }

                return InsertOrUpdateResult<List<(string RobotId, double CurrentStar)>>.Updated(updatedList);
            }

            return new InsertOrUpdateResult<List<(string RobotId, double CurrentStar)>>
            {
                Data = new List<(string RobotId, double CurrentStar)>(),
                OperationType = DatabaseOperationType.None,
                Message = MessageConstants.NOTHING_WAS_UPDATED
            };
        }
        ...
        {
            Debug.LogError("Error: " + ex.Message);
            return InsertOrUpdateResult<List<(string RobotId, double CurrentStar)>>.Failure(ex.Message);
        }
    }
    public async Task<Robots> GetRobotCollectionByIdAsync(string userId, string objectId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            string selectSQL = @"
            SELECT cg.* 
            FROM robots_gallery cg, robots c 
            WHERE cg.user_id = @userId 
                AND cg.robot_id = @objectId 
                AND cg.status = 'available'
                AND cg.robot_id = c.id
    ...
    public async Task<InsertOrUpdateResult<bool>> UpdateRobotGalleryPowerAsync(string userId, string id, Robots robot)
    {
        string connectionString = DatabaseConfig.ConnectionString;
