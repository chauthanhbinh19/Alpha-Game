...
public class AvatarsGalleryRepository : IAvatarsGalleryRepository
{
    public async Task<List<Avatars>> GetAvatarsCollectionAsync(string userId, string search, int pageSize, int offset, string rare)
    {
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                SELECT c.*, 
                       CASE 
                           WHEN cg.avatar_id IS NULL THEN 'block' 
    ...
    public async Task<InsertOrUpdateResult<Avatars>> InsertAvatarGalleryAsync(string userId, string id, Avatars avatar)
    {
        int percent = QualityEvaluatorHelper.CheckQuality(avatar.Rarity);
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        ";
            ...
            if (recordCount > 0)
            {
                return new InsertOrUpdateResult<Avatars>
                {
                    Data = avatar,
            ...
            INSERT INTO avatars_gallery (
                user_id, avatar_id, status, current_star, temp_star, power, health, 
            ...
            insertCommand.Parameters.AddWithValue("@avatar_id", id);
    ...
    public async Task<InsertOrUpdateResult<List<Avatars>>> InsertBatchAvatarsGalleryAsync(string userId, List<Avatars> avatars)
    {
        if (avatars == null || avatars.Count == 0)
        {
            return InsertOrUpdateResult<List<Avatars>>.Inserted(new List<Avatars>());
        }

        string connectionString = DatabaseConfig.ConnectionString;
        var insertedList = new List<Avatars>();
        int batchSize = 300;
        ...
        try
        {
            ...
            {
                var chunk = avatars.Skip(i).Take(batchSize).ToList();
                ...
                try
                {
                    ...
                    // INSERT IGNORE: Tự động bỏ qua các dòng đã trùng Primary Key (user_id, avatar_id)
                    sb.Append(@"
                    INSERT IGNORE INTO avatars_gallery (
                        user_id, avatar_id, status, current_star, temp_star, power, health, 
                        physical_attack, physical_defense, magical_attack, magical_defense, 
                        chemical_attack, chemical_defense, atomic_attack, atomic_defense, 
                        mental_attack, mental_defense, speed, critical_damage_rate, critical_rate,
                    ...
                    for (int j = 0; j < chunk.Count; j++)
                    {
                        ...
                        valueSqls.Add($@"
                        (@user_id{suffix}, @avatar_id{suffix}, 'pending', 0, 0, @power{suffix}, @health{suffix},
                        ...
                        command.Parameters.AddWithValue($"@avatar_id{suffix}", item.Id);
            ...
            return InsertOrUpdateResult<List<Avatars>>.Inserted(insertedList);
        }
        catch (MySqlException ex)
        {
            Debug.LogError("Error Batch Insert: " + ex.Message);
            return InsertOrUpdateResult<List<Avatars>>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateStatusAvatarGalleryAsync(string userId, string id, string status = "available")
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            string updateSQL = @"UPDATE avatars_gallery 
                             SET status = @status 
        ...
        {
            Debug.LogError("Error UpdateStatusAvatarGallery: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateBatchStatusAvatarsGalleryAsync(string userId, string status = "available")
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        {
            Debug.LogError("Error UpdateBatchStatusAvatarsGallery: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
    ...
    public async Task<InsertOrUpdateResult<double>> UpdateCurrentStarAvatarGalleryAsync(string userId, string avatarId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            if (rowsAffected > 0)
            {
                ...
                selectCommand.Parameters.AddWithValue("@avatar_id", avatarId);
    ...
    public async Task<InsertOrUpdateResult<List<(string AvatarId, double CurrentStar)>>> UpdateBatchCurrentStarAvatarsGalleryAsync(string userId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            if (rowsAffected > 0)
            {
                // Bước 2: Lấy danh sách avatar_id và current_star vừa được cập nhật (current_star == temp_star)
                string selectSQL = @"
                SELECT avatar_id, current_star 
                FROM avatars_gallery 
                WHERE user_id = @user_id 
                ...
                var updatedList = new List<(string AvatarId, double CurrentStar)>();

                while (await reader.ReadAsync())
                {
                    string id = reader.GetString("avatar_id");
                    double star = reader.GetDouble("current_star");
                    updatedList.Add((id, star));
                }

                return InsertOrUpdateResult<List<(string AvatarId, double CurrentStar)>>.Updated(updatedList);
            }

            return new InsertOrUpdateResult<List<(string AvatarId, double CurrentStar)>>
            {
                Data = new List<(string AvatarId, double CurrentStar)>(),
                OperationType = DatabaseOperationType.None,
                Message = MessageConstants.NOTHING_WAS_UPDATED
            };
        }
        ...
        {
            Debug.LogError("Error: " + ex.Message);
            return InsertOrUpdateResult<List<(string AvatarId, double CurrentStar)>>.Failure(ex.Message);
        }
    }
    public async Task<Avatars> GetAvatarCollectionByIdAsync(string userId, string objectId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            string selectSQL = @"
            SELECT cg.* 
            FROM avatars_gallery cg, avatars c 
            WHERE cg.user_id = @userId 
                AND cg.avatar_id = @objectId 
                AND cg.status = 'available'
                AND cg.avatar_id = c.id
    ...
    public async Task<InsertOrUpdateResult<bool>> UpdateAvatarGalleryPowerAsync(string userId, string id, Avatars avatar)
    {
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            ...
            ";
                ...
                // Stats
                updateCommand.Parameters.AddWithValue("@power", avatar.Power);
                updateCommand.Parameters.AddWithValue("@health", avatar.Health);
                updateCommand.Parameters.AddWithValue("@physical_attack", avatar.PhysicalAttack);
                updateCommand.Parameters.AddWithValue("@physical_defense", avatar.PhysicalDefense);
                updateCommand.Parameters.AddWithValue("@magical_attack", avatar.MagicalAttack);
                updateCommand.Parameters.AddWithValue("@magical_defense", avatar.MagicalDefense);
                updateCommand.Parameters.AddWithValue("@chemical_attack", avatar.ChemicalAttack);
                updateCommand.Parameters.AddWithValue("@chemical_defense", avatar.ChemicalDefense);
                updateCommand.Parameters.AddWithValue("@atomic_attack", avatar.AtomicAttack);
                updateCommand.Parameters.AddWithValue("@atomic_defense", avatar.AtomicDefense);
                updateCommand.Parameters.AddWithValue("@mental_attack", avatar.MagicalAttack);
                updateCommand.Parameters.AddWithValue("@mental_defense", avatar.MagicalDefense);
                updateCommand.Parameters.AddWithValue("@speed", avatar.Speed);
                updateCommand.Parameters.AddWithValue("@critical_damage_rate", avatar.CriticalDamageRate);
                updateCommand.Parameters.AddWithValue("@critical_rate", avatar.CriticalRate);
                updateCommand.Parameters.AddWithValue("@critical_resistance_rate", avatar.CriticalResistanceRate);
                updateCommand.Parameters.AddWithValue("@ignore_critical_rate", avatar.IgnoreCriticalRate);
                updateCommand.Parameters.AddWithValue("@penetration_rate", avatar.PenetrationRate);
                updateCommand.Parameters.AddWithValue("@penetration_resistance_rate", avatar.PenetrationResistanceRate);
                updateCommand.Parameters.AddWithValue("@evasion_rate", avatar.EvasionRate);
                updateCommand.Parameters.AddWithValue("@damage_absorption_rate", avatar.DamageAbsorptionRate);
                updateCommand.Parameters.AddWithValue("@ignore_damage_absorption_rate", avatar.IgnoreDamageAbsorptionRate);
                updateCommand.Parameters.AddWithValue("@absorbed_damage_rate", avatar.AbsorbedDamageRate);
                updateCommand.Parameters.AddWithValue("@vitality_regeneration_rate", avatar.VitalityRegenerationRate);
                updateCommand.Parameters.AddWithValue("@vitality_regeneration_resistance_rate", avatar.VitalityRegenerationResistanceRate);
                updateCommand.Parameters.AddWithValue("@accuracy_rate", avatar.AccuracyRate);
                updateCommand.Parameters.AddWithValue("@lifesteal_rate", avatar.LifestealRate);
                updateCommand.Parameters.AddWithValue("@shield_strength", avatar.ShieldStrength);
                updateCommand.Parameters.AddWithValue("@tenacity", avatar.Tenacity);
                updateCommand.Parameters.AddWithValue("@resistance_rate", avatar.ResistanceRate);
                updateCommand.Parameters.AddWithValue("@combo_rate", avatar.ComboRate);
                updateCommand.Parameters.AddWithValue("@ignore_combo_rate", avatar.IgnoreComboRate);
                updateCommand.Parameters.AddWithValue("@combo_damage_rate", avatar.ComboDamageRate);
                updateCommand.Parameters.AddWithValue("@combo_resistance_rate", avatar.ComboResistanceRate);
                updateCommand.Parameters.AddWithValue("@stun_rate", avatar.StunRate);
                updateCommand.Parameters.AddWithValue("@ignore_stun_rate", avatar.IgnoreStunRate);
                updateCommand.Parameters.AddWithValue("@reflection_rate", avatar.ReflectionRate);
                updateCommand.Parameters.AddWithValue("@ignore_reflection_rate", avatar.IgnoreReflectionRate);
                updateCommand.Parameters.AddWithValue("@reflection_damage_rate", avatar.ReflectionDamageRate);
                updateCommand.Parameters.AddWithValue("@reflection_resistance_rate", avatar.ReflectionResistanceRate);
                updateCommand.Parameters.AddWithValue("@mana", avatar.Mana);
                updateCommand.Parameters.AddWithValue("@mana_regeneration_rate", avatar.ManaRegenerationRate);
                updateCommand.Parameters.AddWithValue("@damage_to_different_faction_rate", avatar.DamageToDifferentFactionRate);
                updateCommand.Parameters.AddWithValue("@resistance_to_different_faction_rate", avatar.ResistanceToDifferentFactionRate);
                updateCommand.Parameters.AddWithValue("@damage_to_same_faction_rate", avatar.DamageToSameFactionRate);
                updateCommand.Parameters.AddWithValue("@resistance_to_same_faction_rate", avatar.ResistanceToSameFactionRate);
                updateCommand.Parameters.AddWithValue("@normal_damage_rate", avatar.NormalDamageRate);
                updateCommand.Parameters.AddWithValue("@normal_resistance_rate", avatar.NormalResistanceRate);
                updateCommand.Parameters.AddWithValue("@skill_damage_rate", avatar.SkillDamageRate);
                updateCommand.Parameters.AddWithValue("@skill_resistance_rate", avatar.SkillResistanceRate);
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                using (MySqlDataReader reader = await selectCommand.ExecuteReaderAsync())
                {
                    if (await reader.ReadAsync())
                    {
                        sumAvatars.Power = reader["total_power"] as double? ?? 0;
                        sumAvatars.Health = reader["total_health"] as double? ?? 0;
                        sumAvatars.Mana = reader["total_mana"] as double? ?? 0f;

                        sumAvatars.PhysicalAttack = reader["total_physical_attack"] as double? ?? 0;
                        sumAvatars.PhysicalDefense = reader["total_physical_defense"] as double? ?? 0;
                        sumAvatars.MagicalAttack = reader["total_magical_attack"] as double? ?? 0;
                        sumAvatars.MagicalDefense = reader["total_magical_defense"] as double? ?? 0;
                        sumAvatars.ChemicalAttack = reader["total_chemical_attack"] as double? ?? 0;
                        sumAvatars.ChemicalDefense = reader["total_chemical_defense"] as double? ?? 0;
                        sumAvatars.AtomicAttack = reader["total_atomic_attack"] as double? ?? 0;
                        sumAvatars.AtomicDefense = reader["total_atomic_defense"] as double? ?? 0;
                        sumAvatars.MentalAttack = reader["total_mental_attack"] as double? ?? 0;
                        sumAvatars.MentalDefense = reader["total_mental_defense"] as double? ?? 0;

                        sumAvatars.Speed = reader["total_speed"] as double? ?? 0;
                        sumAvatars.CriticalDamageRate = reader["total_critical_damage_rate"] as double? ?? 0;
                        sumAvatars.CriticalRate = reader["total_critical_rate"] as double? ?? 0;
                        sumAvatars.CriticalResistanceRate = reader["total_critical_resistance_rate"] as double? ?? 0;

                        sumAvatars.IgnoreCriticalRate = reader["total_ignore_critical_rate"] as double? ?? 0;
                        sumAvatars.PenetrationRate = reader["total_penetration_rate"] as double? ?? 0;
                        sumAvatars.PenetrationResistanceRate = reader["total_penetration_resistance_rate"] as double? ?? 0;

                        sumAvatars.EvasionRate = reader["total_evasion_rate"] as double? ?? 0;
                        sumAvatars.DamageAbsorptionRate = reader["total_damage_absorption_rate"] as double? ?? 0;
                        sumAvatars.IgnoreDamageAbsorptionRate = reader["total_ignore_damage_absorption_rate"] as double? ?? 0;
                        sumAvatars.AbsorbedDamageRate = reader["total_absorbed_damage_rate"] as double? ?? 0;

                        sumAvatars.VitalityRegenerationRate = reader["total_vitality_regeneration_rate"] as double? ?? 0;
                        sumAvatars.VitalityRegenerationResistanceRate = reader["total_vitality_regeneration_resistance_rate"] as double? ?? 0;

                        sumAvatars.AccuracyRate = reader["total_accuracy_rate"] as double? ?? 0;
                        sumAvatars.LifestealRate = reader["total_lifesteal_rate"] as double? ?? 0;
                        sumAvatars.ShieldStrength = reader["total_shield_strength"] as double? ?? 0;

                        sumAvatars.Tenacity = reader["total_tenacity"] as double? ?? 0;
                        sumAvatars.ResistanceRate = reader["total_resistance_rate"] as double? ?? 0;

                        sumAvatars.ComboRate = reader["total_combo_rate"] as double? ?? 0;
                        sumAvatars.IgnoreComboRate = reader["total_ignore_combo_rate"] as double? ?? 0;
                        sumAvatars.ComboDamageRate = reader["total_combo_damage_rate"] as double? ?? 0;
                        sumAvatars.ComboResistanceRate = reader["total_combo_resistance_rate"] as double? ?? 0;

                        sumAvatars.StunRate = reader["total_stun_rate"] as double? ?? 0;
                        sumAvatars.IgnoreStunRate = reader["total_ignore_stun_rate"] as double? ?? 0;

                        sumAvatars.ReflectionRate = reader["total_reflection_rate"] as double? ?? 0;
                        sumAvatars.IgnoreReflectionRate = reader["total_ignore_reflection_rate"] as double? ?? 0;
                        sumAvatars.ReflectionDamageRate = reader["total_reflection_damage_rate"] as double? ?? 0;
                        sumAvatars.ReflectionResistanceRate = reader["total_reflection_resistance_rate"] as double? ?? 0;

                        sumAvatars.ManaRegenerationRate = reader["total_mana_regeneration_rate"] as double? ?? 0;

                        sumAvatars.DamageToDifferentFactionRate = reader["total_damage_to_different_faction_rate"] as double? ?? 0;
                        sumAvatars.ResistanceToDifferentFactionRate = reader["total_resistance_to_different_faction_rate"] as double? ?? 0;

                        sumAvatars.DamageToSameFactionRate = reader["total_damage_to_same_faction_rate"] as double? ?? 0;
                        sumAvatars.ResistanceToSameFactionRate = reader["total_resistance_to_same_faction_rate"] as double? ?? 0;

                        sumAvatars.NormalDamageRate = reader["total_normal_damage_rate"] as double? ?? 0;
                        sumAvatars.NormalResistanceRate = reader["total_normal_resistance_rate"] as double? ?? 0;

                        sumAvatars.SkillDamageRate = reader["total_skill_damage_rate"] as double? ?? 0;
                        sumAvatars.SkillResistanceRate = reader["total_skill_resistance_rate"] as double? ?? 0;

                        sumAvatars.PercentAllHealth = reader["total_percent_all_health"] as double? ?? 0;
                        sumAvatars.PercentAllPhysicalAttack = reader["total_percent_all_physical_attack"] as double? ?? 0;
                        sumAvatars.PercentAllPhysicalDefense = reader["total_percent_all_physical_defense"] as double? ?? 0;
                        sumAvatars.PercentAllMagicalAttack = reader["total_percent_all_magical_attack"] as double? ?? 0;
                        sumAvatars.PercentAllMagicalDefense = reader["total_percent_all_magical_defense"] as double? ?? 0;
                        sumAvatars.PercentAllChemicalAttack = reader["total_percent_all_chemical_attack"] as double? ?? 0;
                        sumAvatars.PercentAllChemicalDefense = reader["total_percent_all_chemical_defense"] as double? ?? 0;
                        sumAvatars.PercentAllAtomicAttack = reader["total_percent_all_atomic_attack"] as double? ?? 0;
                        sumAvatars.PercentAllAtomicDefense = reader["total_percent_all_atomic_defense"] as double? ?? 0;
                        sumAvatars.PercentAllMentalAttack = reader["total_percent_all_mental_attack"] as double? ?? 0;
                        sumAvatars.PercentAllMentalDefense = reader["total_percent_all_mental_defense"] as double? ?? 0;
                    }
                }
            }
            catch (MySqlException ex)
