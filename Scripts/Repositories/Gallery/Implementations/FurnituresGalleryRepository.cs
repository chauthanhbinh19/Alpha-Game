...
public class FurnituresGalleryRepository : IFurnituresGalleryRepository
{
    ...
    public async Task<InsertOrUpdateResult<Furnitures>> InsertFurnitureGalleryAsync(string userId, string id, Furnitures furniture)
    {
        int percent = QualityEvaluatorHelper.CheckQuality(furniture.Rarity);
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        ";
            ...
            if (recordCount > 0)
            {
                return new InsertOrUpdateResult<Furnitures>
                {
                    Data = furniture,
    ...
    public async Task<InsertOrUpdateResult<List<Furnitures>>> InsertBatchFurnituresGalleryAsync(string userId, List<Furnitures> furnitures)
    {
        if (furnitures == null || furnitures.Count == 0)
        {
            return InsertOrUpdateResult<List<Furnitures>>.Inserted(new List<Furnitures>());
        }

        string connectionString = DatabaseConfig.ConnectionString;
        var insertedList = new List<Furnitures>();
        int batchSize = 300;
        ...
        try
        {
            ...
            for (int i = 0; i < furnitures.Count; i += batchSize)
            {
                var chunk = furnitures.Skip(i).Take(batchSize).ToList();
                ...
                try
                {
                    ...
                    // INSERT IGNORE: Tự động bỏ qua các dòng đã trùng Primary Key (user_id, furniture_id)
                    sb.Append(@"
                    INSERT IGNORE INTO furnitures_gallery (
                        user_id, furniture_id, status, current_star, temp_star, power, health, 
                        physical_attack, physical_defense, magical_attack, magical_defense, 
                        chemical_attack, chemical_defense, atomic_attack, atomic_defense, 
                        mental_attack, mental_defense, speed, critical_damage_rate, critical_rate,
                    ...
                    for (int j = 0; j < chunk.Count; j++)
                    {
                        ...
                        valueSqls.Add($@"
                        (@user_id{suffix}, @furniture_id{suffix}, 'pending', 0, 0, @power{suffix}, @health{suffix},
                        ...
                        command.Parameters.AddWithValue($"@furniture_id{suffix}", item.Id);
            ...
            return InsertOrUpdateResult<List<Furnitures>>.Inserted(insertedList);
        }
        catch (MySqlException ex)
        {
            Debug.LogError("Error Batch Insert: " + ex.Message);
            return InsertOrUpdateResult<List<Furnitures>>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateStatusFurnitureGalleryAsync(string userId, string id, string status = "available")
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        {
            Debug.LogError("Error UpdateStatusFurnitureGallery: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateBatchStatusFurnituresGalleryAsync(string userId, string status = "available")
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        {
            Debug.LogError("Error UpdateBatchStatusFurnituresGallery: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<double>> UpdateTempStarFurnitureGalleryAsync(string userId, string furnitureId, double star)
    {
        ...
        try
        {
            ...
            await using MySqlCommand updateCommand = new MySqlCommand(updateSQL, connection);
            updateCommand.Parameters.AddWithValue("@user_id", userId);
            updateCommand.Parameters.AddWithValue("@furniture_id", furnitureId);
    ...
    public async Task<InsertOrUpdateResult<double>> UpdateCurrentStarFurnitureGalleryAsync(string userId, string furnitureId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            if (rowsAffected > 0)
            {
                ...
                selectCommand.Parameters.AddWithValue("@furniture_id", furnitureId);
    ...
    public async Task<InsertOrUpdateResult<List<(string FurnitureId, double CurrentStar)>>> UpdateBatchCurrentStarFurnituresGalleryAsync(string userId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            if (rowsAffected > 0)
            {
                // Bước 2: Lấy danh sách furniture_id và current_star vừa được cập nhật (current_star == temp_star)
                string selectSQL = @"
                SELECT furniture_id, current_star 
                FROM furnitures_gallery 
                WHERE user_id = @user_id 
                ...
                var updatedList = new List<(string FurnitureId, double CurrentStar)>();

                while (await reader.ReadAsync())
                {
                    string id = reader.GetString("furniture_id");
                    double star = reader.GetDouble("current_star");
                    updatedList.Add((id, star));
                }

                return InsertOrUpdateResult<List<(string FurnitureId, double CurrentStar)>>.Updated(updatedList);
            }

            return new InsertOrUpdateResult<List<(string FurnitureId, double CurrentStar)>>
            {
                Data = new List<(string FurnitureId, double CurrentStar)>(),
                OperationType = DatabaseOperationType.None,
                Message = MessageConstants.NOTHING_WAS_UPDATED
            };
        }
        ...
        {
            Debug.LogError("Error: " + ex.Message);
            return InsertOrUpdateResult<List<(string FurnitureId, double CurrentStar)>>.Failure(ex.Message);
        }
    }
    public async Task<Furnitures> GetFurnitureCollectionByIdAsync(string userId, string objectId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            string selectSQL = @"
            SELECT cg.* 
            FROM furnitures_gallery cg, furnitures c 
            WHERE cg.user_id = @userId 
                AND cg.furniture_id = @objectId 
                AND cg.status = 'available'
                AND cg.furniture_id = c.id
    ...
    public async Task<InsertOrUpdateResult<bool>> UpdateFurnitureGalleryPowerAsync(string userId, string Id, Furnitures furniture)
    {
        string connectionString = DatabaseConfig.ConnectionString;
