...
public class CardSoldiersGalleryRepository : ICardSoldiersGalleryRepository
{
    ...
    public async Task<int> GetCardSoldiersCountAsync(string search, string type, string rare)
    {
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                string selectSQL = @"SELECT COUNT(*) FROM card_admirals m 
                WHERE 1=1 AND m.is_active = TRUE AND m.is_deleted = FALSE";
                if (!string.IsNullOrEmpty(type) && type != "All")
                {
    ...
    public async Task<InsertOrUpdateResult<CardSoldiers>> InsertCardSoldierGalleryAsync(string userId, string id, CardSoldiers cardSoldier)
    {
        int percent = QualityEvaluatorHelper.CheckQuality(cardSoldier.Rarity);
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        ";
            ...
            if (recordCount > 0)
            {
                return new InsertOrUpdateResult<CardSoldiers>
                {
                    Data = cardSoldier,
    ...
    public async Task<InsertOrUpdateResult<List<CardSoldiers>>> InsertBatchCardSoldiersGalleryAsync(string userId, List<CardSoldiers> cardSoldiers)
    {
        if (cardSoldiers == null || cardSoldiers.Count == 0)
        {
            return InsertOrUpdateResult<List<CardSoldiers>>.Inserted(new List<CardSoldiers>());
        }

        string connectionString = DatabaseConfig.ConnectionString;
        var insertedList = new List<CardSoldiers>();
        int batchSize = 300;
        ...
        try
        {
            ...
            {
                var chunk = cardSoldiers.Skip(i).Take(batchSize).ToList();
                ...
                try
                {
                    ...
                    // INSERT IGNORE: Tự động bỏ qua các dòng đã trùng Primary Key (user_id, card_soldier_id)
                    sb.Append(@"
                    INSERT IGNORE INTO card_soldiers_gallery (
                        user_id, card_soldier_id, status, current_star, temp_star, power, health, 
                        physical_attack, physical_defense, magical_attack, magical_defense, 
                        chemical_attack, chemical_defense, atomic_attack, atomic_defense, 
                        mental_attack, mental_defense, speed, critical_damage_rate, critical_rate,
                    ...
                    for (int j = 0; j < chunk.Count; j++)
                    {
                        ...
                        valueSqls.Add($@"
                        (@user_id{suffix}, @card_soldier_id{suffix}, 'pending', 0, 0, @power{suffix}, @health{suffix},
                        ...
                        command.Parameters.AddWithValue($"@card_soldier_id{suffix}", item.Id);
            ...
            return InsertOrUpdateResult<List<CardSoldiers>>.Inserted(insertedList);
        }
        catch (MySqlException ex)
        {
            Debug.LogError("Error Batch Insert: " + ex.Message);
            return InsertOrUpdateResult<List<CardSoldiers>>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateStatusCardSoldierGalleryAsync(string userId, string id, string status = "available")
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            string updateSQL = @"UPDATE card_soldiers_gallery g INNER JOIN card_soldiers p ON p.id = g.card_soldier_id 
                             SET status = @status 
                             WHERE g.user_id = @user_id 
                               AND g.card_soldier_id = @card_soldier_id
        ...
        {
            Debug.LogError("Error UpdateStatusCardSoldierGallery: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateBatchStatusCardSoldiersGalleryAsync(string userId, string status = "available")
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        {
            Debug.LogError("Error UpdateBatchStatusCardSoldiersGallery: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<double>> UpdateTempStarCardSoldierGalleryAsync(string userId, string cardSoldierId, double star)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            string updateSQL = @"
            UPDATE card_soldiers_gallery g INNER JOIN card_soldiers p ON p.id = g.card_soldier_id 
            ...
            await using MySqlCommand updateCommand = new MySqlCommand(updateSQL, connection);
            updateCommand.Parameters.AddWithValue("@user_id", userId);
            updateCommand.Parameters.AddWithValue("@card_soldier_id", cardSoldierId);
    ...
    public async Task<InsertOrUpdateResult<double>> UpdateCurrentStarCardSoldierGalleryAsync(string userId, string cardSoldierId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
    ...
    public async Task<InsertOrUpdateResult<List<(string CardSoldierId, double CurrentStar)>>> UpdateBatchCurrentStarCardSoldiersGalleryAsync(string userId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            if (rowsAffected > 0)
            {
                // Bước 2: Lấy danh sách card_soldier_id và current_star vừa được cập nhật (current_star == temp_star)
                string selectSQL = @"
                SELECT card_soldier_id, current_star 
                FROM card_soldiers_gallery 
                ...
                var updatedList = new List<(string CardSoldierId, double CurrentStar)>();

                while (await reader.ReadAsync())
                {
                    string id = reader.GetString("card_soldier_id");
                    double star = reader.GetDouble("current_star");
                    updatedList.Add((id, star));
                }

                return InsertOrUpdateResult<List<(string CardSoldierId, double CurrentStar)>>.Updated(updatedList);
            }

            return new InsertOrUpdateResult<List<(string CardSoldierId, double CurrentStar)>>
            {
                Data = new List<(string CardSoldierId, double CurrentStar)>(),
                OperationType = DatabaseOperationType.None,
                Message = MessageConstants.NOTHING_WAS_UPDATED
            };
        }
        ...
        {
            Debug.LogError("Error: " + ex.Message);
            return InsertOrUpdateResult<List<(string CardSoldierId, double CurrentStar)>>.Failure(ex.Message);
        }
    }
    public async Task<CardSoldiers> GetCardSoldierCollectionByIdAsync(string userId, string objectId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            string selectSQL = @"
            SELECT cg.* 
            FROM card_soldiers_gallery cg, card_soldiers c 
            WHERE cg.user_id = @userId 
                AND cg.card_soldier_id = @objectId 
                AND cg.status = 'available'
                AND cg.card_soldier_id = c.id
    ...
    public async Task<InsertOrUpdateResult<bool>> UpdateCardSoldierGalleryPowerAsync(string userId, string id, CardSoldiers cardAdmiral)
    {
        string connectionString = DatabaseConfig.ConnectionString;
