...
public class ArtworksGalleryRepository : IArtworksGalleryRepository
{
    ...
    public async Task<Artworks> GetArtworkCollectionByIdAsync(string userId, string objectId)
    {
        ...
        try
        {
            ...
            string selectSQL = @"
            SELECT cg.* 
            FROM artworks_gallery cg, artworks c 
            WHERE cg.user_id = @userId 
                AND cg.artwork_id = @objectId 
                AND cg.status = 'available'
                AND cg.artwork_id = c.id
    ...
    public async Task<InsertOrUpdateResult<bool>> UpdateArtworkGalleryPowerAsync(string userId, string id, Artworks artwork)
    {
        string connectionString = DatabaseConfig.ConnectionString;
