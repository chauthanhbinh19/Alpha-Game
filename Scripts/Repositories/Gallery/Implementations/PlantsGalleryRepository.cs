...
public class PlantsGalleryRepository : IPlantsGalleryRepository
{
    ...
    public async Task<InsertOrUpdateResult<Plants>> InsertPlantGalleryAsync(string userId, string id, Plants plant)
    {
        int percent = QualityEvaluatorHelper.CheckQuality(plant.Rarity);
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        ";
            ...
            if (recordCount > 0)
            {
                return new InsertOrUpdateResult<Plants>
                {
                    Data = plant,
            ...
            INSERT INTO plants_gallery (
                user_id, plant_id, status, current_star, temp_star, power, health, 
            ...
            insertCommand.Parameters.AddWithValue("@plant_id", id);
    ...
    public async Task<InsertOrUpdateResult<List<Plants>>> InsertBatchPlantsGalleryAsync(string userId, List<Plants> plants)
    {
        if (plants == null || plants.Count == 0)
        {
            return InsertOrUpdateResult<List<Plants>>.Inserted(new List<Plants>());
        }

        string connectionString = DatabaseConfig.ConnectionString;
        var insertedList = new List<Plants>();
        int batchSize = 300;
        ...
        try
        {
            ...
            {
                var chunk = plants.Skip(i).Take(batchSize).ToList();
                ...
                try
                {
                    ...
                    // INSERT IGNORE: Tự động bỏ qua các dòng đã trùng Primary Key (user_id, plant_id)
                    sb.Append(@"
                    INSERT IGNORE INTO plants_gallery (
                        user_id, plant_id, status, current_star, temp_star, power, health, 
                        physical_attack, physical_defense, magical_attack, magical_defense, 
                        chemical_attack, chemical_defense, atomic_attack, atomic_defense, 
                        mental_attack, mental_defense, speed, critical_damage_rate, critical_rate,
                    ...
                    for (int j = 0; j < chunk.Count; j++)
                    {
                        ...
                        valueSqls.Add($@"
                        (@user_id{suffix}, @plant_id{suffix}, 'pending', 0, 0, @power{suffix}, @health{suffix},
                        ...
                        command.Parameters.AddWithValue($"@plant_id{suffix}", item.Id);
            ...
            return InsertOrUpdateResult<List<Plants>>.Inserted(insertedList);
        }
        catch (MySqlException ex)
        {
            Debug.LogError("Error Batch Insert: " + ex.Message);
            return InsertOrUpdateResult<List<Plants>>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateStatusPlantGalleryAsync(string userId, string id, string status = "available")
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            string updateSQL = @"UPDATE plants_gallery 
                             SET status = @status 
        ...
        {
            Debug.LogError("Error UpdateStatusPlantGallery: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
    public async Task<InsertOrUpdateResult<bool>> UpdateBatchStatusPlantsGalleryAsync(string userId, string status = "available")
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        {
            Debug.LogError("Error UpdateBatchStatusPlantsGallery: " + ex.Message);
            return InsertOrUpdateResult<bool>.Failure(ex.Message);
        }
    }
    ...
    public async Task<InsertOrUpdateResult<double>> UpdateCurrentStarPlantGalleryAsync(string userId, string plantId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            if (rowsAffected > 0)
            {
                ...
                selectCommand.Parameters.AddWithValue("@plant_id", plantId);
    ...
    public async Task<InsertOrUpdateResult<List<(string PlantId, double CurrentStar)>>> UpdateBatchCurrentStarPlantsGalleryAsync(string userId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            if (rowsAffected > 0)
            {
                // Bước 2: Lấy danh sách plant_id và current_star vừa được cập nhật (current_star == temp_star)
                string selectSQL = @"
                SELECT plant_id, current_star 
                FROM plants_gallery 
                WHERE user_id = @user_id 
                ...
                var updatedList = new List<(string PlantId, double CurrentStar)>();

                while (await reader.ReadAsync())
                {
                    string id = reader.GetString("plant_id");
                    double star = reader.GetDouble("current_star");
                    updatedList.Add((id, star));
                }

                return InsertOrUpdateResult<List<(string PlantId, double CurrentStar)>>.Updated(updatedList);
            }

            return new InsertOrUpdateResult<List<(string PlantId, double CurrentStar)>>
            {
                Data = new List<(string PlantId, double CurrentStar)>(),
                OperationType = DatabaseOperationType.None,
                Message = MessageConstants.NOTHING_WAS_UPDATED
            };
        }
        ...
        {
            Debug.LogError("Error: " + ex.Message);
            return InsertOrUpdateResult<List<(string PlantId, double CurrentStar)>>.Failure(ex.Message);
        }
    }
    public async Task<Plants> GetPlantCollectionByIdAsync(string userId, string objectId)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            string selectSQL = @"
            SELECT cg.* 
            FROM plants_gallery cg, plants c 
            WHERE cg.user_id = @userId 
                AND cg.plant_id = @objectId 
                AND cg.status = 'available'
                AND cg.plant_id = c.id
    ...
    public async Task<InsertOrUpdateResult<bool>> UpdatePlantGalleryPowerAsync(string userId, string id, Plants plant)
    {
        string connectionString = DatabaseConfig.ConnectionString;
