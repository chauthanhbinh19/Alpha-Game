...
public class CardMonstersGalleryRepository : ICardMonstersGalleryRepository
{
    ...
    public async Task<int> GetCardMonstersCountAsync(string search, string type, string rare)
    {
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                string selectSQL = @"SELECT COUNT(*) FROM card_monsters m 
                WHERE 1=1 AND m.is_active = TRUE AND m.is_deleted = FALSE";
                if (!string.IsNullOrEmpty(type) && type != "All")
                {
    ...
    public async Task<InsertOrUpdateResult<bool>> UpdateStatusCardMonsterGalleryAsync(string userId, string id, string status = "available")
    {
        ...
        try
        {
            ...
            string updateSQL = @"UPDATE card_monsters_gallery g INNER JOIN card_monsters p ON p.id = g.card_monster_id 
                             SET status = @status 
                             WHERE g.user_id = @user_id 
                               AND g.card_monster_id = @card_monster_id
    ...
    public async Task<InsertOrUpdateResult<double>> UpdateTempStarCardMonsterGalleryAsync(string userId, string cardMonsterId, double star)
    {
        ...
        try
        {
            ...
            string updateSQL = @"
            UPDATE card_monsters_gallery g INNER JOIN card_monsters p ON p.id = g.card_monster_id 
    ...
    public async Task<CardMonsters> GetCardMonsterCollectionByIdAsync(string userId, string objectId)
    {
        ...
        try
        {
            ...
            string selectSQL = @"
            SELECT cg.* 
            FROM card_monsters_gallery cg, card_monsters c 
            WHERE cg.user_id = @userId 
                AND cg.card_monster_id = @objectId 
                AND cg.status = 'available'
                AND cg.card_monster_id = c.id
    ...
    public async Task<InsertOrUpdateResult<bool>> UpdateCardMonsterGalleryPowerAsync(string userId, string id, CardMonsters cardMonster)
    {
        string connectionString = DatabaseConfig.ConnectionString;
