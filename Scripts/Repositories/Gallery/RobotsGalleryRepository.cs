...
public class RobotsGalleryRepository : IRobotsGalleryRepository
{
    public List<Robots> GetRobotsCollection(int pageSize, int offset, string rare)
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                FROM Robots t LEFT JOIN Robots_gallery tg ON t.id = tg.card_id and tg.user_id = @userId 
    ...
    public void UpdateStatusRobotsGallery(string Id)
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            ...
            {
                connection.Open();
                string query = "update Robots_gallery set status=@status where user_id=@user_id and card_id=@card_id";
                MySqlCommand command = new MySqlCommand(query, connection);
                command.Parameters.AddWithValue("@user_id", User.CurrentUserId);
                command.Parameters.AddWithValue("@card_id", Id);
