...
{ 
    ...
    public List<Cores> GetCores(int pageSize, int offset, string rare)
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            ...
            {
                connection.Open();
                string query = @"Select * from Cores WHERE (@rare = 'All' or rare = @rare)
                ORDER BY Cores.name REGEXP '[0-9]+$',CAST(REGEXP_SUBSTR(Cores.name, '[0-9]+$') AS UNSIGNED), Cores.name limit @limit offset @offset";
                MySqlCommand command = new MySqlCommand(query, connection);
    ...
    public List<Cores> GetCoresWithPrice(int pageSize, int offset)
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                from Cores t, Core_trade tt, currency cu
                where t.id=tt.Core_id and tt.currency_id = cu.id
    ...
    public Cores SumPowerCoresPercent()
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                from Cores a, user_Cores ua
                where a.id=ua.Core_id and ua.user_id=@user_id;";
                MySqlCommand command = new MySqlCommand(query, connection);
