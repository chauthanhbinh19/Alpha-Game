...
{ 
    ...
    public List<Cards> GetCards(int pageSize, int offset, string rare)
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            ...
            {
                connection.Open();
                string query = @"Select * from cards WHERE (@rare = 'All' or rare = @rare)
    ...
    public List<Cards> GetCardsWithPrice(int pageSize, int offset)
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                from cards t, card_trade tt, currency cu
                where t.id=tt.card_id and tt.currency_id = cu.id
    ...
    public Cards SumPowerCardsPercent()
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                from cards a, user_cards ua
                where a.id=ua.card_id and ua.user_id=@user_id;";
                MySqlCommand command = new MySqlCommand(query, connection);
