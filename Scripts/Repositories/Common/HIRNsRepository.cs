...
public class HIRNsRepository : IHIRNsRepository
{
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            ...
            ";
                ...
                using (MySqlCommand command = new MySqlCommand(query, connection))
                {
                    ...
                    command.Parameters.AddWithValue("@hirn_id", id);

                    using (MySqlDataReader reader = await command.ExecuteReaderAsync())
                    {
                        if (await reader.ReadAsync())
                        {
                            researchs.Id = reader.GetStringSafe("hirn_id");
                            researchs.Level = reader.GetIntSafe("hirn_level");
    ...
    public async Task InsertOrUpdateHIRNsAsync(string user_id, HIRNs HIRNs, string id)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        ...
        try
        {
            ...
            await using (var checkCommand = new MySqlCommand(checkQuery, connection))
            {
                ...
                ";
                    ...
                    AddAllParameters(updateCommand, HIRNs, user_id, id);
                ...
                ";
                    ...
                    AddAllParameters(insertCommand, HIRNs, user_id, id);
    ...
    private void AddAllParameters(MySqlCommand cmd, HIRNs a, string user_id, string type)
    {
        cmd.Parameters.AddWithValue("@user_id", user_id);
        cmd.Parameters.AddWithValue("@hirn_id", type);

        cmd.Parameters.AddWithValue("@hirn_level", a.Level == 0 ? 1 : a.Level);
