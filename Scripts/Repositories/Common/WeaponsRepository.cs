...
{ 
    ...
    public List<Weapons> GetWeapons(int pageSize, int offset, string rare)
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            ...
            {
                connection.Open();
                string query = @"Select * from Weapons WHERE (@rare = 'All' or rare = @rare)
                ORDER BY Weapons.name REGEXP '[0-9]+$',CAST(REGEXP_SUBSTR(Weapons.name, '[0-9]+$') AS UNSIGNED), Weapons.name limit @limit offset @offset";
                MySqlCommand command = new MySqlCommand(query, connection);
    ...
    public List<Weapons> GetWeaponsWithPrice(int pageSize, int offset)
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                from Weapons t, Weapon_trade tt, currency cu
                where t.id=tt.Weapon_id and tt.currency_id = cu.id
                ...
                while (reader.Read())
                {
                    ...
                    Weapon.Currency = new Currencies
                    {
    ...
    public Weapons SumPowerWeaponsPercent()
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                from Weapons a, user_Weapons ua
                where a.id=ua.Weapon_id and ua.user_id=@user_id;";
                MySqlCommand command = new MySqlCommand(query, connection);
