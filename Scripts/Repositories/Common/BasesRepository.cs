...
{ 
    ...
    {
        List<Bases> BasesList = new List<Bases>();
        string connectionString = DatabaseConfig.ConnectionString;
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                connection.Open();
                string query = @"SELECT w.*, CASE WHEN uw.Base_id IS NULL THEN 'false' ELSE 'true' END AS status 
                FROM Bases w LEFT JOIN user_Bases uw ON w.id = uw.Base_id and uw.user_id = @userId 
    ...
    public List<Bases> GetBasesWithPrice(int pageSize, int offset)
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                from Bases t, Base_trade tt, currency cu
                where t.id=tt.Base_id and tt.currency_id = cu.id
    ...
    public Bases SumPowerBasesPercent()
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                from Bases a, user_Bases ua
                where a.id=ua.Base_id and ua.user_id=@user_id;";
                MySqlCommand command = new MySqlCommand(query, connection);
