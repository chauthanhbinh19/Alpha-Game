...
{ 
    ...
    public List<Architectures> GetArchitectures(int pageSize, int offset, string rare)
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            ...
            {
                connection.Open();
                string query = @"Select * from architectures WHERE (@rare = 'All' or rare = @rare)
                ORDER BY architectures.name REGEXP '[0-9]+$',CAST(REGEXP_SUBSTR(architectures.name, '[0-9]+$') AS UNSIGNED), architectures.name limit @limit offset @offset";
                MySqlCommand command = new MySqlCommand(query, connection);
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                while (reader.Read())
                {
                    ...
                    architecture.Currency = new Currencies
                    {
    ...
    public Architectures SumPowerArchitecturesPercent()
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                from architectures a, user_architectures ua
                where a.id=ua.architecture_id and ua.user_id=@user_id;";
                MySqlCommand command = new MySqlCommand(query, connection);
