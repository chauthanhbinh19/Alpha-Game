...
{ 
    ...
    public List<MechaBeasts> GetMechaBeasts(int pageSize, int offset, string rare)
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            ...
            {
                connection.Open();
                string query = @"Select * from mecha_beasts WHERE (@rare = 'All' or rare = @rare)
                ORDER BY mecha_beasts.name REGEXP '[0-9]+$',CAST(REGEXP_SUBSTR(mecha_beasts.name, '[0-9]+$') AS UNSIGNED), mecha_beasts.name limit @limit offset @offset";
                MySqlCommand command = new MySqlCommand(query, connection);
                ...
                while (reader.Read())
                {
                    ...
                    MechaBeastsList.Add(MechaBeast);
                }
                connection.Close();
            }
    ...
    public List<MechaBeasts> GetMechaBeastsWithPrice(int pageSize, int offset)
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                from mecha_beasts t, mecha_beast_trade tt, currency cu
                where t.id=tt.mecha_beast_id and tt.currency_id = cu.id
                ...
                while (reader.Read())
                {
                    ...
                    MechaBeast.Currency = new Currencies
                    {
    ...
    public MechaBeasts SumPowerMechaBeastsPercent()
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                from mecha_beasts a, user_mecha_beasts ua
                where a.id=ua.mecha_beast_id and ua.user_id=@user_id;";
                MySqlCommand command = new MySqlCommand(query, connection);
