...
{ 
    ...
    {
        List<Researchs> ResearchsList = new List<Researchs>();
        string connectionString = DatabaseConfig.ConnectionString;
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                connection.Open();
                string query = @"SELECT w.*, CASE WHEN uw.Research_id IS NULL THEN 'block' ELSE 'available' END AS status 
                FROM Researchs w LEFT JOIN user_Researchs uw ON w.id = uw.Research_id and uw.user_id = @userId 
    ...
    public List<Researchs> GetResearchsWithPrice(int pageSize, int offset)
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                from Researchs t, Research_trade tt, currency cu
                where t.id=tt.Research_id and tt.currency_id = cu.id
                ...
                while (reader.Read())
                {
                    ...
                    Research.Currency = new Currencies
                    {
    ...
    public Researchs SumPowerResearchsPercent()
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                from Researchs a, user_Researchs ua
                where a.id=ua.Research_id and ua.user_id=@user_id;";
                MySqlCommand command = new MySqlCommand(query, connection);
