...
public class SpiritBeastRepository : ISpiritBeastRepository
{ 
    public List<string> GetUniqueSpiritBeastId()
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            ...
            string query = "Select distinct id from spirit_beast";
    ...
    public List<SpiritBeast> GetSpiritBeast(int pageSize, int offset, string rare)
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            ...
            {
                connection.Open();
                string query = @"Select * from spirit_beast WHERE (@rare = 'All' or rare = @rare)
                ORDER BY spirit_beast.name REGEXP '[0-9]+$',CAST(REGEXP_SUBSTR(spirit_beast.name, '[0-9]+$') AS UNSIGNED), spirit_beast.name limit @limit offset @offset";
                MySqlCommand command = new MySqlCommand(query, connection);
    ...
    public List<SpiritBeast> GetSpiritBeastWithPrice(int pageSize, int offset)
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                from spirit_beast t, spirit_beast_trade tt, currency cu
                where t.id=tt.spirit_beast_id and tt.currency_id = cu.id
    ...
    public SpiritBeast GetSpiritBeastById(string Id)
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            ...
            {
                connection.Open();
                string query = "Select * from spirit_beast where id=@id";
                MySqlCommand command = new MySqlCommand(query, connection);
                command.Parameters.AddWithValue("@id", Id);
                MySqlDataReader reader = command.ExecuteReader();
                while (reader.Read())
                {
                    SpiritBeast = new SpiritBeast
                    {
                        id = reader.GetString("id"),
                        name = reader.GetString("name"),
                        image = reader.GetString("image"),
    ...
    public SpiritBeast SumPowerSpiritBeastPercent()
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                from spirit_beast a, user_spirit_beast ua
                where a.id=ua.spirit_beast_id and ua.user_id=@user_id;";
                MySqlCommand command = new MySqlCommand(query, connection);
