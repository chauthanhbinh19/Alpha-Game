...
{ 
    ...
    public List<Technologies> GetTechnologies(int pageSize, int offset, string rare)
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            ...
            {
                connection.Open();
                string query = @"Select * from technologies WHERE (@rare = 'All' or rare = @rare)
                ORDER BY technologies.name REGEXP '[0-9]+$',CAST(REGEXP_SUBSTR(technologies.name, '[0-9]+$') AS UNSIGNED), technologies.name limit @limit offset @offset";
                MySqlCommand command = new MySqlCommand(query, connection);
    ...
    public List<Technologies> GetTechnologiesWithPrice(int pageSize, int offset)
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                from technologies t, technology_trade tt, currency cu
                where t.id=tt.technology_id and tt.currency_id = cu.id
                ...
                while (reader.Read())
                {
                    ...
                    technology.Currency = new Currencies
                    {
    ...
    public Technologies SumPowerTechnologiesPercent()
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                from technologies a, user_technologies ua
                where a.id=ua.technology_id and ua.user_id=@user_id;";
                MySqlCommand command = new MySqlCommand(query, connection);
