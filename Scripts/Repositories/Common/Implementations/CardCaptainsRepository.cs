...
public class CardCaptainsRepository : ICardCaptainsRepository
{
    ...
    public async Task<bool> IsCardCaptainDeletedOrInactiveAsync(string id)
    {
        if (string.IsNullOrEmpty(id)) return true;

        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = "SELECT 1 FROM card_captains WHERE id = @id AND (is_deleted = TRUE OR is_active = FALSE) LIMIT 1;";
        ...
        {
            Debug.LogError($"Error checking card captain status: {ex.Message}");
            return true;
        }
    }
