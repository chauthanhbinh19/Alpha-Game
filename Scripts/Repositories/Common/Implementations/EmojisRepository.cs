...
public class EmojisRepository : IEmojisRepository
{
    ...
    public async Task<InsertOrUpdateResult<Emojis>> InsertEmojiAsync(Emojis entity)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = @"
        INSERT INTO emojis (
        ...
        try
        {
            ...
            using (var cmd = new MySqlCommand(sql, conn))
            {
                ...
                if (rowsAffected > 0)
                {
                    return InsertOrUpdateResult<Emojis>.Inserted(entity);
                }
        ...
        {
            return InsertOrUpdateResult<Emojis>.Failure($"Failed When Insert Emoji: {ex.Message}");
        }
    }
    public async Task<InsertOrUpdateResult<Emojis>> UpdateEmojiAsync(Emojis entity)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = @"
        UPDATE emojis SET
        ...
        try
        {
            ...
            using (var cmd = new MySqlCommand(sql, conn))
            {
                ...
                if (rowsAffected > 0)
                {
                    return InsertOrUpdateResult<Emojis>.Updated(entity);
                }
        ...
        {
            return InsertOrUpdateResult<Emojis>.Failure($"Failed When Update Emoji: {ex.Message}");
        }
    }
    public async Task<bool> IsEmojiDeletedOrInactiveAsync(string id)
    {
        if (string.IsNullOrEmpty(id)) return true;

        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = "SELECT 1 FROM emojis WHERE id = @id AND (is_deleted = TRUE OR is_active = FALSE) LIMIT 1;";
        ...
        {
            Debug.LogError($"Error checking emoji status: {ex.Message}");
            return true;
        }
    }
    private void AddParameters(MySqlCommand command, Emojis entity)
    {
