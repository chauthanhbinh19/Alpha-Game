...
public class RelicsRepository : IRelicsRepository
{
    ...
    public async Task<InsertOrUpdateResult<Relics>> InsertRelicAsync(Relics entity)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = @"
        INSERT INTO relics (
        ...
        try
        {
            ...
            using (var cmd = new MySqlCommand(sql, conn))
            {
                ...
                if (rowsAffected > 0)
                {
                    return InsertOrUpdateResult<Relics>.Inserted(entity);
                }
        ...
        {
            return InsertOrUpdateResult<Relics>.Failure($"Failed When Insert Relic: {ex.Message}");
        }
    }
    public async Task<InsertOrUpdateResult<Relics>> UpdateRelicAsync(Relics entity)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = @"
        UPDATE relics SET
        ...
        try
        {
            ...
            using (var cmd = new MySqlCommand(sql, conn))
            {
                ...
                if (rowsAffected > 0)
                {
                    return InsertOrUpdateResult<Relics>.Updated(entity);
                }
        ...
        {
            return InsertOrUpdateResult<Relics>.Failure($"Failed When Update Relic: {ex.Message}");
        }
    }
    public async Task<bool> IsRelicDeletedOrInactiveAsync(string id)
    {
        if (string.IsNullOrEmpty(id)) return true;

        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = "SELECT 1 FROM relics WHERE id = @id AND (is_deleted = TRUE OR is_active = FALSE) LIMIT 1;";
        ...
        {
            Debug.LogError($"Error checking relic status: {ex.Message}");
            return true;
        }
    }
    private void AddParameters(MySqlCommand command, Relics entity)
    {
    ...
    public async Task<Relics> SumPowerRelicsPercentAsync(string userId)
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                FROM relics a
                INNER JOIN user_relics ua ON a.id = ua.relic_id
