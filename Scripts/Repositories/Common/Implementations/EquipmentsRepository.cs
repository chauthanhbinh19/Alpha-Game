...
public class EquipmentsRepository : IEquipmentsRepository
{
    ...
    public async Task<bool> IsEquipmentDeletedOrInactiveAsync(string id)
    {
        if (string.IsNullOrEmpty(id)) return true;

        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = "SELECT 1 FROM equipments WHERE id = @id AND (is_deleted = TRUE OR is_active = FALSE) LIMIT 1;";
