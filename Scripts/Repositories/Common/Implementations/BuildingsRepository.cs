    ...
    public async Task<List<Buildings>> GetBuildingsAsync(string search, string type, string rare, int pageSize, int offset)
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                selectSQL += " ORDER BY Buildings.name REGEXP '[0-9]+$', CAST(REGEXP_SUBSTR(Buildings.name, '[0-9]+$') AS UNSIGNED), Buildings.name";
    ...
    public async Task<List<Buildings>> GetBuildingsWithoutLimitAsync()
    {
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                string selectSQL = @"SELECT id, name, image, rare, quality FROM buildings";
