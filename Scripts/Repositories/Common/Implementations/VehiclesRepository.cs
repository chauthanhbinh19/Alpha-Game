...
public class VehiclesRepository : IVehiclesRepository
{
    ...
    public async Task<InsertOrUpdateResult<Vehicles>> InsertVehicleAsync(Vehicles entity)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = @"
        ...
        try
        {
            ...
            using (var cmd = new MySqlCommand(sql, conn))
            {
                ...
                if (rowsAffected > 0)
                {
                    return InsertOrUpdateResult<Vehicles>.Inserted(entity);
                }
        ...
        {
            return InsertOrUpdateResult<Vehicles>.Failure($"Failed When Insert Vehicle: {ex.Message}");
        }
    }
    public async Task<InsertOrUpdateResult<Vehicles>> UpdateVehicleAsync(Vehicles entity)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = @"
        UPDATE vehicles SET
        ...
        try
        {
            ...
            using (var cmd = new MySqlCommand(sql, conn))
            {
                ...
                if (rowsAffected > 0)
                {
                    return InsertOrUpdateResult<Vehicles>.Updated(entity);
                }
        ...
        {
            return InsertOrUpdateResult<Vehicles>.Failure($"Failed When Update Vehicle: {ex.Message}");
        }
    }
    public async Task<bool> IsVehicleDeletedOrInactiveAsync(string id)
    {
        if (string.IsNullOrEmpty(id)) return true;

        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = "SELECT 1 FROM vehicles WHERE id = @id AND (is_deleted = TRUE OR is_active = FALSE) LIMIT 1;";
        ...
        {
            Debug.LogError($"Error checking vehicle status: {ex.Message}");
            return true;
        }
    }
    private void AddParameters(MySqlCommand command, Vehicles entity)
    {
