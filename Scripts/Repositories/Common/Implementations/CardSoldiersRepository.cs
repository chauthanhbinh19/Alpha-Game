...
public class CardSoldiersRepository : ICardSoldiersRepository
{
    public async Task<List<string>> GetUniqueCardSoldiersTypesAsync()
    {
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                string selectSQL = "SELECT DISTINCT type FROM card_soldiers";
    ...
    public async Task<List<string>> GetUniqueCardSoldiersIdAsync()
    {
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                string selectSQL = "SELECT DISTINCT id FROM card_soldiers";
                await using (MySqlCommand selectCommand = new MySqlCommand(selectSQL, connection))
                await using (MySqlDataReader reader = await selectCommand.ExecuteReaderAsync())
                {
                    while (await reader.ReadAsync())
    ...
    public async Task<int> GetCardSoldiersCountAsync(string search, string type, string rare)
    {
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                string selectSQL = @"SELECT COUNT(*) FROM card_soldiers WHERE 1=1";
            ...
            try
            {
                ...
                string selectSQL = @"
                    ...
                    (
                        ...
                        WHERE che.card_soldier_id = ch.id
                    ) AS emblems_json,
                    (
                        SELECT JSON_ARRAYAGG(
                            JSON_OBJECT(
                        ...
                        WHERE chc.card_soldier_id = ch.id
                    ) AS classes_json
    ...
    public async Task<InsertOrUpdateResult<CardSoldiers>> InsertCardSoldierAsync(CardSoldiers entity)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = @"
        INSERT INTO card_soldiers (
        ...
        try
        {
            ...
            using (var cmd = new MySqlCommand(sql, conn))
            {
                ...
                if (rowsAffected > 0)
                {
                    return InsertOrUpdateResult<CardSoldiers>.Inserted(entity);
                }
        ...
        {
            return InsertOrUpdateResult<CardSoldiers>.Failure($"Failed When Insert Card Soldier: {ex.Message}");
        }
    }
    public async Task<InsertOrUpdateResult<CardSoldiers>> UpdateCardSoldierAsync(CardSoldiers entity)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = @"
        UPDATE card_soldiers SET
            name = @name,
        ...
        try
        {
            ...
            using (var cmd = new MySqlCommand(sql, conn))
            {
                ...
                if (rowsAffected > 0)
                {
                    return InsertOrUpdateResult<CardSoldiers>.Updated(entity);
                }
        ...
        {
            return InsertOrUpdateResult<CardSoldiers>.Failure($"Failed When Update Card Soldier: {ex.Message}");
        }
    }
    public async Task<bool> IsCardSoldierDeletedOrInactiveAsync(string id)
    {
        if (string.IsNullOrEmpty(id)) return true;

        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = "SELECT 1 FROM card_soldiers WHERE id = @id AND (is_deleted = TRUE OR is_active = FALSE) LIMIT 1;";
        ...
        {
            Debug.LogError($"Error checking card soldier status: {ex.Message}");
            return true;
        }
    }
    private void AddParameters(MySqlCommand command, CardSoldiers entity)
    {
    ...
    public async Task<List<CardSoldiers>> GetCardSoldiersRandomAsync(string type, int pageSize)
    {
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                string selectSQL = "SELECT * FROM card_soldiers WHERE type = @type ORDER BY RAND() LIMIT @limit";
                ...
                await using (MySqlCommand selectCommand = new MySqlCommand(selectSQL, connection))
                {
                    ...
                    await using (MySqlDataReader reader = await selectCommand.ExecuteReaderAsync())
                    {
                        while (await reader.ReadAsync())
                        {
                            CardSoldiers captain = new CardSoldiers
                            {
                                Id = reader.GetStringSafe("id"),
                                Name = reader.GetStringSafe("name"),
                                Image = reader.GetStringSafe("image"),
    ...
    public async Task<List<CardSoldiers>> GetAllCardSoldiersAsync(string type)
    {
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                string selectSQL = "SELECT * FROM card_soldiers WHERE type = @type";
    ...
    public async Task<CardSoldiers> GetCardSoldierByIdAsync(string Id)
    {
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                string selectSQL = "SELECT * FROM card_soldiers WHERE id = @id";
    ...
    public async Task<List<CardSoldiers>> GetCardSoldiersWithPriceAsync(string type, int pageSize, int offset)
    {
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                JOIN card_soldier_trade ct ON c.id = ct.card_soldier_id
