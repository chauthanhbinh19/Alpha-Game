...
public class AlchemiesRepository : IAlchemiesRepository
{
    ...
    public async Task<InsertOrUpdateResult<Alchemies>> InsertAlchemyAsync(Alchemies entity)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = @"
        INSERT INTO alchemies (
        ...
        try
        {
            ...
            using (var cmd = new MySqlCommand(sql, conn))
            {
                ...
                if (rowsAffected > 0)
                {
                    return InsertOrUpdateResult<Alchemies>.Inserted(entity);
                }
        ...
        {
            return InsertOrUpdateResult<Alchemies>.Failure($"Failed When Insert Alchemy: {ex.Message}");
        }
    }
    public async Task<InsertOrUpdateResult<Alchemies>> UpdateAlchemyAsync(Alchemies entity)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = @"
        UPDATE alchemies SET
            name = @name,
        ...
        try
        {
            ...
            using (var cmd = new MySqlCommand(sql, conn))
            {
                ...
                if (rowsAffected > 0)
                {
                    return InsertOrUpdateResult<Alchemies>.Updated(entity);
                }
        ...
        {
            return InsertOrUpdateResult<Alchemies>.Failure($"Failed When Update Alchemy: {ex.Message}");
        }
    }
    public async Task<bool> IsAlchemyDeletedOrInactiveAsync(string id)
    {
        if (string.IsNullOrEmpty(id)) return true;

        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = "SELECT 1 FROM alchemies WHERE id = @id AND (is_deleted = TRUE OR is_active = FALSE) LIMIT 1;";
        ...
        {
            Debug.LogError($"Error checking alchemy status: {ex.Message}");
            return true;
        }
    }
    private void AddParameters(MySqlCommand command, Alchemies entity)
    {
