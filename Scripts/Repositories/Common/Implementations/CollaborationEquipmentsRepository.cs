...
public class CollaborationEquipmentsRepository : ICollaborationEquipmentsRepository
{
    ...
    public async Task<InsertOrUpdateResult<CollaborationEquipments>> InsertCollaborationEquipmentAsync(CollaborationEquipments entity)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = @"
        INSERT INTO collaboration_equipments (
        ...
        try
        {
            ...
            using (var cmd = new MySqlCommand(sql, conn))
            {
                ...
                if (rowsAffected > 0)
                {
                    return InsertOrUpdateResult<CollaborationEquipments>.Inserted(entity);
                }
        ...
        {
            return InsertOrUpdateResult<CollaborationEquipments>.Failure($"Failed When Insert Collaboration Equipment: {ex.Message}");
        }
    }
    public async Task<InsertOrUpdateResult<CollaborationEquipments>> UpdateCollaborationEquipmentAsync(CollaborationEquipments entity)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = @"
        UPDATE collaboration_equipments SET
            name = @name,
        ...
        try
        {
            ...
            using (var cmd = new MySqlCommand(sql, conn))
            {
                ...
                if (rowsAffected > 0)
                {
                    return InsertOrUpdateResult<CollaborationEquipments>.Updated(entity);
                }
        ...
        {
            return InsertOrUpdateResult<CollaborationEquipments>.Failure($"Failed When Update Collaboration Equipment: {ex.Message}");
        }
    }
    public async Task<bool> IsCollaborationEquipmentDeletedOrInactiveAsync(string id)
    {
        if (string.IsNullOrEmpty(id)) return true;

        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = "SELECT 1 FROM collaboration_equipments WHERE id = @id AND (is_deleted = TRUE OR is_active = FALSE) LIMIT 1;";
        ...
        {
            Debug.LogError($"Error checking collaboration equipment status: {ex.Message}");
            return true;
        }
    }
    private void AddParameters(MySqlCommand command, CollaborationEquipments entity)
    {
