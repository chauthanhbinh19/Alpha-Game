...
public class CardMonstersRepository : ICardMonstersRepository
{
    ...
    public async Task<List<CardMonsters>> GetCardMonstersWithoutLimitAsync()
    {
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                string selectSQL = @"SELECT * FROM card_monsters";
    ...
    public async Task<InsertOrUpdateResult<CardMonsters>> InsertCardMonsterAsync(CardMonsters entity)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = @"
        INSERT INTO card_monsters (
        ...
        try
        {
            ...
            using (var cmd = new MySqlCommand(sql, conn))
            {
                ...
                if (rowsAffected > 0)
                {
                    return InsertOrUpdateResult<CardMonsters>.Inserted(entity);
                }
        ...
        {
            return InsertOrUpdateResult<CardMonsters>.Failure($"Failed When Insert Card Monster: {ex.Message}");
        }
    }
    public async Task<InsertOrUpdateResult<CardMonsters>> UpdateCardMonsterAsync(CardMonsters entity)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = @"
        UPDATE card_monsters SET
            name = @name,
        ...
        try
        {
            ...
            using (var cmd = new MySqlCommand(sql, conn))
            {
                ...
                if (rowsAffected > 0)
                {
                    return InsertOrUpdateResult<CardMonsters>.Updated(entity);
                }
        ...
        {
            return InsertOrUpdateResult<CardMonsters>.Failure($"Failed When Update Card Monster: {ex.Message}");
        }
    }
    public async Task<bool> IsCardMonsterDeletedOrInactiveAsync(string id)
    {
        if (string.IsNullOrEmpty(id)) return true;

        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = "SELECT 1 FROM card_monsters WHERE id = @id AND (is_deleted = TRUE OR is_active = FALSE) LIMIT 1;";
        ...
        {
            Debug.LogError($"Error checking card monster status: {ex.Message}");
            return true;
        }
    }
    private void AddParameters(MySqlCommand command, CardMonsters entity)
    {
