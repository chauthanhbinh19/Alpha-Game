...
public class BooksRepository : IBooksRepository
{
    ...
    public async Task<InsertOrUpdateResult<Books>> InsertBookAsync(Books entity)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = @"
        INSERT INTO books (
        ...
        try
        {
            ...
            using (var cmd = new MySqlCommand(sql, conn))
            {
                ...
                if (rowsAffected > 0)
                {
                    return InsertOrUpdateResult<Books>.Inserted(entity);
                }
        ...
        {
            return InsertOrUpdateResult<Books>.Failure($"Failed When Insert Book: {ex.Message}");
        }
    }
    public async Task<InsertOrUpdateResult<Books>> UpdateBookAsync(Books entity)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = @"
        UPDATE books SET
        ...
        try
        {
            ...
            using (var cmd = new MySqlCommand(sql, conn))
            {
                ...
                if (rowsAffected > 0)
                {
                    return InsertOrUpdateResult<Books>.Updated(entity);
                }
