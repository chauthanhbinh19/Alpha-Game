...
public class ForgesRepository : IForgesRepository
{
    public async Task<List<string>> GetUniqueForgesTypesAsync()
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            ...
            string selectSQL = "SELECT DISTINCT type FROM forges";
    ...
    public async Task<List<string>> GetUniqueForgesIdAsync()
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            ...
            string selectSQL = "SELECT DISTINCT id FROM forges";
    ...
    public async Task<int> GetForgesCountAsync(string search, string type, string rare)
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                string selectSQL = @"SELECT COUNT(*) FROM forges WHERE 1=1";
    ...
    public async Task<InsertOrUpdateResult<Forges>> InsertForgeAsync(Forges entity)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = @"
        INSERT INTO forges (
        ...
        try
        {
            ...
            using (var cmd = new MySqlCommand(sql, conn))
            {
                ...
                if (rowsAffected > 0)
                {
                    return InsertOrUpdateResult<Forges>.Inserted(entity);
                }
        ...
        {
            return InsertOrUpdateResult<Forges>.Failure($"Failed When Insert Forge: {ex.Message}");
        }
    }
    public async Task<InsertOrUpdateResult<Forges>> UpdateForgeAsync(Forges entity)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = @"
        UPDATE forges SET
        ...
        try
        {
            ...
            using (var cmd = new MySqlCommand(sql, conn))
            {
                ...
                if (rowsAffected > 0)
                {
                    return InsertOrUpdateResult<Forges>.Updated(entity);
                }
        ...
        {
            return InsertOrUpdateResult<Forges>.Failure($"Failed When Update Forge: {ex.Message}");
        }
    }
    public async Task<bool> IsForgeDeletedOrInactiveAsync(string id)
    {
        if (string.IsNullOrEmpty(id)) return true;

        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = "SELECT 1 FROM forges WHERE id = @id AND (is_deleted = TRUE OR is_active = FALSE) LIMIT 1;";
        ...
        {
            Debug.LogError($"Error checking forge status: {ex.Message}");
            return true;
        }
    }
    private void AddParameters(MySqlCommand command, Forges entity)
    {
    ...
    public async Task<Forges> GetForgeByIdAsync(string Id)
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                string selectSQL = "SELECT * FROM forges WHERE id = @id";
