...
public class CardColonelsRepository : ICardColonelsRepository
{
    ...
    public async Task<bool> IsCardColonelDeletedOrInactiveAsync(string id)
    {
        if (string.IsNullOrEmpty(id)) return true;

        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = "SELECT 1 FROM card_colonels WHERE id = @id AND (is_deleted = TRUE OR is_active = FALSE) LIMIT 1;";
        ...
        {
            Debug.LogError($"Error checking card colonel status: {ex.Message}");
            return true;
        }
    }
