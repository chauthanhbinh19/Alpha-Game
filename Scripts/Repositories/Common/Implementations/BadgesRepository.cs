...
public class BadgesRepository : IBadgesRepository
{
    ...
    public async Task<InsertOrUpdateResult<Badges>> InsertBadgeAsync(Badges entity)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = @"
        INSERT INTO badges (
        ...
        try
        {
            ...
            using (var cmd = new MySqlCommand(sql, conn))
            {
                ...
                if (rowsAffected > 0)
                {
                    return InsertOrUpdateResult<Badges>.Inserted(entity);
                }
        ...
        {
            return InsertOrUpdateResult<Badges>.Failure($"Failed When Insert Badge: {ex.Message}");
        }
    }
    public async Task<InsertOrUpdateResult<Badges>> UpdateBadgeAsync(Badges entity)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = @"
        UPDATE badges SET
        ...
        try
        {
            ...
            using (var cmd = new MySqlCommand(sql, conn))
            {
                ...
                if (rowsAffected > 0)
                {
                    return InsertOrUpdateResult<Badges>.Updated(entity);
                }
