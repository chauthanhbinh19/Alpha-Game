...
public class CardHeroesRepository : ICardHeroesRepository
{
    ...
    public async Task<List<CardHeroes>> GetCardHeroesWithoutLimitAsync()
    {
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                string selectSQL = @"SELECT * FROM card_heroes";
    ...
    public async Task<InsertOrUpdateResult<CardHeroes>> InsertCardHeroAsync(CardHeroes entity)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = @"
        INSERT INTO card_heroes (
        ...
        try
        {
            ...
            using (var cmd = new MySqlCommand(sql, conn))
            {
                ...
                if (rowsAffected > 0)
                {
                    return InsertOrUpdateResult<CardHeroes>.Inserted(entity);
                }
        ...
        {
            return InsertOrUpdateResult<CardHeroes>.Failure($"Failed When Insert Card Hero: {ex.Message}");
        }
    }
    public async Task<InsertOrUpdateResult<CardHeroes>> UpdateCardHeroAsync(CardHeroes entity)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = @"
        UPDATE card_heroes SET
            name = @name,
        ...
        try
        {
            ...
            using (var cmd = new MySqlCommand(sql, conn))
            {
                ...
                if (rowsAffected > 0)
                {
                    return InsertOrUpdateResult<CardHeroes>.Updated(entity);
                }
        ...
        {
            return InsertOrUpdateResult<CardHeroes>.Failure($"Failed When Update Card Hero: {ex.Message}");
        }
    }
    public async Task<bool> IsCardHeroDeletedOrInactiveAsync(string id)
    {
        if (string.IsNullOrEmpty(id)) return true;

        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = "SELECT 1 FROM card_heroes WHERE id = @id AND (is_deleted = TRUE OR is_active = FALSE) LIMIT 1;";
        ...
        {
            Debug.LogError($"Error checking card hero status: {ex.Message}");
            return true;
        }
    }
    private void AddParameters(MySqlCommand command, CardHeroes entity)
    {
