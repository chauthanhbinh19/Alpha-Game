...
public class AvatarsRepository : IAvatarsRepository
{
    ...
    public async Task<InsertOrUpdateResult<Avatars>> InsertAvatarAsync(Avatars entity)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = @"
        INSERT INTO avatars (
        ...
        try
        {
            ...
            using (var cmd = new MySqlCommand(sql, conn))
            {
                ...
                if (rowsAffected > 0)
                {
                    return InsertOrUpdateResult<Avatars>.Inserted(entity);
                }
        ...
        {
            return InsertOrUpdateResult<Avatars>.Failure($"Failed When Insert Avatar: {ex.Message}");
        }
    }
    public async Task<InsertOrUpdateResult<Avatars>> UpdateAvatarAsync(Avatars entity)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = @"
        UPDATE avatars SET
        ...
        try
        {
            ...
            using (var cmd = new MySqlCommand(sql, conn))
            {
                ...
                if (rowsAffected > 0)
                {
                    return InsertOrUpdateResult<Avatars>.Updated(entity);
                }
        ...
        {
            return InsertOrUpdateResult<Avatars>.Failure($"Failed When Update Avatar: {ex.Message}");
        }
    }
    public async Task<bool> IsAvatarDeletedOrInactiveAsync(string id)
    {
        if (string.IsNullOrEmpty(id)) return true;

        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = "SELECT 1 FROM avatars WHERE id = @id AND (is_deleted = TRUE OR is_active = FALSE) LIMIT 1;";
        ...
        {
            Debug.LogError($"Error checking avatar status: {ex.Message}");
            return true;
        }
    }
    private void AddParameters(MySqlCommand command, Avatars entity)
    {
