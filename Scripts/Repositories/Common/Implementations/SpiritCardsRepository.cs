...
public class SpiritCardsRepository : ISpiritCardsRepository
{
    ...
    public async Task<InsertOrUpdateResult<SpiritCards>> InsertSpiritCardAsync(SpiritCards entity)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = @"
        INSERT INTO spirit_cards (
        ...
        try
        {
            ...
            using (var cmd = new MySqlCommand(sql, conn))
            {
                ...
                if (rowsAffected > 0)
                {
                    return InsertOrUpdateResult<SpiritCards>.Inserted(entity);
                }
        ...
        {
            return InsertOrUpdateResult<SpiritCards>.Failure($"Failed When Insert Spirit Card: {ex.Message}");
        }
    }
    public async Task<InsertOrUpdateResult<SpiritCards>> UpdateSpiritCardAsync(SpiritCards entity)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = @"
        UPDATE spirit_cards SET
            name = @name,
        ...
        try
        {
            ...
            using (var cmd = new MySqlCommand(sql, conn))
            {
                ...
                if (rowsAffected > 0)
                {
                    return InsertOrUpdateResult<SpiritCards>.Updated(entity);
                }
        ...
        {
            return InsertOrUpdateResult<SpiritCards>.Failure($"Failed When Update Spirit Card: {ex.Message}");
        }
    }
    public async Task<bool> IsSpiritCardDeletedOrInactiveAsync(string id)
    {
        if (string.IsNullOrEmpty(id)) return true;

        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = "SELECT 1 FROM spirit_cards WHERE id = @id AND (is_deleted = TRUE OR is_active = FALSE) LIMIT 1;";
        ...
        {
            Debug.LogError($"Error checking spirit card status: {ex.Message}");
            return true;
        }
    }
    private void AddParameters(MySqlCommand command, SpiritCards entity)
    {
    ...
    public async Task<SpiritCards> SumPowerSpiritCardsPercentAsync(string userId)
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                FROM spirit_cards a
                INNER JOIN user_spirit_cards ua ON a.id = ua.spirit_card_id
