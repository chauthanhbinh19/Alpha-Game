...
public class MagicFormationCirclesRepository : IMagicFormationCirclesRepository
{
    ...
    public async Task<InsertOrUpdateResult<MagicFormationCircles>> InsertMagicFormationCircleAsync(MagicFormationCircles entity)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = @"
        INSERT INTO magic_formation_circles (
        ...
        try
        {
            ...
            using (var cmd = new MySqlCommand(sql, conn))
            {
                ...
                if (rowsAffected > 0)
                {
                    return InsertOrUpdateResult<MagicFormationCircles>.Inserted(entity);
                }
        ...
        {
            return InsertOrUpdateResult<MagicFormationCircles>.Failure($"Failed When Insert Magic Formation Circle: {ex.Message}");
        }
    }
    public async Task<InsertOrUpdateResult<MagicFormationCircles>> UpdateMagicFormationCircleAsync(MagicFormationCircles entity)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = @"
        UPDATE magic_formation_circles SET
            name = @name,
        ...
        try
        {
            ...
            using (var cmd = new MySqlCommand(sql, conn))
            {
                ...
                if (rowsAffected > 0)
                {
                    return InsertOrUpdateResult<MagicFormationCircles>.Updated(entity);
                }
        ...
        {
            return InsertOrUpdateResult<MagicFormationCircles>.Failure($"Failed When Update Magic Formation Circle: {ex.Message}");
        }
    }
    public async Task<bool> IsMagicFormationCircleDeletedOrInactiveAsync(string id)
    {
        if (string.IsNullOrEmpty(id)) return true;

        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = "SELECT 1 FROM magic_formation_circles WHERE id = @id AND (is_deleted = TRUE OR is_active = FALSE) LIMIT 1;";
        ...
        {
            Debug.LogError($"Error checking magic formation circle status: {ex.Message}");
            return true;
        }
    }
    private void AddParameters(MySqlCommand command, MagicFormationCircles entity)
    {
