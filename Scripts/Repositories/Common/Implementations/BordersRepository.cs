...
public class BordersRepository : IBordersRepository
{
    ...
    public async Task<InsertOrUpdateResult<Borders>> InsertBorderAsync(Borders entity)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = @"
        INSERT INTO borders (
        ...
        try
        {
            ...
            using (var cmd = new MySqlCommand(sql, conn))
            {
                ...
                if (rowsAffected > 0)
                {
                    return InsertOrUpdateResult<Borders>.Inserted(entity);
                }
        ...
        {
            return InsertOrUpdateResult<Borders>.Failure($"Failed When Insert Border: {ex.Message}");
        }
    }
    public async Task<InsertOrUpdateResult<Borders>> UpdateBorderAsync(Borders entity)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = @"
        UPDATE borders SET
        ...
        try
        {
            ...
            using (var cmd = new MySqlCommand(sql, conn))
            {
                ...
                if (rowsAffected > 0)
                {
                    return InsertOrUpdateResult<Borders>.Updated(entity);
                }
        ...
        {
            return InsertOrUpdateResult<Borders>.Failure($"Failed When Update Border: {ex.Message}");
        }
    }
    public async Task<bool> IsBorderDeletedOrInactiveAsync(string id)
    {
        if (string.IsNullOrEmpty(id)) return true;

        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = "SELECT 1 FROM borders WHERE id = @id AND (is_deleted = TRUE OR is_active = FALSE) LIMIT 1;";
        ...
        {
            Debug.LogError($"Error checking border status: {ex.Message}");
            return true;
        }
    }
    private void AddParameters(MySqlCommand command, Borders entity)
    {
