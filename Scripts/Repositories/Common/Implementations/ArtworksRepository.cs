...
public class ArtworksRepository : IArtworksRepository
{
    ...
    public async Task<bool> IsArtworkDeletedOrInactiveAsync(string id)
    {
        if (string.IsNullOrEmpty(id)) return true;

        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = "SELECT 1 FROM artworks WHERE id = @id AND (is_deleted = TRUE OR is_active = FALSE) LIMIT 1;";
        ...
        {
            Debug.LogError($"Error checking artwork status: {ex.Message}");
            return true;
        }
    }
