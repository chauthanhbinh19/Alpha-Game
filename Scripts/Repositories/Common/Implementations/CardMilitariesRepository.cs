...
public class CardMilitariesRepository : ICardMilitariesRepository
{
    public async Task<List<string>> GetUniqueCardMilitariesTypesAsync()
    {
        ...
        try
        {
            ...
            string selectSQL = "SELECT DISTINCT type FROM card_militaries";
    ...
    public async Task<List<string>> GetUniqueCardMilitariesIdAsync()
    {
        ...
        try
        {
            ...
            string selectSQL = "SELECT DISTINCT id FROM card_militaries";
            await using var selectCommand = new MySqlCommand(selectSQL, connection);
            await using var reader = await selectCommand.ExecuteReaderAsync();
        ...
        try
        {
            ...
            SELECT 
                    ...
                    (
                        ...
                        WHERE che.card_military_id = ch.id
                    ) AS emblems_json,
                    (
                        SELECT JSON_ARRAYAGG(
                            JSON_OBJECT(
                        ...
                        WHERE chc.card_military_id = ch.id
                    ) AS classes_json
                FROM card_militaries ch
            WHERE 1=1";
            ...
            while (await reader.ReadAsync())
            {
                ...
                if (!string.IsNullOrEmpty(emblemsJson))
                {
                    ...
                    {
                        // Phòng trường hợp Military không có emblem, MySQL sinh ra chuỗi "[null]"
    ...
    public async Task<int> GetCardMilitariesCountAsync(string search, string type, string rare)
    {
        ...
        try
        {
            ...
            string selectSQL = @"SELECT COUNT(*) FROM card_militaries WHERE 1=1";
    ...
    public async Task<InsertOrUpdateResult<CardMilitaries>> InsertCardMilitaryAsync(CardMilitaries entity)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = @"
        INSERT INTO card_militaries (
        ...
        try
        {
            ...
            using (var cmd = new MySqlCommand(sql, conn))
            {
                ...
                if (rowsAffected > 0)
                {
                    return InsertOrUpdateResult<CardMilitaries>.Inserted(entity);
                }
        ...
        {
            return InsertOrUpdateResult<CardMilitaries>.Failure($"Failed When Insert Card Military: {ex.Message}");
        }
    }
    public async Task<InsertOrUpdateResult<CardMilitaries>> UpdateCardMilitaryAsync(CardMilitaries entity)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = @"
        UPDATE card_militaries SET
            name = @name,
        ...
        try
        {
            ...
            using (var cmd = new MySqlCommand(sql, conn))
            {
                ...
                if (rowsAffected > 0)
                {
                    return InsertOrUpdateResult<CardMilitaries>.Updated(entity);
                }
        ...
        {
            return InsertOrUpdateResult<CardMilitaries>.Failure($"Failed When Update Card Military: {ex.Message}");
        }
    }
    public async Task<bool> IsCardMilitaryDeletedOrInactiveAsync(string id)
    {
        if (string.IsNullOrEmpty(id)) return true;

        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = "SELECT 1 FROM card_militaries WHERE id = @id AND (is_deleted = TRUE OR is_active = FALSE) LIMIT 1;";
        ...
        {
            Debug.LogError($"Error checking card military status: {ex.Message}");
            return true;
        }
    }
    private void AddParameters(MySqlCommand command, CardMilitaries entity)
    {
    ...
    public async Task<List<CardMilitaries>> GetCardMilitariesRandomAsync(string type, int pageSize)
    {
        ...
        try
        {
            ...
            string selectSQL = "SELECT * FROM card_militaries WHERE type = @type ORDER BY RAND() LIMIT @limit";
            await using var selectCommand = new MySqlCommand(selectSQL, connection);
            selectCommand.Parameters.AddWithValue("@type", type);
            selectCommand.Parameters.AddWithValue("@limit", pageSize);
    ...
    public async Task<List<CardMilitaries>> GetAllCardMilitariesAsync(string type)
    {
        ...
        try
        {
            ...
            string selectSQL = "SELECT * FROM card_militaries WHERE type = @type";
            await using var selectCommand = new MySqlCommand(selectSQL, connection);
            selectCommand.Parameters.AddWithValue("@type", type);
    ...
    public async Task<CardMilitaries> GetCardMilitaryByIdAsync(string Id)
    {
        ...
        try
        {
            ...
            string selectSQL = "SELECT * FROM card_militaries WHERE id = @id";
            await using var selectCommand = new MySqlCommand(selectSQL, connection);
