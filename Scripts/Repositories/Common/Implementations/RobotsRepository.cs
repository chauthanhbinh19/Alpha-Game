...
public class RobotsRepository : IRobotsRepository
{
    ...
    public async Task<InsertOrUpdateResult<Robots>> InsertRobotAsync(Robots entity)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = @"
        INSERT INTO robots (
        ...
        try
        {
            ...
            using (var cmd = new MySqlCommand(sql, conn))
            {
                ...
                if (rowsAffected > 0)
                {
                    return InsertOrUpdateResult<Robots>.Inserted(entity);
                }
        ...
        {
            return InsertOrUpdateResult<Robots>.Failure($"Failed When Insert Robot: {ex.Message}");
        }
    }
    public async Task<InsertOrUpdateResult<Robots>> UpdateRobotAsync(Robots entity)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = @"
        UPDATE robots SET
        ...
        try
        {
            ...
            using (var cmd = new MySqlCommand(sql, conn))
            {
                ...
                if (rowsAffected > 0)
                {
                    return InsertOrUpdateResult<Robots>.Updated(entity);
                }
