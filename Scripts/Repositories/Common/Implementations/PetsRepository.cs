...
public class PetsRepository : IPetsRepository
{
    public async Task<List<string>> GetUniquePetsTypesAsync()
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                string selectSQL = "SELECT DISTINCT type FROM Pets";
    ...
    public async Task<List<string>> GetUniquePetsIdAsync()
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                string selectSQL = "SELECT DISTINCT id FROM Pets";
    ...
    public async Task<List<Pets>> GetPetsAsync(string search, string type, string rare, int pageSize, int offset)
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                string selectSQL = @"SELECT * FROM Pets WHERE 1=1";
    ...
    public async Task<int> GetPetsCountAsync(string search, string type, string rare)
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                string selectSQL = @"SELECT COUNT(*) FROM Pets WHERE 1=1";
    ...
    public async Task<InsertOrUpdateResult<Pets>> InsertPetAsync(Pets entity)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = @"
        INSERT INTO pets (
        ...
        try
        {
            ...
            using (var cmd = new MySqlCommand(sql, conn))
            {
                ...
                if (rowsAffected > 0)
                {
                    return InsertOrUpdateResult<Pets>.Inserted(entity);
                }
        ...
        {
            return InsertOrUpdateResult<Pets>.Failure($"Failed When Insert Pet: {ex.Message}");
        }
    }
    public async Task<InsertOrUpdateResult<Pets>> UpdatePetAsync(Pets entity)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = @"
        UPDATE pets SET
        ...
        try
        {
            ...
            using (var cmd = new MySqlCommand(sql, conn))
            {
                ...
                if (rowsAffected > 0)
                {
                    return InsertOrUpdateResult<Pets>.Updated(entity);
                }
        ...
        {
            return InsertOrUpdateResult<Pets>.Failure($"Failed When Update Pet: {ex.Message}");
        }
    }
    public async Task<bool> IsPetDeletedOrInactiveAsync(string id)
    {
        if (string.IsNullOrEmpty(id)) return true;

        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = "SELECT 1 FROM pets WHERE id = @id AND (is_deleted = TRUE OR is_active = FALSE) LIMIT 1;";
        ...
        {
            Debug.LogError($"Error checking pet status: {ex.Message}");
            return true;
        }
    }
    private void AddParameters(MySqlCommand command, Pets entity)
    {
    ...
    public async Task<List<Pets>> GetPetsWithPriceAsync(string type, int pageSize, int offset)
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                string selectSQL = @"
                SELECT p.*, pt.price, cu.image AS currency_image, cu.id AS currency_id
                FROM pets p
                JOIN pet_trade pt ON p.id = pt.pet_id
                JOIN currencies cu ON pt.currency_id = cu.id
                WHERE p.type = @type
    ...
    public async Task<Pets> GetPetByIdAsync(string Id)
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                string selectSQL = "SELECT * FROM pets WHERE id = @id";
                using (MySqlCommand selectCommand = new MySqlCommand(selectSQL, connection))
                {
