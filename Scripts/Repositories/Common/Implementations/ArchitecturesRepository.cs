...
public class ArchitecturesRepository : IArchitecturesRepository
{
    ...
    public async Task<InsertOrUpdateResult<Architectures>> InsertArchitectureAsync(Architectures entity)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = @"
        ...
        try
        {
            ...
            using (var cmd = new MySqlCommand(sql, conn))
            {
                ...
                if (rowsAffected > 0)
                {
                    return InsertOrUpdateResult<Architectures>.Inserted(entity);
                }
        ...
        {
            return InsertOrUpdateResult<Architectures>.Failure($"Failed When Insert Architecture: {ex.Message}");
        }
    }
    public async Task<InsertOrUpdateResult<Architectures>> UpdateArchitectureAsync(Architectures entity)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = @"
        ...
        try
        {
            ...
            using (var cmd = new MySqlCommand(sql, conn))
            {
                ...
                if (rowsAffected > 0)
                {
                    return InsertOrUpdateResult<Architectures>.Updated(entity);
                }
        ...
        {
            return InsertOrUpdateResult<Architectures>.Failure($"Failed When Update Architecture: {ex.Message}");
        }
    }
    public async Task<bool> IsArchitectureDeletedOrInactiveAsync(string id)
    {
        if (string.IsNullOrEmpty(id)) return true;

        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = "SELECT 1 FROM architectures WHERE id = @id AND (is_deleted = TRUE OR is_active = FALSE) LIMIT 1;";
        ...
        {
            Debug.LogError($"Error checking architecture status: {ex.Message}");
            return true;
        }
    }
    private void AddParameters(MySqlCommand command, Architectures entity)
    {
