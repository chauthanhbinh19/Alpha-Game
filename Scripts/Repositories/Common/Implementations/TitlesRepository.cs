...
public class TitlesRepository : ITitlesRepository
{
    ...
    public async Task<List<Titles>> GetTitlesWithoutLimitAsync()
    {
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                string selectSQL = @"SELECT * FROM titles";
    ...
    public async Task<InsertOrUpdateResult<Titles>> InsertTitleAsync(Titles entity)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = @"
        INSERT INTO titles (
        ...
        try
        {
            ...
            using (var cmd = new MySqlCommand(sql, conn))
            {
                ...
                if (rowsAffected > 0)
                {
                    return InsertOrUpdateResult<Titles>.Inserted(entity);
                }
        ...
        {
            return InsertOrUpdateResult<Titles>.Failure($"Failed When Insert Title: {ex.Message}");
        }
    }
    public async Task<InsertOrUpdateResult<Titles>> UpdateTitleAsync(Titles entity)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = @"
        UPDATE titles SET
        ...
        try
        {
            ...
            using (var cmd = new MySqlCommand(sql, conn))
            {
                ...
                if (rowsAffected > 0)
                {
                    return InsertOrUpdateResult<Titles>.Updated(entity);
                }
        ...
        {
            return InsertOrUpdateResult<Titles>.Failure($"Failed When Update Title: {ex.Message}");
        }
    }
    public async Task<bool> IsTitleDeletedOrInactiveAsync(string id)
    {
        if (string.IsNullOrEmpty(id)) return true;

        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = "SELECT 1 FROM titles WHERE id = @id AND (is_deleted = TRUE OR is_active = FALSE) LIMIT 1;";
        ...
        {
            Debug.LogError($"Error checking title status: {ex.Message}");
            return true;
        }
    }
    private void AddParameters(MySqlCommand command, Titles entity)
    {
