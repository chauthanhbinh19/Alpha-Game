...
public class CardLivesRepository : ICardLivesRepository
{
    ...
    public async Task<bool> IsCardLifeDeletedOrInactiveAsync(string id)
    {
        if (string.IsNullOrEmpty(id)) return true;

        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = "SELECT 1 FROM card_lives WHERE id = @id AND (is_deleted = TRUE OR is_active = FALSE) LIMIT 1;";
        ...
        {
            Debug.LogError($"Error checking card life status: {ex.Message}");
            return true;
        }
    }
