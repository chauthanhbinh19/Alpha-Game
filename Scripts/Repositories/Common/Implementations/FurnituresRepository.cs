...
public class FurnituresRepository : IFurnituresRepository
{
    ...
    public async Task<InsertOrUpdateResult<Furnitures>> InsertFurnitureAsync(Furnitures entity)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = @"
        ...
        try
        {
            ...
            using (var cmd = new MySqlCommand(sql, conn))
            {
                ...
                if (rowsAffected > 0)
                {
                    return InsertOrUpdateResult<Furnitures>.Inserted(entity);
                }
        ...
        {
            return InsertOrUpdateResult<Furnitures>.Failure($"Failed When Insert Furniture: {ex.Message}");
        }
    }
    public async Task<InsertOrUpdateResult<Furnitures>> UpdateFurnitureAsync(Furnitures entity)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = @"
        UPDATE furnitures SET
            name = @name,
        ...
        try
        {
            ...
            using (var cmd = new MySqlCommand(sql, conn))
            {
                ...
                if (rowsAffected > 0)
                {
                    return InsertOrUpdateResult<Furnitures>.Updated(entity);
                }
        ...
        {
            return InsertOrUpdateResult<Furnitures>.Failure($"Failed When Update Furniture: {ex.Message}");
        }
    }
    public async Task<bool> IsFurnitureDeletedOrInactiveAsync(string id)
    {
        if (string.IsNullOrEmpty(id)) return true;

        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = "SELECT 1 FROM furnitures WHERE id = @id AND (is_deleted = TRUE OR is_active = FALSE) LIMIT 1;";
        ...
        {
            Debug.LogError($"Error checking furniture status: {ex.Message}");
            return true;
        }
    }
    private void AddParameters(MySqlCommand command, Furnitures entity)
    {
    ...
    public async Task<Furnitures> SumPowerFurnituresPercentAsync(string userId)
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                FROM Furnitures a
                JOIN user_Furnitures ua ON a.id = ua.Furniture_id
