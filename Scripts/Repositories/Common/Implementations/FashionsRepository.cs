    ...
    public async Task<List<Fashions>> GetFashionsAsync(string search, string type, string rare, int pageSize, int offset)
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                selectSQL += " ORDER BY Fashions.name REGEXP '[0-9]+$', CAST(REGEXP_SUBSTR(Fashions.name, '[0-9]+$') AS UNSIGNED), Fashions.name";
    ...
    public async Task<List<Fashions>> GetFashionsWithoutLimitAsync()
    {
        ...
        await using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                string selectSQL = @"SELECT id, name, image, rare, quality FROM fashions";
