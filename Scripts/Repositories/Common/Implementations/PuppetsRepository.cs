...
public class PuppetsRepository : IPuppetsRepository
{
    ...
    public async Task<InsertOrUpdateResult<Puppets>> InsertPuppetAsync(Puppets entity)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = @"
        INSERT INTO puppets (
        ...
        try
        {
            ...
            using (var cmd = new MySqlCommand(sql, conn))
            {
                ...
                if (rowsAffected > 0)
                {
                    return InsertOrUpdateResult<Puppets>.Inserted(entity);
                }
        ...
        {
            return InsertOrUpdateResult<Puppets>.Failure($"Failed When Insert Puppet: {ex.Message}");
        }
    }
    public async Task<InsertOrUpdateResult<Puppets>> UpdatePuppetAsync(Puppets entity)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = @"
        UPDATE puppets SET
        ...
        try
        {
            ...
            using (var cmd = new MySqlCommand(sql, conn))
            {
                ...
                if (rowsAffected > 0)
                {
                    return InsertOrUpdateResult<Puppets>.Updated(entity);
                }
        ...
        {
            return InsertOrUpdateResult<Puppets>.Failure($"Failed When Update Puppet: {ex.Message}");
        }
    }
    public async Task<bool> IsPuppetDeletedOrInactiveAsync(string id)
    {
        if (string.IsNullOrEmpty(id)) return true;

        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = "SELECT 1 FROM puppets WHERE id = @id AND (is_deleted = TRUE OR is_active = FALSE) LIMIT 1;";
        ...
        {
            Debug.LogError($"Error checking puppet status: {ex.Message}");
            return true;
        }
    }
    private void AddParameters(MySqlCommand command, Puppets entity)
    {
