...
public class SkillsRepository : ISkillsRepository
{
    ...
    public async Task<bool> IsSkillDeletedOrInactiveAsync(string id)
    {
        if (string.IsNullOrEmpty(id)) return true;

        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = "SELECT 1 FROM skills WHERE id = @id AND (is_deleted = TRUE OR is_active = FALSE) LIMIT 1;";
        ...
        {
            Debug.LogError($"Error checking skill status: {ex.Message}");
            return true;
        }
    }
