...
public class CardGeneralsRepository : ICardGeneralsRepository
{
    public async Task<List<string>> GetUniqueCardGeneralsTypesAsync()
    {
        ...
        try
        {
            ...
            string selectSQL = "SELECT DISTINCT type FROM card_generals";
    ...
    public async Task<List<string>> GetUniqueCardGeneralsIdAsync()
    {
        ...
        try
        {
            ...
            string selectSQL = "SELECT DISTINCT id FROM card_generals";
            await using var selectCommand = new MySqlCommand(selectSQL, connection);
        ...
        try
        {
            ...
            SELECT 
                ...
                (
                    ...
                    WHERE che.card_general_id = ch.id
                ) AS emblems_json,
                (
                    SELECT JSON_ARRAYAGG(
                        JSON_OBJECT(
                    ...
                    WHERE chc.card_general_id = ch.id
                ) AS classes_json
            ...
            while (await reader.ReadAsync())
            {
                var cardGeneral = new CardGenerals
                {
    ...
    public async Task<int> GetCardGeneralsCountAsync(string search, string type, string rare)
    {
        ...
        try
        {
            ...
            string selectSQL = @"SELECT COUNT(*) FROM card_generals WHERE 1=1";
    ...
    public async Task<InsertOrUpdateResult<CardGenerals>> InsertCardGeneralAsync(CardGenerals entity)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = @"
        INSERT INTO card_generals (
        ...
        try
        {
            ...
            using (var cmd = new MySqlCommand(sql, conn))
            {
                ...
                if (rowsAffected > 0)
                {
                    return InsertOrUpdateResult<CardGenerals>.Inserted(entity);
                }
        ...
        {
            return InsertOrUpdateResult<CardGenerals>.Failure($"Failed When Insert Card General: {ex.Message}");
        }
    }
    public async Task<InsertOrUpdateResult<CardGenerals>> UpdateCardGeneralAsync(CardGenerals entity)
    {
        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = @"
        UPDATE card_generals SET
            name = @name,
        ...
        try
        {
            ...
            using (var cmd = new MySqlCommand(sql, conn))
            {
                ...
                if (rowsAffected > 0)
                {
                    return InsertOrUpdateResult<CardGenerals>.Updated(entity);
                }
        ...
        {
            return InsertOrUpdateResult<CardGenerals>.Failure($"Failed When Update Card General: {ex.Message}");
        }
    }
    public async Task<bool> IsCardGeneralDeletedOrInactiveAsync(string id)
    {
        if (string.IsNullOrEmpty(id)) return true;

        string connectionString = DatabaseConfig.ConnectionString;
        const string sql = "SELECT 1 FROM card_generals WHERE id = @id AND (is_deleted = TRUE OR is_active = FALSE) LIMIT 1;";
        ...
        {
            Debug.LogError($"Error checking card general status: {ex.Message}");
            return true;
        }
    }
    private void AddParameters(MySqlCommand command, CardGenerals entity)
    {
    ...
    public async Task<List<CardGenerals>> GetCardGeneralsRandomAsync(string type, int pageSize)
    {
        ...
        try
        {
            ...
            string selectSQL = "SELECT * FROM card_generals WHERE type = @type ORDER BY RAND() LIMIT @limit";
            await using var selectCommand = new MySqlCommand(selectSQL, connection);
            selectCommand.Parameters.AddWithValue("@type", type);
            selectCommand.Parameters.AddWithValue("@limit", pageSize);
    ...
    public async Task<List<CardGenerals>> GetAllCardGeneralsAsync(string type)
    {
        ...
        try
        {
            ...
            string selectSQL = "SELECT * FROM card_generals WHERE type = @type";
            await using var selectCommand = new MySqlCommand(selectSQL, connection);
            selectCommand.Parameters.AddWithValue("@type", type);
    ...
    public async Task<CardGenerals> GetCardGeneralByIdAsync(string Id)
    {
        ...
        try
        {
            ...
            string selectSQL = "SELECT * FROM card_generals WHERE id = @id";
            await using var selectCommand = new MySqlCommand(selectSQL, connection);
