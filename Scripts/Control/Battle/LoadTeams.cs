...
public class LoadTeams : MonoBehaviour
{
    ...
    public void LoadPlayerTeamCard(int team_id)
    {
        ...
        for (int i = 1; i <= 10; i++)
        {
            ...
            if (type == "cardHeroes")
            {
                ...
                cardHeroesList.AddRange(c.GetUserCardHeroesTeam(1, team_id, position));
            }
            else if (type == "cardCaptains")
            {
                CardCaptains c = new CardCaptains();
                cardCaptainsList.AddRange(c.GetUserCardCaptainsTeam(User.CurrentUserId, team_id, position));
            }
            else if (type == "CardColonels")
            {
                CardColonels c = new CardColonels();
                cardColonelsList.AddRange(c.GetUserCardColonelsTeam(User.CurrentUserId, team_id, position));
            }
            else if (type == "CardGenerals")
            {
                CardGenerals c = new CardGenerals();
                cardGeneralsList.AddRange(c.GetUserCardGeneralsTeam(User.CurrentUserId, team_id, position));
            }
            else if (type == "CardAdmirals")
            {
                CardAdmirals c = new CardAdmirals();
                cardAdmiralsList.AddRange(c.GetUserCardAdmiralsTeam(User.CurrentUserId, team_id, position));
            }
            else if (type == "CardMonsters")
            {
                CardMonsters c = new CardMonsters();
                cardMonstersList.AddRange(c.GetUserCardMonstersTeam(User.CurrentUserId, team_id, position));
            }
            else if (type == "CardMilitary")
            {
                CardMilitary c = new CardMilitary();
                cardMilitaryList.AddRange(c.GetUserCardMilitaryTeam(User.CurrentUserId, team_id, position));
            }
            else if (type == "CardSpell")
            {
                CardSpell c = new CardSpell();
                cardSpellList.AddRange(c.GetUserCardSpellTeam(User.CurrentUserId, team_id, position));
            }
        }

        LoadPlayerCardHeroes(cardHeroesList);
    }
    public void LoadEnemyTeamCard(int team_id)
    {
        List<CardHeroes> cardHeroesList = new List<CardHeroes>();
        List<CardCaptains> cardCaptainsList = new List<CardCaptains>();
        List<CardColonels> cardColonelsList = new List<CardColonels>();
        ...
        for (int i = 1; i <= 10; i++)
        {
            ...
            {
                CardHeroes c = new CardHeroes();
                cardHeroesList.AddRange(c.GetUserCardHeroesTeam(2, team_id, position));
            }
            else if (type == "cardCaptains")
            {
                CardCaptains c = new CardCaptains();
    ...
    public void LoadPlayerCardHeroes(List<CardHeroes> cardHeroesList)
    {
        for (int i = 1; i <= 10; i++)
        {
            ...
            {
                battleScript.cardName = card.name;
                battleScript.health = card.all_health;
                battleScript.attackPower = card.all_physical_attack;
