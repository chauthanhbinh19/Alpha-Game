...
public static class NumberFormatterHelper
{
    ...
    public static string FormatNumberExtended(double number, bool shorten = true)
    {
        if (!shorten)
            return Math.Round(number).ToString("F0");
        ...
        // nếu chưa có suffix thì scale lần đầu
        if (suffixIndex == 0)
        {
            shortenedNumber /= 1000.0;
            suffixIndex++;
        ...
        return shortenedNumber % 1 == 0
            ? $"{shortenedNumber:0}{suffixes[suffixIndex]}"
            : $"{shortenedNumber:0.##}{suffixes[suffixIndex]}";
    }
}
