...
public class PowerManagerService : IPowerManagerService
{
    ...
    public async Task<PowerManager> GetCardsPowerAsync()
    {
        ...
        // Titles titles = new Titles();
        IArtifactsGalleryRepository artifactsGalleryRepository = new ArtifactsGalleryRepository();
        ArtifactsGalleryService artifactsGalleryService = new ArtifactsGalleryService(artifactsGalleryRepository);
        // Gallery
        Artifacts artifact = await artifactsGalleryService.SumPowerArtifactsGalleryAsync();
        ...
        IUserArtifactsRepository userArtifactsRepository = new UserArtifactsRepository();
        UserArtifactsService userArtifactsService = new UserArtifactsService(userArtifactsRepository);
        // User Artifacts (Gallery)
        artifact = await userArtifactsService.SumPowerUserArtifactsAsync(); // Giả định SumPowerUserArtifacts cũng trả về một đối tượng Artifacts mới hoặc đã được reset
        powerManager.Power += artifact.Power;
        powerManager.Health += artifact.Health;
        powerManager.PhysicalAttack += artifact.PhysicalAttack;
        powerManager.PhysicalDefense += artifact.PhysicalDefense;
        ...
        // Percent
        artifact = await artifactsService.SumPowerArtifactsPercentAsync(); // Giả định SumPowerTitlesPercent cũng trả về một đối tượng Titles mới hoặc đã được reset
        powerManager.PercentAllHealth += artifact.PercentAllHealth;
        powerManager.PercentAllPhysicalAttack += artifact.PercentAllPhysicalAttack;
        powerManager.PercentAllPhysicalDefense += artifact.PercentAllPhysicalDefense;
        powerManager.PercentAllMagicalAttack += artifact.PercentAllMagicalAttack;
