...
public class HICAXManager : MonoBehaviour
{
    public static HICAXManager Instance { get; private set; }
    ...
    public async Task CreateHICAXManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetHICAFeaturesByTypeAsync(AppConstants.HICA.HICA_X);
    ...
    public async Task CreateMainHICAPanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.HICA.HICA_X}");
        rankImage.texture = rankTexture;
        RawImage background = transform.Find("Background").GetComponent<RawImage>();
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.HICA.HICA_X_BACKGROUND_URL);
