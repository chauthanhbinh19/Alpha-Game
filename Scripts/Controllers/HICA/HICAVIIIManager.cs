...
public class HICAVIIIManager : MonoBehaviour
{
    public static HICAVIIIManager Instance { get; private set; }
    ...
    public async Task CreateHICAVIIIManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetHICAFeaturesByTypeAsync(AppConstants.HICA.HICA_VIII);
    ...
    public async Task CreateMainHICAPanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.HICA.HICA_VIII}");
        rankImage.texture = rankTexture;
        RawImage background = transform.Find("Background").GetComponent<RawImage>();
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.HICA.HICA_VIII_BACKGROUND_URL);
