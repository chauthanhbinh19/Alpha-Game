...
public class HICAIIIManager : MonoBehaviour
{
    ...
    public async Task CreateMainHICAPanelAsync(string featureId, string featureName)
    {
        ...
        upgradeOneLevelButton.onClick.AddListener(async () =>
        {
            ...
            UpgradeResultDTO result = await UpgradeService.Create().UpgradeOneLevelAsync(featureName, currentLevel, hica.MaxLevel, User.CurrentUserId);
            if (result.Success)
            {
        ...
        {
            AudioManager.Instance.PlaySFX(AudioConstants.SFX.SWITCH_CLICK_SOUND);
            UpgradeResultDTO result = await UpgradeService.Create().UpgradeMaxLevelAsync(featureName, currentLevel, hica.MaxLevel, User.CurrentUserId);
            if (result.Success)
            {
                userHICA = EnhanceHelper.EnhanceHICAs(userHICA, result.UpgradedLevels, hica.BaseMultiplier);
                await UserHICAsService.Create().InsertOrUpdateUserHICAsAsync(User.CurrentUserId, userHICA, featureId);
