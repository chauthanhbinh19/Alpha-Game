...
public class HICAIIIManager : MonoBehaviour
{
    public static HICAIIIManager Instance { get; private set; }
    private Transform MainPanel;
    private GameObject HICAPanelPrefab;
    private GameObject HICAButtonPrefab;
    private GameObject PopupHICAPanelPrefab;
    private GameObject PopupHICAQuantityPanelPrefab;
    private GameObject PopupHICAButtonPrefab;
    private GameObject MainHICAPanelPrefab;
    private GameObject HICAItemPrefab;
    private Transform Content;
    private const int ITEMS_PER_PAGE = 50;
    private int CurrentPage = 0;
    private List<KeyValuePair<string, FeatureHICADTO>> FeatureList;
    ...
    public async Task CreateHICAIIIManagerAsync()
    {
        GameObject currentObject = Instantiate(PopupHICAPanelPrefab, MainPanel);
        ...
        uniqueTypes = await FeaturesService.Create().GetHICAFeaturesByTypeAsync(AppConstants.HICA.HICA_III);
    ...
    private void RenderPage()
    {
        ...
        for (int i = start; i < end; i++)
        {
            ...
            GameObject button = Instantiate(PopupHICAButtonPrefab, Content);
            ...
            if (btn != null)
            {
                ...
                btn.onClick.AddListener(async () =>
                {
                    ...
                    AudioManager.Instance.PlaySFX(AudioConstants.SFX.BUTTON_CLICK_SOUND);
                    await CreateMainHICAPanelAsync(currentFeatureId, currentSubtype);
                });
            }
        }
    }
    ...
    public async Task CreateMainHICAPanelAsync(string featureId, string featureName)
    {
        GameObject currentObject = Instantiate(MainHICAPanelPrefab, MainPanel);
        Transform transform = currentObject.transform;
        ...
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.HICA.HICA_III}");
        rankImage.texture = rankTexture;
        RawImage background = transform.Find("Background").GetComponent<RawImage>();
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.HICA.HICA_III_BACKGROUND_URL);
        ...
        List<RecipeItemDto> recipeItems = await RecipeService.Create().GetRecipeItemsAsync(featureName, userHICA.Level, User.CurrentUserId);
        ...
        for (int i = 0; i < total; i++)
        {
            ...
            GameObject itemGO = Instantiate(HICAItemPrefab, parent);

            SetupHICAItemUI(itemGO, recipeItems[i]);
        }

        int currentLevel = userHICA?.Level ?? 0;
        ...
        async Task RefreshPanelAsync()
        {
            ...
            List<RecipeItemDto> refreshedRecipeItems = await RecipeService.Create().GetRecipeItemsAsync(featureName, userHICA.Level, User.CurrentUserId);
            ...
            for (int i = 0; i < refreshedTotal; i++)
            {
                ...
                GameObject itemGO = Instantiate(HICAItemPrefab, parent);
                SetupHICAItemUI(itemGO, refreshedRecipeItems[i]);
            }
        }
        ...
        void CreatePopupUpgradePanelAsync()
        {
            GameObject gameObject =
                Instantiate(PopupHICAQuantityPanelPrefab, MainPanel);
            ...
            int maxLevel = hica != null ? hica.MaxLevel : popupCurrentLevel;
            ...
            if (userHICA != null)
            {
                StatsManager.Instance.CreateStatsManager(userHICA, currentStatsContent);
                StatsManager.Instance.CreateStatsManager(userHICA, nextStatsContent);
            }
            ...
            async Task UpdatePreviewAsync()
            {
                ...
                {
                    UserHICAs previewHICA = userHICA.CloneUserHICA(userHICA);
                    EnhanceHelper.EnhanceHICAs(previewHICA, preview.UpgradedLevels, hica.BaseMultiplier);
                    StatsManager.Instance.CreateStatsManager(previewHICA, nextStatsContent);
                }
                else if (userHICA != null)
                {
                    StatsManager.Instance.CreateStatsManager(userHICA, nextStatsContent);
                }
            ...
            confirmButton.onClick.AddListener(async () =>
            {
                ...
                if (result.Success)
                {
                    userHICA = EnhanceHelper.EnhanceHICAs(userHICA, result.UpgradedLevels, hica.BaseMultiplier);
                    await UserHICAsService.Create().InsertOrUpdateUserHICAsAsync(User.CurrentUserId, userHICA, featureId);
    ...
    private void SetupHICAItemUI(GameObject itemGO,RecipeItemDto data)
    {
