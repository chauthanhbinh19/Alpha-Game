...
public class HICAVIManager : MonoBehaviour
{
    public static HICAVIManager Instance { get; private set; }
    ...
    public async Task CreateHICAVIManagerAsync()
    {
    ...
    public async Task CreateMainHICAPanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.HICA.HICA_VI}");
        rankImage.texture = rankTexture;
        RawImage background = transform.Find("Background").GetComponent<RawImage>();
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.HICA.HICA_VI_BACKGROUND_URL);
