...
public class HICAIManager : MonoBehaviour
{
    public static HICAIManager Instance { get; private set; }
    ...
    public async Task CreateHICAIManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetHICAFeaturesByTypeAsync(AppConstants.HICA.HICA_I);
    ...
    public async Task CreateMainHICAPanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.HICA.HICA_I}");
        rankImage.texture = rankTexture;
        RawImage background = transform.Find("Background").GetComponent<RawImage>();
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.HICA.HICA_I_BACKGROUND_URL);
