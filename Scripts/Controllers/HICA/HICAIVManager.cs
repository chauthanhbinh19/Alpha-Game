...
public class HICAIVManager : MonoBehaviour
{
    public static HICAIVManager Instance { get; private set; }
    ...
    public async Task CreateHICAIVManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetHICAFeaturesByTypeAsync(AppConstants.HICA.HICA_IV);
    ...
    public async Task CreateMainHICAPanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.HICA.HICA_IV}");
        rankImage.texture = rankTexture;
        RawImage background = transform.Find("Background").GetComponent<RawImage>();
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.HICA.HICA_IV_BACKGROUND_URL);
