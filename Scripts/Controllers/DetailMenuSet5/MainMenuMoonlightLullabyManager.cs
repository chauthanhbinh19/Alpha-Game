...
{
    public static MainMenuMoonlightLullabyManager Instance { get; private set; }
    ...
    public async Task CreateMainMenuMoonlightLullabyManagerAsync(IStats stat)
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetRankFeaturesByTypeAsync(AppConstants.MainMenuSet5.MOONLIGHT_LULLABY, stat);
    ...
    public async Task CreateMainRankPanelAsync(string featureId, string featureName)
    {
        ...
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.MainMenuSet5.MOONLIGHT_LULLABY);
