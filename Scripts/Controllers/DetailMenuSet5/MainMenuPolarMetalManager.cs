...
{
    public static MainMenuPolarMetalManager Instance { get; private set; }
    ...
    public async Task CreateMainMenuPolarMetalManagerAsync(IStats stat)
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetRankFeaturesByTypeAsync(AppConstants.MainMenuSet5.POLAR_METAL, stat);
    ...
    public async Task CreateMainRankPanelAsync(string featureId, string featureName)
    {
        ...
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.MainMenuSet5.POLAR_METAL);
