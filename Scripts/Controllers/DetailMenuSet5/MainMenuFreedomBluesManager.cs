...
{
    public static MainMenuFreedomBluesManager Instance { get; private set; }
    ...
    public async Task CreateMainMenuFreedomBluesManagerAsync(IStats stat)
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetRankFeaturesByTypeAsync(AppConstants.MainMenuSet5.FREEDOM_BLUES, stat);
    ...
    public async Task CreateMainRankPanelAsync(string featureId, string featureName)
    {
        ...
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.MainMenuSet5.FREEDOM_BLUES);
