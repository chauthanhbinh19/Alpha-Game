...
{
    public static MainMenuThunderMetalManager Instance { get; private set; }
    ...
    public async Task CreateMainMenuThunderMetalManagerAsync(IStats stat)
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetRankFeaturesByTypeAsync(AppConstants.MainMenuSet5.THUNDER_METAL, stat);
    ...
    public async Task CreateMainRankPanelAsync(string featureId, string featureName)
    {
        ...
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.MainMenuSet5.THUNDER_METAL);
