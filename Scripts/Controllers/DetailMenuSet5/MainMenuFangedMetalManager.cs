...
{
    public static MainMenuFangedMetalManager Instance { get; private set; }
    ...
    public async Task CreateMainMenuFangedMetalManagerAsync(IStats stat)
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetRankFeaturesByTypeAsync(AppConstants.MainMenuSet5.FANGED_METAL, stat);
    ...
    public async Task CreateMainRankPanelAsync(string featureId, string featureName)
    {
        ...
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.MainMenuSet5.FANGED_METAL);
