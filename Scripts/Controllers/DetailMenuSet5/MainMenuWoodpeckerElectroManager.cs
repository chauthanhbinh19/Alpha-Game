...
{
    public static MainMenuWoodpeckerElectroManager Instance { get; private set; }
    ...
    public async Task CreateMainMenuWoodpeckerElectroManagerAsync(IStats stat)
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetRankFeaturesByTypeAsync(AppConstants.MainMenuSet5.WOODPECKER_ELECTRO, stat);
    ...
    public async Task CreateMainRankPanelAsync(string featureId, string featureName)
    {
        ...
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.MainMenuSet5.WOODPECKER_ELECTRO);
