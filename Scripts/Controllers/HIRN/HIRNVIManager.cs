...
public class HIRNVIManager : MonoBehaviour
{
    public static HIRNVIManager Instance { get; private set; }
    ...
    public async Task CreateHIRNVIManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetHIRNFeaturesByTypeAsync(AppConstants.HIRN.HIRN_VI);
    ...
    public async Task CreateMainHIRNPanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.HIRN.HIRN_VI}");
