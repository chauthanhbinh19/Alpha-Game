...
public class HIRNIVManager : MonoBehaviour
{
    public static HIRNIVManager Instance { get; private set; }
    ...
    public async Task CreateHIRNIVManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetHIRNFeaturesByTypeAsync(AppConstants.HIRN.HIRN_IV);
    ...
    public async Task CreateMainHIRNPanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.HIRN.HIRN_IV}");
        rankImage.texture = rankTexture;
        RawImage background = transform.Find("Background").GetComponent<RawImage>();
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.HIRN.HIRN_IV_BACKGROUND_URL);
