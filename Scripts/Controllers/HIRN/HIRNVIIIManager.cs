...
public class HIRNVIIIManager : MonoBehaviour
{
    public static HIRNVIIIManager Instance { get; private set; }
    ...
    public async Task CreateMainHIRNPanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.HIRN.HIRN_VIII}");
        rankImage.texture = rankTexture;
        RawImage background = transform.Find("Background").GetComponent<RawImage>();
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.HIRN.HIRN_VIII_BACKGROUND_URL);
