...
public class HIRNVIIManager : MonoBehaviour
{
    public static HIRNVIIManager Instance { get; private set; }
    private Transform MainPanel;
    private GameObject HIRNPanelPrefab;
    private GameObject HIRNButtonPrefab;
    private GameObject PopupHIRNPanelPrefab;
    private GameObject PopupHIRNQuantityPanelPrefab;
    ...
    private List<KeyValuePair<string, FeatureHIRNDTO>> FeatureList;
    ...
    public async Task CreateHIRNVIIManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetHIRNFeaturesByTypeAsync(AppConstants.HIRN.HIRN_VII);
        ...
        for (int i = start; i < end; i++)
        {
            ...
            if (btn != null)
            {
                ...
                btn.onClick.AddListener(async () =>
                {
                    ...
                    AudioManager.Instance.PlaySFX(AudioConstants.SFX.BUTTON_CLICK_SOUND);
                    await CreateMainHIRNPanelAsync(currentFeatureId, currentSubtype);
                });
            }
        }
    }
    ...
    public async Task CreateMainHIRNPanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.HIRN.HIRN_VII}");
        rankImage.texture = rankTexture;
        RawImage background = transform.Find("Background").GetComponent<RawImage>();
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.HIRN.HIRN_VII_BACKGROUND_URL);
        ...
        List<RecipeItemDto> recipeItems = await RecipeService.Create().GetRecipeItemsAsync(featureName, userHIRN.Level, User.CurrentUserId);
        ...
        async Task RefreshPanelAsync()
        {
            ...
            List<RecipeItemDto> refreshedRecipeItems = await RecipeService.Create().GetRecipeItemsAsync(featureName, userHIRN.Level, User.CurrentUserId);
        ...
        void CreatePopupUpgradePanelAsync()
        {
            GameObject gameObject =
                Instantiate(PopupHIRNQuantityPanelPrefab, MainPanel);
            ...
            int maxLevel = hirn != null ? hirn.MaxLevel : popupCurrentLevel;
            ...
            if (userHIRN != null)
            {
                StatsManager.Instance.CreateStatsManager(userHIRN, currentStatsContent);
                StatsManager.Instance.CreateStatsManager(userHIRN, nextStatsContent);
            }
            ...
            async Task UpdatePreviewAsync()
            {
                ...
                {
                    UserHIRNs previewHIRN = userHIRN.CloneUserHIRN(userHIRN);
                    EnhanceHelper.EnhanceHIRNs(previewHIRN, preview.UpgradedLevels, hirn.BaseMultiplier);
                    StatsManager.Instance.CreateStatsManager(previewHIRN, nextStatsContent);
                }
                else if (userHIRN != null)
                {
                    StatsManager.Instance.CreateStatsManager(userHIRN, nextStatsContent);
                }
