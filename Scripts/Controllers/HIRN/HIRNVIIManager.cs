...
public class HIRNVIIManager : MonoBehaviour
{
    ...
    public async Task CreateMainHIRNPanelAsync(string featureId, string featureName)
    {
        ...
        upgradeOneLevelButton.onClick.AddListener(async () =>
        {
            ...
            UpgradeResultDTO result = await UpgradeService.Create().UpgradeOneLevelAsync(featureName, currentLevel, hirn.MaxLevel, User.CurrentUserId);
            if (result.Success)
            {
        ...
        {
            AudioManager.Instance.PlaySFX(AudioConstants.SFX.SWITCH_CLICK_SOUND);
            UpgradeResultDTO result = await UpgradeService.Create().UpgradeMaxLevelAsync(featureName, currentLevel, hirn.MaxLevel, User.CurrentUserId);
            if (result.Success)
            {
                userHIRN = EnhanceHelper.EnhanceHIRNs(userHIRN, result.UpgradedLevels, hirn.BaseMultiplier);
                await UserHIRNsService.Create().InsertOrUpdateUserHIRNsAsync(User.CurrentUserId, userHIRN, featureId);
