...
public class HIRNXIVManager : MonoBehaviour
{
    public static HIRNXIVManager Instance { get; private set; }
    ...
    public async Task CreateHIRNXIVManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetHIRNFeaturesByTypeAsync(AppConstants.HIRN.HIRN_XIV);
    ...
    public async Task CreateMainHIRNPanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = Resources.Load<Texture2D>($"UI/Rank_Research/{AppConstants.HIRN.HIRN_XIV}");
