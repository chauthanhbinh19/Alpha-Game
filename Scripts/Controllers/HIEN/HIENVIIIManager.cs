...
public class HIENVIIIManager : MonoBehaviour
{
    ...
    public async Task CreateMainHIENPanelAsync(string featureId, string featureName)
    {
        ...
        upgradeOneLevelButton.onClick.AddListener(async () =>
        {
            ...
            UpgradeResultDTO result = await UpgradeService.Create().UpgradeOneLevelAsync(featureName, currentLevel, hien.MaxLevel, User.CurrentUserId);
            if (result.Success)
            {
        ...
        {
            AudioManager.Instance.PlaySFX(AudioConstants.SFX.SWITCH_CLICK_SOUND);
            UpgradeResultDTO result = await UpgradeService.Create().UpgradeMaxLevelAsync(featureName, currentLevel, hien.MaxLevel, User.CurrentUserId);
            if (result.Success)
            {
                userHIEN = EnhanceHelper.EnhanceHIENs(userHIEN, result.UpgradedLevels, hien.BaseMultiplier);
                await UserHIENsService.Create().InsertOrUpdateUserHIENsAsync(User.CurrentUserId, userHIEN, featureId);
