...
public class HIENVIIIManager : MonoBehaviour
{
    public static HIENVIIIManager Instance { get; private set; }
    private Transform MainPanel;
    private GameObject HIENPanelPrefab;
    private GameObject HIENButtonPrefab;
    private GameObject PopupHIENPanelPrefab;
    private GameObject PopupHIENQuantityPanelPrefab;
    ...
    private List<KeyValuePair<string, FeatureHIENDTO>> FeatureList;
    ...
    public void Initialize()
    {
        ...
        PopupHIENQuantityPanelPrefab = UIManager.Instance.Get("PopupHIENQuantityPanelPrefab");
    ...
    public async Task CreateHIENVIIIManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetHIENFeaturesByTypeAsync(AppConstants.HIEN.HIEN_VIII);
        ...
        for (int i = start; i < end; i++)
        {
            ...
            if (btn != null)
            {
                ...
                btn.onClick.AddListener(async () =>
                {
                    ...
                    AudioManager.Instance.PlaySFX(AudioConstants.SFX.BUTTON_CLICK_SOUND);
                    await CreateMainHIENPanelAsync(currentFeatureId, currentSubtype);
                });
            }
        }
    }
    ...
    public async Task CreateMainHIENPanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.HIEN.HIEN_VIII}");
        rankImage.texture = rankTexture;
        RawImage background = transform.Find("Background").GetComponent<RawImage>();
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.HIEN.HIEN_VIII_BACKGROUND_URL);
        ...
        void CreatePopupUpgradePanelAsync()
        {
            GameObject gameObject =
                Instantiate(PopupHIENQuantityPanelPrefab, MainPanel);
            ...
            confirmButton.onClick.AddListener(async () =>
            {
                ...
                if (result.Success)
                {
                    userHIEN = EnhanceHelper.EnhanceHIENs(userHIEN, result.UpgradedLevels, hien.BaseMultiplier);
