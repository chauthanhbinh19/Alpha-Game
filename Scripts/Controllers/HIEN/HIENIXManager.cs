...
public class HIENIXManager : MonoBehaviour
{
    public static HIENIXManager Instance { get; private set; }
    ...
    public async Task CreateHIENIXManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetHIENFeaturesByTypeAsync(AppConstants.HIEN.HIEN_IX);
    ...
    public async Task CreateMainHIENPanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.HIEN.HIEN_IX}");
        rankImage.texture = rankTexture;
        RawImage background = transform.Find("Background").GetComponent<RawImage>();
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.HIEN.HIEN_IX_BACKGROUND_URL);
