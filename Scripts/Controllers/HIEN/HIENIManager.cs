...
public class HIENIManager : MonoBehaviour
{
    public static HIENIManager Instance { get; private set; }
    ...
    public async Task CreateHIENIManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetHIENFeaturesByTypeAsync(AppConstants.HIEN.HIEN_I);
    ...
    public async Task CreateMainHIENPanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.HIEN.HIEN_I}");
        rankImage.texture = rankTexture;
        RawImage background = transform.Find("Background").GetComponent<RawImage>();
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.HIEN.HIEN_I_BACKGROUND_URL);
