...
public class HIHNIManager : MonoBehaviour
{
    public static HIHNIManager Instance { get; private set; }
    ...
    public async Task CreateHIHNIManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetHIHNFeaturesByTypeAsync(AppConstants.HIHN.HIHN_I);
    ...
    public async Task CreateMainHIHNPanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.HIHN.HIHN_I}");
        rankImage.texture = rankTexture;
        RawImage background = transform.Find("Background").GetComponent<RawImage>();
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.HIHN.HIHN_I_BACKGROUND_URL);
