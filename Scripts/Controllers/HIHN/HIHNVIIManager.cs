...
public class HIHNVIIManager : MonoBehaviour
{
    public static HIHNVIIManager Instance { get; private set; }
    ...
    private List<KeyValuePair<string, FeatureHIHNDTO>> FeatureList;
    ...
    public async Task CreateHIHNVIIManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetHIHNFeaturesByTypeAsync(AppConstants.HIHN.HIHN_VII);
        ...
        for (int i = start; i < end; i++)
        {
            ...
            if (btn != null)
            {
                ...
                btn.onClick.AddListener(async () =>
                {
                    ...
                    AudioManager.Instance.PlaySFX(AudioConstants.SFX.BUTTON_CLICK_SOUND);
                    await CreateMainHIHNPanelAsync(currentFeatureId, currentSubtype);
                });
            }
        }
    }
    ...
    public async Task CreateMainHIHNPanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.HIHN.HIHN_VII}");
        rankImage.texture = rankTexture;
        RawImage background = transform.Find("Background").GetComponent<RawImage>();
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.HIHN.HIHN_VII_BACKGROUND_URL);
        ...
        List<RecipeItemDto> recipeItems = await RecipeService.Create().GetRecipeItemsAsync(featureName, userHIHN.Level, User.CurrentUserId);
        ...
        async Task RefreshPanelAsync()
        {
            ...
            List<RecipeItemDto> refreshedRecipeItems = await RecipeService.Create().GetRecipeItemsAsync(featureName, userHIHN.Level, User.CurrentUserId);
        ...
        void CreatePopupUpgradePanelAsync()
        {
            ...
            if (userHIHN != null)
            {
                StatsManager.Instance.CreateStatsManager(userHIHN, currentStatsContent);
                StatsManager.Instance.CreateStatsManager(userHIHN, nextStatsContent);
            }
            ...
            async Task UpdatePreviewAsync()
            {
                ...
                {
                    UserHIHNs previewHIHN = userHIHN.CloneUserHIHN(userHIHN);
                    EnhanceHelper.EnhanceHIHNs(previewHIHN, preview.UpgradedLevels, hihn.BaseMultiplier);
                    StatsManager.Instance.CreateStatsManager(previewHIHN, nextStatsContent);
                }
                else if (userHIHN != null)
                {
                    StatsManager.Instance.CreateStatsManager(userHIHN, nextStatsContent);
                }
