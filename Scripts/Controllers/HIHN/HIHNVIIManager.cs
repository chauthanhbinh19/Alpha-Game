...
public class HIHNVIIManager : MonoBehaviour
{
    ...
    public async Task CreateMainHIHNPanelAsync(string featureId, string featureName)
    {
        ...
        upgradeOneLevelButton.onClick.AddListener(async () =>
        {
            ...
            UpgradeResultDTO result = await UpgradeService.Create().UpgradeOneLevelAsync(featureName, currentLevel, hihn.MaxLevel, User.CurrentUserId);
            if (result.Success)
            {
        ...
        {
            AudioManager.Instance.PlaySFX(AudioConstants.SFX.SWITCH_CLICK_SOUND);
            UpgradeResultDTO result = await UpgradeService.Create().UpgradeMaxLevelAsync(featureName, currentLevel, hihn.MaxLevel, User.CurrentUserId);
            if (result.Success)
            {
                userHIHN = EnhanceHelper.EnhanceHIHNs(userHIHN, result.UpgradedLevels, hihn.BaseMultiplier);
                await UserHIHNsService.Create().InsertOrUpdateUserHIHNsAsync(User.CurrentUserId, userHIHN, featureId);
