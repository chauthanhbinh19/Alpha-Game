...
public class HIHNXManager : MonoBehaviour
{
    public static HIHNXManager Instance { get; private set; }
    ...
    public async Task CreateHIHNXManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetHIHNFeaturesByTypeAsync(AppConstants.HIHN.HIHN_X);
    ...
    public async Task CreateMainHIHNPanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.HIHN.HIHN_X}");
        rankImage.texture = rankTexture;
        RawImage background = transform.Find("Background").GetComponent<RawImage>();
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.HIHN.HIHN_X_BACKGROUND_URL);
