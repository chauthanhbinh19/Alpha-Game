...
public class HIHNXIIIManager : MonoBehaviour
{
    public static HIHNXIIIManager Instance { get; private set; }
    ...
    public async Task CreateHIHNXIIIManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetHIHNFeaturesByTypeAsync(AppConstants.HIHN.HIHN_XIII);
    ...
    public async Task CreateMainHIHNPanelAsync(string featureId, string featureName)
    {
        ...
        Texture mapTexture = Resources.Load<Texture2D>("UI/Background2/Chapter_15");
        mapImage.texture = mapTexture; 
        RawImage rankImage = currentObject.transform.Find("GroupBackground/RankImage").GetComponent<RawImage>();
        Texture rankTexture = Resources.Load<Texture2D>($"UI/Rank_Research/{AppConstants.HIHN.HIHN_XIII}");
