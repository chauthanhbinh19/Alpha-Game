...
public class HIHNVIManager : MonoBehaviour
{
    public static HIHNVIManager Instance { get; private set; }
    ...
    public async Task CreateHIHNVIManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetHIHNFeaturesByTypeAsync(AppConstants.HIHN.HIHN_VI);
    ...
    public async Task CreateMainHIHNPanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.HIHN.HIHN_VI}");
        rankImage.texture = rankTexture;
        RawImage background = transform.Find("Background").GetComponent<RawImage>();
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.HIHN.HIHN_VI_BACKGROUND_URL);
