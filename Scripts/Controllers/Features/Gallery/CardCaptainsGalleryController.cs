...
public class CardCaptainsGalleryController : MonoBehaviour
{
    ...
    public void Initialize()
    {
        ...
        CardCaptainBlockButtonPrefab = UIManager.Instance.Get(AppConstants.Prefab.Component.CARD_CAPTAIN_BLOCK_BUTTON_PREFAB);
    }
    public void CreateCardCaptainsGallery(List<CardCaptains> cardCaptains, Transform contentPanel)
    {
        ...
        foreach (var cardCaptain in cardCaptains)
        {
            ...
            unlockButton.onClick.AddListener(async () =>
            {
                ...
                await cardCaptainsGalleryService.UpdateStatusCardCaptainGalleryAsync(User.CurrentUserId, cardCaptain.Id);
                blockImage.gameObject.SetActive(false);
                unlockButton.gameObject.SetActive(false);
                image.color = Color.white;
            ...
            {
                AudioManager.Instance.PlaySFX(AudioConstants.SFX.BUTTON_CLICK_SOUND);
                await CardCaptainsGalleryService.Create().UpdateCardCaptainGalleryPowerAsync(User.CurrentUserId, cardCaptain.Id);
            });
        }
