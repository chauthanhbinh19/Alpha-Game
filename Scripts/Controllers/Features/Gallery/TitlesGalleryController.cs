...
public class TitlesGalleryController : MonoBehaviour
{
    ...
    private GameObject TitleBlockButtonPrefab;
    private void Awake()
    {
    ...
    public void CreateTitlesGallery(List<Titles> titles, Transform contentPanel)
    {
        foreach (var title in titles)
        {
            try
            {
                ...
                RawImage backgroundImage = titleObject.transform.Find("RectMask2/Background").GetComponent<RawImage>();
                backgroundImage.texture = TextureHelper.LoadTextureCached(ImageConstants.Background.TITLE_BUTTON_BACKGROUND_URL);
                ...
                if (title.Status.Equals(AppConstants.Status.AVAILABLE))
                {
                    blockImage.gameObject.SetActive(false);
                    Unlock.gameObject.SetActive(false);
                    image.color = Color.white;
                }
                else if (title.Status.Equals(AppConstants.Status.PENDING))
                {
                    blockImage.gameObject.SetActive(true);
                    Unlock.gameObject.SetActive(true);
                }
                else if (title.Status.Equals(AppConstants.Status.BLOCK))
                {
                    blockImage.gameObject.SetActive(true);
                    Unlock.gameObject.SetActive(false);
                }
                ...
                if ((title.CurrentStar < title.TempStar) && title.Status.Equals(AppConstants.Status.AVAILABLE))
                {
