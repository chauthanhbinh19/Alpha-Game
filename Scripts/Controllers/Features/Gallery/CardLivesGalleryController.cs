...
public class CardLivesGalleryController : MonoBehaviour
{
    ...
    public void Initialize()
    {
        ...
        CardLifeBlockButtonPrefab = UIManager.Instance.Get(AppConstants.Prefab.Component.CARD_LIFE_BLOCK_BUTTON_PREFAB);
    }
    public void CreateCardLivesGallery(List<CardLives> cardLives, Transform contentPanel)
    {
        ...
        foreach (var cardLife in cardLives)
        {
            ...
            unlockButton.onClick.AddListener(async () =>
            {
                ...
                await cardLifeGalleryService.UpdateStatusCardLifeGalleryAsync(User.CurrentUserId, cardLife.Id);
                blockImage.gameObject.SetActive(false);
                unlockButton.gameObject.SetActive(false);
                image.color = Color.white;
            ...
            {
                AudioManager.Instance.PlaySFX(AudioConstants.SFX.BUTTON_CLICK_SOUND);
                await CardLivesGalleryService.Create().UpdateCardLifeGalleryPowerAsync(User.CurrentUserId, cardLife.Id);
            });
        }
