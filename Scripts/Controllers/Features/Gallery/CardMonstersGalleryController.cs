...
public class CardMonstersGalleryController : MonoBehaviour
{
    ...
    public void Initialize()
    {
        ...
        CardMonsterBlockButtonPrefab = UIManager.Instance.Get(AppConstants.Prefab.Component.CARD_MONSTER_BLOCK_BUTTON_PREFAB);
    }
    public void CreateCardMonstersGallery(List<CardMonsters> cardMonsters, Transform contentPanel)
    {
        ...
        foreach (var cardMonster in cardMonsters)
        {
            ...
            unlockButton.onClick.AddListener(async () =>
            {
                ...
                await monsterGalleryService.UpdateStatusCardMonsterGalleryAsync(User.CurrentUserId, cardMonster.Id);
                blockImage.gameObject.SetActive(false);
                unlockButton.gameObject.SetActive(false);
                image.color = Color.white;
            ...
            {
                AudioManager.Instance.PlaySFX(AudioConstants.SFX.BUTTON_CLICK_SOUND);
                await CardMonstersGalleryService.Create().UpdateCardMonsterGalleryPowerAsync(User.CurrentUserId, cardMonster.Id);
            });
        }
