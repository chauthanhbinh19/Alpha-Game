...
public class UserBadgesController : MonoBehaviour
{
    ...
    public void CreateUserBadges(List<Badges> badges, Transform contentPanel)
    {
        ...
        foreach (var badge in badges)
        {
            ...
            button.onClick.AddListener(() =>
            {
                ...
                MainMenuDetailsManager.Instance.PopupDetails(badge, MainPanel);
            });
    ...
    public void ShowBadgeDetails(Badges badge, GameObject currentObject, int buttonType = 1)
    {
        ...
        {
            GetDetails(badge, currentObject);
            ButtonLoader.Instance.OnButtonClicked("Button_1", RightButtonContent);
        });
        ButtonEvent.Instance.AssignButtonEvent("Button_2", RightButtonContent, () =>
        {
            _ = GetLevelAsync(badge, currentObject);
            ButtonLoader.Instance.OnButtonClicked("Button_2", RightButtonContent);
        });
        ButtonEvent.Instance.AssignButtonEvent("Button_4", RightButtonContent, () =>
        {
            _ = GetUpgradeAsync(badge, currentObject);
    ...
    public void GetDetails(object obj, GameObject currentObject)
    {
        ...
        if (obj is Badges badge)
        {
            ...
            powerText.text = NumberFormatter.FormatNumber(badge.Power, false);
            ...
            UIManager.Instance.CreatePropertyUI(1, properties, badge, currentObject);
        }
    }
    public async Task GetLevelAsync(object obj, GameObject currentObject)
    {
        ...
        {
            PropertyInfo[] properties = badge.GetType().GetProperties();
            UIManager.Instance.CreatePropertyLevelUI(properties, badge, increasePerLevel, currentObject);
            ...
            {
                AudioManager.Instance.PlaySFX(AudioConstants.SFX.BUTTON_CLICK_SOUND);
                Badges currentBadge = new Badges();
                currentBadge = await UserBadgesService.Create().GetUserBadgeByIdAsync(User.CurrentUserId, badge.Id);
                double totalExperiment = currentBadge.Experiment;
                int currentLevel = currentBadge.Level;
                ...
                if (canLevel)
                {
                    ...
                    newBadge = await UserBadgesService.Create().GetNewLevelPowerAsync(badge, increasePerLevel);
                    await UserBadgesService.Create().UpdateBadgeLevelAsync(newBadge, currentLevel + 1);
            ...
            {
                AudioManager.Instance.PlaySFX(AudioConstants.SFX.BUTTON_CLICK_SOUND);
                Badges currentBadge = await UserBadgesService.Create().GetUserBadgeByIdAsync(User.CurrentUserId, badge.Id);
                double totalExperiment = currentBadge.Experiment;
                int currentLevel = currentBadge.Level;
                ...
                if (canLevel)
                {
                    ...
                    Badges newBadge = await UserBadgesService.Create().GetNewLevelPowerAsync(badge, levelsGained * increasePerLevel);
                    await UserBadgesService.Create().UpdateBadgeLevelAsync(newBadge, currentLevel);
                    double newPower = await teamsService.GetTeamsPowerAsync(User.CurrentUserId);
                    double currentPower = User.CurrentUserPower;
                    User.CurrentUserPower = newPower;
                    FindObjectOfType<PowerController>().ShowPower(currentPower, newPower - currentPower, 1);
    ...
    public async Task GetUpgradeAsync(object obj, GameObject currentObject)
    {
        ...
        if (obj is Badges badge)
        {
            ...
            foreach (var property in properties)
            {
                ...
                object value = property.GetValue(badge, null);
            ...
            foreach (Items item in items)
            {
                ...
                eQuantity.text = item.Quantity.ToString() + "/" + (badge.Star + 1).ToString();
            }
            GameObject badgeObject = Instantiate(ElementDetails2Prefab, UpgradeMaterialContent);
            Transform badgeTransform = badgeObject.transform;

            RawImage badgeImage = badgeTransform.Find("MaterialImage").GetComponent<RawImage>();
            fileNameWithoutExtension = ImageExtensionHandler.RemoveImageExtension(badge.Image);
            Texture badgeTexture = TextureHelper.LoadTextureCached($"{fileNameWithoutExtension}");
            badgeImage.texture = badgeTexture;

            TextMeshProUGUI badgeQuantity = badgeTransform.Find("QuantityText").GetComponent<TextMeshProUGUI>();
            badgeQuantity.text = badge.Quantity.ToString() + "/" + (badge.Star + 1).ToString();

            UIManager.Instance.CreateStarUI(badge.Star, currentObject);
            breakthroughButton.onClick.RemoveAllListeners();
            breakthroughButton.onClick.AddListener(async () =>
            {
                AudioManager.Instance.PlaySFX(AudioConstants.SFX.BUTTON_CLICK_SOUND);
                double requiredQuantity = badge.Star + 1;
                double totalItemQuantity = 0;

                // Kiểm tra số lượng danh hiệu
                bool hasEnoughBadge = badge.Quantity >= requiredQuantity;
                ...
                bool hasEnoughItem = totalItemQuantity + badge.Quantity >= requiredQuantity;

                if (hasEnoughBadge || hasEnoughItem)
                {
                    // Giảm số lượng danh hiệu trước
                    if (badge.Quantity >= requiredQuantity)
                    {
                        badge.Quantity -= requiredQuantity;
                    }
                    else
                    {
                        // Nếu danh hiệu không đủ, dùng cả danh hiệu + vật phẩm để bù vào
                        double remainingRequired = requiredQuantity - badge.Quantity;
                        badge.Quantity = 0; // Dùng hết danh hiệu
                    ...
                    newBadge = await UserBadgesService.Create().GetNewBreakthroughPowerAsync(badge, increasePerUpgrade);
                    await UserBadgesService.Create().UpdateBadgeBreakthroughAsync(newBadge, badge.Star + 1, badge.Quantity);
                    double newPower = await teamsService.GetTeamsPowerAsync(User.CurrentUserId);
                    double currentPower = User.CurrentUserPower;
                    User.CurrentUserPower = newPower;
                    FindObjectOfType<PowerController>().ShowPower(currentPower, newPower - currentPower, 1);

                    await BadgesGalleryService.Create().UpdateStarBadgeGalleryAsync(badge.Id, badge.Star + 1);
