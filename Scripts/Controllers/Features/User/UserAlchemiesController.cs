...
public class UserAlchemiesController : MonoBehaviour
{
    ...
    public async Task GetLevelAsync(object obj, GameObject currentObject)
    {
        ...
        if (obj is Alchemies alchemy)
        {
            ...
            {
                AudioManager.Instance.PlaySFX(AudioConstants.SFX.BUTTON_CLICK_SOUND);
                Alchemies currentAlchemy = new Alchemies();
                currentAlchemy = await UserAlchemiesService.Create().GetUserAlchemyByIdAsync(User.CurrentUserId, alchemy.Id);
                double totalExperiment = currentAlchemy.Experiment;
                int currentLevel = currentAlchemy.Level;
            ...
            {
                AudioManager.Instance.PlaySFX(AudioConstants.SFX.BUTTON_CLICK_SOUND);
                Alchemies currentAlchemy = await UserAlchemiesService.Create().GetUserAlchemyByIdAsync(User.CurrentUserId, alchemy.Id);
                double totalExperiment = currentAlchemy.Experiment;
                int currentLevel = currentAlchemy.Level;
    ...
    public async Task GetUpgradeAsync(object obj, GameObject currentObject)
    {
        ...
        if (obj is Alchemies alchemy)
        {
            ...
            foreach (Items item in items)
            {
                ...
                eQuantity.text = item.Quantity.ToString() + "/" + (alchemy.Star + 1).ToString();
            }
            GameObject alchemyObject = Instantiate(ElementDetails2Prefab, UpgradeMaterialContent);
            Transform alchemyTransform = alchemyObject.transform;

            RawImage alchemyImage = alchemyTransform.Find("MaterialImage").GetComponent<RawImage>();
            fileNameWithoutExtension = alchemy.Image.Replace(".png", "");
            Texture alchemyTexture = TextureHelper.LoadTextureCached($"{fileNameWithoutExtension}");
            alchemyImage.texture = alchemyTexture;

            TextMeshProUGUI alchemyQuantity = alchemyTransform.Find("QuantityText").GetComponent<TextMeshProUGUI>();
            alchemyQuantity.text = alchemy.Quantity.ToString() + "/" + (alchemy.Star + 1).ToString();
            ...
            breakthroughButton.onClick.AddListener(async () =>
            {
                ...
                bool hasEnoughAlchemy = alchemy.Quantity >= requiredQuantity;
                ...
                bool hasEnoughItem = totalItemQuantity + alchemy.Quantity >= requiredQuantity;

                if (hasEnoughAlchemy || hasEnoughItem)
                {
