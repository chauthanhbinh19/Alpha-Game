...
public class UserAvatarsController : MonoBehaviour
{
    public static UserAvatarsController Instance { get; private set; }
    ...
    public void CreateUserAvatars(List<Avatars> avatars, Transform contentPanel)
    {
        ...
        foreach (var avatar in avatars)
        {
            ...
            Transform transform = avatarObject.transform;
            ...
            rareText.color = ColorHelper.HexToColor(QualityEvaluatorHelper.CheckRareColor(avatar.Rarity));
            rareText.text = avatar.Rarity;
        }
    ...
    public void ShowAvatarDetails(Avatars avatar, int buttonType = 1)
    {
        ...
        ButtonLoader.Instance.CreateSetButtonGroup(avatar, setButtonGroupPanel);
    ...
    {
        RefreshDetailsUI(avatar, currentObject);

        BindButton(avatar, currentObject);
    }
    public void RefreshDetailsUI(Avatars avatar, GameObject currentObject)
    {
        Transform transform = currentObject.transform;
        RawImage image = transform.Find("DictionaryCards/CardImage").GetComponent<RawImage>();
        string fileNameWithoutExtension = ImageHelper.RemoveImageExtension(avatar.Image); // Lấy giá trị của image từ đối tượng Card
        Texture texture = TextureHelper.LoadTextureCached($"{fileNameWithoutExtension}");
        image.texture = texture;
        ...
        powerText.text = NumberFormatterHelper.FormatNumber(avatar.Power, false);
        ...
        Texture rareTexture = TextureHelper.LoadTextureCached($"UI/UI/{avatar.Rarity}");
        rareImage.texture = rareTexture;

        Transform starGridLayout = transform.Find("DictionaryCards/DetailsPanel/Group2/Star/GridLayout");
        TextureHelper.SetupStars(starGridLayout, avatar.Star);
        ...
        SetupStat(transform, "Health", AppConstants.StatFields.HEALTH, AppDisplayConstants.StatFieldsShort.HEALTH, avatar.Health);
        SetupStat(transform, "PhysicalAttack", AppConstants.StatFields.PHYSICAL_ATTACK, AppDisplayConstants.StatFieldsShort.PHYSICAL_ATTACK, avatar.PhysicalAttack);
        SetupStat(transform, "PhysicalDefense", AppConstants.StatFields.PHYSICAL_DEFENSE, AppDisplayConstants.StatFieldsShort.PHYSICAL_DEFENSE, avatar.PhysicalDefense);
        SetupStat(transform, "MagicalAttack", AppConstants.StatFields.MAGICAL_ATTACK, AppDisplayConstants.StatFieldsShort.MAGICAL_ATTACK, avatar.MagicalAttack);
        SetupStat(transform, "MagicalDefense", AppConstants.StatFields.MAGICAL_DEFENSE, AppDisplayConstants.StatFieldsShort.MAGICAL_DEFENSE, avatar.MagicalDefense);
        SetupStat(transform, "ChemicalAttack", AppConstants.StatFields.CHEMICAL_ATTACK, AppDisplayConstants.StatFieldsShort.CHEMICAL_ATTACK, avatar.ChemicalAttack);
        SetupStat(transform, "ChemicalDefense", AppConstants.StatFields.CHEMICAL_DEFENSE, AppDisplayConstants.StatFieldsShort.CHEMICAL_DEFENSE, avatar.ChemicalDefense);
        SetupStat(transform, "AtomicAttack", AppConstants.StatFields.ATOMIC_ATTACK, AppDisplayConstants.StatFieldsShort.ATOMIC_ATTACK, avatar.AtomicAttack);
        SetupStat(transform, "AtomicDefense", AppConstants.StatFields.ATOMIC_DEFENSE, AppDisplayConstants.StatFieldsShort.ATOMIC_DEFENSE, avatar.AtomicDefense);
        SetupStat(transform, "MentalAttack", AppConstants.StatFields.MENTAL_ATTACK, AppDisplayConstants.StatFieldsShort.MENTAL_ATTACK, avatar.MentalAttack);
        SetupStat(transform, "MentalDefense", AppConstants.StatFields.MENTAL_DEFENSE, AppDisplayConstants.StatFieldsShort.MENTAL_DEFENSE, avatar.MentalDefense);
        SetupStat(transform, "Speed", AppConstants.StatFields.SPEED, AppDisplayConstants.StatFieldsShort.SPEED, avatar.Speed);
    }
    public void BindButton(Avatars avatar, GameObject currentObject)
    {
        Transform transform = currentObject.transform;
        ...
        {
            AudioManager.Instance.PlaySFX(AudioConstants.SFX.BUTTON_CLICK_SOUND);
            StatsManager.Instance.CreateStatsManager(avatar);
        });
        ...
        upgradeLevelButton.onClick.AddListener(async () =>
        {
            ...
            LevelController.Instance.CreateLevelPanel(avatar, itemExperienceDTO, MAX_LEVEL, level => Math.Max(level, 1) * 500d);
        });
        ...
        {
            AudioManager.Instance.PlaySFX(AudioConstants.SFX.BUTTON_CLICK_SOUND);
            StarController.Instance.CreateStarPanel(avatar, MAX_LEVEL, level => Math.Max(level, 1) * 2d);
        });
        ...
        {
            AudioManager.Instance.PlaySFX(AudioConstants.SFX.BUTTON_CLICK_SOUND);
            ModuleManager.Instance.CreateModule(avatar);
        });
        ...
        {
            AudioManager.Instance.PlaySFX(AudioConstants.SFX.BUTTON_CLICK_SOUND);
            UpgradeManager.Instance.CreateUpgrade(avatar);
        });
    }
