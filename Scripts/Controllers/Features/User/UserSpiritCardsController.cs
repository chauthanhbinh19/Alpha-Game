...
public class UserSpiritCardsController : MonoBehaviour
{
    ...
    public void CreateUserSpiritCards(List<SpiritCards> spiritCards, Transform contentPanel)
    {
        ...
        foreach (var spiritCard in spiritCards)
        {
            ...
            button.onClick.AddListener(() =>
            {
                ...
                MainMenuDetailsManager.Instance.PopupDetails(spiritCard, MainPanel);
            });
    ...
    public void ShowSpiritCardDetails(SpiritCards spiritCard, GameObject currentObject, int buttonType = 1)
    {
        ...
        {
            GetDetails(spiritCard, currentObject);
        ...
        switch (buttonType)
        {
            ...
            case 2:
                ...
                break;
            case 3:
                GetSkills(spiritCard, currentObject);
    ...
    public void GetDetails(object obj, GameObject currentObject)
    {
        ...
        if (obj is SpiritCards spiritCard)
        {
            ...
            powerText.text = NumberFormatter.FormatNumber(spiritCard.Power, false);
            ...
            UIManager.Instance.CreatePropertyUI(1, properties, spiritCard, currentObject);
        }
    }
    public async Task GetLevelAsync(object obj, GameObject currentObject)
    {
        ...
        if (obj is SpiritCards spiritCard)
        {
            ...
            {
                AudioManager.Instance.PlaySFX(AudioConstants.SFX.BUTTON_CLICK_SOUND);
                SpiritCards currentSpiritCard = new SpiritCards();
                currentSpiritCard = await UserSpiritCardsService.Create().GetUserSpiritCardByIdAsync(User.CurrentUserId, spiritCard.Id);
                double totalExperiment = currentSpiritCard.Experiment;
                int currentLevel = currentSpiritCard.Level;
            ...
            {
                AudioManager.Instance.PlaySFX(AudioConstants.SFX.BUTTON_CLICK_SOUND);
                SpiritCards currentSpiritCard = await UserSpiritCardsService.Create().GetUserSpiritCardByIdAsync(User.CurrentUserId, spiritCard.Id);
                double totalExperiment = currentSpiritCard.Experiment;
                int currentLevel = currentSpiritCard.Level;
    ...
    public async Task GetUpgradeAsync(object obj, GameObject currentObject)
    {
        ...
        if (obj is SpiritCards spiritBeast)
        {
            ...
            GameObject spiritCardObject = Instantiate(ElementDetails2Prefab, UpgradeMaterialContent);
            Transform spiritCardTransform = spiritCardObject.transform;

            RawImage spiritCardImage = spiritCardTransform.Find("MaterialImage").GetComponent<RawImage>();
            fileNameWithoutExtension = ImageExtensionHandler.RemoveImageExtension(spiritBeast.Image);
            Texture spiritCardTexture = TextureHelper.LoadTextureCached($"{fileNameWithoutExtension}");
            spiritCardImage.texture = spiritCardTexture;

            TextMeshProUGUI spiritCardQuantity = spiritCardTransform.Find("QuantityText").GetComponent<TextMeshProUGUI>();
            spiritCardQuantity.text = spiritBeast.Quantity.ToString() + "/" + (spiritBeast.Star + 1).ToString();
            ...
            breakthroughButton.onClick.AddListener(async () =>
            {
                ...
                if (hasEnoughSpiritCard || hasEnoughItem)
                {
                    ...
                    await SpiritCardsGalleryService.Create().UpdateStarSpiritCardGalleryAsync(spiritBeast.Id, spiritBeast.Star + 1);
