...
public class UserPlantsController : MonoBehaviour
{
    ...
    public void CreateUserPlants(List<Plants> plants, Transform contentPanel)
    {
        ...
        Texture bgTexture = TextureHelper.LoadTextureCached(ImageConstants.Background.PLANT_BUTTON_BACKGROUND_URL);

        foreach (var plant in plants)
        {
            GameObject plantObject = Instantiate(PlantButtonPrefab, contentPanel);
            Transform transform = plantObject.transform;

            TextMeshProUGUI titleText = transform.Find("TitleText").GetComponent<TextMeshProUGUI>();
            titleText.text = plant.Name.Replace("_", " ");

            RawImage image = transform.Find("Image").GetComponent<RawImage>();
            string fileNameWithoutExtension = ImageHelper.RemoveImageExtension(plant.Image);
            ...
            rareText.color = ColorHelper.HexToColor(QualityEvaluatorHelper.CheckRareColor(plant.Rarity));
            rareText.text = plant.Rarity;
        }
    ...
    public void ShowPlantDetails(Plants plant, int buttonType = 1)
    {
        ...
        titleText.text = LocalizationManager.Get(AppDisplayConstants.Title.PLANT);
        ...
        closeButton.onClick.AddListener(async () =>
        {
            ...
            await MainMenuManager.Instance.GetTypeAsync(AppConstants.MainType.PLANT);
        });
        TempCurrentObject = currentObject;
        CreateDetailsUI(plant, currentObject);
    }
    public void CreateDetailsUI(Plants plant, GameObject currentObject)
    {
        RefreshDetailsUI(plant, currentObject);

        BindButton(plant, currentObject);
    }
    public void RefreshDetailsUI(Plants plant, GameObject currentObject)
    {
        Transform transform = currentObject.transform;
        RawImage image = transform.Find("DictionaryCards/CardImage").GetComponent<RawImage>();
        string fileNameWithoutExtension = ImageHelper.RemoveImageExtension(plant.Image); // Lấy giá trị của image từ đối tượng Card
        Texture texture = TextureHelper.LoadTextureCached($"{fileNameWithoutExtension}");
        image.texture = texture;
        ...
        powerText.text = NumberFormatterHelper.FormatNumber(plant.Power, false);
        ...
        SetupStat(transform, "Health", AppConstants.StatFields.HEALTH, AppDisplayConstants.StatFieldsShort.HEALTH, plant.Health);
        SetupStat(transform, "PhysicalAttack", AppConstants.StatFields.PHYSICAL_ATTACK, AppDisplayConstants.StatFieldsShort.PHYSICAL_ATTACK, plant.PhysicalAttack);
        SetupStat(transform, "PhysicalDefense", AppConstants.StatFields.PHYSICAL_DEFENSE, AppDisplayConstants.StatFieldsShort.PHYSICAL_DEFENSE, plant.PhysicalDefense);
        SetupStat(transform, "MagicalAttack", AppConstants.StatFields.MAGICAL_ATTACK, AppDisplayConstants.StatFieldsShort.MAGICAL_ATTACK, plant.MagicalAttack);
        SetupStat(transform, "MagicalDefense", AppConstants.StatFields.MAGICAL_DEFENSE, AppDisplayConstants.StatFieldsShort.MAGICAL_DEFENSE, plant.MagicalDefense);
        SetupStat(transform, "ChemicalAttack", AppConstants.StatFields.CHEMICAL_ATTACK, AppDisplayConstants.StatFieldsShort.CHEMICAL_ATTACK, plant.ChemicalAttack);
        SetupStat(transform, "ChemicalDefense", AppConstants.StatFields.CHEMICAL_DEFENSE, AppDisplayConstants.StatFieldsShort.CHEMICAL_DEFENSE, plant.ChemicalDefense);
        SetupStat(transform, "AtomicAttack", AppConstants.StatFields.ATOMIC_ATTACK, AppDisplayConstants.StatFieldsShort.ATOMIC_ATTACK, plant.AtomicAttack);
        SetupStat(transform, "AtomicDefense", AppConstants.StatFields.ATOMIC_DEFENSE, AppDisplayConstants.StatFieldsShort.ATOMIC_DEFENSE, plant.AtomicDefense);
        SetupStat(transform, "MentalAttack", AppConstants.StatFields.MENTAL_ATTACK, AppDisplayConstants.StatFieldsShort.MENTAL_ATTACK, plant.MentalAttack);
        SetupStat(transform, "MentalDefense", AppConstants.StatFields.MENTAL_DEFENSE, AppDisplayConstants.StatFieldsShort.MENTAL_DEFENSE, plant.MentalDefense);
        SetupStat(transform, "Speed", AppConstants.StatFields.SPEED, AppDisplayConstants.StatFieldsShort.SPEED, plant.Speed);
    }
    public void BindButton(Plants plant, GameObject currentObject)
    {
        Transform transform = currentObject.transform;
        ...
        {
            AudioManager.Instance.PlaySFX(AudioConstants.SFX.BUTTON_CLICK_SOUND);
            StatsManager.Instance.CreateStatsManager(plant);
        });
        ...
        upgradeLevelButton.onClick.AddListener(async () =>
        {
            ...
            LevelController.Instance.CreateLevelPanel(plant, itemExperienceDTO, MAX_LEVEL, level => Math.Max(level, 1) * 500d);
        });
        ...
        {
            AudioManager.Instance.PlaySFX(AudioConstants.SFX.BUTTON_CLICK_SOUND);
            StarController.Instance.CreateStarPanel(plant, MAX_LEVEL, level => Math.Max(level, 1) * 2d);
        });
        ...
        {
            AudioManager.Instance.PlaySFX(AudioConstants.SFX.BUTTON_CLICK_SOUND);
            ModuleManager.Instance.CreateModule(plant);
        });
        ...
        {
            AudioManager.Instance.PlaySFX(AudioConstants.SFX.BUTTON_CLICK_SOUND);
            UpgradeManager.Instance.CreateUpgrade(plant);
        });
    }
    public void RefreshCurrentDetailsUI(Plants plant)
    {
