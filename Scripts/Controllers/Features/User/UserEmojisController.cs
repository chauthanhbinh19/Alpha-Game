...
public class UserEmojisController : MonoBehaviour
{
    ...
    public void CreateUserEmojis(List<Emojis> cores, Transform contentPanel)
    {
    ...
    public void ShowEmojiDetails(Emojis Emojis, GameObject currentObject, int buttonType = 1)
    {
        ...
        {
            GetDetails(Emojis, currentObject);
            ButtonLoader.Instance.OnButtonClicked("Button_1", RightButtonContent);
        });
        ButtonEvent.Instance.AssignButtonEvent("Button_2", RightButtonContent, () =>
        {
            _ = GetLevelAsync(Emojis, currentObject);
            ButtonLoader.Instance.OnButtonClicked("Button_2", RightButtonContent);
        });
        ButtonEvent.Instance.AssignButtonEvent("Button_4", RightButtonContent, () =>
        {
            _ = GetUpgradeAsync(Emojis, currentObject);
        ...
        switch (buttonType)
        {
            ...
            case 2:
                ...
                break;
            case 3:
                GetSkills(Emojis, currentObject);
    ...
    public async Task GetLevelAsync(object obj, GameObject currentObject)
    {
        ...
        if (obj is Emojis title)
        {
            ...
            {
                AudioManager.Instance.PlaySFX(AudioConstants.SFX.BUTTON_CLICK_SOUND);
                Emojis currentCard = new Emojis();
                currentCard = await UserEmojisService.Create().GetUserEmojiByIdAsync(User.CurrentUserId, title.Id);
                ...
                if (canLevel)
                {
                    Emojis newCard = new Emojis();

                    newCard = await UserEmojisService.Create().GetNewLevelPowerAsync(title, increasePerLevel);
                    await UserEmojisService.Create().UpdateEmojiLevelAsync(newCard, currentLevel + 1);
            ...
            {
                AudioManager.Instance.PlaySFX(AudioConstants.SFX.BUTTON_CLICK_SOUND);
                Emojis currentCard = await UserEmojisService.Create().GetUserEmojiByIdAsync(User.CurrentUserId, title.Id);
                double totalExperiment = currentCard.Experiment;
                int currentLevel = currentCard.Level;
                ...
                if (canLevel)
                {
                    ...
                    Emojis newCard = await UserEmojisService.Create().GetNewLevelPowerAsync(title, levelsGained * increasePerLevel);
                    await UserEmojisService.Create().UpdateEmojiLevelAsync(newCard, currentLevel);
                    double newPower = await teamsService.GetTeamsPowerAsync(User.CurrentUserId);
                    double currentPower = User.CurrentUserPower;
                    User.CurrentUserPower = newPower;
                    FindObjectOfType<PowerController>().ShowPower(currentPower, newPower - currentPower, 1);
    ...
    public async Task GetUpgradeAsync(object obj, GameObject currentObject)
    {
        ...
        if (obj is Emojis title)
        {
            ...
            breakthroughButton.onClick.AddListener(async () =>
            {
                ...
                bool hasEnoughEmojis = title.Quantity >= requiredQuantity;
                ...
                if (hasEnoughEmojis || hasEnoughItems)
                {
                    ...
                    Emojis newTitle = new Emojis();

                    newTitle = await UserEmojisService.Create().GetNewBreakthroughPowerAsync(title, increasePerUpgrade);
                    await UserEmojisService.Create().UpdateEmojiBreakthroughAsync(newTitle, title.Star + 1, title.Quantity);
                    double newPower = await teamsService.GetTeamsPowerAsync(User.CurrentUserId);
                    double currentPower = User.CurrentUserPower;
                    User.CurrentUserPower = newPower;
                    FindObjectOfType<PowerController>().ShowPower(currentPower, newPower - currentPower, 1);

                    await EmojisGalleryService.Create().UpdateStarEmojiGalleryAsync(title.Id, title.Star + 1);
