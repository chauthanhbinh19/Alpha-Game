...
public class UserMedalsController : MonoBehaviour
{
    ...
    public void ShowMedalDetails(Medals medal, GameObject currentObject, int buttonType = 1)
    {
        ...
        {
            GetDetails(medal, currentObject);
            ButtonLoader.Instance.OnButtonClicked("Button_1", RightButtonContent);
        });
        ButtonEvent.Instance.AssignButtonEvent("Button_2", RightButtonContent, () =>
        {
            _=GetLevelAsync(medal, currentObject);
            ButtonLoader.Instance.OnButtonClicked("Button_2", RightButtonContent);
        });
        ButtonEvent.Instance.AssignButtonEvent("Button_4", RightButtonContent, () =>
        {
            _=GetUpgradeAsync(medal, currentObject);
    ...
    public async Task GetLevelAsync(object obj, GameObject currentObject)
    {
        ...
        if (obj is Medals medal)
        {
            ...
            {
                AudioManager.Instance.PlaySFX(AudioConstants.SFX.BUTTON_CLICK_SOUND);
                Medals currentMedal = new Medals();
                currentMedal = await UserMedalsService.Create().GetUserMedalByIdAsync(User.CurrentUserId, medal.Id);
                double totalExperiment = currentMedal.Experiment;
                int currentLevel = currentMedal.Level;
            ...
            {
                AudioManager.Instance.PlaySFX(AudioConstants.SFX.BUTTON_CLICK_SOUND);
                Medals currentMedal = await UserMedalsService.Create().GetUserMedalByIdAsync(User.CurrentUserId, medal.Id);
                double totalExperiment = currentMedal.Experiment;
                int currentLevel = currentMedal.Level;
    ...
    public async Task GetUpgradeAsync(object obj, GameObject currentObject)
    {
        ...
        if (obj is Medals medal)
        {
            ...
            breakthroughButton.onClick.AddListener(async () =>
            {
                ...
                bool hasEnoughMedal = medal.Quantity >= requiredQuantity;
                ...
                bool hasEnoughItem = totalItemQuantity + medal.Quantity >= requiredQuantity;

                if (hasEnoughMedal || hasEnoughItem)
                {
