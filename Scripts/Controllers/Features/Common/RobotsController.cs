...
public class RobotsController : MonoBehaviour
{
    ...
    public async Task CreateRobotsTradeAsync(List<Robots> robots, string subType, Transform currentContent, Transform currencyPanel, Transform popupPanel)
    {
        // Xóa bớt animation cũ nếu có để tránh lỗi chồng đè
        ...
        foreach (var robot in robots)
        {
            ...
            fileNameWithoutExtension = ImageHelper.RemoveImageExtension(robot.Currency.Image);
            ...
            currencyText.text = NumberFormatterHelper.FormatNumber(robot.Currency.Quantity, false);
    ...
    public void GetQuantity(double originPrice, object obj, string subType, Transform popupPanel, Transform currencyPanel)
    {
        ...
        confirmButton.onClick.AddListener(async () =>
        {
            ...
            if (obj is Robots robot)
            {
                ...
                var result = await UserRobotsService.Create().InsertOrUpdateUserRobotAsync(User.CurrentUserId, robot);
                ...
                if (result.Data || result.OperationType != DatabaseOperationType.None || result.OperationType != DatabaseOperationType.Failed)
                {
                    ...
                    // robots.InsertUserRobots(robots);
                    currencies = await UserCurrenciesService.Create().GetRobotsCurrencyAsync(subType);
