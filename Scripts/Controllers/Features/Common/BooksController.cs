...
public class BooksController : MonoBehaviour
{
    ...
    public async Task CreateBooksTradeAsync(List<Books> books, string subType, Transform currentContent, Transform currencyPanel, Transform popupPanel)
    {
        // Xóa bớt animation cũ nếu có để tránh lỗi chồng đè
        ...
        foreach (var book in books)
        {
            ...
            fileNameWithoutExtension = ImageHelper.RemoveImageExtension(book.Currency.Image);
            ...
            currencyText.text = NumberFormatterHelper.FormatNumber(book.Currency.Quantity, false);
    ...
    public void GetQuantity(double originPrice, object obj, string subType, Transform popupPanel, Transform currencyPanel)
    {
        ...
        confirmButton.onClick.AddListener(async () =>
        {
            ...
            if (obj is Books book)
            {
                ...
                var result = await UserBooksService.Create().InsertOrUpdateUserBookAsync(User.CurrentUserId, book);
