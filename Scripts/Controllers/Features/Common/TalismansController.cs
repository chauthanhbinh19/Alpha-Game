...
public class TalismansController : MonoBehaviour
{
    ...
    public async Task CreateTalismansTradeAsync(List<Talismans> talismans, string subType, Transform currentContent, Transform currencyPanel, Transform popupPanel)
    {
        // Xóa bớt animation cũ nếu có để tránh lỗi chồng đè
        ...
        foreach (var talisman in talismans)
        {
            ...
            fileNameWithoutExtension = ImageHelper.RemoveImageExtension(talisman.Currency.Image);
            ...
            currencyText.text = NumberFormatterHelper.FormatNumber(talisman.Currency.Quantity, false);
    ...
    public void GetQuantity(double originPrice, object obj, string subType, Transform popupPanel, Transform currencyPanel)
    {
        ...
        confirmButton.onClick.AddListener(async () =>
        {
            ...
            if (obj is Talismans talisman)
            {
                ...
                var result = await UserTalismansService.Create().InsertOrUpdateUserTalismanAsync(User.CurrentUserId, talisman);
                ...
                if (result.Data || result.OperationType != DatabaseOperationType.None || result.OperationType != DatabaseOperationType.Failed)
                {
                    ...
                    // talismans.InsertUserTalismans(talismans);
                    currencies = await UserCurrenciesService.Create().GetTalismansCurrencyAsync(subType);
