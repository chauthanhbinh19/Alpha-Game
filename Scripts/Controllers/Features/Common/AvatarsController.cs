...
public class AvatarsController : MonoBehaviour
{
    ...
    public void CreateAvatarsGallery(List<Avatars> avatars, Transform contentPanel)
    {
        ...
        Texture bgTexture = TextureHelper.LoadTextureCached(ImageConstants.Background.AVATAR_BUTTON_BACKGROUND_URL);
    ...
    public async Task CreateAvatarsTradeAsync(List<Avatars> avatars, string subType, Transform currentContent, Transform currencyPanel, Transform popupPanel)
    {
        // Xóa bớt animation cũ nếu có để tránh lỗi chồng đè
        ...
        foreach (var avatar in avatars)
        {
            ...
            fileNameWithoutExtension = ImageHelper.RemoveImageExtension(avatar.Currency.Image);
            ...
            currencyText.text = NumberFormatterHelper.FormatNumber(avatar.Currency.Quantity, false);
    ...
    public void GetQuantity(double originPrice, object obj, string subType, Transform popupPanel, Transform currencyPanel)
    {
        ...
        confirmButton.onClick.AddListener((UnityEngine.Events.UnityAction)(async () =>
        {
            ...
            if (obj is Avatars avatar)
            {
                ...
                await UserCurrenciesService.Create().UpdateUserCurrencyAsync(User.CurrentUserId, avatar.Currency.Id, price);
                var result = await UserAvatarsService.Create().InsertOrUpdateUserAvatarAsync(User.CurrentUserId, avatar);
                ...
                if (result.Data || result.OperationType != DatabaseOperationType.None || result.OperationType != DatabaseOperationType.Failed)
                {
                    ...
                    // avatars.InsertUserAvatars(avatars);
                    currencies = await UserCurrenciesService.Create().GetAvatarsCurrencyAsync(subType);
                    fileNameWithoutExtension = ImageHelper.RemoveImageExtension(avatar.Image);
