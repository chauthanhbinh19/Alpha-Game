...
public class SpiritCardsController : MonoBehaviour
{
    ...
    public async Task CreateSpiritCardTradeAsync(List<SpiritCards> spiritCards, string subType, Transform currentContent, Transform currencyPanel, Transform popupPanel)
    {
        // Xóa bớt animation cũ nếu có để tránh lỗi chồng đè
        ...
        foreach (var spiritCard in spiritCards)
        {
            ...
            fileNameWithoutExtension = ImageHelper.RemoveImageExtension(spiritCard.Currency.Image);
            ...
            currencyText.text = NumberFormatterHelper.FormatNumber(spiritCard.Currency.Quantity, false);
            ...
            {
                AudioManager.Instance.PlaySFX(AudioConstants.SFX.BUTTON_CLICK_SOUND);
                GetQuantity(spiritCard.Currency.Quantity, spiritCard, subType, popupPanel, currencyPanel);
            });
        }
    ...
    public void GetQuantity(double originPrice, object obj, string subType, Transform popupPanel, Transform currencyPanel)
    {
        ...
        confirmButton.onClick.AddListener(async () =>
        {
            ...
            if (obj is SpiritCards spiritCard)
            {
                ...
                var result = await UserSpiritCardsService.Create().InsertOrUpdateUserSpiritCardAsync(User.CurrentUserId, spiritCard);
                ...
                if (result.Data || result.OperationType != DatabaseOperationType.None || result.OperationType != DatabaseOperationType.Failed)
                {
                    ...
                    // spiritCards.InsertUserSpiritCards(spiritCards);
                    currencies = await UserCurrenciesService.Create().GetSpiritCardsCurrencyAsync(subType);
