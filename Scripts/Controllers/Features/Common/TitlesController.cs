...
public class TitlesController : MonoBehaviour
{
    ...
    public async Task CreateTitlesTradeAsync(List<Titles> titles, string subType, Transform currentContent, Transform currencyPanel, Transform popupPanel)
    {
        // Xóa bớt animation cũ nếu có để tránh lỗi chồng đè
        ...
        foreach (var title in titles)
        {
            ...
            fileNameWithoutExtension = ImageHelper.RemoveImageExtension(title.Currency.Image);
            ...
            currencyText.text = NumberFormatterHelper.FormatNumber(title.Currency.Quantity, false);
    ...
    public void GetQuantity(double originPrice, object obj, string subType, Transform popupPanel, Transform currencyPanel)
    {
        ...
        confirmButton.onClick.AddListener((UnityEngine.Events.UnityAction)(async () =>
        {
            ...
            if (obj is Titles title)
            {
                ...
                await UserCurrenciesService.Create().UpdateUserCurrencyAsync(User.CurrentUserId, title.Currency.Id, price);
                var result = await UserTitlesService.Create().InsertOrUpdateUserTitleAsync(User.CurrentUserId, title);
                ...
                if (result.Data || result.OperationType != DatabaseOperationType.None || result.OperationType != DatabaseOperationType.Failed)
                {
                    ...
                    // titles.InsertUserTitles(titles);
                    currencies = await UserCurrenciesService.Create().GetTitlesCurrencyAsync(subType);
