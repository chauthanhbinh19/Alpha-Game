...
public class SymbolsController : MonoBehaviour
{
    ...
    public async Task CreateSymbolsTradeAsync(List<Symbols> symbols, string subType, Transform currentContent, Transform currencyPanel, Transform popupPanel)
    {
        // Xóa bớt animation cũ nếu có để tránh lỗi chồng đè
        ...
        foreach (var symbol in symbols)
        {
            ...
            fileNameWithoutExtension = ImageHelper.RemoveImageExtension(symbol.Currency.Image);
            ...
            currencyText.text = NumberFormatterHelper.FormatNumber(symbol.Currency.Quantity, false);
    ...
    public void GetQuantity(double originPrice, object obj, string subType, Transform popupPanel, Transform currencyPanel)
    {
        ...
        confirmButton.onClick.AddListener(async () =>
        {
            ...
            if (obj is Symbols symbol)
            {
                ...
                var result = await UserSymbolsService.Create().InsertOrUpdateUserSymbolAsync(User.CurrentUserId, symbol);
                ...
                if (result.Data || result.OperationType != DatabaseOperationType.None || result.OperationType != DatabaseOperationType.Failed)
                {
                    ...
                    // symbols.InsertUserSymbols(symbols);
                    currencies = await UserCurrenciesService.Create().GetSymbolsCurrencyAsync(subType);
