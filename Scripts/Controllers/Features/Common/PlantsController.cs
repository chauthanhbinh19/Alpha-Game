...
public class PlantsController : MonoBehaviour
{
    ...
    public async Task CreatePlantsTradeAsync(List<Plants> plants, string subType, Transform currentContent, Transform currencyPanel, Transform popupPanel)
    {
        // Xóa bớt animation cũ nếu có để tránh lỗi chồng đè
        ...
        foreach (var plant in plants)
        {
            ...
            fileNameWithoutExtension = ImageHelper.RemoveImageExtension(plant.Currency.Image);
            ...
            currencyText.text = NumberFormatterHelper.FormatNumber(plant.Currency.Quantity, false);
    ...
    public void GetQuantity(double originPrice, object obj, string subType, Transform popupPanel, Transform currencyPanel)
    {
        ...
        confirmButton.onClick.AddListener((UnityEngine.Events.UnityAction)(async () =>
        {
            ...
            if (obj is Plants plant)
            {
                ...
                await UserCurrenciesService.Create().UpdateUserCurrencyAsync(User.CurrentUserId, plant.Currency.Id, price);
                var result = await UserPlantsService.Create().InsertOrUpdateUserPlantAsync(User.CurrentUserId, plant);
                ...
                if (result.Data || result.OperationType != DatabaseOperationType.None || result.OperationType != DatabaseOperationType.Failed)
                {
                    ...
                    // plants.InsertUserPlants(plants);
                    currencies = await UserCurrenciesService.Create().GetPlantsCurrencyAsync(subType);
                    fileNameWithoutExtension = ImageHelper.RemoveImageExtension(plant.Image);
