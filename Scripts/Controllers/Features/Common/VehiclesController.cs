...
public class VehiclesController : MonoBehaviour
{
    ...
    public async Task CreateVehiclesTradeAsync(List<Vehicles> vehicles, string subType, Transform currentContent, Transform currencyPanel, Transform popupPanel)
    {
        // Xóa bớt animation cũ nếu có để tránh lỗi chồng đè
        ...
        foreach (var vehicle in vehicles)
        {
            ...
            fileNameWithoutExtension = ImageHelper.RemoveImageExtension(vehicle.Currency.Image);
            ...
            currencyText.text = NumberFormatterHelper.FormatNumber(vehicle.Currency.Quantity, false);
    ...
    public void GetQuantity(double originPrice, object obj, string subType, Transform popupPanel, Transform currencyPanel)
    {
        ...
        confirmButton.onClick.AddListener(async () =>
        {
            ...
            if (obj is Vehicles vehicle)
            {
                ...
                var result = await UserVehiclesService.Create().InsertOrUpdateUserVehicleAsync(User.CurrentUserId, vehicle);
                ...
                if (result.Data || result.OperationType != DatabaseOperationType.None || result.OperationType != DatabaseOperationType.Failed)
                {
                    ...
                    // vehicles.InsertUserVehicles(vehicles);
                    currencies = await UserCurrenciesService.Create().GetVehiclesCurrencyAsync(subType);
                    fileNameWithoutExtension = ImageHelper.RemoveImageExtension(vehicle.Image);
