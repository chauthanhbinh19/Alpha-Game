...
public class FurnituresController : MonoBehaviour
{
    ...
    public async Task CreateFurnituresTradeAsync(List<Furnitures> furnitures, string subType, Transform currentContent, Transform currencyPanel, Transform popupPanel)
    {
        // Xóa bớt animation cũ nếu có để tránh lỗi chồng đè
    ...
    public void GetQuantity(double originPrice, object obj, string subType, Transform popupPanel, Transform currencyPanel)
    {
        ...
        confirmButton.onClick.AddListener(async () =>
        {
            ...
            if (obj is Furnitures furniture)
            {
                ...
                var result = await UserFurnituresService.Create().InsertOrUpdateUserFurnitureAsync(User.CurrentUserId, furniture);
                ...
                if (result.Data || result.OperationType != DatabaseOperationType.None || result.OperationType != DatabaseOperationType.Failed)
                {
                    ...
                    // furnitures.InsertUserFurnitures(furnitures);
                    currencies = await UserCurrenciesService.Create().GetFurnituresCurrencyAsync(subType);
