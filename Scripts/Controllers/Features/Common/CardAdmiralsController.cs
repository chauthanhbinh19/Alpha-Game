...
public class CardAdmiralsController : MonoBehaviour
{
    ...
    public void CreateCardAdmiralsGallery(List<CardAdmirals> cardAdmirals, Transform contentPanel)
    {
        ...
        foreach (var cardAdmiral in cardAdmirals)
        {
            ...
            rareBackground.color = ColorHelper.HexToColor(QualityEvaluatorHelper.CheckRareColor(cardAdmiral.Rarity));
            ...
            rareText.color = ColorHelper.HexToColor(QualityEvaluatorHelper.CheckRareColor(cardAdmiral.Rarity));
            rareText.text = cardAdmiral.Rarity;
        }
    ...
    public async Task CreateCardAdmiralsTradeAsync(List<CardAdmirals> cardAdmirals, string subType, Transform currentContent, Transform currencyPanel, Transform popupPanel)
    {
        // Xóa bớt animation cũ nếu có để tránh lỗi chồng đè
    ...
    public void GetQuantity(double originPrice, object obj, string subType, Transform popupPanel, Transform currencyPanel)
    {
        ...
        confirmButton.onClick.AddListener(async () =>
        {
            ...
            if (obj is CardAdmirals cardAdmiral)
            {
                ...
                var result = await UserCardAdmiralsService.Create().InsertOrUpdateUserCardAdmiralAsync(User.CurrentUserId, cardAdmiral);
                ...
                if (result.Data || result.OperationType != DatabaseOperationType.None || result.OperationType != DatabaseOperationType.Failed)
                {
                    ...
                    // cardAdmirals.InsertUserCardAdmirals(cardAdmirals);
                    currencies = await UserCurrenciesService.Create().GetCardAdmiralsCurrencyAsync(subType);
