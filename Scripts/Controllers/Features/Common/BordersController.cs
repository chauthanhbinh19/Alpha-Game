...
public class BordersController : MonoBehaviour
{
    ...
    public void CreateBordersGallery(List<Borders> borders, Transform contentPanel)
    {
        ...
        Texture bgTexture = TextureHelper.LoadTextureCached(ImageConstants.Background.BORDER_BUTTON_BACKGROUND_URL);
    ...
    public async Task CreateBordersTradeAsync(List<Borders> borders, string subType, Transform currentContent, Transform currencyPanel, Transform popupPanel)
    {
        // Xóa bớt animation cũ nếu có để tránh lỗi chồng đè
        ...
        foreach (var border in borders)
        {
            ...
            fileNameWithoutExtension = ImageHelper.RemoveImageExtension(border.Currency.Image);
            ...
            currencyText.text = NumberFormatterHelper.FormatNumber(border.Currency.Quantity, false);
    ...
    public void GetQuantity(double originPrice, object obj, string subType, Transform popupPanel, Transform currencyPanel)
    {
        ...
        confirmButton.onClick.AddListener(async () =>
        {
            ...
            if (obj is Borders border)
            {
                ...
                var result = await UserBordersService.Create().InsertOrUpdateUserBorderAsync(User.CurrentUserId, border);
                ...
                if (result.Data || result.OperationType != DatabaseOperationType.None || result.OperationType != DatabaseOperationType.Failed)
                {
                    ...
                    // borders.InsertUserBorders(borders);
                    currencies = await UserCurrenciesService.Create().GetBordersCurrencyAsync(subType);
