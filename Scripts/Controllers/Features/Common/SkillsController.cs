...
public class SkillsController : MonoBehaviour
{
    ...
    public async Task CreateSkillsTradeAsync(List<Skills> skills, string subType, Transform currentContent, Transform currencyPanel, Transform popupPanel)
    {
        // Xóa bớt animation cũ nếu có để tránh lỗi chồng đè
        ...
        foreach (var skill in skills)
        {
            ...
            fileNameWithoutExtension = ImageHelper.RemoveImageExtension(skill.Currency.Image);
            ...
            currencyText.text = NumberFormatterHelper.FormatNumber(skill.Currency.Quantity, false);
    ...
    public void GetQuantity(double originPrice, object obj, string subType, Transform popupPanel, Transform currencyPanel)
    {
        ...
        confirmButton.onClick.AddListener(async () =>
        {
            ...
            if (obj is Skills skill)
            {
                ...
                var result = await UserSkillsService.Create().InsertOrUpdateUserSkillAsync(User.CurrentUserId, skill);
                ...
                if (result.Data || result.OperationType != DatabaseOperationType.None || result.OperationType != DatabaseOperationType.Failed)
                {
                    ...
                    // skills.InsertUserSkills(skills);
                    currencies = await UserCurrenciesService.Create().GetSkillsCurrencyAsync(subType);
