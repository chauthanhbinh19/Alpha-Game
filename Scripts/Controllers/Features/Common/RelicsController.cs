...
public class RelicsController : MonoBehaviour
{
    ...
    public async Task CreateRelicsTradeAsync(List<Relics> relics, string subType, Transform currentContent, Transform currencyPanel, Transform popupPanel)
    {
        // Xóa bớt animation cũ nếu có để tránh lỗi chồng đè
        ...
        foreach (var relic in relics)
        {
            ...
            fileNameWithoutExtension = ImageHelper.RemoveImageExtension(relic.Currency.Image);
            ...
            currencyText.text = NumberFormatterHelper.FormatNumber(relic.Currency.Quantity, false);
    ...
    public void GetQuantity(double originPrice, object obj, string subType, Transform popupPanel, Transform currencyPanel)
    {
        ...
        confirmButton.onClick.AddListener(async () =>
        {
            ...
            if (obj is Relics relic)
            {
                ...
                var result = await UserRelicsService.Create().InsertOrUpdateUserRelicAsync(User.CurrentUserId, relic);
                ...
                if (result.Data || result.OperationType != DatabaseOperationType.None || result.OperationType != DatabaseOperationType.Failed)
                {
                    ...
                    // relics.InsertUserRelics(relics);
                    currencies = await UserCurrenciesService.Create().GetRelicsCurrencyAsync(subType);
