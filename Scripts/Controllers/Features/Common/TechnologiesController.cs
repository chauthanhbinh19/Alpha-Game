...
public class TechnologiesController : MonoBehaviour
{
    ...
    public async Task CreateTechnologiesTradeAsync(List<Technologies> technologies, string subType, Transform currentContent, Transform currencyPanel, Transform popupPanel)
    {
        // Xóa bớt animation cũ nếu có để tránh lỗi chồng đè
        ...
        foreach (var technology in technologies)
        {
            ...
            fileNameWithoutExtension = ImageHelper.RemoveImageExtension(technology.Currency.Image);
            ...
            currencyText.text = NumberFormatterHelper.FormatNumber(technology.Currency.Quantity, false);
    ...
    public void GetQuantity(double originPrice, object obj, string subType, Transform popupPanel, Transform currencyPanel)
    {
        ...
        confirmButton.onClick.AddListener(async () =>
        {
            ...
            if (obj is Technologies technology)
            {
                ...
                var result = await UserTechnologiesService.Create().InsertOrUpdateUserTechnologyAsync(User.CurrentUserId, technology);
                ...
                if (result.Data || result.OperationType != DatabaseOperationType.None || result.OperationType != DatabaseOperationType.Failed)
                {
                    ...
                    // technologys.InsertUserTechnologys(technologys);
                    currencies = await UserCurrenciesService.Create().GetTechnologiesCurrencyAsync(subType);
