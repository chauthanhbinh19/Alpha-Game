...
public class RunesController : MonoBehaviour
{
    ...
    public async Task CreateRunesTradeAsync(List<Runes> runes, string subType, Transform currentContent, Transform currencyPanel, Transform popupPanel)
    {
        // Xóa bớt animation cũ nếu có để tránh lỗi chồng đè
        ...
        foreach (var rune in runes)
        {
            ...
            fileNameWithoutExtension = ImageHelper.RemoveImageExtension(rune.Currency.Image);
            ...
            currencyText.text = NumberFormatterHelper.FormatNumber(rune.Currency.Quantity, false);
    ...
    public void GetQuantity(double originPrice, object obj, string subType, Transform popupPanel, Transform currencyPanel)
    {
        ...
        confirmButton.onClick.AddListener(async () =>
        {
            ...
            if (obj is Runes rune)
            {
                ...
                var result = await UserRunesService.Create().InsertOrUpdateUserRuneAsync(User.CurrentUserId, rune);
