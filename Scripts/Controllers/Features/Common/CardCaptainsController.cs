    ...
    public void GetQuantity(double originPrice, object obj, string subType, Transform popupPanel, Transform currencyPanel)
    {
        ...
        confirmButton.onClick.AddListener(async () =>
        {
            ...
            if (obj is CardCaptains cardCaptain)
            {
                ...
                var result = await UserCardCaptainsService.Create().InsertOrUpdateUserCardCaptainAsync(User.CurrentUserId, cardCaptain);
                ...
                if (result.Data || result.OperationType != DatabaseOperationType.None || result.OperationType != DatabaseOperationType.Failed)
                {
                    ...
                    // cardCaptains.InsertUserCardCaptains(cardCaptains);
                    currencies = await UserCurrenciesService.Create().GetCardCaptainsCurrencyAsync(subType);
