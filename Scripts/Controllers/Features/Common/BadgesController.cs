...
public class BadgesController : MonoBehaviour
{
    ...
    public async Task CreateBadgesTradeAsync(List<Badges> badges, string subType, Transform currentContent, Transform currencyPanel, Transform popupPanel)
    {
        // Xóa bớt animation cũ nếu có để tránh lỗi chồng đè
        ...
        foreach (var badge in badges)
        {
            ...
            fileNameWithoutExtension = ImageHelper.RemoveImageExtension(badge.Currency.Image);
            ...
            currencyText.text = NumberFormatterHelper.FormatNumber(badge.Currency.Quantity, false);
    ...
    public void GetQuantity(double originPrice, object obj, string subType, Transform popupPanel, Transform currencyPanel)
    {
        ...
        confirmButton.onClick.AddListener(async () =>
        {
            ...
            if (obj is Badges badge)
            {
                ...
                var result = await UserBadgesService.Create().InsertOrUpdateUserBadgeAsync(User.CurrentUserId, badge);
                ...
                if (result.Data || result.OperationType != DatabaseOperationType.None || result.OperationType != DatabaseOperationType.Failed)
                {
                    ...
                    // badges.InsertUserBadges(badges);
                    currencies = await UserCurrenciesService.Create().GetBadgesCurrencyAsync(subType);
