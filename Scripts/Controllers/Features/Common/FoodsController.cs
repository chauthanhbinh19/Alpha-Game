...
{
    public static FoodsController Instance { get; private set; }
    ...
    public async Task CreateFoodsTradeAsync(List<Foods> foods, string subType, Transform currentContent, Transform currencyPanel, Transform popupPanel)
    {
        // Xóa bớt animation cũ nếu có để tránh lỗi chồng đè
        ...
        foreach (var food in foods)
        {
            ...
            fileNameWithoutExtension = ImageHelper.RemoveImageExtension(food.Currency.Image);
            ...
            currencyText.text = NumberFormatterHelper.FormatNumber(food.Currency.Quantity, false);
    ...
    public void GetQuantity(double originPrice, object obj, string subType, Transform popupPanel, Transform currencyPanel)
    {
        ...
        confirmButton.onClick.AddListener((UnityEngine.Events.UnityAction)(async () =>
        {
            ...
            if (obj is Foods food)
            {
                ...
                await UserCurrenciesService.Create().UpdateUserCurrencyAsync(User.CurrentUserId, food.Currency.Id, price);
                var result = await UserFoodsService.Create().InsertOrUpdateUserFoodAsync(User.CurrentUserId, food);
                ...
                if (result.Data || result.OperationType != DatabaseOperationType.None || result.OperationType != DatabaseOperationType.Failed)
                {
                    ...
                    // foods.InsertUserFoods(foods);
                    currencies = await UserCurrenciesService.Create().GetFoodsCurrencyAsync(subType);
                    fileNameWithoutExtension = ImageHelper.RemoveImageExtension(food.Image);
