...
{
    public static OutfitsController Instance { get; private set; }
    ...
    public async Task CreateOutfitsTradeAsync(List<Outfits> outfits, string subType, Transform currentContent, Transform currencyPanel, Transform popupPanel)
    {
        // Xóa bớt animation cũ nếu có để tránh lỗi chồng đè
        ...
        foreach (var outfit in outfits)
        {
            GameObject outfitObject = Instantiate(EquipmentShopPrefab, currentContent);
            Transform transform = outfitObject.transform;
            ...
            circleImage.color = ColorHelper.HexToColor(ColorConstants.ORANGE_COLOR);
            Outline bottomOutline = transform.Find("BottomImage").GetComponent<Outline>();
            bottomOutline.effectColor = ColorHelper.HexToColor(ColorConstants.ORANGE_COLOR);
            ...
            fileNameWithoutExtension = ImageHelper.RemoveImageExtension(outfit.Currency.Image);
            ...
            currencyText.text = NumberFormatterHelper.FormatNumber(outfit.Currency.Quantity, false);
            ...
            buttonBackgroundImage.color = ColorHelper.HexToColor(ColorConstants.ORANGE_COLOR);
            buyButton.onClick.AddListener(() =>
            {
                AudioManager.Instance.PlaySFX(AudioConstants.SFX.BUTTON_CLICK_SOUND);
                GetQuantity(outfit.Currency.Quantity, outfit, subType, popupPanel, currencyPanel);
            });
        }

        List<Currencies> currencies = new List<Currencies>();
        currencies = await UserCurrenciesService.Create().GetOutfitsCurrencyAsync(subType);
        ...
        confirmButton.onClick.AddListener(async () =>
        {
            ...
            if (obj is Outfits outfit)
            {
                ...
                if (allSuccess)
                {
                    ...
                    await OutfitsGalleryService.Create().InsertOutfitGalleryAsync(outfit.Id);
                    currencies = await UserCurrenciesService.Create().GetOutfitsCurrencyAsync(subType);
                    fileNameWithoutExtension = ImageHelper.RemoveImageExtension(outfit.Image);
