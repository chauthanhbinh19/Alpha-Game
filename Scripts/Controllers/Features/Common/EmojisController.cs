...
{
    public static EmojisController Instance { get; private set; }
    ...
    public async Task CreateEmojisTradeAsync(List<Emojis> emojis, string subType, Transform currentContent, Transform currencyPanel, Transform popupPanel)
    {
        // Xóa bớt animation cũ nếu có để tránh lỗi chồng đè
        ...
        foreach (var emoji in emojis)
        {
            ...
            fileNameWithoutExtension = ImageHelper.RemoveImageExtension(emoji.Currency.Image);
            ...
            currencyText.text = NumberFormatterHelper.FormatNumber(emoji.Currency.Quantity, false);
    ...
    public void GetQuantity(double originPrice, object obj, string subType, Transform popupPanel, Transform currencyPanel)
    {
        ...
        confirmButton.onClick.AddListener(async () =>
        {
            ...
            if (obj is Emojis emoji)
            {
                ...
                var result = await UserEmojisService.Create().InsertOrUpdateUserEmojiAsync(User.CurrentUserId, emoji);
                ...
                if (result.Data || result.OperationType != DatabaseOperationType.None || result.OperationType != DatabaseOperationType.Failed)
                {
                    ...
                    // emojis.InsertUserEmojis(emojis);
                    currencies = await UserCurrenciesService.Create().GetEmojisCurrencyAsync(subType);
