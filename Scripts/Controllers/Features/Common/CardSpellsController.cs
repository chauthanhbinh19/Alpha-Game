...
public class CardSpellsController : MonoBehaviour
{
    ...
    public async Task CreateCardSpellsTradeAsync(List<CardSpells> cardSpells, string subType, Transform currentContent, Transform currencyPanel, Transform popupPanel)
    {
        // Xóa bớt animation cũ nếu có để tránh lỗi chồng đè
        ...
        foreach (var cardSpell in cardSpells)
        {
            ...
            fileNameWithoutExtension = ImageHelper.RemoveImageExtension(cardSpell.Currency.Image);
            ...
            currencyText.text = NumberFormatterHelper.FormatNumber(cardSpell.Currency.Quantity, false);
        ...
        currencies = await UserCurrenciesService.Create().GetCardSpellsCurrencyAsync(subType);
    ...
    public void GetQuantity(double originPrice, object obj, string subType, Transform popupPanel, Transform currencyPanel)
    {
        ...
        confirmButton.onClick.AddListener(async () =>
        {
            ...
            if (obj is CardSpells cardSpell)
            {
                ...
                var result = await UserCardSpellsService.Create().InsertOrUpdateUserCardSpellAsync(User.CurrentUserId, cardSpell);
                ...
                if (result.Data || result.OperationType != DatabaseOperationType.None || result.OperationType != DatabaseOperationType.Failed)
                {
                    ...
                    // cardSpells.InsertUserCardSpells(cardSpells);
                    currencies = await UserCurrenciesService.Create().GetCardSpellsCurrencyAsync(subType);
