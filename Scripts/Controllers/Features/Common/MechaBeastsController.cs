...
public class MechaBeastsController : MonoBehaviour
{
    ...
    public async Task CreateMechaBeastsTradeAsync(List<MechaBeasts> mechaBeasts, string subType, Transform currentContent, Transform currencyPanel, Transform popupPanel)
    {
        // Xóa bớt animation cũ nếu có để tránh lỗi chồng đè
        ...
        foreach (var mechaBeast in mechaBeasts)
        {
            ...
            fileNameWithoutExtension = ImageHelper.RemoveImageExtension(mechaBeast.Currency.Image);
            ...
            currencyText.text = NumberFormatterHelper.FormatNumber(mechaBeast.Currency.Quantity, false);
    ...
    public void GetQuantity(double originPrice, object obj, string subType, Transform popupPanel, Transform currencyPanel)
    {
        ...
        maxButton.onClick.AddListener(async () =>
        {
            ...
            {
                userCurrency = await UserCurrenciesService.Create().GetUserCurrencyByIdAsync(User.CurrentUserId, mechaBeast.Currency.Id);
            }
        ...
        confirmButton.onClick.AddListener(async () =>
        {
            ...
            if (obj is MechaBeasts mechaBeast)
            {
                ...
                await UserCurrenciesService.Create().UpdateUserCurrencyAsync(User.CurrentUserId, mechaBeast.Currency.Id, price);
                var result = await UserMechaBeastsService.Create().InsertOrUpdateUserMechaBeastAsync(User.CurrentUserId, mechaBeast);
                ...
                if (result.Data || result.OperationType != DatabaseOperationType.None || result.OperationType != DatabaseOperationType.Failed)
                {
                    ...
                    // mechaBeasts.InsertUserMechaBeasts(mechaBeasts);
                    currencies = await UserCurrenciesService.Create().GetMechaBeastsCurrencyAsync(subType);
