...
public class ArtworksController : MonoBehaviour
{
    ...
    public async Task CreateArtworksTradeAsync(List<Artworks> artworks, string subType, Transform currentContent, Transform currencyPanel, Transform popupPanel)
    {
        ...
        foreach (var artwork in artworks)
        {
            ...
            fileNameWithoutExtension = ImageHelper.RemoveImageExtension(artwork.Currency.Image);
            ...
            currencyText.text = NumberFormatterHelper.FormatNumber(artwork.Currency.Quantity, false);
        ...
        currencies = await UserCurrenciesService.Create().GetArtworksCurrencyAsync(subType);
    ...
    public void GetQuantity(double originPrice, object obj, string subType, Transform popupPanel, Transform currencyPanel)
    {
        ...
        confirmButton.onClick.AddListener(async () =>
        {
            ...
            if (obj is Artworks artwork)
            {
                ...
                var result = await UserArtworksService.Create().InsertOrUpdateUserArtworkAsync(User.CurrentUserId, artwork);
                ...
                if (result.Data || result.OperationType != DatabaseOperationType.None || result.OperationType != DatabaseOperationType.Failed)
                {
                    ...
                    // artworks.InsertUserArtworks(artworks);
                    currencies = await UserCurrenciesService.Create().GetArtworksCurrencyAsync(subType);
                    fileNameWithoutExtension = ImageHelper.RemoveImageExtension(artwork.Image);
