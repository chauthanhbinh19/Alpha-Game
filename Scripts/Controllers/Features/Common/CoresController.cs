...
public class CoresController : MonoBehaviour
{
    ...
    public async Task CreateCoresTradeAsync(List<Cores> cores, string subType, Transform currentContent, Transform currencyPanel, Transform popupPanel)
    {
        // Xóa bớt animation cũ nếu có để tránh lỗi chồng đè
        ...
        foreach (var core in cores)
        {
            ...
            fileNameWithoutExtension = ImageHelper.RemoveImageExtension(core.Currency.Image);
            ...
            currencyText.text = NumberFormatterHelper.FormatNumber(core.Currency.Quantity, false);
    ...
    public void GetQuantity(double originPrice, object obj, string subType, Transform popupPanel, Transform currencyPanel)
    {
        ...
        confirmButton.onClick.AddListener(async () =>
        {
            ...
            if (obj is Cores core)
            {
                ...
                var result = await UserCoresService.Create().InsertOrUpdateUserCoreAsync(User.CurrentUserId, core);
