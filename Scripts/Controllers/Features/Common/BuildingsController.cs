...
public class BuildingsController : MonoBehaviour
{
    ...
    public async Task CreateBuildingsTradeAsync(List<Buildings> buildings, string subType, Transform currentContent, Transform currencyPanel, Transform popupPanel)
    {
        // Xóa bớt animation cũ nếu có để tránh lỗi chồng đè
        ...
        foreach (var building in buildings)
        {
            ...
            fileNameWithoutExtension = ImageHelper.RemoveImageExtension(building.Currency.Image);
            ...
            currencyText.text = NumberFormatterHelper.FormatNumber(building.Currency.Quantity, false);
    ...
    public void GetQuantity(double originPrice, object obj, string subType, Transform popupPanel, Transform currencyPanel)
    {
        ...
        confirmButton.onClick.AddListener(async () =>
        {
            ...
            if (obj is Buildings building)
            {
                ...
                var result = await UserBuildingsService.Create().InsertOrUpdateUserBuildingAsync(User.CurrentUserId, building);
                ...
                if (result.Data || result.OperationType != DatabaseOperationType.None || result.OperationType != DatabaseOperationType.Failed)
                {
                    ...
                    // buildings.InsertUserBuildings(buildings);
                    currencies = await UserCurrenciesService.Create().GetBuildingsCurrencyAsync(subType);
