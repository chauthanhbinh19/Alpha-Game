...
public class CardLivesController : MonoBehaviour
{
    public static CardLivesController Instance { get; private set; }
    ...
    public async Task CreateCardLivesTradeAsync(List<CardLives> cardLives, string subType, Transform currentContent, Transform currencyPanel, Transform popupPanel)
    {
        // Xóa bớt animation cũ nếu có để tránh lỗi chồng đè
        ...
        foreach (var cardLife in cardLives)
        {
            ...
            fileNameWithoutExtension = ImageHelper.RemoveImageExtension(cardLife.Currency.Image);
            ...
            currencyText.text = NumberFormatterHelper.FormatNumber(cardLife.Currency.Quantity, false);
        ...
        currencies = await UserCurrenciesService.Create().GetCardLivesCurrencyAsync(subType);
    ...
    public void GetQuantity(double originPrice, object obj, string subType, Transform popupPanel, Transform currencyPanel)
    {
        ...
        confirmButton.onClick.AddListener(async () =>
        {
            ...
            if (obj is CardLives cardLife)
            {
                ...
                var result = await UserCardLivesService.Create().InsertOrUpdateUserCardLifeAsync(User.CurrentUserId, cardLife);
                ...
                if (result.Data || result.OperationType != DatabaseOperationType.None || result.OperationType != DatabaseOperationType.Failed)
                {
                    ...
                    // cardLifes.InsertUserCardLifes(cardLifes);
                    currencies = await UserCurrenciesService.Create().GetCardLivesCurrencyAsync(subType);
