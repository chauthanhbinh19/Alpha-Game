...
public class PuppetsController : MonoBehaviour
{
    ...
    public async Task CreatePuppetsTradeAsync(List<Puppets> puppets, string subType, Transform currentContent, Transform currencyPanel, Transform popupPanel)
    {
        // Xóa bớt animation cũ nếu có để tránh lỗi chồng đè
        ...
        foreach (var puppet in puppets)
        {
            ...
            fileNameWithoutExtension = ImageHelper.RemoveImageExtension(puppet.Currency.Image);
            ...
            currencyText.text = NumberFormatterHelper.FormatNumber(puppet.Currency.Quantity, false);
    ...
    public void GetQuantity(double originPrice, object obj, string subType, Transform popupPanel, Transform currencyPanel)
    {
        ...
        confirmButton.onClick.AddListener(async () =>
        {
            ...
            if (obj is Puppets puppet)
            {
                ...
                var result = await UserPuppetsService.Create().InsertOrUpdateUserPuppetAsync(User.CurrentUserId, puppet);
                ...
                if (result.Data || result.OperationType != DatabaseOperationType.None || result.OperationType != DatabaseOperationType.Failed)
                {
                    ...
                    // puppets.InsertUserPuppets(puppets);
                    currencies = await UserCurrenciesService.Create().GetPuppetsCurrencyAsync(subType);
