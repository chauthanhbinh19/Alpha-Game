...
{
    public static BeveragesController Instance { get; private set; }
    ...
    public async Task CreateBeveragesTradeAsync(List<Beverages> beverages, string subType, Transform currentContent, Transform currencyPanel, Transform popupPanel)
    {
        // Xóa bớt animation cũ nếu có để tránh lỗi chồng đè
        ...
        foreach (var beverage in beverages)
        {
            ...
            fileNameWithoutExtension = ImageHelper.RemoveImageExtension(beverage.Currency.Image);
            ...
            currencyText.text = NumberFormatterHelper.FormatNumber(beverage.Currency.Quantity, false);
    ...
    public void GetQuantity(double originPrice, object obj, string subType, Transform popupPanel, Transform currencyPanel)
    {
        ...
        confirmButton.onClick.AddListener((UnityEngine.Events.UnityAction)(async () =>
        {
            ...
            if (obj is Beverages beverage)
            {
                ...
                await UserCurrenciesService.Create().UpdateUserCurrencyAsync(User.CurrentUserId, beverage.Currency.Id, price);
                var result = await UserBeveragesService.Create().InsertOrUpdateUserBeverageAsync(User.CurrentUserId, beverage);
                ...
                if (result.Data || result.OperationType != DatabaseOperationType.None || result.OperationType != DatabaseOperationType.Failed)
                {
                    ...
                    // beverages.InsertUserBeverages(beverages);
                    currencies = await UserCurrenciesService.Create().GetBeveragesCurrencyAsync(subType);
