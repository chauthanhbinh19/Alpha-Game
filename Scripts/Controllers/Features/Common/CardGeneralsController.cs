...
public class CardGeneralsController : MonoBehaviour
{
    ...
    public async Task CreateCardGeneralsTradeAsync(List<CardGenerals> cardGenerals, string subType, Transform currentContent, Transform currencyPanel, Transform popupPanel)
    {
        // Xóa bớt animation cũ nếu có để tránh lỗi chồng đè
        ...
        foreach (var cardGeneral in cardGenerals)
        {
            ...
            fileNameWithoutExtension = ImageHelper.RemoveImageExtension(cardGeneral.Currency.Image);
            ...
            currencyText.text = NumberFormatterHelper.FormatNumber(cardGeneral.Currency.Quantity, false);
    ...
    public void GetQuantity(double originPrice, object obj, string subType, Transform popupPanel, Transform currencyPanel)
    {
        ...
        confirmButton.onClick.AddListener(async () =>
        {
            ...
            if (obj is CardGenerals cardGeneral)
            {
                ...
                var result = await UserCardGeneralsService.Create().InsertOrUpdateUserCardGeneralAsync(User.CurrentUserId, cardGeneral);
                ...
                if (result.Data || result.OperationType != DatabaseOperationType.None || result.OperationType != DatabaseOperationType.Failed)
                {
                    ...
                    // cardGenerals.InsertUserCardGenerals(cardGenerals);
                    currencies = await UserCurrenciesService.Create().GetCardGeneralsCurrencyAsync(subType);
