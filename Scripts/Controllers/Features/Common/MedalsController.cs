...
public class MedalsController : MonoBehaviour
{
    ...
    public async Task CreateMedalsTradeAsync(List<Medals> medals, string subType, Transform currentContent, Transform currencyPanel, Transform popupPanel)
    {
        // Xóa bớt animation cũ nếu có để tránh lỗi chồng đè
    ...
    public void GetQuantity(double originPrice, object obj, string subType, Transform popupPanel, Transform currencyPanel)
    {
        ...
        confirmButton.onClick.AddListener(async () =>
        {
            ...
            if (obj is Medals medal)
            {
                ...
                var result = await UserMedalsService.Create().InsertOrUpdateUserMedalAsync(User.CurrentUserId, medal);
                ...
                if (result.Data || result.OperationType != DatabaseOperationType.None || result.OperationType != DatabaseOperationType.Failed)
                {
                    ...
                    // medals.InsertUserMedals(medals);
                    currencies = await UserCurrenciesService.Create().GetMedalsCurrencyAsync(subType);
