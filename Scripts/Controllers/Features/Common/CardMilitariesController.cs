...
public class CardMilitariesController : MonoBehaviour
{
    ...
    public async Task CreateCardMilitariesTradeAsync(List<CardMilitaries> cardMilitaries, string subType, Transform currentContent, Transform currencyPanel, Transform popupPanel)
    {
        ...
        foreach (var cardMilitary in cardMilitaries)
        {
            ...
            fileNameWithoutExtension = ImageHelper.RemoveImageExtension(cardMilitary.Currency.Image);
            ...
            currencyText.text = NumberFormatterHelper.FormatNumber(cardMilitary.Currency.Quantity, false);
    ...
    public void GetQuantity(double originPrice, object obj, string subType, Transform popupPanel, Transform currencyPanel)
    {
        ...
        confirmButton.onClick.AddListener(async () =>
        {
            ...
            if (obj is CardMilitaries cardMilitary)
            {
                ...
                var result = await UserCardMilitariesService.Create().InsertOrUpdateUserCardMilitaryAsync(User.CurrentUserId, cardMilitary);
                ...
                if (result.Data || result.OperationType != DatabaseOperationType.None || result.OperationType != DatabaseOperationType.Failed)
                {
                    ...
                    // cardMilitarys.InsertUserCardMilitarys(cardMilitarys);
                    currencies = await UserCurrenciesService.Create().GetCardMilitariesCurrencyAsync(subType);
                    fileNameWithoutExtension = ImageHelper.RemoveImageExtension(cardMilitary.Image);
