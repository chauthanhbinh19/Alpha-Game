...
public class ForgesController : MonoBehaviour
{
    ...
    public async Task CreateForgesTradeAsync(List<Forges> forges, string subType, Transform currentContent, Transform currencyPanel, Transform popupPanel)
    {
        // Xóa bớt animation cũ nếu có để tránh lỗi chồng đè
        ...
        foreach (var forge in forges)
        {
            ...
            fileNameWithoutExtension = ImageHelper.RemoveImageExtension(forge.Currency.Image);
            ...
            currencyText.text = NumberFormatterHelper.FormatNumber(forge.Currency.Quantity, false);
    ...
    public void GetQuantity(double originPrice, object obj, string subType, Transform popupPanel, Transform currencyPanel)
    {
        ...
        confirmButton.onClick.AddListener(async () =>
        {
            ...
            if (obj is Forges forge)
            {
                ...
                var result = await UserForgesService.Create().InsertOrUpdateUserForgeAsync(User.CurrentUserId, forge);
                ...
                if (result.Data || result.OperationType != DatabaseOperationType.None || result.OperationType != DatabaseOperationType.Failed)
                {
                    ...
                    // forges.InsertUserForges(forges);
                    currencies = await UserCurrenciesService.Create().GetForgesCurrencyAsync(subType);
