...
{
    public static ArtifactsController Instance { get; private set; }
    ...
    public async Task CreateArtifactsTradeAsync(List<Artifacts> artifacts, string subType, Transform currentContent, Transform currencyPanel, Transform popupPanel)
    {
        // Xóa bớt animation cũ nếu có để tránh lỗi chồng đè
        ...
        foreach (var artifact in artifacts)
        {
            ...
            fileNameWithoutExtension = ImageHelper.RemoveImageExtension(artifact.Currency.Image);
            ...
            currencyText.text = NumberFormatterHelper.FormatNumber(artifact.Currency.Quantity, false);
    ...
    public void GetQuantity(double originPrice, object obj, string subType, Transform popupPanel, Transform currencyPanel)
    {
        ...
        confirmButton.onClick.AddListener(async () =>
        {
            ...
            if (obj is Artifacts artifact)
            {
                ...
                var result = await UserArtifactsService.Create().InsertOrUpdateUserArtifactAsync(User.CurrentUserId, artifact);
                ...
                if (result.Data || result.OperationType != DatabaseOperationType.None || result.OperationType != DatabaseOperationType.Failed)
                {
                    ...
                    // artifacts.InsertUserArtifacts(artifacts);
                    currencies = await UserCurrenciesService.Create().GetArtifactsCurrencyAsync(subType);
