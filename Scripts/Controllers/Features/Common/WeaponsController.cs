...
public class WeaponsController : MonoBehaviour
{
    ...
    public async Task CreateWeaponsTradeAsync(List<Weapons> weapons, string subType, Transform currentContent, Transform currencyPanel, Transform popupPanel)
    {
        // Xóa bớt animation cũ nếu có để tránh lỗi chồng đè
        ...
        foreach (var weapon in weapons)
        {
            ...
            fileNameWithoutExtension = ImageHelper.RemoveImageExtension(weapon.Currency.Image);
            ...
            currencyText.text = NumberFormatterHelper.FormatNumber(weapon.Currency.Quantity, false);
    ...
    public void GetQuantity(double originPrice, object obj, string subType, Transform popupPanel, Transform currencyPanel)
    {
        ...
        confirmButton.onClick.AddListener(async () =>
        {
            ...
            if (obj is Weapons weapon)
            {
                ...
                var result = await UserWeaponsService.Create().InsertOrUpdateUserWeaponAsync(User.CurrentUserId, weapon);
                ...
                if (result.Data || result.OperationType != DatabaseOperationType.None || result.OperationType != DatabaseOperationType.Failed)
                {
                    ...
                    // weapons.InsertUserWeapons(weapons);
                    currencies = await UserCurrenciesService.Create().GetWeaponsCurrencyAsync(subType);
