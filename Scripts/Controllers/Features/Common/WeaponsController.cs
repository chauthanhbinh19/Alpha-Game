...
public class WeaponsController : MonoBehaviour
{
    ...
    public void CreateWeaponsGallery(List<Weapons> WeaponsList, Transform DictionaryContentPanel)
    {
        foreach (var weapon in WeaponsList)
        {
            GameObject weaponObject = Instantiate(equipmentsPrefab, DictionaryContentPanel);
    ...
    Transform currencyPanel, Transform popupPanel)
    {
        foreach (var weapon in WeaponsList)
        {
            ...
            Outline middleOutline = weaponObject.transform.Find("MiddleImage").GetComponent<Outline>();
            bottomOutline.effectColor = ColorHelper.ToColor(ColorConstants.BLUE_COLOR);

            RawImage currencyImage = weaponObject.transform.Find("CurrencyImage").GetComponent<RawImage>();
            fileNameWithoutExtension = ImageExtensionHandler.RemoveImageExtension(weapon.Currency.Image);
            Texture currencyTexture = Resources.Load<Texture>($"{fileNameWithoutExtension}");
            currencyImage.texture = currencyTexture;

            TextMeshProUGUI currencyText = weaponObject.transform.Find("CurrencyText").GetComponent<TextMeshProUGUI>();
            currencyText.text = NumberFormatter.FormatNumber(weapon.Currency.Quantity, false);

            Button buy = weaponObject.transform.Find("Buy").GetComponent<Button>();
            ...
            {
                AudioManager.Instance.PlaySFX(AudioConstants.SFX.BUTTON_CLICK_SOUND);
                GetQuantity(weapon.Currency.Quantity, weapon, subType, popupPanel, currencyPanel);
            });
        }
    ...
    public void GetQuantity(double originPrice, object obj, string subType, Transform popupPanel, Transform currencyPanel)
    {
        ...
        confirmButton.onClick.AddListener(() =>
        {
            ...
            if (obj is Weapons Weapons)
            {
                Weapons.Quantity = Weapons.Quantity + quantity;
                UserCurrencyService.Create().UpdateUserCurrency(Weapons.Currency.Id, price);
                bool success = UserWeaponsService.Create().InsertUserWeapons(Weapons);
                if (!success)
                {
                    allSuccess = false;
                }
                ...
                if (allSuccess)
                {
                    ...
                    WeaponsGalleryService.Create().InsertWeaponsGallery(Weapons.Id);
                    currencies = UserCurrencyService.Create().GetWeaponsCurrency(subType);
                    fileNameWithoutExtension = ImageExtensionHandler.RemoveImageExtension(Weapons.Image);
