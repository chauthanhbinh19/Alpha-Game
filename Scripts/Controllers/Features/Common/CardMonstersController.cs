...
public class CardMonstersController : MonoBehaviour
{
    ...
    public void GetQuantity(double originPrice, object obj, string subType, Transform popupPanel, Transform currencyPanel)
    {
        ...
        maxButton.onClick.AddListener(async () =>
        {
            ...
            {
                userCurrency = await UserCurrenciesService.Create().GetUserCurrencyByIdAsync(User.CurrentUserId, cardMonster.Currency.Id);
            }
        ...
        confirmButton.onClick.AddListener(async () =>
        {
            ...
            if (obj is CardMonsters cardMonster)
            {
                cardMonster.Quantity = cardMonster.Quantity + quantity;
                await UserCurrenciesService.Create().UpdateUserCurrencyAsync(User.CurrentUserId, cardMonster.Currency.Id, price);
                var result = await UserCardMonstersService.Create().InsertOrUpdateUserCardMonsterAsync(User.CurrentUserId, cardMonster);
                ...
                if (result.Data || result.OperationType != DatabaseOperationType.None || result.OperationType != DatabaseOperationType.Failed)
                {
                    ...
                    // cardMonsters.InsertUserCardMonsters(cardMonsters);
                    currencies = await UserCurrenciesService.Create().GetCardMonstersCurrencyAsync(subType);
