...
public class SpiritBeastsController : MonoBehaviour
{
    public static SpiritBeastsController Instance { get; private set; }
    ...
    public async Task CreateSpiritBeastsTradeAsync(List<SpiritBeasts> spiritBeasts, string subType, Transform currentContent, Transform currencyPanel, Transform popupPanel)
    {
        // Xóa bớt animation cũ nếu có để tránh lỗi chồng đè
        ...
        foreach (var spiritBeast in spiritBeasts)
        {
            ...
            circleImage.color = ColorHelper.HexToColor(ColorConstants.GRAY_COLOR);
            ...
            fileNameWithoutExtension = ImageHelper.RemoveImageExtension(spiritBeast.Currency.Image);
            ...
            currencyText.text = NumberFormatterHelper.FormatNumber(spiritBeast.Currency.Quantity, false);
            ...
            buttonBackgroundImage.color = ColorHelper.HexToColor(ColorConstants.GRAY_COLOR);
            buyButton.onClick.AddListener(() =>
            {
                AudioManager.Instance.PlaySFX(AudioConstants.SFX.BUTTON_CLICK_SOUND);
                GetQuantity(spiritBeast.Currency.Quantity, spiritBeast, subType, popupPanel, currencyPanel);
            });
        }
    ...
    public void GetQuantity(double originPrice, object obj, string subType, Transform popupPanel, Transform currencyPanel)
    {
        ...
        confirmButton.onClick.AddListener(async () =>
        {
            ...
            if (obj is SpiritBeasts spiritBeast)
            {
                ...
                var result = await UserSpiritBeastsService.Create().InsertOrUpdateUserSpiritBeastAsync(User.CurrentUserId, spiritBeast);
                ...
                if (result.Data || result.OperationType != DatabaseOperationType.None || result.OperationType != DatabaseOperationType.Failed)
                {
                    ...
                    // spiritBeasts.InsertUserSpiritBeasts(spiritBeasts);
                    currencies = await UserCurrenciesService.Create().GetSpiritBeastsCurrencyAsync(subType);
