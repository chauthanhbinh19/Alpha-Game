...
public class CollaborationsController : MonoBehaviour
{
    ...
    public async Task CreateCollaborationTradeAsync(List<Collaborations> collaborations, string subType, Transform currentContent, Transform currencyPanel, Transform popupPanel)
    {
        // Xóa bớt animation cũ nếu có để tránh lỗi chồng đè
        ...
        foreach (var collaboration in collaborations)
        {
            GameObject collaborationObject = Instantiate(EquipmentShopPrefab, currentContent);
            Transform transform = collaborationObject.transform;
            ...
            fileNameWithoutExtension = ImageHelper.RemoveImageExtension(collaboration.Currency.Image);
            ...
            currencyText.text = NumberFormatterHelper.FormatNumber(collaboration.Currency.Quantity, false);
    ...
    public void GetQuantity(double originPrice, object obj, string subType, Transform popupPanel, Transform currencyPanel)
    {
        ...
        confirmButton.onClick.AddListener(async () =>
        {
            ...
            if (obj is Collaborations collaboration)
            {
                ...
                var result = await UserCollaborationsService.Create().InsertOrUpdateUserCollaborationAsync(User.CurrentUserId, collaboration);
                ...
                if (result.Data || result.OperationType != DatabaseOperationType.None || result.OperationType != DatabaseOperationType.Failed)
                {
                    ...
                    // collaborations.InsertUserCollaborations(collaborations);
                    currencies = await UserCurrenciesService.Create().GetCollaborationsCurrencyAsync(subType);
