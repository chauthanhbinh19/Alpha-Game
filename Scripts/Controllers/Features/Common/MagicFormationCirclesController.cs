...
public class MagicFormationCirclesController : MonoBehaviour
{
    ...
    public void Initialize()
    {
        ...
        MagicFormationCircleButtonPrefab = UIManager.Instance.Get(AppConstants.Prefab.Component.MAGIC_FORMATION_CIRCLE_BUTTON_PREFAB);
    ...
    public async Task CreateMagicFormationCirclesTradeAsync(List<MagicFormationCircles> magicFormationCircles, string subType, Transform currentContent, Transform currencyPanel, Transform popupPanel)
    {
        ...
        foreach (var magicFormationCircle in magicFormationCircles)
        {
            ...
            fileNameWithoutExtension = ImageHelper.RemoveImageExtension(magicFormationCircle.Currency.Image);
            ...
            currencyText.text = NumberFormatterHelper.FormatNumber(magicFormationCircle.Currency.Quantity, false);
    ...
    public void GetQuantity(double originPrice, object obj, string subType, Transform popupPanel, Transform currencyPanel)
    {
        ...
        confirmButton.onClick.AddListener(async () =>
        {
            ...
            if (obj is MagicFormationCircles magicFormationCircle)
            {
                ...
                var result = await UserMagicFormationCirclesService.Create().InsertOrUpdateUserMagicFormationCircleAsync(User.CurrentUserId, magicFormationCircle);
                ...
                if (result.Data || result.OperationType != DatabaseOperationType.None || result.OperationType != DatabaseOperationType.Failed)
                {
                    ...
                    // magicFormationCircles.InsertUserMagicFormationCircles(magicFormationCircles);
                    currencies = await UserCurrenciesService.Create().GetMagicFormationCirclesCurrencyAsync(subType);
