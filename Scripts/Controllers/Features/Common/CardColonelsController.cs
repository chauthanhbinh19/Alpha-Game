...
public class CardColonelsController : MonoBehaviour
{
    ...
    Transform currencyPanel, Transform popupPanel)
    {
        foreach (var cardColonel in cardColonels)
        {
            ...
            // Gán sự kiện click
            ButtonEvent.Instance.AddClickListener(eventTrigger, () => FindObjectOfType<PopupDetailsManager>().PopupDetails(cardColonel, MainPanel));
            // Thêm sự kiện Scroll để chuyển tiếp sự kiện cuộn
            EventTrigger.Entry scrollEntry = new EventTrigger.Entry { eventID = EventTriggerType.Scroll };
            scrollEntry.callback.AddListener((eventData) =>
            {
            ...
            currencyText.text = cardColonel.currency.quantity.ToString();
