...
public class CardColonelsController : MonoBehaviour
{
    ...
    public async Task CreateCardColonelsTradeAsync(List<CardColonels> cardColonels, string subType, Transform currentContent, Transform currencyPanel, Transform popupPanel)
    {
        // Xóa bớt animation cũ nếu có để tránh lỗi chồng đè
        ...
        foreach (var cardColonel in cardColonels)
        {
            ...
            fileNameWithoutExtension = ImageHelper.RemoveImageExtension(cardColonel.Currency.Image);
            ...
            currencyText.text = NumberFormatterHelper.FormatNumber(cardColonel.Currency.Quantity, false);
    ...
    public void GetQuantity(double originPrice, object obj, string subType, Transform popupPanel, Transform currencyPanel)
    {
        ...
        maxButton.onClick.AddListener(async () =>
        {
            ...
            {
                userCurrency = await UserCurrenciesService.Create().GetUserCurrencyByIdAsync(User.CurrentUserId, cardColonel.Currency.Id);
            }
        ...
        confirmButton.onClick.AddListener(async () =>
        {
            ...
            if (obj is CardColonels cardColonel)
            {
                cardColonel.Quantity = cardColonel.Quantity;
                await UserCurrenciesService.Create().UpdateUserCurrencyAsync(User.CurrentUserId, cardColonel.Currency.Id, price);
                bool success = await UserCardColonelsService.Create().InsertUserCardColonelAsync(User.CurrentUserId, cardColonel);
                if (!success)
                {
                    allSuccess = false;
                }
                ...
                if (allSuccess)
                {
                    ...
                    await CardColonelsGalleryService.Create().InsertCardColonelGalleryAsync(cardColonel.Id);
                    currencies = await UserCurrenciesService.Create().GetCardColonelsCurrencyAsync(subType);
