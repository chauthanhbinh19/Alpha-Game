...
public class CardColonelsController : MonoBehaviour
{
    ...
    public async Task CreateCardColonelsTradeAsync(List<CardColonels> cardColonels, string subType, Transform currentContent,
    Transform currencyPanel, Transform popupPanel)
    {
        foreach (var cardColonel in cardColonels)
        {
            ...
            RawImage circleImage = cardColonelObject.transform.Find("BackgroundContent/CircleImage").GetComponent<RawImage>();
            circleImage.color = ColorHelper.ToColor(ColorConstants.GREEN_COLOR);
            Outline bottomOutline = cardColonelObject.transform.Find("BottomImage").GetComponent<Outline>();
            bottomOutline.effectColor = ColorHelper.ToColor(ColorConstants.GREEN_COLOR);
            Outline middleOutline = cardColonelObject.transform.Find("MiddleImage").GetComponent<Outline>();
            bottomOutline.effectColor = ColorHelper.ToColor(ColorConstants.GREEN_COLOR);
            ...
            {
                AudioManager.Instance.PlaySFX(AudioConstants.SFX.BUTTON_CLICK_SOUND);
                GetQuantity(cardColonel.Currency.Quantity, cardColonel, subType, popupPanel, currencyPanel);
            });
        }

        List<Currencies> currencies = new List<Currencies>();
        currencies = await UserCurrenciesService.Create().GetCardColonelsCurrencyAsync(subType);
    ...
    public void GetQuantity(double originPrice, object obj, string subType, Transform popupPanel, Transform currencyPanel)
    {
        ...
        maxButton.onClick.AddListener(async () =>
        {
            ...
            {
                userCurrency = await UserCurrenciesService.Create().GetUserCurrencyByIdAsync(cardColonels.Currency.Id);
            }
        ...
        confirmButton.onClick.AddListener(async () =>
        {
            ...
            if (obj is CardColonels cardColonels)
            {
                cardColonels.Quantity = cardColonels.Quantity;
                await UserCurrenciesService.Create().UpdateUserCurrencyAsync(cardColonels.Currency.Id, price);
                bool success = await UserCardColonelsService.Create().InsertUserCardColonelAsync(cardColonels);
                if (!success)
                {
                    allSuccess = false;
                }
                ...
                if (allSuccess)
                {
                    ...
                    await CardColonelsGalleryService.Create().InsertCardColonelGalleryAsync(cardColonels.Id);
                    currencies = await UserCurrenciesService.Create().GetCardColonelsCurrencyAsync(subType);
