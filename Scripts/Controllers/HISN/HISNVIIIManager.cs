...
public class HISNVIIIManager : MonoBehaviour
{
    ...
    private List<KeyValuePair<string, FeatureHISNDTO>> FeatureList;
        ...
        for (int i = start; i < end; i++)
        {
            ...
            if (btn != null)
            {
                ...
                btn.onClick.AddListener(async () =>
                {
                    ...
                    AudioManager.Instance.PlaySFX(AudioConstants.SFX.BUTTON_CLICK_SOUND);
                    await CreateMainHISNPanelAsync(currentFeatureId, currentSubtype);
                });
            }
        }
    }
    ...
    public async Task CreateMainHISNPanelAsync(string featureId, string featureName)
    {
        ...
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.HISN.HISN_VIII_BACKGROUND_URL);
        ...
        int currentLevel = userHISN?.Level ?? 0;
        ...
        async Task RefreshPanelAsync()
        {
            ...
            for (int i = 0; i < refreshedTotal; i++)
            {
                ...
                GameObject itemGO = Instantiate(HISNItemPrefab, parent);
                SetupHISNItemUI(itemGO, refreshedRecipeItems[i]);
            }
        }
