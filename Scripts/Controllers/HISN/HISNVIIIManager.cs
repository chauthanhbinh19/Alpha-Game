...
public class HISNVIIIManager : MonoBehaviour
{
    public static HISNVIIIManager Instance { get; private set; }
    ...
    private List<KeyValuePair<string, FeatureHISNDTO>> _featureList;
    ...
    public async Task CreateHISNVIIIManagerAsync()
    {
        GameObject currentObject = Instantiate(PopupHISNPanelPrefab, MainPanel);
        ...
        uniqueTypes = await FeaturesService.Create().GetHISNFeaturesByTypeAsync(AppConstants.HISN.HISN_VIII);
    ...
    private void RenderPage()
    {
        ...
        for (int i = start; i < end; i++)
        {
            ...
            GameObject button = Instantiate(PopupHISNButtonPrefab, content);
            ...
            {
                AudioManager.Instance.PlaySFX(AudioConstants.SFX.BUTTON_CLICK_SOUND);
                await CreateMainHISNPanelAsync(featureId, subtype);
            });
        }
    }
    ...
    public async Task CreateMainHISNPanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.HISN.HISN_VIII}");
        rankImage.texture = rankTexture;

        List<RecipeItemDto> recipeItems = await RecipeService.Create().GetRecipeItemsAsync(featureName, User.CurrentUserLevel, User.CurrentUserId);
        HISNs research = await HISNsService.Create().GetHISNsAsync(featureId);
        ...
        for (int i = 0; i < total; i++)
        {
            ...
            GameObject itemGO = Instantiate(HISNItemPrefab, parent);

            SetupHISNItemUI(itemGO, recipeItems[i]);
        }
        ...
        upgradeOneLevelButton.onClick.AddListener(async () =>
        {
            ...
            if (result.Success)
            {
                research = HISNsService.Create().EnhanceHISNs(research, result.UpgradedLevels, 1000);
                await HISNsService.Create().InsertOrUpdateHISNsAsync(User.CurrentUserId, research, featureId);
                Destroy(currentObject);
                ...
                await CreateMainHISNPanelAsync(featureId, featureName);
            }
            else
            {
    ...
    private void SetupHISNItemUI(GameObject itemGO,RecipeItemDto data)
    {
