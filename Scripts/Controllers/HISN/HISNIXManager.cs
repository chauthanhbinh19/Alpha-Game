...
public class HISNIXManager : MonoBehaviour
{
    public static HISNIXManager Instance { get; private set; }
    ...
    public async Task CreateHISNIXManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetHISNFeaturesByTypeAsync(AppConstants.HISN.HISN_IX);
    ...
    public async Task CreateMainHISNPanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.HISN.HISN_IX}");
