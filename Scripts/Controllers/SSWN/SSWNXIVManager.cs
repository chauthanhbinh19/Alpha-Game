...
public class SSWNXIVManager : MonoBehaviour
{
    public static SSWNXIVManager Instance { get; private set; }
    ...
    public async Task CreateSSWNXIVManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetSSWNFeaturesByTypeAsync(AppConstants.SSWN.SSWN_XIV);
    ...
    public async Task CreateMainSSWNPanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = Resources.Load<Texture2D>($"UI/Rank_Research/{AppConstants.SSWN.SSWN_XIV}");
