...
public class SSWNXIIIManager : MonoBehaviour
{
    public static SSWNXIIIManager Instance { get; private set; }
    ...
    public async Task CreateSSWNXIIIManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetSSWNFeaturesByTypeAsync(AppConstants.SSWN.SSWN_XIII);
    ...
    public async Task CreateMainSSWNPanelAsync(string featureId, string featureName)
    {
        ...
        Texture mapTexture = Resources.Load<Texture2D>("UI/Background2/Chapter_14");
        mapImage.texture = mapTexture; 
        RawImage rankImage = currentObject.transform.Find("GroupBackground/RankImage").GetComponent<RawImage>();
        Texture rankTexture = Resources.Load<Texture2D>($"UI/Rank_Research/{AppConstants.SSWN.SSWN_XIII}");
        rankImage.texture = rankTexture;

        List<RecipeItemDto> recipeItems = await RecipeService.Create().GetRecipeItemsAsync(featureName, User.CurrentUserLevel, User.CurrentUserId);
        SSWNs researchs = await SSWNsService.Create().GetSSWNsAsync(featureId);
