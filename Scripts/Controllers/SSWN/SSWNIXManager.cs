...
public class SSWNIXManager : MonoBehaviour
{
    public static SSWNIXManager Instance { get; private set; }
    ...
    public async Task CreateSSWNIXManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetSSWNFeaturesByTypeAsync(AppConstants.SSWN.SSWN_IX);
    ...
    public async Task CreateMainSSWNPanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.SSWN.SSWN_IX}");
        rankImage.texture = rankTexture;
        RawImage background = transform.Find("Background").GetComponent<RawImage>();
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.SSWN.SSWN_IX_BACKGROUND_URL);
