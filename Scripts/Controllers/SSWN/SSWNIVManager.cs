...
public class SSWNIVManager : MonoBehaviour
{
    public static SSWNIVManager Instance { get; private set; }
    ...
    public async Task CreateSSWNIVManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetSSWNFeaturesByTypeAsync(AppConstants.SSWN.SSWN_IV);
    ...
    public async Task CreateMainSSWNPanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.SSWN.SSWN_IV}");
        rankImage.texture = rankTexture;
        RawImage background = transform.Find("Background").GetComponent<RawImage>();
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.SSWN.SSWN_IV_BACKGROUND_URL);
