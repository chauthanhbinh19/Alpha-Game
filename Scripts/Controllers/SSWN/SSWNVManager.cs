...
public class SSWNVManager : MonoBehaviour
{
    public static SSWNVManager Instance { get; private set; }
    ...
    public async Task CreateSSWNVManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetSSWNFeaturesByTypeAsync(AppConstants.SSWN.SSWN_V);
    ...
    public async Task CreateMainSSWNPanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.SSWN.SSWN_V}");
        rankImage.texture = rankTexture;
        RawImage background = transform.Find("Background").GetComponent<RawImage>();
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.SSWN.SSWN_V_BACKGROUND_URL);
