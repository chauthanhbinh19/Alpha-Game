...
public class SSWNVIIIManager : MonoBehaviour
{
    ...
    public async Task CreateMainSSWNPanelAsync(string featureId, string featureName)
    {
        ...
        upgradeOneLevelButton.onClick.AddListener(async () =>
        {
            ...
            UpgradeResultDTO result = await UpgradeService.Create().UpgradeOneLevelAsync(featureName, currentLevel, sswn.MaxLevel, User.CurrentUserId);
            if (result.Success)
            {
        ...
        {
            AudioManager.Instance.PlaySFX(AudioConstants.SFX.SWITCH_CLICK_SOUND);
            UpgradeResultDTO result = await UpgradeService.Create().UpgradeMaxLevelAsync(featureName, currentLevel, sswn.MaxLevel, User.CurrentUserId);
            if (result.Success)
            {
                userSSWN = EnhanceHelper.EnhanceSSWNs(userSSWN, result.UpgradedLevels, sswn.BaseMultiplier);
                await UserSSWNsService.Create().InsertOrUpdateUserSSWNsAsync(User.CurrentUserId, userSSWN, featureId);
