...
public class SSWNVIIIManager : MonoBehaviour
{
    public static SSWNVIIIManager Instance { get; private set; }
    ...
    private List<KeyValuePair<string, FeatureSSWNDTO>> FeatureList;
    ...
    public async Task CreateSSWNVIIIManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetSSWNFeaturesByTypeAsync(AppConstants.SSWN.SSWN_VIII);
        ...
        for (int i = start; i < end; i++)
        {
            ...
            if (btn != null)
            {
                ...
                btn.onClick.AddListener(async () =>
                {
                    ...
                    AudioManager.Instance.PlaySFX(AudioConstants.SFX.BUTTON_CLICK_SOUND);
                    await CreateMainSSWNPanelAsync(currentFeatureId, currentSubtype);
                });
            }
        }
    }
    ...
    public async Task CreateMainSSWNPanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.SSWN.SSWN_VIII}");
        rankImage.texture = rankTexture;
        RawImage background = transform.Find("Background").GetComponent<RawImage>();
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.SSWN.SSWN_VIII_BACKGROUND_URL);
        ...
        List<RecipeItemDto> recipeItems = await RecipeService.Create().GetRecipeItemsAsync(featureName, userSSWN.Level, User.CurrentUserId);
        ...
        async Task RefreshPanelAsync()
        {
            ...
            List<RecipeItemDto> refreshedRecipeItems = await RecipeService.Create().GetRecipeItemsAsync(featureName, userSSWN.Level, User.CurrentUserId);
        ...
        void CreatePopupUpgradePanelAsync()
        {
            ...
            if (userSSWN != null)
            {
                StatsManager.Instance.CreateStatsManager(userSSWN, currentStatsContent);
                StatsManager.Instance.CreateStatsManager(userSSWN, nextStatsContent);
            }
            ...
            async Task UpdatePreviewAsync()
            {
                ...
                {
                    UserSSWNs previewSSWN = userSSWN.CloneUserSSWN(userSSWN);
                    EnhanceHelper.EnhanceSSWNs(previewSSWN, preview.UpgradedLevels, sswn.BaseMultiplier);
                    StatsManager.Instance.CreateStatsManager(previewSSWN, nextStatsContent);
                }
                else if (userSSWN != null)
                {
                    StatsManager.Instance.CreateStatsManager(userSSWN, nextStatsContent);
                }
