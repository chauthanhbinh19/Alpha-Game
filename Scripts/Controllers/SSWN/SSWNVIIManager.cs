...
public class SSWNVIIManager : MonoBehaviour
{
    public static SSWNVIIManager Instance { get; private set; }
    ...
    public async Task CreateSSWNVIIManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetSSWNFeaturesByTypeAsync(AppConstants.SSWN.SSWN_VII);
    ...
    public async Task CreateMainSSWNPanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.SSWN.SSWN_VII}");
