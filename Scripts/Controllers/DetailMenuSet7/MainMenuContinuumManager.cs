...
{
    public static MainMenuContinuumManager Instance { get; private set; }
    ...
    public async Task CreateMainMenuContinuumManagerAsync(IStats stat)
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetRankFeaturesByTypeAsync(AppConstants.MainMenuSet7.CONTINUUM, stat);
    ...
    public async Task CreateMainRankPanelAsync(string featureId, string featureName)
    {
        ...
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.MainMenuSet7.CONTINUUM);
