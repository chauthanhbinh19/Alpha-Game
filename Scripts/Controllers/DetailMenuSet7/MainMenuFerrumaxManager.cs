...
{
    public static MainMenuFerrumaxManager Instance { get; private set; }
    ...
    public async Task CreateMainMenuFerrumaxManagerAsync(IStats stat)
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetRankFeaturesByTypeAsync(AppConstants.MainMenuSet7.FERRUMAX, stat);
    ...
    public async Task CreateMainRankPanelAsync(string featureId, string featureName)
    {
        ...
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.MainMenuSet7.FERRUMAX);
