...
{
    public static MainMenuLuminaryManager Instance { get; private set; }
    ...
    public async Task CreateMainMenuLuminaryManagerAsync(IStats stat)
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetRankFeaturesByTypeAsync(AppConstants.MainMenuSet7.LUMINARY, stat);
    ...
    public async Task CreateMainRankPanelAsync(string featureId, string featureName)
    {
        ...
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.MainMenuSet7.LUMINARY);
