...
{
    public static MainMenuThrenodyManager Instance { get; private set; }
    ...
    public async Task CreateMainMenuThrenodyManagerAsync(IStats stat)
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetRankFeaturesByTypeAsync(AppConstants.MainMenuSet7.THRENODY, stat);
    ...
    public async Task CreateMainRankPanelAsync(string featureId, string featureName)
    {
        ...
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.MainMenuSet7.THRENODY);
