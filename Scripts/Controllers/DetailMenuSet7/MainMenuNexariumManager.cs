...
{
    public static MainMenuNexariumManager Instance { get; private set; }
    ...
    public async Task CreateMainMenuNexariumManagerAsync(IStats stat)
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetRankFeaturesByTypeAsync(AppConstants.MainMenuSet7.NEXARIUM, stat);
    ...
    public async Task CreateMainRankPanelAsync(string featureId, string featureName)
    {
        ...
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.MainMenuSet7.NEXARIUM);
