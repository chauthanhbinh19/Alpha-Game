...
{
    public static MainMenuOmnivisionManager Instance { get; private set; }
    ...
    public async Task CreateMainMenuOmnivisionManagerAsync(IStats stat)
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetRankFeaturesByTypeAsync(AppConstants.MainMenuSet1.OMNIVISION, stat);
    ...
    public async Task CreateMainRankPanelAsync(string featureId, string featureName)
    {
        ...
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.MainMenuSet1.OMNIVISION);
