...
{
    public static MainMenuOmnipresenceManager Instance { get; private set; }
    ...
    public async Task CreateMainMenuOmnipresenceManagerAsync(IStats stat)
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetRankFeaturesByTypeAsync(AppConstants.MainMenuSet1.OMNIPRESENCE, stat);
    ...
    public async Task CreateMainRankPanelAsync(string featureId, string featureName)
    {
        ...
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.MainMenuSet1.OMNIPRESENCE);
