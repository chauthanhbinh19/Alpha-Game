...
public class MainMenuRealmManager : MonoBehaviour
{
    public static MainMenuRealmManager Instance { get; private set; }
    ...
    public async Task CreateMainMenuRealmManagerAsync(IStats stat)
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetRankFeaturesByTypeAsync(AppConstants.MainMenuSet1.REALM, stat);
    ...
    public async Task CreateMainRankPanelAsync(string featureId, string featureName)
    {
        ...
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.MainMenuSet1.REALM);
