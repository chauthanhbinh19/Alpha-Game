...
{
    public static MainMenuPhysiqueManager Instance { get; private set; }
    ...
    public async Task CreateMainMenuPhysiqueManagerAsync(IStats stat)
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetRankFeaturesByTypeAsync(AppConstants.MainMenuSet1.PHYSIQUE, stat);
    ...
    public async Task CreateMainRankPanelAsync(string featureId, string featureName)
    {
        ...
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.MainMenuSet1.PHYSIQUE);
