...
{
    public static MainMenuAptitudeManager Instance { get; private set; }
    ...
    public async Task CreateMainMenuAptitudeManagerAsync(IStats stat)
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetRankFeaturesByTypeAsync(AppConstants.MainMenuSet1.APTITUDE, stat);
    ...
    public async Task CreateMainRankPanelAsync(string featureId, string featureName)
    {
        ...
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.MainMenuSet1.APTITUDE);
