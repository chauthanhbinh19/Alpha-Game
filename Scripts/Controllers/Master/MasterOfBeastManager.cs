...
public class MasterOfBeastManager : MonoBehaviour
{
    public static MasterOfBeastManager Instance { get; private set; }
    private Transform MainPanel;
    private GameObject MasterPanelPrefab;
    private GameObject MasterButtonPrefab;
    private GameObject PopupMasterPanelPrefab;
    private GameObject PopupMasterQuantityPanelPrefab;
    private GameObject PopupMasterButtonPrefab;
    private GameObject MainMasterPanelPrefab;
    private GameObject MasterItemPrefab;
    private Transform content;
    private const int ITEMS_PER_PAGE = 50;
    private int _currentPage = 0;
    private List<KeyValuePair<string, FeatureMasterDTO>> _featureList;
    ...
    public async Task CreateMasterOfBeastManagerAsync(IStats stat)
    {
        GameObject currentObject = Instantiate(PopupMasterPanelPrefab, MainPanel);
        ...
        Dictionary<string, FeatureMasterDTO> uniqueTypes = new Dictionary<string, FeatureMasterDTO>();
        uniqueTypes = await FeaturesService.Create().GetMasterFeaturesByTypeAsync(AppConstants.Master.MASTER_OF_BEAST, stat);
    ...
    private void RenderPage()
    {
        ...
        for (int i = start; i < end; i++)
        {
            ...
            GameObject button = Instantiate(PopupMasterButtonPrefab, content);
            ...
            btn.onClick.AddListener(async () =>
            {
                ...
                AudioManager.Instance.PlaySFX(AudioConstants.SFX.BUTTON_CLICK_SOUND);
                await CreateMainMasterPanelAsync(featureId, subtype);
            });
        }
    }
    ...
    public async Task CreateMainMasterPanelAsync(string featureId, string featureName)
    {
        GameObject currentObject = Instantiate(MainMasterPanelPrefab, MainPanel);
        Transform transform = currentObject.transform;
        ...
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.Master.MASTER_OF_BEAST_BACKGROUND_URL);
        ...
        for (int i = 0; i < total; i++)
        {
            ...
            GameObject itemGO = Instantiate(MasterItemPrefab, parent);

            SetupMasterItemUI(itemGO, recipeItems[i]);
        }

        int currentLevel = userMaster?.Level ?? 0;
        ...
        async Task RefreshPanelAsync()
        {
            ...
            for (int i = 0; i < refreshedTotal; i++)
            {
                ...
                GameObject itemGO = Instantiate(MasterItemPrefab, parent);
                SetupMasterItemUI(itemGO, refreshedRecipeItems[i]);
            }
        }
        ...
        void CreatePopupUpgradePanelAsync()
        {
            GameObject gameObject =
                Instantiate(PopupMasterQuantityPanelPrefab, MainPanel);
            ...
            confirmButton.onClick.AddListener(async () =>
            {
                ...
                if (result.Success)
                {
                    ...
                    await UserMastersService.Create().InsertOrUpdateUserMastersAsync(User.CurrentUserId, userMaster, featureId, _stat);
    ...
    private void SetupMasterItemUI(GameObject itemGO, RecipeItemDto data)
    {
