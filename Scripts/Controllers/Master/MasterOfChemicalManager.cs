...
public class MasterOfChemicalManager : MonoBehaviour
{
    public static MasterOfChemicalManager Instance { get; private set; }
    ...
    private List<KeyValuePair<string, FeatureMasterDTO>> FeatureList;
    private IStats Stat;
    ...
    public async Task CreateMasterOfChemicalManagerAsync(IStats stat)
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetMasterFeaturesByTypeAsync(AppConstants.Master.MASTER_OF_CHEMICAL, stat);
        ...
        for (int i = start; i < end; i++)
        {
            ...
            if (btn != null)
            {
                ...
                btn.onClick.AddListener(async () =>
                {
                    ...
                    AudioManager.Instance.PlaySFX(AudioConstants.SFX.BUTTON_CLICK_SOUND);
                    await CreateMainMasterPanelAsync(currentFeatureId, currentSubtype);
                });
            }
        }
    }
    ...
    public async Task CreateMainMasterPanelAsync(string featureId, string featureName)
    {
        ...
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.Master.MASTER_OF_CHEMICAL_BACKGROUND_URL);
