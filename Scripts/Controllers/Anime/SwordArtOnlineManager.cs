...
public class SwordArtOnlineManager : MonoBehaviour
{
    public static SwordArtOnlineManager Instance { get; private set; }
    private Transform MainPanel;
    private GameObject AnimePanelPrefab;
    private GameObject AnimeButtonPrefab;
    private GameObject PopupAnimePanelPrefab;
    private GameObject PopupAnimeQuantityPanelPrefab;
    private GameObject PopupAnimeButtonPrefab;
    private GameObject MainAnimePanelPrefab;
    private GameObject AnimeItemPrefab;
    private Transform Content;
    private const int ITEMS_PER_PAGE = 50;
    private int CurrentPage = 0;
    private List<KeyValuePair<string, FeatureAnimeDTO>> FeatureList;
    ...
    {
        GameObject currentObject = Instantiate(PopupAnimePanelPrefab, MainPanel);
        ...
        Dictionary<string, FeatureAnimeDTO> uniqueTypes = new Dictionary<string, FeatureAnimeDTO>();
        uniqueTypes = await FeaturesService.Create().GetAnimeFeaturesByTypeAsync(AppConstants.Anime.SWORD_ART_ONLINE);
    ...
    private void RenderPage()
    {
        ...
        for (int i = start; i < end; i++)
        {
            ...
            GameObject button = Instantiate(PopupAnimeButtonPrefab, Content);
            ...
            if (btn != null)
            {
                ...
                btn.onClick.AddListener(async () =>
                {
                    ...
                    AudioManager.Instance.PlaySFX(AudioConstants.SFX.BUTTON_CLICK_SOUND);
                    await CreateMainAnimePanelAsync(currentFeatureId, currentSubtype);
                });
            }
        }
    }
    ...
    public async Task CreateMainAnimePanelAsync(string featureId, string featureName)
    {
        GameObject currentObject = Instantiate(MainAnimePanelPrefab, MainPanel);
        Transform transform = currentObject.transform;
        ...
        for (int i = 0; i < total; i++)
        {
            ...
            GameObject itemGO = Instantiate(AnimeItemPrefab, parent);

            SetupAnimeItemUI(itemGO, recipeItems[i]);
        }

        int currentLevel = userAnime?.Level ?? 0;
        ...
        async Task RefreshPanelAsync()
        {
            ...
            for (int i = 0; i < refreshedTotal; i++)
            {
                ...
                GameObject itemGO = Instantiate(AnimeItemPrefab, parent);
                SetupAnimeItemUI(itemGO, refreshedRecipeItems[i]);
            }
        }
        ...
        void CreatePopupUpgradePanelAsync()
        {
            GameObject gameObject =
                Instantiate(PopupAnimeQuantityPanelPrefab, MainPanel);
            ...
            confirmButton.onClick.AddListener(async () =>
            {
                ...
                if (result.Success)
                {
                    userAnime = EnhanceHelper.EnhanceAnimes(userAnime, result.UpgradedLevels, universe.BaseMultiplier);
    ...
    private void SetupAnimeItemUI(GameObject itemGO,RecipeItemDto data)
    {
