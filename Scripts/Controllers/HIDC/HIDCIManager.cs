...
public class HIDCIManager : MonoBehaviour
{
    public static HIDCIManager Instance { get; private set; }
    ...
    public async Task CreateHIDCIManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetHIDCFeaturesByTypeAsync(AppConstants.HIDC.HIDC_I);
    ...
    public async Task CreateMainHIDCPanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.HIDC.HIDC_I}");
        rankImage.texture = rankTexture;
        RawImage background = transform.Find("Background").GetComponent<RawImage>();
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.HIDC.HIDC_I_BACKGROUND_URL);
