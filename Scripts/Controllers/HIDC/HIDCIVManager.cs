...
public class HIDCIVManager : MonoBehaviour
{
    public static HIDCIVManager Instance { get; private set; }
    ...
    public async Task CreateHIDCIVManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetHIDCFeaturesByTypeAsync(AppConstants.HIDC.HIDC_IV);
    ...
    public async Task CreateMainHIDCPanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.HIDC.HIDC_IV}");
        rankImage.texture = rankTexture;
        RawImage background = transform.Find("Background").GetComponent<RawImage>();
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.HIDC.HIDC_IV_BACKGROUND_URL);
