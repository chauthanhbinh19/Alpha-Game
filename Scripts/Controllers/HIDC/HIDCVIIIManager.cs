...
public class HIDCVIIIManager : MonoBehaviour
{
    public static HIDCVIIIManager Instance { get; private set; }
    ...
    public async Task CreateHIDCVIIIManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetHIDCFeaturesByTypeAsync(AppConstants.HIDC.HIDC_VIII);
    ...
    public async Task CreateMainHIDCPanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.HIDC.HIDC_VIII}");
        ...
        upgradeOneLevelButton.onClick.AddListener(async () =>
        {
            ...
            UpgradeResultDTO result = await UpgradeService.Create().UpgradeOneLevelAsync(featureName, currentLevel, hidc.MaxLevel, User.CurrentUserId);
            if (result.Success)
            {
                userHIDC = EnhanceHelper.EnhanceHIDCs(userHIDC, result.UpgradedLevels, 1000);
        ...
        {
            AudioManager.Instance.PlaySFX(AudioConstants.SFX.SWITCH_CLICK_SOUND);
            UpgradeResultDTO result = await UpgradeService.Create().UpgradeMaxLevelAsync(featureName, currentLevel, hidc.MaxLevel, User.CurrentUserId);
            if (result.Success)
            {
                userHIDC = EnhanceHelper.EnhanceHIDCs(userHIDC, result.UpgradedLevels, 1000);
                await UserHIDCsService.Create().InsertOrUpdateUserHIDCsAsync(User.CurrentUserId, userHIDC, featureId);
