...
public class HIDCXManager : MonoBehaviour
{
    public static HIDCXManager Instance { get; private set; }
    ...
    public async Task CreateHIDCXManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetHIDCFeaturesByTypeAsync(AppConstants.HIDC.HIDC_X);
    ...
    public async Task CreateMainHIDCPanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.HIDC.HIDC_X}");
        rankImage.texture = rankTexture;
        RawImage background = transform.Find("Background").GetComponent<RawImage>();
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.HIDC.HIDC_X_BACKGROUND_URL);
