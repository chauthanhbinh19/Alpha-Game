...
public class HIDCVIManager : MonoBehaviour
{
    public static HIDCVIManager Instance { get; private set; }
    ...
    public async Task CreateHIDCVIManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetHIDCFeaturesByTypeAsync(AppConstants.HIDC.HIDC_VI);
    ...
    public async Task CreateMainHIDCPanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.HIDC.HIDC_VI}");
        rankImage.texture = rankTexture;
        RawImage background = transform.Find("Background").GetComponent<RawImage>();
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.HIDC.HIDC_VI_BACKGROUND_URL);
