...
public class ManufacturingManager : MonoBehaviour
{
    public static ManufacturingManager Instance { get; private set; }
    ...
    public async Task CreateManufacturingManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetResearchFeaturesByTypeAsync(AppConstants.Research.MANUFACTURING);
    ...
    public async Task CreateMainResearchPanelAsync(string featureId, string featureName)
    {
        ...
        Texture mapTexture = TextureHelper.LoadTexture2DCached("UI/Background2/Chapter_2");
        mapImage.texture = mapTexture; 
        RawImage rankImage = transform.Find("GroupBackground/RankImage").GetComponent<RawImage>();
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.Research.MANUFACTURING}");
