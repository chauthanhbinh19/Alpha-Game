...
public class PowerResearchManager : MonoBehaviour
{
    public static PowerResearchManager Instance { get; private set; }
    ...
    public async Task CreatePowerManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetResearchFeaturesByTypeAsync(AppConstants.Research.POWER);
    ...
    public async Task CreateMainResearchPanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.Research.POWER}");
