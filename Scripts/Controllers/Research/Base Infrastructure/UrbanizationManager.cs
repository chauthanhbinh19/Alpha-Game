...
public class UrbanizationManager : MonoBehaviour
{
    public static UrbanizationManager Instance { get; private set; }
    ...
    public async Task CreateUrbanizationManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetResearchFeaturesByTypeAsync(AppConstants.Research.URBANIZATION);
    ...
    public async Task CreateMainResearchPanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.Research.URBANIZATION}");
