...
public class SanitationManager : MonoBehaviour
{
    public static SanitationManager Instance { get; private set; }
    ...
    public async Task CreateSanitationManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetResearchFeaturesByTypeAsync(AppConstants.Research.SANITATION);
    ...
    public async Task CreateMainResearchPanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.Research.SANITATION}");
