...
{
    public static MegastructureManager Instance { get; private set; }
    ...
    public async Task CreateMegastructureManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetResearchFeaturesByTypeAsync(AppConstants.Research.MEGASTRUCTURE);
    ...
    public async Task CreateMainResearchPanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.Research.MEGASTRUCTURE}");
