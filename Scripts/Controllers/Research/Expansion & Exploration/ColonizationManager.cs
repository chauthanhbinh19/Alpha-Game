...
public class ColonizationManager : MonoBehaviour
{
    public static ColonizationManager Instance { get; private set; }
    ...
    public async Task CreateColonizationManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetResearchFeaturesByTypeAsync(AppConstants.Research.COLONIZATION);
    ...
    public async Task CreateMainResearchPanelAsync(string featureId, string featureName)
    {
        ...
        Texture mapTexture = TextureHelper.LoadTexture2DCached("UI/Background2/Chapter_6");
        mapImage.texture = mapTexture; 
        RawImage rankImage = transform.Find("GroupBackground/RankImage").GetComponent<RawImage>();
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.Research.COLONIZATION}");
