...
public class DemographyManager : MonoBehaviour
{
    public static DemographyManager Instance { get; private set; }
    ...
    public async Task CreateDemographyManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetResearchFeaturesByTypeAsync(AppConstants.Research.DEMOGRAPHY);
    ...
    public async Task CreateMainResearchPanelAsync(string featureId, string featureName)
    {
        ...
        Texture mapTexture = TextureHelper.LoadTexture2DCached("UI/Background2/Chapter_10");
        mapImage.texture = mapTexture; 
        RawImage rankImage = transform.Find("GroupBackground/RankImage").GetComponent<RawImage>();
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.Research.DEMOGRAPHY}");
