...
public class CivicsManager : MonoBehaviour
{
    public static CivicsManager Instance { get; private set; }
    ...
    public async Task CreateCivicsManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetResearchFeaturesByTypeAsync(AppConstants.Research.CIVICS);
    ...
    public async Task CreateMainResearchPanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.Research.CIVICS}");
