...
public class ImmunologyManager : MonoBehaviour
{
    public static ImmunologyManager Instance { get; private set; }
    ...
    public async Task CreateImmunologyManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetResearchFeaturesByTypeAsync(AppConstants.Research.IMMUNOLOGY);
    ...
    public async Task CreateMainResearchPanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.Research.IMMUNOLOGY}");
