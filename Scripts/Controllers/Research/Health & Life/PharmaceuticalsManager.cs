...
public class PharmaceuticalsManager : MonoBehaviour
{
    public static PharmaceuticalsManager Instance { get; private set; }
    ...
    public async Task CreatePharmaceuticalsManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetResearchFeaturesByTypeAsync(AppConstants.Research.PHARMACEUTICALS);
    ...
    public async Task CreateMainResearchPanelAsync(string featureId, string featureName)
    {
        ...
        Texture mapTexture = TextureHelper.LoadTexture2DCached("UI/Background2/Chapter_7");
        mapImage.texture = mapTexture; 
        RawImage rankImage = transform.Find("GroupBackground/RankImage").GetComponent<RawImage>();
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.Research.PHARMACEUTICALS}");
        ...
        upgradeOneLevelButton.onClick.AddListener(async () =>
        {
            ...
            UpgradeResultDTO result = await UpgradeService.Create().UpgradeOneLevelAsync(featureName, currentLevel, research.MaxLevel, User.CurrentUserId);
            if (result.Success)
            {
        ...
        {
            AudioManager.Instance.PlaySFX(AudioConstants.SFX.SWITCH_CLICK_SOUND);
            UpgradeResultDTO result = await UpgradeService.Create().UpgradeMaxLevelAsync(featureName, currentLevel, research.MaxLevel, User.CurrentUserId);
            if (result.Success)
            {
                userResearch = EnhanceHelper.EnhanceResearchs(userResearch, result.UpgradedLevels, research.BaseMultiplier);
                await UserResearchsService.Create().InsertOrUpdateUserResearchsAsync(User.CurrentUserId, userResearch, featureId);
