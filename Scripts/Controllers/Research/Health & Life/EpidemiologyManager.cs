...
public class EpidemiologyManager : MonoBehaviour
{
    public static EpidemiologyManager Instance { get; private set; }
    ...
    public async Task CreateEpidemiologyManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetResearchFeaturesByTypeAsync(AppConstants.Research.EPIDEMIOLOGY);
    ...
    public async Task CreateMainResearchPanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.Research.EPIDEMIOLOGY}");
