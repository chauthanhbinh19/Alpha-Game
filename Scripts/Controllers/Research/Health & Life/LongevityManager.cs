...
public class LongevityManager : MonoBehaviour
{
    public static LongevityManager Instance { get; private set; }
    ...
    public async Task CreateLongevityManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetResearchFeaturesByTypeAsync(AppConstants.Research.LONGEVITY);
    ...
    public async Task CreateMainResearchPanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.Research.LONGEVITY}");
