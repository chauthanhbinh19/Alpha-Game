...
public class RenewablesManager : MonoBehaviour
{
    public static RenewablesManager Instance { get; private set; }
    ...
    public async Task CreateRenewablesManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetResearchFeaturesByTypeAsync(AppConstants.Research.RENEWABLES);
    ...
    public async Task CreateMainResearchPanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.Research.RENEWABLES}");
