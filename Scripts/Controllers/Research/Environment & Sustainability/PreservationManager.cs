...
public class PreservationManager : MonoBehaviour
{
    public static PreservationManager Instance { get; private set; }
    ...
    public async Task CreatePreservationManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetResearchFeaturesByTypeAsync(AppConstants.Research.PRESERVATION);
    ...
    public async Task CreateMainResearchPanelAsync(string featureId, string featureName)
    {
        ...
        Texture mapTexture = TextureHelper.LoadTexture2DCached("UI/Background2/Chapter_5");
        mapImage.texture = mapTexture; 
        RawImage rankImage = transform.Find("GroupBackground/RankImage").GetComponent<RawImage>();
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.Research.PRESERVATION}");
