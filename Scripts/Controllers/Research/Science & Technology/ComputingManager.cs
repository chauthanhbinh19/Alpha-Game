...
public class ComputingManager : MonoBehaviour
{
    ...
    public async Task CreateMainResearchPanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = Resources.Load<Texture2D>($"UI/Rank_Research/{AppConstants.Research.COMPUTING}");
        rankImage.texture = rankTexture;
        RawImage background = transform.Find("Background").GetComponent<RawImage>();
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.Research.COMPUTING_URL);
