...
public class NanotechnologyManager : MonoBehaviour
{
    public static NanotechnologyManager Instance { get; private set; }
    ...
    public async Task CreateNanotechnologyManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetResearchFeaturesByTypeAsync(AppConstants.Research.NANOTECHNOLOGY);
    ...
    public async Task CreateMainResearchPanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.Research.NANOTECHNOLOGY}");
