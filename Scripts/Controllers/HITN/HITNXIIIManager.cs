...
public class HITNXIIIManager : MonoBehaviour
{
    public static HITNXIIIManager Instance { get; private set; }
    ...
    public async Task CreateHITNXIIIManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetHITNFeaturesByTypeAsync(AppConstants.HITN.HITN_XIII);
    ...
    public async Task CreateMainHITNPanelAsync(string featureId, string featureName)
    {
        ...
        Texture mapTexture = Resources.Load<Texture2D>("UI/Background2/Chapter_13");
        mapImage.texture = mapTexture; 
        RawImage rankImage = currentObject.transform.Find("GroupBackground/RankImage").GetComponent<RawImage>();
        Texture rankTexture = Resources.Load<Texture2D>($"UI/Rank_Research/{AppConstants.HITN.HITN_XIII}");
        rankImage.texture = rankTexture;

        List<RecipeItemDto> recipeItems = await RecipeService.Create().GetRecipeItemsAsync(featureName, User.CurrentUserLevel, User.CurrentUserId);
        HITNs researchs = await HITNsService.Create().GetHITNsAsync(featureId);
