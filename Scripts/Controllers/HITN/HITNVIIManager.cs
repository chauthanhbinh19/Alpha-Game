...
public class HITNVIIManager : MonoBehaviour
{
    public static HITNVIIManager Instance { get; private set; }
    private Transform MainPanel;
    private GameObject HITNPanelPrefab;
    private GameObject HITNButtonPrefab;
    private GameObject PopupHITNPanelPrefab;
    private GameObject PopupHITNButtonPrefab;
    private GameObject MainHITNPanelPrefab;
    private GameObject HITNItemPrefab;
    private Transform content;
    private const int ITEMS_PER_PAGE = 50;
    private int _currentPage = 0;
    private List<KeyValuePair<string, FeatureHITNDTO>> _featureList;
    ...
    public async Task CreateHITNVIIManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetHITNFeaturesByTypeAsync(AppConstants.HITN.HITN_VII);
    ...
    private void RenderPage()
    {
        ...
        for (int i = start; i < end; i++)
        {
            ...
            GameObject button = Instantiate(PopupHITNButtonPrefab, content);
            ...
            {
                AudioManager.Instance.PlaySFX(AudioConstants.SFX.BUTTON_CLICK_SOUND);
                await CreateMainHITNPanelAsync(featureId, subtype);
            });
        }
    }
    ...
    public async Task CreateMainHITNPanelAsync(string featureId, string featureName)
    {
        GameObject currentObject = Instantiate(MainHITNPanelPrefab, MainPanel);
        Transform transform = currentObject.transform;
        ...
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.HITN.HITN_VII}");
        ...
        for (int i = 0; i < total; i++)
        {
            ...
            GameObject itemGO = Instantiate(HITNItemPrefab, parent);

            SetupHITNItemUI(itemGO, recipeItems[i]);
        }

        int currentLevel = userHITN?.Level ?? 0;
        ...
        async Task RefreshPanelAsync()
        {
            ...
            for (int i = 0; i < refreshedTotal; i++)
            {
                ...
                GameObject itemGO = Instantiate(HITNItemPrefab, parent);
                SetupHITNItemUI(itemGO, refreshedRecipeItems[i]);
            }
        }
        ...
        upgradeOneLevelButton.onClick.AddListener(async () =>
        {
            ...
            UpgradeResultDTO result = await UpgradeService.Create().UpgradeOneLevelAsync(featureName, currentLevel, hitn.MaxLevel, User.CurrentUserId);
            if (result.Success)
            {
        ...
        {
            AudioManager.Instance.PlaySFX(AudioConstants.SFX.SWITCH_CLICK_SOUND);
            UpgradeResultDTO result = await UpgradeService.Create().UpgradeMaxLevelAsync(featureName, currentLevel, hitn.MaxLevel, User.CurrentUserId);
            if (result.Success)
            {
                userHITN = EnhanceHelper.EnhanceHITNs(userHITN, result.UpgradedLevels, hitn.BaseMultiplier);
                await UserHITNsService.Create().InsertOrUpdateUserHITNsAsync(User.CurrentUserId, userHITN, featureId);
