...
public class ArchiveXIVManager : MonoBehaviour
{
    public static ArchiveXIVManager Instance { get; private set; }
    ...
    public async Task CreateArchiveXIVManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetArchiveFeaturesByTypeAsync(AppConstants.Archive.ARCHIVE_XIV);
    ...
    public async Task CreateMainArchivePanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = Resources.Load<Texture2D>($"UI/Rank_Research/{AppConstants.Archive.ARCHIVE_XIV}");
        ...
        UserArchives research = await UserArchivesService.Create().GetUserArchivesAsync(featureId);
        ...
        {
            research = await UserArchivesService.Create().GetUserArchivesAsync(featureId);
            currentLevel = research?.Level ?? 0;
            levelText.text = currentLevel.ToString();
        ...
        upgradeOneLevelButton.onClick.AddListener(async () =>
        {
            ...
            int levelToUpgrade = research?.Level ?? 0;
            ...
            if (result.Success)
            {
                research = EnhanceHelper.EnhanceArchives(research, result.UpgradedLevels, archive.BaseMultiplier);
                await UserArchivesService.Create().InsertOrUpdateUserArchivesAsync(User.CurrentUserId, research, featureId);
