...
public class ArchiveXXVIManager : MonoBehaviour
{
    public static ArchiveXXVIManager Instance { get; private set; }
    ...
    public async Task CreateArchiveXXVIManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetArchiveFeaturesByTypeAsync(AppConstants.Archive.ARCHIVE_XXVI);
    ...
    public async Task CreateMainArchivePanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.Archive.ARCHIVE_XXVI}");
        rankImage.texture = rankTexture;
        RawImage background = transform.Find("Background").GetComponent<RawImage>();
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.Archive.ARCHIVE_XXVI_BACKGROUND_URL);
