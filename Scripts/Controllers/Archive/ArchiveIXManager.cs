...
public class ArchiveIXManager : MonoBehaviour
{
    public static ArchiveIXManager Instance { get; private set; }
    ...
    public async Task CreateArchiveIXManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetArchiveFeaturesByTypeAsync(AppConstants.Archive.ARCHIVE_IX);
    ...
    public async Task CreateMainArchivePanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.Archive.ARCHIVE_IX}");
        rankImage.texture = rankTexture;
        RawImage background = transform.Find("Background").GetComponent<RawImage>();
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.Archive.ARCHIVE_IX_BACKGROUND_URL);
