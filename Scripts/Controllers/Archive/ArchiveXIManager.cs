...
public class ArchiveXIManager : MonoBehaviour
{
    public static ArchiveXIManager Instance { get; private set; }
    ...
    public async Task CreateArchiveXIManagerAsync()
    {
    ...
    public async Task CreateMainArchivePanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.Archive.ARCHIVE_XI}");
        rankImage.texture = rankTexture;
        RawImage background = transform.Find("Background").GetComponent<RawImage>();
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.Archive.ARCHIVE_XI_BACKGROUND_URL);
