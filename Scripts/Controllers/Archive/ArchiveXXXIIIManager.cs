...
public class ArchiveXXXIIIManager : MonoBehaviour
{
    public static ArchiveXXXIIIManager Instance { get; private set; }
    ...
    public async Task CreateArchiveXXXIIIManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetArchiveFeaturesByTypeAsync(AppConstants.Archive.ARCHIVE_XXXIII);
    ...
    public async Task CreateMainArchivePanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.Archive.ARCHIVE_XXXIII}");
        rankImage.texture = rankTexture;
        RawImage background = transform.Find("Background").GetComponent<RawImage>();
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.Archive.ARCHIVE_XXXIII_BACKGROUND_URL);
