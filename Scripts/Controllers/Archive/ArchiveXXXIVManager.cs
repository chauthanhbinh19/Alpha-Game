...
public class ArchiveXXXIVManager : MonoBehaviour
{
    public static ArchiveXXXIVManager Instance { get; private set; }
    ...
    public async Task CreateArchiveXXXIVManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetArchiveFeaturesByTypeAsync(AppConstants.Archive.ARCHIVE_XXXIV);
    ...
    public async Task CreateMainArchivePanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.Archive.ARCHIVE_XXXIV}");
        rankImage.texture = rankTexture;
        RawImage background = transform.Find("Background").GetComponent<RawImage>();
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.Archive.ARCHIVE_XXXIV_BACKGROUND_URL);
