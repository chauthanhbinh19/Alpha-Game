...
public class HIINIIIManager : MonoBehaviour
{
    public static HIINIIIManager Instance { get; private set; }
    ...
    public async Task CreateHIINIIIManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetHIINFeaturesByTypeAsync(AppConstants.HIIN.HIIN_III);
    ...
    public async Task CreateMainHIINPanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.HIIN.HIIN_III}");
        rankImage.texture = rankTexture;
        RawImage background = transform.Find("Background").GetComponent<RawImage>();
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.HIIN.HIIN_III_BACKGROUND_URL);
