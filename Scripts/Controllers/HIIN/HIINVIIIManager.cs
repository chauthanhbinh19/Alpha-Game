...
public class HIINVIIIManager : MonoBehaviour
{
    ...
    public async Task CreateMainHIINPanelAsync(string featureId, string featureName)
    {
        ...
        upgradeOneLevelButton.onClick.AddListener(async () =>
        {
            ...
            UpgradeResultDTO result = await UpgradeService.Create().UpgradeOneLevelAsync(featureName, currentLevel, hiin.MaxLevel, User.CurrentUserId);
            if (result.Success)
            {
        ...
        {
            AudioManager.Instance.PlaySFX(AudioConstants.SFX.SWITCH_CLICK_SOUND);
            UpgradeResultDTO result = await UpgradeService.Create().UpgradeMaxLevelAsync(featureName, currentLevel, hiin.MaxLevel, User.CurrentUserId);
            if (result.Success)
            {
                userHIIN = EnhanceHelper.EnhanceHIINs(userHIIN, result.UpgradedLevels, hiin.BaseMultiplier);
                await UserHIINsService.Create().InsertOrUpdateUserHIINsAsync(User.CurrentUserId, userHIIN, featureId);
