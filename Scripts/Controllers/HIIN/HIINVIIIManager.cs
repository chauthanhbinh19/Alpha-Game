...
public class HIINVIIIManager : MonoBehaviour
{
    public static HIINVIIIManager Instance { get; private set; }
    private Transform MainPanel;
    private GameObject HIINPanelPrefab;
    private GameObject HIINButtonPrefab;
    private GameObject PopupHIINPanelPrefab;
    private GameObject PopupHIINQuantityPanelPrefab;
    ...
    private List<KeyValuePair<string, FeatureHIINDTO>> FeatureList;
    ...
    public async Task CreateHIINVIIIManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetHIINFeaturesByTypeAsync(AppConstants.HIIN.HIIN_VIII);
        ...
        for (int i = start; i < end; i++)
        {
            ...
            if (btn != null)
            {
                ...
                btn.onClick.AddListener(async () =>
                {
                    ...
                    AudioManager.Instance.PlaySFX(AudioConstants.SFX.BUTTON_CLICK_SOUND);
                    await CreateMainHIINPanelAsync(currentFeatureId, currentSubtype);
                });
            }
        }
    }
    ...
    public async Task CreateMainHIINPanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.HIIN.HIIN_VIII}");
        rankImage.texture = rankTexture;
        RawImage background = transform.Find("Background").GetComponent<RawImage>();
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.HIIN.HIIN_VIII_BACKGROUND_URL);
        ...
        List<RecipeItemDto> recipeItems = await RecipeService.Create().GetRecipeItemsAsync(featureName, userHIIN.Level, User.CurrentUserId);
        ...
        int currentLevel = userHIIN?.Level ?? 0;
        ...
        async Task RefreshPanelAsync()
        {
            ...
            List<RecipeItemDto> refreshedRecipeItems = await RecipeService.Create().GetRecipeItemsAsync(featureName, userHIIN.Level, User.CurrentUserId);
            ...
            for (int i = 0; i < refreshedTotal; i++)
            {
                ...
                GameObject itemGO = Instantiate(HIINItemPrefab, parent);
                SetupHIINItemUI(itemGO, refreshedRecipeItems[i]);
            }
        }
        ...
        void CreatePopupUpgradePanelAsync()
        {
            GameObject gameObject =
                Instantiate(PopupHIINQuantityPanelPrefab, MainPanel);
            ...
            int maxLevel = hiin != null ? hiin.MaxLevel : popupCurrentLevel;
            ...
            if (userHIIN != null)
            {
                StatsManager.Instance.CreateStatsManager(userHIIN, currentStatsContent);
                StatsManager.Instance.CreateStatsManager(userHIIN, nextStatsContent);
            }
            ...
            async Task UpdatePreviewAsync()
            {
                ...
                {
                    UserHIINs previewHIIN = userHIIN.CloneUserHIIN(userHIIN);
                    EnhanceHelper.EnhanceHIINs(previewHIIN, preview.UpgradedLevels, hiin.BaseMultiplier);
                    StatsManager.Instance.CreateStatsManager(previewHIIN, nextStatsContent);
                }
                else if (userHIIN != null)
                {
                    StatsManager.Instance.CreateStatsManager(userHIIN, nextStatsContent);
                }
            ...
            confirmButton.onClick.AddListener(async () =>
            {
                ...
                if (result.Success)
                {
                    userHIIN = EnhanceHelper.EnhanceHIINs(userHIIN, result.UpgradedLevels, hiin.BaseMultiplier);
                    await UserHIINsService.Create().InsertOrUpdateUserHIINsAsync(User.CurrentUserId, userHIIN, featureId);
