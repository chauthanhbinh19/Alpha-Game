...
public class HIINXIIIManager : MonoBehaviour
{
    public static HIINXIIIManager Instance { get; private set; }
    ...
    public async Task CreateHIINXIIIManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetHIINFeaturesByTypeAsync(AppConstants.HIIN.HIIN_XIII);
    ...
    public async Task CreateMainHIINPanelAsync(string featureId, string featureName)
    {
        ...
        Texture mapTexture = Resources.Load<Texture2D>("UI/Background2/Chapter_11");
        mapImage.texture = mapTexture; 
        RawImage rankImage = currentObject.transform.Find("GroupBackground/RankImage").GetComponent<RawImage>();
        Texture rankTexture = Resources.Load<Texture2D>($"UI/Rank_Research/{AppConstants.HIIN.HIIN_XIII}");
        rankImage.texture = rankTexture;

        List<RecipeItemDto> recipeItems = await RecipeService.Create().GetRecipeItemsAsync(featureName, User.CurrentUserLevel, User.CurrentUserId);
        HIINs researchs = await HIINsService.Create().GetHIINsAsync(featureId);
