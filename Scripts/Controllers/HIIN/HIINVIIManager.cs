...
public class HIINVIIManager : MonoBehaviour
{
    public static HIINVIIManager Instance { get; private set; }
    ...
    public async Task CreateHIINVIIManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetHIINFeaturesByTypeAsync(AppConstants.HIIN.HIIN_VII);
    ...
    public async Task CreateMainHIINPanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.HIIN.HIIN_VII}");
        rankImage.texture = rankTexture;
        RawImage background = transform.Find("Background").GetComponent<RawImage>();
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.HIIN.HIIN_VII_BACKGROUND_URL);
