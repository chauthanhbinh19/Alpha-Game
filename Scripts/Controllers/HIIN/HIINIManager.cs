...
public class HIINIManager : MonoBehaviour
{
    public static HIINIManager Instance { get; private set; }
    ...
    public async Task CreateHIINIManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetHIINFeaturesByTypeAsync(AppConstants.HIIN.HIIN_I);
    ...
    public async Task CreateMainHIINPanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.HIIN.HIIN_I}");
        rankImage.texture = rankTexture;
        RawImage background = transform.Find("Background").GetComponent<RawImage>();
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.HIIN.HIIN_I_BACKGROUND_URL);
