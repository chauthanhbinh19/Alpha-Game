...
public class HICBXManager : MonoBehaviour
{
    public static HICBXManager Instance { get; private set; }
    ...
    public async Task CreateHICBXManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetHICBFeaturesByTypeAsync(AppConstants.HICB.HICB_X);
    ...
    public async Task CreateMainHICBPanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.HICB.HICB_X}");
        rankImage.texture = rankTexture;
        RawImage background = transform.Find("Background").GetComponent<RawImage>();
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.HICB.HICB_X_BACKGROUND_URL);
