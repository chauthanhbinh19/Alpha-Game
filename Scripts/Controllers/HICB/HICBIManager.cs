...
public class HICBIManager : MonoBehaviour
{
    public static HICBIManager Instance { get; private set; }
    ...
    public async Task CreateHICBIManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetHICBFeaturesByTypeAsync(AppConstants.HICB.HICB_I);
    ...
    public async Task CreateMainHICBPanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.HICB.HICB_I}");
        rankImage.texture = rankTexture;
        RawImage background = transform.Find("Background").GetComponent<RawImage>();
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.HICB.HICB_I_BACKGROUND_URL);
