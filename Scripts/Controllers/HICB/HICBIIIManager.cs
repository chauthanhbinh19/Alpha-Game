...
public class HICBIIIManager : MonoBehaviour
{
    ...
    public async Task CreateMainHICBPanelAsync(string featureId, string featureName)
    {
        ...
        upgradeOneLevelButton.onClick.AddListener(async () =>
        {
            ...
            UpgradeResultDTO result = await UpgradeService.Create().UpgradeOneLevelAsync(featureName, currentLevel, hicb.MaxLevel, User.CurrentUserId);
            if (result.Success)
            {
                ...
                await CreateMainHICBPanelAsync(featureId, featureName);
            }
            else
            {
        ...
        {
            AudioManager.Instance.PlaySFX(AudioConstants.SFX.SWITCH_CLICK_SOUND);
            UpgradeResultDTO result = await UpgradeService.Create().UpgradeMaxLevelAsync(featureName, currentLevel, hicb.MaxLevel, User.CurrentUserId);
            if (result.Success)
            {
                userHICB = EnhanceHelper.EnhanceHICBs(userHICB, result.UpgradedLevels, hicb.BaseMultiplier);
                await UserHICBsService.Create().InsertOrUpdateUserHICBsAsync(User.CurrentUserId, userHICB, featureId);
