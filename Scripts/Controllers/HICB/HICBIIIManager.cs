...
public class HICBIIIManager : MonoBehaviour
{
    public static HICBIIIManager Instance { get; private set; }
    private Transform MainPanel;
    private GameObject HICBPanelPrefab;
    private GameObject HICBButtonPrefab;
    private GameObject PopupHICBPanelPrefab;
    private GameObject PopupHICBQuantityPanelPrefab;
    private GameObject PopupHICBButtonPrefab;
    private GameObject MainHICBPanelPrefab;
    private GameObject HICBItemPrefab;
    private Transform Content;
    private const int ITEMS_PER_PAGE = 50;
    private int CurrentPage = 0;
    private List<KeyValuePair<string, FeatureHICBDTO>> FeatureList;
    ...
    public async Task CreateHICBIIIManagerAsync()
    {
        GameObject currentObject = Instantiate(PopupHICBPanelPrefab, MainPanel);
        ...
        Dictionary<string, FeatureHICBDTO> uniqueTypes = new Dictionary<string, FeatureHICBDTO>();
        uniqueTypes = await FeaturesService.Create().GetHICBFeaturesByTypeAsync(AppConstants.HICB.HICB_III);
    ...
    private void RenderPage()
    {
        ...
        for (int i = start; i < end; i++)
        {
            ...
            GameObject button = Instantiate(PopupHICBButtonPrefab, Content);
            ...
            if (btn != null)
            {
                ...
                btn.onClick.AddListener(async () =>
                {
                    ...
                    AudioManager.Instance.PlaySFX(AudioConstants.SFX.BUTTON_CLICK_SOUND);
                    await CreateMainHICBPanelAsync(currentFeatureId, currentSubtype);
                });
            }
        }
    }
    ...
    public async Task CreateMainHICBPanelAsync(string featureId, string featureName)
    {
        GameObject currentObject = Instantiate(MainHICBPanelPrefab, MainPanel);
        Transform transform = currentObject.transform;
        ...
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.HICB.HICB_III}");
        rankImage.texture = rankTexture;
        RawImage background = transform.Find("Background").GetComponent<RawImage>();
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.HICB.HICB_III_BACKGROUND_URL);
        ...
        List<RecipeItemDto> recipeItems = await RecipeService.Create().GetRecipeItemsAsync(featureName, userHICB.Level, User.CurrentUserId);
        ...
        for (int i = 0; i < total; i++)
        {
            ...
            GameObject itemGO = Instantiate(HICBItemPrefab, parent);

            SetupHICBItemUI(itemGO, recipeItems[i]);
        }

        int currentLevel = userHICB?.Level ?? 0;
        ...
        async Task RefreshPanelAsync()
        {
            ...
            List<RecipeItemDto> refreshedRecipeItems = await RecipeService.Create().GetRecipeItemsAsync(featureName, userHICB.Level, User.CurrentUserId);
            ...
            for (int i = 0; i < refreshedTotal; i++)
            {
                ...
                GameObject itemGO = Instantiate(HICBItemPrefab, parent);
                SetupHICBItemUI(itemGO, refreshedRecipeItems[i]);
            }
        }
        ...
        void CreatePopupUpgradePanelAsync()
        {
            GameObject gameObject =
                Instantiate(PopupHICBQuantityPanelPrefab, MainPanel);
            ...
            int maxLevel = hicb != null ? hicb.MaxLevel : popupCurrentLevel;
            ...
            if (userHICB != null)
            {
                StatsManager.Instance.CreateStatsManager(userHICB, currentStatsContent);
                StatsManager.Instance.CreateStatsManager(userHICB, nextStatsContent);
            }
            ...
            async Task UpdatePreviewAsync()
            {
                ...
                {
                    UserHICBs previewHICB = userHICB.CloneUserHICB(userHICB);
                    EnhanceHelper.EnhanceHICBs(previewHICB, preview.UpgradedLevels, hicb.BaseMultiplier);
                    StatsManager.Instance.CreateStatsManager(previewHICB, nextStatsContent);
                }
                else if (userHICB != null)
                {
                    StatsManager.Instance.CreateStatsManager(userHICB, nextStatsContent);
                }
            ...
            confirmButton.onClick.AddListener(async () =>
            {
                ...
                if (result.Success)
                {
                    userHICB = EnhanceHelper.EnhanceHICBs(userHICB, result.UpgradedLevels, hicb.BaseMultiplier);
                    await UserHICBsService.Create().InsertOrUpdateUserHICBsAsync(User.CurrentUserId, userHICB, featureId);
    ...
    private void SetupHICBItemUI(GameObject itemGO,RecipeItemDto data)
    {
