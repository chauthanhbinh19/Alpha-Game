...
public class HICBVIManager : MonoBehaviour
{
    public static HICBVIManager Instance { get; private set; }
    ...
    public async Task CreateHICBVIManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetHICBFeaturesByTypeAsync(AppConstants.HICB.HICB_VI);
    ...
    public async Task CreateMainHICBPanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.HICB.HICB_VI}");
        rankImage.texture = rankTexture;
        RawImage background = transform.Find("Background").GetComponent<RawImage>();
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.HICB.HICB_VI_BACKGROUND_URL);
