...
public class HICBIXManager : MonoBehaviour
{
    public static HICBIXManager Instance { get; private set; }
    ...
    public async Task CreateHICBIXManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetHICBFeaturesByTypeAsync(AppConstants.HICB.HICB_IX);
    ...
    public async Task CreateMainHICBPanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.HICB.HICB_IX}");
        rankImage.texture = rankTexture;
        RawImage background = transform.Find("Background").GetComponent<RawImage>();
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.HICB.HICB_IX_BACKGROUND_URL);
