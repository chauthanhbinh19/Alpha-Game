...
public class HICBVIIIManager : MonoBehaviour
{
    public static HICBVIIIManager Instance { get; private set; }
    ...
    public async Task CreateHICBVIIIManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetHICBFeaturesByTypeAsync(AppConstants.HICB.HICB_VIII);
    ...
    public async Task CreateMainHICBPanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.HICB.HICB_VIII}");
        rankImage.texture = rankTexture;
        RawImage background = transform.Find("Background").GetComponent<RawImage>();
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.HICB.HICB_VIII_BACKGROUND_URL);
