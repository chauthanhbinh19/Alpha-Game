...
{
    public static MainMenuNanotechManager Instance { get; private set; }
    ...
    public async Task CreateMainMenuNanotechManagerAsync(IStats stat)
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetRankFeaturesByTypeAsync(AppConstants.MainMenuSet2.NANOTECH, stat);
    ...
    public async Task CreateMainRankPanelAsync(string featureId, string featureName)
    {
        ...
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.MainMenuSet2.NANOTECH);
