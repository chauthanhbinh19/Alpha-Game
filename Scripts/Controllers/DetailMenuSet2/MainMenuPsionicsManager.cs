...
{
    public static MainMenuPsionicsManager Instance { get; private set; }
    ...
    public async Task CreateMainMenuPsionicsManagerAsync(IStats stat)
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetRankFeaturesByTypeAsync(AppConstants.MainMenuSet2.PSIONICS, stat);
    ...
    public async Task CreateMainRankPanelAsync(string featureId, string featureName)
    {
        ...
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.MainMenuSet2.PSIONICS);
