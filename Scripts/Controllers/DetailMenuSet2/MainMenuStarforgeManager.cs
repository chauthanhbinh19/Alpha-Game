...
{
    public static MainMenuStarforgeManager Instance { get; private set; }
    ...
    public async Task CreateMainMenuStarforgeManagerAsync(IStats stat)
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetRankFeaturesByTypeAsync(AppConstants.MainMenuSet2.STARFORGE, stat);
    ...
    public async Task CreateMainRankPanelAsync(string featureId, string featureName)
    {
        ...
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.MainMenuSet2.STARFORGE);
