...
{
    public static MainMenuSeraphirosManager Instance { get; private set; }
    ...
    public async Task CreateMainMenuSeraphirosManagerAsync(IStats stat)
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetRankFeaturesByTypeAsync(AppConstants.MainMenuSet3.SERAPHIROS, stat);
    ...
    public async Task CreateMainRankPanelAsync(string featureId, string featureName)
    {
        ...
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.MainMenuSet3.SERAPHIROS);
