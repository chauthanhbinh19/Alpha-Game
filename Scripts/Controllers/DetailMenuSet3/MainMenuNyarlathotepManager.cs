...
{
    public static MainMenuNyarlathotepManager Instance { get; private set; }
    ...
    public async Task CreateMainMenuNyarlathotepManagerAsync(IStats stat)
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetRankFeaturesByTypeAsync(AppConstants.MainMenuSet3.NYARLATHOTEP, stat);
    ...
    public async Task CreateMainRankPanelAsync(string featureId, string featureName)
    {
        ...
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.MainMenuSet3.NYARLATHOTEP);
