...
{
    public static MainMenuDolomathManager Instance { get; private set; }
    ...
    public async Task CreateMainMenuDolomathManagerAsync(IStats stat)
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetRankFeaturesByTypeAsync(AppConstants.MainMenuSet3.DOLOMATH, stat);
    ...
    public async Task CreateMainRankPanelAsync(string featureId, string featureName)
    {
        ...
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.MainMenuSet3.DOLOMATH);
