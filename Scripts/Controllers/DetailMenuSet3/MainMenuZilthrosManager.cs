...
{
    public static MainMenuZilthrosManager Instance { get; private set; }
    ...
    public async Task CreateMainMenuZilthrosManagerAsync(IStats stat)
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetRankFeaturesByTypeAsync(AppConstants.MainMenuSet3.ZILTHROS, stat);
    ...
    public async Task CreateMainRankPanelAsync(string featureId, string featureName)
    {
        ...
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.MainMenuSet3.ZILTHROS);
