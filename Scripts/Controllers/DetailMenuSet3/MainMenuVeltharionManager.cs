...
{
    public static MainMenuVeltharionManager Instance { get; private set; }
    ...
    public async Task CreateMainMenuVeltharionManagerAsync(IStats stat)
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetRankFeaturesByTypeAsync(AppConstants.MainMenuSet3.VELTHARION, stat);
    ...
    public async Task CreateMainRankPanelAsync(string featureId, string featureName)
    {
        ...
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.MainMenuSet3.VELTHARION);
