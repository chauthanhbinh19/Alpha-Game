...
{
    public static MainMenuCataclysmManager Instance { get; private set; }
    ...
    public async Task CreateMainMenuCataclysmManagerAsync(IStats stat)
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetRankFeaturesByTypeAsync(AppConstants.MainMenuSet6.CATACLYSM, stat);
    ...
    public async Task CreateMainRankPanelAsync(string featureId, string featureName)
    {
        ...
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.MainMenuSet6.CATACLYSM);
