...
public class UniverseIVManager : MonoBehaviour
{
    ...
    public async Task CreateMainUniversePanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.Universe.UNIVERSE_IV}");
        rankImage.texture = rankTexture;
        RawImage background = transform.Find("Background").GetComponent<RawImage>();
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.Universe.UNIVERSE_IV_BACKGROUND_URL);
