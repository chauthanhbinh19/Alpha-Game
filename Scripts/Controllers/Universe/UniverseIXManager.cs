...
public class UniverseIXManager : MonoBehaviour
{
    public static UniverseIXManager Instance { get; private set; }
    ...
    public async Task CreateUniverseIXManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetUniverseFeaturesByTypeAsync(AppConstants.Universe.UNIVERSE_IX);
    ...
    public async Task CreateMainUniversePanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.Universe.UNIVERSE_IX}");
        rankImage.texture = rankTexture;
        RawImage background = transform.Find("Background").GetComponent<RawImage>();
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.Universe.UNIVERSE_IX_BACKGROUND_URL);
