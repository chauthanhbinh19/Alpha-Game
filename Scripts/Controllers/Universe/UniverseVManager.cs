...
public class UniverseVManager : MonoBehaviour
{
    public static UniverseVManager Instance { get; private set; }
    ...
    public async Task CreateUniverseVManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetUniverseFeaturesByTypeAsync(AppConstants.Universe.UNIVERSE_V);
    ...
    public async Task CreateMainUniversePanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.Universe.UNIVERSE_V}");
        rankImage.texture = rankTexture;
        RawImage background = transform.Find("Background").GetComponent<RawImage>();
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.Universe.UNIVERSE_V_BACKGROUND_URL);
