...
public class UniverseVIIManager : MonoBehaviour
{
    public static UniverseVIIManager Instance { get; private set; }
    ...
    public async Task CreateUniverseVIIManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetUniverseFeaturesByTypeAsync(AppConstants.Universe.UNIVERSE_VII);
    ...
    public async Task CreateMainUniversePanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.Universe.UNIVERSE_VII}");
        rankImage.texture = rankTexture;
        RawImage background = transform.Find("Background").GetComponent<RawImage>();
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.Universe.UNIVERSE_VII_BACKGROUND_URL);
