...
public class UniverseIManager : MonoBehaviour
{
    public static UniverseIManager Instance { get; private set; }
    ...
    public async Task CreateMainUniversePanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = TextureHelper.LoadTexture2DCached($"UI/Rank_Research/{AppConstants.Universe.UNIVERSE_I}");
        rankImage.texture = rankTexture;
        RawImage background = transform.Find("Background").GetComponent<RawImage>();
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.Universe.UNIVERSE_I_BACKGROUND_URL);
