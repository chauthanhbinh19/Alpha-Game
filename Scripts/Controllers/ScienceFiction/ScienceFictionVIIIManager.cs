...
public class ScienceFictionVIIIManager : MonoBehaviour
{
    public static ScienceFictionVIIIManager Instance { get; private set; }
    ...
    public async Task CreateScienceFictionVIIIManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetScienceFictionFeaturesByTypeAsync(AppConstants.ScienceFiction.SCIENCE_FICTION_VIII);
    ...
    public async Task CreateMainScienceFictionPanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = Resources.Load<Texture2D>($"UI/Rank_Research/{AppConstants.ScienceFiction.SCIENCE_FICTION_VIII}");
        rankImage.texture = rankTexture;
        RawImage background = transform.Find("Background").GetComponent<RawImage>();
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.ScienceFiction.SCIENCE_FICTION_NUMBER_8_BACKGROUND_URL);
