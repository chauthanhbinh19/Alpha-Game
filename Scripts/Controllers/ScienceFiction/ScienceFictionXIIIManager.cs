...
public class ScienceFictionXIIIManager : MonoBehaviour
{
    public static ScienceFictionXIIIManager Instance { get; private set; }
    ...
    public async Task CreateScienceFictionXIIIManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetScienceFictionFeaturesByTypeAsync(AppConstants.ScienceFiction.SCIENCE_FICTION_XIII);
    ...
    public async Task CreateMainScienceFictionPanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = Resources.Load<Texture2D>($"UI/Rank_Research/{AppConstants.ScienceFiction.SCIENCE_FICTION_XIII}");
        rankImage.texture = rankTexture;
        RawImage background = transform.Find("Background").GetComponent<RawImage>();
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.ScienceFiction.SCIENCE_FICTION_NUMBER_13_BACKGROUND_URL);
