...
public class ScienceFictionVIIManager : MonoBehaviour
{
    public static ScienceFictionVIIManager Instance { get; private set; }
    ...
    public async Task CreateScienceFictionVIIManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetScienceFictionFeaturesByTypeAsync(AppConstants.ScienceFiction.SCIENCE_FICTION_VII);
    ...
    public async Task CreateMainScienceFictionPanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = Resources.Load<Texture2D>($"UI/Rank_Research/{AppConstants.ScienceFiction.SCIENCE_FICTION_VII}");
        rankImage.texture = rankTexture;
        RawImage background = transform.Find("Background").GetComponent<RawImage>();
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.ScienceFiction.SCIENCE_FICTION_NUMBER_7_BACKGROUND_URL);
