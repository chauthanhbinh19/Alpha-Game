...
public class ScienceFictionIIManager : MonoBehaviour
{
    public static ScienceFictionIIManager Instance { get; private set; }
    ...
    public async Task CreateScienceFictionIIManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetScienceFictionFeaturesByTypeAsync(AppConstants.ScienceFiction.SCIENCE_FICTION_II);
    ...
    public async Task CreateMainScienceFictionPanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = Resources.Load<Texture2D>($"UI/Rank_Research/{AppConstants.ScienceFiction.SCIENCE_FICTION_II}");
        rankImage.texture = rankTexture;
        RawImage background = transform.Find("Background").GetComponent<RawImage>();
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.ScienceFiction.SCIENCE_FICTION_NUMBER_2_BACKGROUND_URL);
