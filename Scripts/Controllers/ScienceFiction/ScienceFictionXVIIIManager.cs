...
public class ScienceFictionXVIIIManager : MonoBehaviour
{
    public static ScienceFictionXVIIIManager Instance { get; private set; }
    ...
    public async Task CreateScienceFictionXVIIIManagerAsync()
    {
        ...
        uniqueTypes = await FeaturesService.Create().GetScienceFictionFeaturesByTypeAsync(AppConstants.ScienceFiction.SCIENCE_FICTION_XVIII);
        ...
        for (int i = start; i < end; i++)
        {
            ...
            if (btn != null)
            {
                ...
                btn.onClick.AddListener(async () =>
                {
                    ...
                    AudioManager.Instance.PlaySFX(AudioConstants.SFX.BUTTON_CLICK_SOUND);
                    await CreateMainScienceFictionPanelAsync(currentFeatureId, currentSubtype);
                });
            }
        }
    }
    ...
    public async Task CreateMainScienceFictionPanelAsync(string featureId, string featureName)
    {
        ...
        Texture rankTexture = Resources.Load<Texture2D>($"UI/Rank_Research/{AppConstants.ScienceFiction.SCIENCE_FICTION_XVIII}");
        rankImage.texture = rankTexture;
        RawImage background = transform.Find("Background").GetComponent<RawImage>();
        background.texture = TextureHelper.LoadTexture2DCached(ImageConstants.ScienceFiction.SCIENCE_FICTION_NUMBER_18_BACKGROUND_URL);
